...
fileprivate let mainCurrentlyIncreaseMessage:[UInt8] = [0x53,0x54,0x53,0x4e,0x12,0x59,0x55,0x5e,0x5f,0x48,0x0,0x13,0x1a,0x52,0x5b,0x49,0x1a,0x54,0x55,0x4e,0x1a,0x58,0x5f,0x5f,0x54,0x1a,0x53,0x57,0x4a,0x56,0x5f,0x57,0x5f,0x54,0x4e,0x5f,0x5e]

private func saleLab(burden num: UInt8) -> UInt8 {
    return num ^ 58
}
...
@objc protocol PromiseSize: NSObjectProtocol {
    ///
    //: @objc optional func startScroll()
    @objc optional func phoneOfficial()

    //: @objc optional func endScroll()
    @objc optional func weekBass()
}

//: class TalkingVideoDanmuMsgListTableView: UIView {
class SignificantBecome: UIView {
    ...
    override init(frame: CGRect) {
        ...
        let tapGesture = UITapGestureRecognizer(target: self, action: #selector(atStream))
    ...
    required init?(coder: NSCoder) {
        ...
        fatalError(String(bytes: mainCurrentlyIncreaseMessage.map{saleLab(burden: $0)}, encoding: .utf8)!)
    }
...
extension SignificantBecome {
    ...
    func replaceReading() {
        ...
        refreshTimer = Timer.scheduledTimer(timeInterval: reloadTimeSpan, target: self, selector: #selector(despiteActual), userInfo: nil, repeats: true)
    }
...
extension SignificantBecome {
    ...
    func orientationFor(msgModel: RenderProjection?) {
    ...
    func asFlexible() {
        ...
        if AllHeight > show_aroundTitle {
            //: if self.msgTableView.height < VideoMsgTableViewHeight {
            if self.msgTableView.height < show_aroundTitle {
                ...
                self.msgTableView.height = show_aroundTitle
            }
            //: } else {
        } else {
            //: var frame = self.msgTableView.frame
            ...
            frame.origin.y = show_aroundTitle - AllHeight
        ...
        compose(animated: false)
    }

    /** 执行插入动画并滚动 */
    //: func scrollToBottom(animated: Bool) {
    func compose(animated: Bool) {
    ...
    func pureOnModel(inPending: Bool) {
...
extension SignificantBecome: UITableViewDelegate, UITableViewDataSource {
    ...
    func betweenLogAt(_ scrollView: UIScrollView) {
        //: self.delegate?.endScroll?()
        self.delegate?.weekBass?()
    ...
    func tableView(_ tableView: UITableView, heightForRowAt indexPath: IndexPath) -> CGFloat {
        ...
        return msgModel.msgHeight + Double(mainFailureStr)
    }
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        let cell = ExtendSpacePad.keyCeiling(tableView: tableView, msg: msgModel, indexPath: indexPath)
...
extension SignificantBecome {
    ...
    func anSkirtDrown() {
        ...
        self.msgTableView.frame = CGRect(x: 0, y: 0, width: mainAverageMsg, height: show_aroundTitle)
    }
}
