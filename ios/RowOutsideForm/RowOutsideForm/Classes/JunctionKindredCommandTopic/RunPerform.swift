...
/*: "num" :*/
fileprivate let appShakeStr:[Character] = ["n","u","m"]
...
@objc public enum VisibleListen: Int {
    //: case normal  = 1
    case normal = 1 // 普通礼物
    //: case myStery = 2
    case myStery = 2 // 盲盒礼物
...
struct RunPerform: HandyJSON {
    ...
    mutating func fillWill(giftNum: Int) {
        //: self.num = giftNum
        self.num = giftNum
    }
}
...
struct ExamineionConstant: HandyJSON {
    //: init() {
    init() {}
    ...
    mutating func mapping(mapper: HelpingMapper) {
        ...
        mapper <<<
            //: self.giftNum <-- "num"
            self.giftNum <-- (String(appShakeStr))
    }
}
