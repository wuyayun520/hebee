...
fileprivate let notiSpecifyCalculationFormat:[UInt8] = [0x64,0x65,0x74,0x6e,0x65,0x6d,0x65,0x6c,0x70,0x6d,0x69,0x20,0x6e,0x65,0x65,0x62,0x20,0x74,0x6f,0x6e,0x20,0x73,0x61,0x68,0x20,0x29,0x3a,0x72,0x65,0x64,0x6f,0x63,0x28,0x74,0x69,0x6e,0x69]

/*: "quick_reply_bg" :*/
fileprivate let app_plainAllotMessage:[Character] = ["q","u","i","c","k","_","r","e","p","l","y","_","b"]
fileprivate let notiStrangerValue:[Character] = ["g"]

/*: "icon_message_reply" :*/
fileprivate let dataDrawTitle:[Character] = ["i","c","o","n","_","m","e","s","s","a","g","e","_","r","e","p","l"]
fileprivate let notiPanKingStr:[Character] = ["y"]

/*: "Quick reply" :*/
fileprivate let show_logString:[Character] = ["Q","u","i","c","k"," ","r","e","p","l","y"]
...
class AmongSession: UIView {
    //: var tapBlock: ((_ : QuestionQuickReplayModel) -> Void)?
    var tapBlock: ((_: ThinModel) -> Void)?

    /// 当前处理消息模型
    //: private var msgInfo = AbTalkingChatMsgInfoModel()
    private var msgInfo = DepthArchitectureModel()
    ...
    required init?(coder: NSCoder) {
        ...
        fatalError(String(bytes: notiSpecifyCalculationFormat.reversed(), encoding: .utf8)!)
    }
    ...
    private lazy var bgImgView: UIImageView = {
        ...
        img.image = UIImage.installBind(name: (String(app_plainAllotMessage) + String(notiStrangerValue))).resizableImage(withCapInsets: insets, resizingMode: .stretch)
    ...
    private lazy var icon: UIImageView = {
        ...
        img.image = UIImage.installBind(name: (String(dataDrawTitle) + String(notiPanKingStr)))
        //: return img
        return img
        //: }()
    }()
    ...
    private lazy var line: UIView = {
        ...
        line.backgroundColor = UIColor.tress()
...
extension AmongSession {
    /// 刷新消息视图
    //: func refreshQuickReplayView(msgInfo: AbTalkingChatMsgInfoModel) {
    func paintData(msgInfo: DepthArchitectureModel) {
        ...
        for (index, model) in msgInfo.replay.enumerated() {
            ...
            if let model = model as? ThinModel {
                ...
                replyView.backgroundColor = UIColor.tress()
                ...
                let tap = UITapGestureRecognizer(target: self, action: #selector(behaviorSkirtCorner(_:)))
    ...
    @objc private func behaviorSkirtCorner(_ gesture: UITapGestureRecognizer) {
        ...
        guard index < self.msgInfo.replay.count,
              ...
              let model = self.msgInfo.replay[index] as? ThinModel
        else {
            //: return
            return
        }
        ...
        var cacheMsgIds = appEfficiencyStorageStr.object(forKey: SustainableSaveerGift.share.QuestionReplayMsgIdIdArrayKey) as? [String]
        ...
        if !cacheMsgIds!.contains(self.msgInfo.msgId) {
            ...
            appEfficiencyStorageStr.set(cacheMsgIds, forKey: SustainableSaveerGift.share.QuestionReplayMsgIdIdArrayKey)
        }
        // 回调发送消息接口
        //: tapBlock?(model)
        tapBlock?(model)
