...
/*: "MF:LiveChatWelMsg" :*/
fileprivate let notiMusicPath:[Character] = ["M","F",":","L","i","v","e","C","h","a"]
fileprivate let appDailyTooName:[Character] = ["t","W","e","l","M","s","g"]

/*: "MF:LiveChatPrizeMsg" :*/
fileprivate let constEasyData:[Character] = ["M","F",":","L","i","v","e","C","h","a","t","P","r"]
fileprivate let main_inmateName:String = "auto blueizeMsg"

/*: "MF:LiveChatAttentionMsg" :*/
fileprivate let noti_listenerKey:String = "eager warn sameMF:LiveC"
fileprivate let mainWaitInfrastructureId:String = "hawithout"
fileprivate let k_performFastMsg:String = "pending road counsel comment procedureAtte"

/*: "#BAFBFF" :*/
fileprivate let showReachKey:[Character] = ["#","B","A","F","B","F","F"]

/*: "uid" :*/
fileprivate let kOperateKey:String = "uiaccelerate"

/*: "<at>@[\\S\\s]+?</at>" :*/
fileprivate let show_teamTitle:[Character] = ["<","a","t",">","@","[","\\","S","\\","s","]","+"]
fileprivate let const_minString:String = "analyze pic he selection?</at>"

/*: "Sent " :*/
fileprivate let kObtainEasyExerciseFormat:[Character] = ["S","e","n","t"," "]

/*: "#FFA4CF" :*/
fileprivate let notiReplyUrl:[Character] = ["#"]
fileprivate let constYesterdayStr:[Character] = ["F","F","A","4","C","F"]

/*: "Here comes " :*/
fileprivate let showChoosePath:String = "scientific isolate craft failHere "
fileprivate let showCorePath:[Character] = ["c","o","m","e","s"," "]

/*: "#FFE386" :*/
fileprivate let kPoolMessage:String = "stranger ready variable split alive#F"
fileprivate let k_rocketValue:[Character] = ["F","E","3","8","6"]

/*: "followed the anchor" :*/
fileprivate let mainWarnStr:String = "foindependentindependent"
fileprivate let mainDestinationStr:[Character] = [" ","t","h","e"," ","a","n","c","h","o","r"]

/*: "level_ :*/
fileprivate let mainAccessibleString:String = "ladditionel"
fileprivate let main_renderId:[Character] = ["_"]

/*: "icon_lounge" :*/
fileprivate let data_withinCompleteValue:String = "creation behindicon_lo"
fileprivate let const_programFormat:[Character] = ["u","n","g","e"]
...
class CouldLarge: NSObject {
    ...
    func topic(model: CheckerUser) -> CheckerUser {
        //: self.model = model
        self.model = model
        /// 普通文本消息,礼物消息
        ...
        } else if self.model.MsgExtension == (String(notiMusicPath) + String(appDailyTooName)) {
            //: caculateWelMsg()
            sharedPic()
            /// 中奖消息
            //: } else if self.model.MsgExtension == "MF:LiveChatPrizeMsg" {
        } else if self.model.MsgExtension == (String(constEasyData) + String(main_inmateName.suffix(6))) {
        ...
        } else if self.model.MsgExtension == (String(noti_listenerKey.suffix(8)) + mainWaitInfrastructureId.replacingOccurrences(of: "without", with: "t") + String(k_performFastMsg.suffix(4)) + "ntionMsg") {
            //: caculAttionMsg()
            processWithoutSet()
        }
        //: return self.model
    ...
    func plainResign() {
        ...
        let nameAttr = searchionHistory(text: firstStr, font: UIFont.theSearcher(fontSize: 14), color: UIColor(hex: (String(showReachKey)))!, isTap: true, atUid: String(self.model.user?.uid ?? 0))
        ...
        if self.model.msgInfo?.messageType == 1 {
            ...
            let content = searchionHistory(text: self.model.msgInfo?.content ?? "", font: UIFont.theSearcher(fontSize: 14), color: UIColor.white, isTap: false, atUid: nil)
            ...
            let atUid = jumps?[(kOperateKey.replacingOccurrences(of: "accelerate", with: "d"))] as? Int ?? 0
            //: let text = self.model.msgInfo?.content ?? ""
            let text = self.model.msgInfo?.content ?? ""
            //: let regex = try! NSRegularExpression(pattern: "<at>@[\\S\\s]+?</at>", options: [])
            let regex = try! NSRegularExpression(pattern: (String(show_teamTitle) + String(const_minString.suffix(6))), options: [])
            ...
            if matchRange?.length ?? 0 > 9 {
                ...
                let subStr = String.listenerByQuarterback(str: text, from: (matchRange?.location ?? 0) + 4, to: (matchRange?.length ?? 0) - 5)
                //: let contentStr = String.substringByUnicodeIndex(str: text, from: (matchRange?.length ?? 0), to: text.utf16.count )
                let contentStr = String.listenerByQuarterback(str: text, from: (matchRange?.length ?? 0), to: text.utf16.count)
                ...
                let atName = searchionHistory(text: subStr, font: UIFont.theSearcher(fontSize: 14), color: UIColor(hex: (String(showReachKey)))!, isTap: true, atUid: "\(atUid)")
                ...
                let content = searchionHistory(text: contentStr, font: UIFont.theSearcher(fontSize: 14), color: UIColor.white, isTap: false, atUid: nil)
            ...
            let sentAttr = searchionHistory(text: (String(kObtainEasyExerciseFormat)).localized, font: UIFont.theSearcher(fontSize: 14), color: UIColor.white, isTap: false, atUid: nil)
            ...
            if let toUser = self.model.toUser { // 送礼接收人
                ...
                let nickNameAttr = searchionHistory(text: "\(toUser.nickname) ", font: UIFont.statinEssay(type: .Medium, fontSize: 14), color: UIColor(hex: (String(notiReplyUrl) + String(constYesterdayStr)))!, isTap: true, atUid: String(toUser.uid))
                //: nameAttr.append(nickNameAttr)
                nameAttr.append(nickNameAttr)
            }
            //: let giftnameStr = changeGiftName(giftmodel: self.model.gift!)
            let giftnameStr = behindDifficult(giftmodel: self.model.gift!)
            ...
            let giftAttr = searchionHistory(text: giftnameStr, font: UIFont.statinEssay(type: .Medium, fontSize: 14), color: UIColor(hex: (String(notiReplyUrl) + String(constYesterdayStr)))!, isTap: false, atUid: nil)
            ...
            let imag = self.model.gift?.giftImg?.funeralDelay(with: size)
            ...
            let giftIcon = NSMutableAttributedString.yy_attachmentString(withContent: imag, contentMode: .center, attachmentSize: size, alignTo: UIFont.theSearcher(fontSize: 14), alignment: .center)
            //: giftIcon.yy_paragraphStyle = paragraphStyle()
            giftIcon.yy_paragraphStyle = link()
            ...
            let numAttr = self.searchionHistory(text: "*\(self.model.gift?.num ?? 0)", font: UIFont.theSearcher(fontSize: 14), color: UIColor(hex: (String(notiReplyUrl) + String(constYesterdayStr)))!, isTap: false, atUid: nil)
        ...
        projectionCollection(attribText: self.model.msgAttribText)
    }

    /// 公告富文本
    //: func showAnnouncementMsg(model: TalkingLiveRoomDanmuModel) -> TalkingLiveRoomDanmuModel {
    func load(model: CheckerUser) -> CheckerUser {
        //: self.model = model
        self.model = model
        // 内容
        //: let paraStyle = paragraphStyle()
        ...
        let content = searchionHistory(text: self.model.msgInfo?.content ?? "", font: UIFont.theSearcher(fontSize: 14), color: UIColor(hex: (String(showReachKey)))!, isTap: false, atUid: nil)
    ...
    func sharedPic() {
        ...
        let textView = openSpring()
        //: let strnor = "Here comes ".localized
        let strnor = (String(showChoosePath.suffix(5)) + String(showCorePath)).localized
        ...
        let content = searchionHistory(text: strnor, font: UIFont.theSearcher(fontSize: 14), color: UIColor(hex: (String(kPoolMessage.suffix(2)) + String(k_rocketValue)))!, isTap: false, atUid: nil)
        ...
        let attachText = searchionHistory(text: strc, font: UIFont.theSearcher(fontSize: 14), color: UIColor(hex: (String(showReachKey)))!, isTap: true, atUid: String(self.model.user?.uid ?? 0))
    ...
    func section() {
        ...
        let content = searchionHistory(text: firstStr, font: UIFont.theSearcher(fontSize: 14), color: UIColor.white, isTap: true, atUid: String(self.model.user?.uid ?? 0))
        //: textView.append(content)
        textView.append(content)
        //: self.model.msgAttribText = textView
        self.model.msgAttribText = textView
    ...
    func processWithoutSet() {
        ...
        let strnor = (mainWarnStr.replacingOccurrences(of: "independent", with: "l") + "owed" + String(mainDestinationStr)).localized
        //: let strc =  "\(self.model.user?.nickname ?? "") "
        let strc = "\(self.model.user?.nickname ?? "") "
        ...
        let content = searchionHistory(text: strnor, font: UIFont.theSearcher(fontSize: 14), color: UIColor.white, isTap: false, atUid: nil)
        ...
        let nameText = searchionHistory(text: strc, font: UIFont.theSearcher(fontSize: 14), color: UIColor(hex: (String(showReachKey)))!, isTap: true, atUid: String(self.model.user?.uid ?? 0))
    ...
    func link() -> NSMutableParagraphStyle {
    ...
    func searchionHistory(text: String, font: UIFont, color: UIColor, isTap: Bool, atUid: String?) -> NSMutableAttributedString {
...
//            attribute.yy_writingDirection = [3]
        ...
        attribute.yy_paragraphStyle = link()
        ...
        if isTap == true {
            ...
            attribute.yy_setTextHighlight(range, color: color, backgroundColor: .clear, userInfo: nil) { containerView, text, range, rect in
                ...
                /// 不能@自己
                //: if atUid != VideoMagnitudeerval.share.loginUserMode.userID {
                if atUid != VideoMagnitudeerval.share.loginUserMode.userID {
    ...
    func projectionCollection(attribText: NSMutableAttributedString) {
        // 距离左边8  距离右边也为8
        //: let maxWidth = MsgTableViewWidth - 16
        let maxWidth = noti_createTitle - 16
    ...
    private func behindDifficult(giftmodel: RunPerform) -> String {
...
extension CouldLarge {
    /// 获取弹幕富文本图片
    //: func getImagesAttributedString() -> NSMutableAttributedString {
    func openSpring() -> NSMutableAttributedString {
        ...
        levelImgV.image = UIImage.installBind(name: (mainAccessibleString.replacingOccurrences(of: "addition", with: "ev") + String(main_renderId)) + "\(self.model.user?.level ?? 1)")
        //: let levelAttr = getAttachText(imageView: levelImgV)
        let levelAttr = discountDeviceClose(imageView: levelImgV)
        ...
        if self.model.user?.loungePlus == true {
            ...
            vipImgV.image = UIImage.installBind(name: (String(data_withinCompleteValue.suffix(7)) + String(const_programFormat)))
            //: let vipAttr = getAttachText(imageView: vipImgV)
            let vipAttr = discountDeviceClose(imageView: vipImgV)
    ...
    func discountDeviceClose(imageView: UIImageView) -> NSMutableAttributedString {
        ...
        let attachText = NSMutableAttributedString.yy_attachmentString(withContent: imageView.image, contentMode: .scaleToFill, attachmentSize: imageView.size, alignTo: UIFont.sumervalSort(fontSize: 12), alignment: .center)
        //: attachText.yy_paragraphStyle = paragraphStyle()
        attachText.yy_paragraphStyle = link()
        //: return attachText
        return attachText
    }
}
