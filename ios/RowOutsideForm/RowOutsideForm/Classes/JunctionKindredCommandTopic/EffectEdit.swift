...
fileprivate let user_generateStr:[UInt8] = [0x8f,0x88,0x8f,0x92,0xce,0x85,0x89,0x82,0x83,0x94,0xdc,0xcf,0xc6,0x8e,0x87,0x95,0xc6,0x88,0x89,0x92,0xc6,0x84,0x83,0x83,0x88,0xc6,0x8f,0x8b,0x96,0x8a,0x83,0x8b,0x83,0x88,0x92,0x83,0x82]

private func appearMark(mine num: UInt8) -> UInt8 {
    return num ^ 230
}

/*: "%@.mp4" :*/
fileprivate let dataAcceptBucketUrl:String = "fresh employ link%@.mp4"

/*: "http" :*/
fileprivate let app_essaySignificantlyTitle:String = "hspotspotp"
...
class EffectEdit: SceneInput {
    ...
    required init?(coder: NSCoder) {
        ...
        fatalError(String(bytes: user_generateStr.map{appearMark(mine: $0)}, encoding: .utf8)!)
    }
    ...
    public lazy var vapPlayerView: UIView? = {
        ...
        let tap = UITapGestureRecognizer(target: self, action: #selector(sendSquare))
    ...
    override func one() {
        ...
        let path = TitleDescription.shared.block(fileName: self.giftEffectModel?.mainFile ?? "", model: self.effectMsgModel ?? BeyondUsed())

        //: if  path.count > 0 {
        if path.count > 0 {
            //: let mp4Path = String.init(format: "%@.mp4", path)
            let mp4Path = String(format: (String(dataAcceptBucketUrl.suffix(6))), path)
            ...
            self.delegate?.robot(effectItemView: self)
        }
    }

    //: override func stopAnimating() {
    override func attachMight() {
        //: self.vapPlayerView?.stopHWDMP4()
        self.vapPlayerView?.stopHWDMP4()
    }

    //: override func cleanAnimating() {
    override func currency() {
    ...
    @objc func sendSquare() {}
}

///  --- HWDMP4PlayDelegate

//: extension TalkingVapAnimationView: HWDMP4PlayDelegate {
extension EffectEdit: HWDMP4PlayDelegate {
    ...
    func loadVapImage(withURL urlStr: String!, context: [AnyHashable: Any]!, completion completionBlock: VAPImageCompletionBlock!) {
        ...
        DispatchQueue.main.async {
            ...
            if urlStr.hasPrefix((app_essaySignificantlyTitle.replacingOccurrences(of: "spot", with: "t"))) {
