...
fileprivate let notiConstructDisappearStr:String = "business persistDocume"
fileprivate let notiDomainSucceedAccessMsg:String = "genuine fit unlessnts/"
fileprivate let main_shareData:String = "/voice/show make filter strength"

/*: "Documents/User/record/" :*/
fileprivate let app_revealId:String = "odd timing evolution clipDocum"
fileprivate let k_builderCharacterUrl:String = "/User/challenge child dump"
fileprivate let k_recommendationName:[Character] = ["r","e","c","o","r","d","/"]

/*: "cancelled" :*/
fileprivate let dataOutsideData:String = "brushanbrushel"
fileprivate let k_meetValue:[Character] = ["l","e","d"]

/*: .wav" :*/
fileprivate let const_recommendData:String = ".wavfemale above void"
...
typealias OneKeyGreetDownloadBlock = (_ model: CanModel) -> Void

//: protocol VoiceDownloadTaskManagerDelegate: NSObjectProtocol {
protocol ProviderObserve: NSObjectProtocol {
    /// 下载完成
...
extension TapMark {
    ...
    func succeed(_ tasksArray: [CanModel]) {
        ...
        for model in filterArray {
            ...
            if model.zipFilePath.isEmpty {
                ...
                model.zipFilePath = model.waitToOption()
            }

            // 重编译，相对路径会变
            //: if model.zipFilePath.contains("Documents/User/voice/") {
            if model.zipFilePath.contains((String(notiConstructDisappearStr.suffix(6)) + String(notiDomainSucceedAccessMsg.suffix(4)) + "User" + String(main_shareData.prefix(7)))) {
                //: model.zipFilePath = "\(AppCacheDefine.getChatVoicePath())\((model.zipFilePath as NSString).lastPathComponent)"
                model.zipFilePath = "\(CustomSpot.starArray())\((model.zipFilePath as NSString).lastPathComponent)"
                //: } else if model.zipFilePath.contains("Documents/User/record/") {
            } else if model.zipFilePath.contains((String(app_revealId.suffix(5)) + "ents" + String(k_builderCharacterUrl.prefix(6)) + String(k_recommendationName))) {
                //: model.zipFilePath = "\(AppCacheDefine.getChatRecordPath())\((model.zipFilePath as NSString).lastPathComponent)"
                model.zipFilePath = "\(CustomSpot.giftMarried())\((model.zipFilePath as NSString).lastPathComponent)"
            }
            ...
            DensityRedRender.angle(msgId: model.taskId, status: "\(CapBottom.prepare.rawValue)", filePath: model.zipFilePath)
    ...
    func localObtain(_ audioInfo: [String: Any], completeHandle: @escaping OneKeyGreetDownloadBlock) {
        //: var taskModel = VoiceDownloadTaskModel.getOneKeyGreetModel(audioInfo)
        var taskModel = CanModel.originDoing(audioInfo)
        ...
        if self.downloadingModelDic.keys.contains(taskModel.downloadUri) {
            ...
            taskModel = self.downloadingModelDic[taskModel.downloadUri] ?? CanModel()
        }

        // 重编译，相对路径会变
        //: if taskModel.zipFilePath.contains("Documents/User/voice/") {
        if taskModel.zipFilePath.contains((String(notiConstructDisappearStr.suffix(6)) + String(notiDomainSucceedAccessMsg.suffix(4)) + "User" + String(main_shareData.prefix(7)))) {
            //: taskModel.zipFilePath = "\(AppCacheDefine.getChatVoicePath())\((taskModel.zipFilePath as NSString).lastPathComponent)"
            taskModel.zipFilePath = "\(CustomSpot.starArray())\((taskModel.zipFilePath as NSString).lastPathComponent)"
            //: } else if taskModel.zipFilePath.contains("Documents/User/record/") {
        } else if taskModel.zipFilePath.contains((String(app_revealId.suffix(5)) + "ents" + String(k_builderCharacterUrl.prefix(6)) + String(k_recommendationName))) {
            //: taskModel.zipFilePath = "\(AppCacheDefine.getChatRecordPath())\((taskModel.zipFilePath as NSString).lastPathComponent)"
            taskModel.zipFilePath = "\(CustomSpot.giftMarried())\((taskModel.zipFilePath as NSString).lastPathComponent)"
        }
...
extension TapMark: URLSessionDataDelegate {
    // 开始
    ...
    func urlSession(_ session: URLSession, dataTask: URLSessionDataTask, didReceive response: URLResponse, completionHandler: @escaping (URLSession.ResponseDisposition) -> Void) {
        ...
        let totalBytesWritten = CanModel.meAcross(path: taskModel.zipFilePath)
    ...
    func urlSession(_ session: URLSession, dataTask: URLSessionDataTask, didReceive data: Data) {
        ...
        self.delegate?.frame(model: taskModel)
    }
    ...
    func urlSession(_ session: URLSession, task: URLSessionTask, didCompleteWithError error: Error?) {
        ...
        if error != nil || errorCode != 200 {
            ...
            if let error = error, (error as NSError).userInfo[NSLocalizedDescriptionKey] as? String == (dataOutsideData.replacingOccurrences(of: "brush", with: "c") + String(k_meetValue)) {
                //: if taskModel.taskStatus == .pause {
                if taskModel.taskStatus == .pause {
                    //: pauseTask(url: task.taskDescription ?? "", isSuspend: true)
                    sh(url: task.taskDescription ?? "", isSuspend: true)
                    //: } else if taskModel.taskStatus == .cancel {
                } else if taskModel.taskStatus == .cancel {
                    //: self.cancelTask(url: task.taskDescription ?? "")
                    self.burn(url: task.taskDescription ?? "")
                }
                //: } else {
            } else {
                ...
                sh(url: task.taskDescription ?? "", isSuspend: false)
            }
            //: } else {
        } else {
            ...
            self.anyLeave(url: task.taskDescription ?? "")
        }
...
extension TapMark {
    ...
    func sh(url: String, isSuspend: Bool) {
        ...
        case .chatRecord:
            ...
            let status = isSuspend ? "\(CapBottom.pause.rawValue)" : "\(CapBottom.error.rawValue)"
            //: WCDBVoiceMsgTable.db_updateVoiceMsg(msgId: taskModel.taskId, status: status)
            DensityRedRender.angle(msgId: taskModel.taskId, status: status)
            //: if isSuspend == false {
            if isSuspend == false {
                //: self.delegate?.voiceDownloadError(model: taskModel)
                self.delegate?.notLibrary(model: taskModel)
            }
    ...
    func burn(url: String) {
        ...
        case .chatRecord:
            ...
            DensityRedRender.nearPull(msgId: taskModel.taskId)
            //: self.delegate?.voiceDownloadCancel(model: taskModel)
            self.delegate?.kindDown(model: taskModel)
    ...
    func anyLeave(url: String) {
        ...
        let size = CanModel.meAcross(path: taskModel.zipFilePath)
        //: if size > 0 {
        if size > 0 {
            ...
            if taskModel.taskTypeId == .chatRecord {
                ...
                DensityRedRender.angle(msgId: taskModel.taskId, status: "\(CapBottom.finish.rawValue)")
            }

            //: let filePath = "\(taskModel.zipFilePath).wav"
            let filePath = "\(taskModel.zipFilePath)" + (String(const_recommendData.prefix(4)))
            ...
            self.brownOnly(at: taskModel.zipFilePath, withKeyId: taskModel.taskId, withTypeId: taskModel.taskTypeId) { _ in
                //: switch taskModel.taskTypeId {
                switch taskModel.taskTypeId {
                //: case .chatRecord:
                case .chatRecord:
                    //: self.delegate?.voiceDownloadFinish(model: taskModel)
                    self.delegate?.service(model: taskModel)
            ...
            case .chatRecord:
                ...
                DensityRedRender.angle(msgId: taskModel.taskId, status: "\(CapBottom.expired.rawValue)")
                //: self.delegate?.voiceDownloadExpired(model: taskModel)
                self.delegate?.contrast(model: taskModel)
    ...
    func brownOnly(at path: String, withKeyId keyId: String, withTypeId typeId: WaitRoute, complete: @escaping (Error?) -> Void) {
        //: DispatchQueue.global().async {
        DispatchQueue.global().async {
            //: if let data = try? Data(contentsOf: URL(fileURLWithPath: path)) {
            if let data = try? Data(contentsOf: URL(fileURLWithPath: path)) {
                //: let newPath = "\(path).wav"
                let newPath = "\(path)" + (String(const_recommendData.prefix(4)))
                ...
                if let handle = FileHandle(forWritingAtPath: newPath) {
                    ...
                    let unzippedData = data.incredible()
                    ...
                    if typeId == .chatRecord {
                        ...
                        DensityRedRender.angle(msgId: keyId, filePath: newPath)
                    }
