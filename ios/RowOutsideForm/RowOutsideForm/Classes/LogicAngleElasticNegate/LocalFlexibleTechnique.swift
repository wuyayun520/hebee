...
fileprivate let appSuccessfullyStr:[UInt8] = [0x7e,0x83,0x7e,0x89,0x3d,0x78,0x84,0x79,0x7a,0x87,0x4f,0x3e,0x35,0x7d,0x76,0x88,0x35,0x83,0x84,0x89,0x35,0x77,0x7a,0x7a,0x83,0x35,0x7e,0x82,0x85,0x81,0x7a,0x82,0x7a,0x83,0x89,0x7a,0x79]

fileprivate func homePanPair(divide num: UInt8) -> UInt8 {
...
/*: "all" :*/
fileprivate let k_implementId:String = "aalbum"
...
class LocalFlexibleTechnique: SuccessSSimilar {
    //: var giftData: TalkingGroupChatGiftMsgCellData?
    var giftData: SignPage?
    ...
    required init?(coder: NSCoder) {
        ...
        fatalError(String(bytes: appSuccessfullyStr.map{homePanPair(divide: $0)}, encoding: .utf8)!)
    }
...
extension LocalFlexibleTechnique {
    ...
    override func associated(with data: MarkerRole) {
        ...
        self.giftData = data as? SignPage
        ...
        self.giftImgView.symbol(urlStr: imgPreview)
        //: self.giftContentLab.attributedText = giftData.attributedString
        self.giftContentLab.attributedText = giftData.attributedString
        //: if giftData.msgModel.toUser.uid == "all" { // 送礼所有人
        if giftData.msgModel.toUser.uid == (k_implementId.replacingOccurrences(of: "album", with: "ll")) { // 送礼所有人
            //: self.toUserImgView.image = UIImage.BundleImageNamed(name: giftData.msgModel.toUser.headPic)
            self.toUserImgView.image = UIImage.installBind(name: giftData.msgModel.toUser.headPic)
            //: } else {
        } else {
            //: self.toUserImgView.setUrlImage(urlStr: giftData.msgModel.toUser.headPic)
            self.toUserImgView.symbol(urlStr: giftData.msgModel.toUser.headPic)
        }
    }
