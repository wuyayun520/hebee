...
fileprivate let user_sortMirrorStr:String = "TSystenear precise critical"
fileprivate let showReceiveTotalString:String = "position mitt giftCell"

/*: "init(coder:) has not been implemented" :*/
fileprivate let mainViaRankId:[UInt8] = [0x64,0x65,0x74,0x6e,0x65,0x6d,0x65,0x6c,0x70,0x6d,0x69,0x20,0x6e,0x65,0x65,0x62,0x20,0x74,0x6f,0x6e,0x20,0x73,0x61,0x68,0x20,0x29,0x3a,0x72,0x65,0x64,0x6f,0x63,0x28,0x74,0x69,0x6e,0x69]

/*: "icon_talk_notic" :*/
fileprivate let userPhotoStr:String = "denycon"
fileprivate let kConDespiteFormat:String = "yesterday specie span_notic"
...
@objc public protocol SkipBridge: NSObjectProtocol {
    /**
     *  长按消息回调
     ...
     *  @param cell 委托者，消息单元
     */
    //: func onLongPressMessage(cell: OffUsed)
    func okBurn(cell: OffUsed)
     ...
     *  @param cell 委托者，消息单元
     */
    //: func onRetryMessage(cell: OffUsed)
    func behaviorMethod(cell: OffUsed)
     ...
     *  @param cell 委托者，消息单元
     */
    //: func onSelectMessage(cell: OffUsed)
    func valid(cell: OffUsed)
     ...
     *  @param cell 委托者，消息单元
     */
    //: func onSelectMessageAvatar(cell: OffUsed)
    func anyPromise(cell: OffUsed)

    /**
     *  点击消息单元中引用消息
     *  @param cell 委托者，消息单元
     */
    //: func onReplyMessage(cell: OffUsed)
    func searched(cell: OffUsed)
}

//: @objcMembers public class OffUsed: TCommonTableViewCell {
@objcMembers public class OffUsed: NameWindowOperation {
    /**
     *  信息数据类。
     *  存储了该massageCell中所需的信息。包括发送者 ID，发送者头像、信息发送状态、信息气泡图标等。
     *  messageData 详细信息请参考：Section\Chat\CellData\TaskAction.h
     */
    //: public var messageData: TaskAction?
    public var messageData: TaskAction?

    /// 复用cell标识符
    //: @objc public static let TSystemMessageCell_ReuseId = "TSystemMessageCell_ReuseId"
    public static let TSystemMessageCell_ReuseId = (String(user_sortMirrorStr.prefix(6)) + "mMessage" + String(showReceiveTotalString.suffix(4)) + "_ReuseId")

    /**
     *  协议委托
     *  负责实现 SkipBridge 协议中的功能。
     */
    //: public weak var delegate: TMessageCellDelegate?
    public weak var delegate: SkipBridge?
    ...
    required init?(coder: NSCoder) {
        ...
        fatalError(String(bytes: mainViaRankId.reversed(), encoding: .utf8)!)
    }
    ...
    override public func associated(with data: MarkerRole) {
        //: super.fill(with: data)
        super.associated(with: data)
        //: let newData: TaskAction = data as! TaskAction
        let newData: TaskAction = data as! TaskAction
    ...
    override public func updateConstraints() {
        ...
        let csize = self.messageData?.cancel()
    ...
    lazy var avatarView: UIImageView = {
        ...
        let tap1 = UITapGestureRecognizer(target: self, action: #selector(request(recognizer:)))
    ...
    lazy var nameLabel: UILabel = {
        ...
        lb.font = UIFont.theSearcher(fontSize: 13)
        //: lb.textColor = .gray
        lb.textColor = .gray
    ...
    public lazy var container: UIView = {
        ...
        let tap = UITapGestureRecognizer(target: self, action: #selector(contrastDeploy(recognizer:)))
        ...
        let longPress = UILongPressGestureRecognizer(target: self, action: #selector(account(recognizer:)))
    ...
    lazy var retryView: UIImageView = {
        ...
        imagV.image = UIImage.installBind(name: (userPhotoStr.replacingOccurrences(of: "deny", with: "i") + "_talk" + String(kConDespiteFormat.suffix(6))))
        ...
        let resendTap = UITapGestureRecognizer(target: self, action: #selector(denseTool(recognizer:)))
...
extension OffUsed {
    ...
    @objc func request(recognizer: UIGestureRecognizer) {
        //: self.delegate?.onSelectMessageAvatar(cell: self)
        self.delegate?.anyPromise(cell: self)
    }

    //: @objc func onLongPress(recognizer: UIGestureRecognizer) {
    @objc func account(recognizer: UIGestureRecognizer) {
    ...
    @objc func denseTool(recognizer: UIGestureRecognizer) {
        //: if self.messageData?.status == .Msg_Status_Fail {
        if self.messageData?.status == .Msg_Status_Fail {
            //: self.delegate?.onRetryMessage(cell: self)
            self.delegate?.behaviorMethod(cell: self)
        }
    }

    //: @objc func onSelectMessage(recognizer: UIGestureRecognizer) {
    @objc func contrastDeploy(recognizer: UIGestureRecognizer) {
