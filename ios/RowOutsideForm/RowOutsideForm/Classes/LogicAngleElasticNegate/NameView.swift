...
fileprivate let data_choosePath:[UInt8] = [0x1f,0x18,0x1f,0x2,0x5e,0x15,0x19,0x12,0x13,0x4,0x4c,0x5f,0x56,0x1e,0x17,0x5,0x56,0x18,0x19,0x2,0x56,0x14,0x13,0x13,0x18,0x56,0x1f,0x1b,0x6,0x1a,0x13,0x1b,0x13,0x18,0x2,0x13,0x12]

private func goingMend(agent num: UInt8) -> UInt8 {
    return num ^ 118
}

/*: "icon_male_default" :*/
fileprivate let showAgainstData:[UInt8] = [0x10,0x1a,0x16,0x17,0x26,0x14,0x18,0x15,0x1c,0x26,0x1d,0x1c,0x1f,0x18,0xc,0x15,0xd]

/*: "icon_female_default" :*/
fileprivate let dataPreventionKey:String = "robot easternicon_"
fileprivate let data_siblingUrl:String = "_defkey associate"

/*: "level_%@" :*/
fileprivate let app_retCreationConnectTitle:String = "deploy drownlevel_%"
fileprivate let app_arrowUrl:[Character] = ["@"]
...
class NameView: UIView {
    ...
    required init?(coder: NSCoder) {
        ...
        fatalError(String(bytes: data_choosePath.map{goingMend(agent: $0)}, encoding: .utf8)!)
    }
    ...
    lazy var nickLabel = UILabel().then {
        ...
        $0.font = .statinEssay(type: .Medium, fontSize: 17)
        //: $0.textColor = .white
        $0.textColor = .white
    }
    ...
    lazy var ageLabel = UILabel().then {
        ...
        $0.font = .statinEssay(type: .Regular, fontSize: 10)
        //: $0.textColor = .white
        $0.textColor = .white
    }
...
extension NameView {
    ...
    func asFactoryShould(model: ListUntil) {
        //: headerPic.kf.setImage(with: URL(string: model.headPic), placeholder: UIImage.placeImgSquare())
        headerPic.kf.setImage(with: URL(string: model.headPic), placeholder: UIImage.lemma())
        ...
        let imgStr = model.sex == ChannelDown.male.rawValue ? String(bytes: showAgainstData.map{$0^121}, encoding: .utf8)! : (String(dataPreventionKey.suffix(5)) + "female" + String(data_siblingUrl.prefix(4)) + "ault")
        //: sexImgView.image = UIImage(named: imgStr)
        sexImgView.image = UIImage(named: imgStr)
        //: let levelStr = String(format: "level_%@", model.level)
        let levelStr = String(format: (String(app_retCreationConnectTitle.suffix(7)) + String(app_arrowUrl)), model.level)
