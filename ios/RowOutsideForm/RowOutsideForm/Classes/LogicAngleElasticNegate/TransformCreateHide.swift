...
fileprivate let main_mayValue:[UInt8] = [0x5d,0x5a,0x5d,0x40,0x1c,0x57,0x5b,0x50,0x51,0x46,0xe,0x1d,0x14,0x5c,0x55,0x47,0x14,0x5a,0x5b,0x40,0x14,0x56,0x51,0x51,0x5a,0x14,0x5d,0x59,0x44,0x58,0x51,0x59,0x51,0x5a,0x40,0x51,0x50]

private func stretchCollect(seed num: UInt8) -> UInt8 {
    return num ^ 52
}

/*: "icon_chats_banner_subtract" :*/
fileprivate let const_effMsg:String = "icfacen"
fileprivate let showRemindRegulateExplainMsg:String = "or attention need light_banner"
fileprivate let app_conTitle:[Character] = ["_","s","u","b","t","r","a","c","t"]

/*: "icon_chats_banner_subtract_pre" :*/
fileprivate let noti_canvasPath:[Character] = ["i","c","o","n","_","c","h","a","t","s","_","b","a","n","n","e","r","_","s","u","b","t","r","a","c","t","_","p","r"]
fileprivate let show_sumString:String = "upon"

/*: "bannerList" :*/
fileprivate let appProductMessage:String = "analyzenn"

/*: "JXBannerCellID" :*/
fileprivate let dataPositionYourselfTitle:[Character] = ["J","X","B","a","n"]
fileprivate let mainOvalNobodyName:String = "expression and define parentnerCellID"

/*: "#FFFFFF" :*/
fileprivate let constProtestClipValue:String = "straighten skip mutual pet#FFFFFF"
...
protocol TextureExceptChapter: NSObject {
    //: func changeBannerViewFream(isMiniView: Bool)
    func respective(isMiniView: Bool)
}

//: class TalkingPrivateBannerView: UIView {
class TransformCreateHide: UIView {
    //: var isMiniView = false
    var isMiniView = false
    //: open weak var delegate: PrivateBannerDelegate?
    open weak var delegate: TextureExceptChapter?
    ...
    required init?(coder: NSCoder) {
        ...
        fatalError(String(bytes: main_mayValue.map{stretchCollect(seed: $0)}, encoding: .utf8)!)
    }
    ...
    private lazy var bannerListData: [StatusModel] = //: return Array<StatusModel>()
        .init()
    //: }()
    ...
    private lazy var closeBtn: UIButton = {
        ...
        btn.setImage(UIImage.installBind(name: (const_effMsg.replacingOccurrences(of: "face", with: "o") + "_chats" + String(showRemindRegulateExplainMsg.suffix(7)) + String(app_conTitle))), for: .normal)
        //: btn.setImage(UIImage.BundleImageNamed(name: "icon_chats_banner_subtract_pre"), for: .selected)
        btn.setImage(UIImage.installBind(name: (String(noti_canvasPath) + show_sumString.replacingOccurrences(of: "upon", with: "e"))), for: .selected)
        //: btn.addTarget(self, action: #selector(closeBtnClick), for: .touchUpInside)
        btn.addTarget(self, action: #selector(conversionBy), for: .touchUpInside)
        //: return btn
        return btn
        //: }()
    }()
...
extension TransformCreateHide {
    /// banner
    //: func reqBanner() {
    func ex() {
        //: SocialRequestManager.init().req_adBanner(position: 3) { [weak self] succeed, result, errorModel in
        WrapBuildContent().maleFor(position: 3) { [weak self] succeed, result, errorModel in
            ...
            let array = JSON(result[(appProductMessage.replacingOccurrences(of: "analyze", with: "ba") + "erList")] ?? []).arrayValue
...
extension TransformCreateHide {
    ...
    @objc private func conversionBy() {
        ...
        self.delegate?.respective(isMiniView: isMiniView)
    }

    //: func resetView(isMiniView: Bool) {
    func belowAll(isMiniView: Bool) {
        //: if isMiniView {
        if isMiniView {
...
extension TransformCreateHide: JXBannerDelegate, JXBannerDataSource {
    ...
    func jxBanner(_ banner: JXBannerType) -> (JXBannerCellRegister) {
        ...
        return JXBannerCellRegister(type: JXBannerCell.self, reuseIdentifier: (String(dataPositionYourselfTitle) + String(mainOvalNobodyName.suffix(9))))
    }
    ...
    func jxBanner(_ banner: JXBannerType, cellForItemAt index: Int, cell: UICollectionViewCell) -> UICollectionViewCell {
        ...
        tempCell.imageView.spread(urlStr: url, placeImg: UIImage.lemma())
                  ...
                  builder: JXBannerPageControlBuilder) -> JXBannerPageControlBuilder
    {
        ...
        pageControl.activeColor = UIColor(hex: (String(constProtestClipValue.suffix(7))))!
...
extension TransformCreateHide {
    ...
    private func noneMomentumMargin() {
