...
/*: "type" :*/
fileprivate let dataFamilyString:String = "typking"

/*: "You are currently on a call, please try again later" :*/
fileprivate let constSaveerCanvasFreshMessage:[UInt8] = [0xb6,0x80,0x9a,0xcf,0x8e,0x9d,0x8a,0xcf,0x8c,0x9a,0x9d,0x9d,0x8a,0x81,0x9b,0x83,0x96,0xcf,0x80,0x81,0xcf,0x8e,0xcf,0x8c,0x8e,0x83,0x83,0xc3,0xcf,0x9f,0x83,0x8a,0x8e,0x9c,0x8a,0xcf,0x9b,0x9d,0x96,0xcf,0x8e,0x88,0x8e,0x86,0x81,0xcf,0x83,0x8e,0x9b,0x8a,0x9d]

/*: "onCallResume" :*/
fileprivate let kYetAlwaysMsg:String = "race min radio lemmaonCa"
...
// __CLOSE_PRINT__
//
//  VideoMagnitudeerval+VideoChat.swift
//  AbroadTalking
//
...
extension VideoMagnitudeerval {
    ...
    func beforeToInstall(info: [String: Any], situation: SessionBehavior = .Start) {
        //: let json = JSON(info)
        let json = JSON(info)
        //: let type = json["type"].intValue
        let type = json[(dataFamilyString.replacingOccurrences(of: "king", with: "e"))].intValue
        ...
        if type == 0 { // 视频
            ...
            (vc as! ShadeRounding).chatModel = RespectiveModel.graphEnvironment(info: info, situation: situation)
            //: } else {
        } else { // 音频
            //: vc = TalkingVoiceChatViewController()
            vc = EmptyContainerScientific()
            //: (vc as! TalkingVoiceChatViewController).chatModel = TalkingVideoChatModel.getVideoModel(info: info, situation: situation)
            (vc as! EmptyContainerScientific).chatModel = RespectiveModel.graphEnvironment(info: info, situation: situation)
        }

        //: self.tryToPushTalkRoom(roomVC: vc!, isVideoTalk: (type == 0 ? true : false), resumeTalk: false, resumeData: [:])
        self.moreEnableBreeze(roomVC: vc!, isVideoTalk: type == 0 ? true : false, resumeTalk: false, resumeData: [:])
    }

    /// 接收到视频通话
    /// - Parameter info: 数据
    //: func recieve1v1TalkRequest(info: [String: Any]) {
    func relation(info: [String: Any]) {
        //: handle1v1AudioRequest(info: info, resumeTalk: false)
        setShout(info: info, resumeTalk: false)
    }

    /// 恢复视频通话
    /// - Parameter info: 数据
    //: func recieve1v1TalkResumeRequest(info: [String: Any]) {
    func noneJudge(info: [String: Any]) {
        //: handle1v1AudioRequest(info: info, resumeTalk: true)
        setShout(info: info, resumeTalk: true)
    }
}
...
extension VideoMagnitudeerval {
    ...
    private func setShout(info: [String: Any], resumeTalk: Bool) {
        //: if !resumeTalk && TalkingSocketManager.shared.versusScope {
        if !resumeTalk && QuantityensityAction.shared.versusScope {
            //: self.func__showStatusBarErrorMsg(showMsg: "You are currently on a call, please try again later".localized)
            self.conclusion(showMsg: String(bytes: constSaveerCanvasFreshMessage.map{$0^239}, encoding: .utf8)!.localized)
            //: return
            return
        }
        ...
        if congressionalFor()!.isKind(of: ShadeRounding.self) ||
            //: currentViewController()!.isKind(of: TalkingVoiceChatViewController.self) ||
            congressionalFor()!.isKind(of: EmptyContainerScientific.self) ||
            //: currentViewController()!.isKind(of: TalkingVideoInitivCallVC.self) {
            congressionalFor()!.isKind(of: MissionClean.self)
        {
            //: return
            return
        }
        ...
        if (HoldTask.shared as! HoldTask).inviteCallView != nil {
            //: (TalkingApplication.shared as! TalkingApplication).inviteCallView?.closeView()
            (HoldTask.shared as! HoldTask).inviteCallView?.aroundAttach()
        }
        ...
        if type == 0 { // 视频通话
            ...
            (vc as! ShadeRounding).chatModel = RespectiveModel.graphEnvironment(info: info, situation: resumeTalk ? .Resume : .Recieved)
            //: } else {
        } else { // 语音通话
            //: vc = TalkingVoiceChatViewController()
            vc = EmptyContainerScientific()
            ...
            (vc as! EmptyContainerScientific).chatModel = RespectiveModel.graphEnvironment(info: info, situation: resumeTalk ? .Resume : .Recieved)
        }

        //: let callResume = json["onCallResume"].stringValue
        let callResume = json[(String(kYetAlwaysMsg.suffix(4)) + "llResume")].stringValue
        ...
        if resumeTalk, callResume.count > 0, callResume == "1" { // 恢复通话
            ...
            DispatchQueue.main.asyncAfter(deadline: DispatchTime.now() + 1) {
                ...
                self.moreEnableBreeze(roomVC: vc!, isVideoTalk: type == 0 ? true : false, resumeTalk: resumeTalk, resumeData: info)
            }
            //: } else {
        } else { // 构建音视频通话
            //: self.tryToPushTalkRoom(roomVC: vc!, isVideoTalk: (type == 0 ? true : false), resumeTalk: resumeTalk, resumeData: info)
    ...
    private func moreEnableBreeze(roomVC: UIViewController, isVideoTalk: Bool, resumeTalk: Bool, resumeData: [String: Any]) {
        //: guard var currentVc = currentViewController() else { return }
        guard var currentVc = congressionalFor() else { return }
        ...
        if currentVc.isKind(of: UINavigationController.self) {
            ...
            //: let rootVc = FormatDistribution.getWindow().rootViewController as! UITabBarController
            let rootVc = FormatDistribution.group().rootViewController as! UITabBarController
        ...
        if !isVideoTalk {
            ...
            talkClass = NSStringFromClass(EmptyContainerScientific.self)
            //: } else {
        } else {
            //: talkClass = NSStringFromClass(TalkingVideoChatViewController.self)
            talkClass = NSStringFromClass(ShadeRounding.self)
        }
        //: var chatIndex = -1
        var chatIndex = -1
        //: var index = -1
        ...
        if isVideoTalk {
            ...
            if currentChatVc != nil, currentChatVc?.chatModel?.pairUid == pushChatVc.chatModel?.pairUid {
                ...
                if resumeTalk {
                    ...
                    currentChatVc?.doinger(resumeData: resumeData)
                }
                //: return
                return
            }
            ...
            if navVc!.isKind(of: FinishController.self) {
                //: let nav = navVc as! TalkingNavigationController
                let nav = navVc as! FinishController
            ...
            var currentChatVc: EmptyContainerScientific?
            //: let pushChatVc = roomVC as! TalkingVoiceChatViewController
            let pushChatVc = roomVC as! EmptyContainerScientific

            //: if chatIndex >= 0 {
            if chatIndex >= 0 {
                //: currentChatVc = navVc?.viewControllers[chatIndex] as? TalkingVoiceChatViewController
                currentChatVc = navVc?.viewControllers[chatIndex] as? EmptyContainerScientific
            }
            ...
            if currentChatVc != nil, currentChatVc?.chatModel?.pairUid == pushChatVc.chatModel?.pairUid {
                ...
                if resumeTalk {
                    //: currentChatVc?.updateInfoViaResumeTalkData(resumeData: resumeData)
                    currentChatVc?.exceptResBottom(resumeData: resumeData)
                }
                //: return
                return
            }
