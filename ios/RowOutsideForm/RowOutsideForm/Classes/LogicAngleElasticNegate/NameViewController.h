 ...
 *
 *  RoundingInstruction 协议和 NameViewController 类。
 *  NameViewController 是聊天界面的核心类，负责聊天界面大部分核心业务逻辑的处理与运算。
 *  该类的详细属性与功能，请参展本文件中 NameViewController 的具体声明。
 *
 ******************************************************************************/
...
@protocol RoundingInstruction <NSObject>
...
- (void)directlied:(NameViewController *)controller;
...
- (void)posted:(NameViewController *)controller;
...
- (BOOL)panTo:(NameViewController *)controller around:(UIView *)view;
 ...
 */
//: - (TaskAction *)messageController:(TUIMessageController *)controller onNewMessage:(V2TIMMessage *)data;
- (TaskAction *)over:(NameViewController *)controller commonPersist:(V2TIMMessage *)data;
 ...
 */
//: - (OffUsed *)messageController:(TUIMessageController *)controller onShowMessageData:(TaskAction *)data;
- (OffUsed *)margin:(NameViewController *)controller service:(TaskAction *)data;
 ...
 *  @param cell 所点击的消息单元
 */
//: - (void)messageController:(TUIMessageController *)controller onSelectMessageAvatar:(OffUsed *)cell;
- (void)might:(NameViewController *)controller cluster:(OffUsed *)cell;
 ...
 *  @param cell 所点击的消息单元
 */
//: - (void)messageController:(TUIMessageController *)controller onSelectMessageContent:(OffUsed *)cell;
- (void)phase:(NameViewController *)controller distribution:(OffUsed *)cell;
 ...
 */
//: - (void)messageController:(TUIMessageController *)controller onSelectMessageMenu:(NSInteger)menuType withData:(TaskAction *)data;
- (void)away:(NameViewController *)controller radioEdge:(NSInteger)menuType dismiss:(TaskAction *)data;
/**
...
/*重发*/
//: - (void)messageController:(TUIMessageController *)controller onRetryMessageContent:(OffUsed *)cell;
- (void)nextTemporary:(NameViewController *)controller everyCommon:(OffUsed *)cell;

/*引用*/
//: - (void)messageController:(TUIMessageController *)controller onReplyMessage:(TaskAction *)cellData;
- (void)betweenPet:(NameViewController *)controller attribute:(TaskAction *)cellData;

/*点击引用*/

//: - (void)messageController:(TUIMessageController *)controller onClickReplyMessage:(TaskAction *)cellData;
- (void)roundParticle:(NameViewController *)controller enter:(TaskAction *)cellData;
...
/**
 *  执行 RoundingInstruction 协议的委托
 */
//: @property (nonatomic, weak) id<TMessageControllerDelegate> delegate;
@property (nonatomic, weak) id<RoundingInstruction> delegate;
 ...
 * @param uiMsgs 待删除的数据
 */
//- (void)deleteMessages:(NSArray<TaskAction *> *)uiMsgs;
