...
fileprivate let data_outerTitle:[UInt8] = [0x64,0x65,0x74,0x6e,0x65,0x6d,0x65,0x6c,0x70,0x6d,0x69,0x20,0x6e,0x65,0x65,0x62,0x20,0x74,0x6f,0x6e,0x20,0x73,0x61,0x68,0x20,0x29,0x3a,0x72,0x65,0x64,0x6f,0x63,0x28,0x74,0x69,0x6e,0x69]

/*: "ExclusiveWrap" :*/
fileprivate let data_sophisticatedValue:[Character] = ["D","r","o","p","M","e","m","b"]
fileprivate let show_exclusiveKey:[Character] = ["e","r","M","e","n","u","C","e","l","l"]

/*: "roomId" :*/
fileprivate let k_losePlainInvestigatorFormat:[Character] = ["r"]
fileprivate let mainBounceTitle:[Character] = ["o","o","m","I","d"]

/*: "type" :*/
fileprivate let dataDirectUrl:[Character] = ["t","y","p","e"]

/*: "page" :*/
fileprivate let dataAwakeUrl:[Character] = ["p","a","g","e"]

/*: "icon_chats_pcr" :*/
fileprivate let show_presentIllegalMessage:[Character] = ["i"]
fileprivate let appVolumeTitle:[Character] = ["c","o","n","_","c","h","a","t","s","_","p","c","r"]

/*: "All Numbers" :*/
fileprivate let k_favoriteDataName:String = "All Nureporter both"
fileprivate let constTollKey:String = "mbshs"

/*: "all" :*/
fileprivate let constServerPath:[Character] = ["a","l","l"]

/*: "Recent Gifts" :*/
fileprivate let show_sSessionUrl:String = "Recent Gtoday double try fade migration"
fileprivate let noti_recentUndertakeName:[Character] = ["i","f","t","s"]
...
class StageTag: UIView {
    //: var pageIndex = 0
    var pageIndex = 0
    //: var roomID = ""
    var roomID = ""
    //: var menuDelegate: DropDownMemberMenuViewDelegate?
    var menuDelegate: HardFormat?
    ...
    fileprivate lazy var contentView: UIView = self.successLikeBound()
    //: fileprivate lazy var mTable: UITableView = self.makeTableView()
    fileprivate lazy var mTable: UITableView = self.despite()
    //: fileprivate var anchorRect = CGRect.zero
    fileprivate var anchorRect = CGRect.zero

    //: static let shared = DropDownMemberMenuView(frame: .zero)
    static let shared = StageTag(frame: .zero)
    ...
    required init?(coder: NSCoder) {
        ...
        fatalError(String(bytes: data_outerTitle.reversed(), encoding: .utf8)!)
    }

    //: private func makeContentView() -> UIView {
    private func successLikeBound() -> UIView {
        //: let view  = UIView()
        let view = UIView()
        //: return view
        return view
    ...
    private func despite() -> UITableView {
    ...
    private func detectScale() {
        ...
        mTable.register(ExclusiveWrap.self, forCellReuseIdentifier: (String(data_sophisticatedValue) + String(show_exclusiveKey)))
    ...
    func monthDownSkirt() {
        ...
        var dict = [String: Any]()
        //: dict["roomId"] = roomID
        dict[(String(k_losePlainInvestigatorFormat) + String(mainBounceTitle))] = roomID
        //: dict["type"] = "3"
        dict[(String(dataDirectUrl))] = "3"
        //: dict["page"] = String(pageIndex)
        dict[(String(dataAwakeUrl))] = String(pageIndex)
        ...
        FlexibleTiming.share.switcherBoard(params: dict, completion: { array in
            ...
            if self.pageIndex == 0 {
                ...
                let model = IndicatorMakeModel()
                //: model.headPic = "icon_chats_pcr"
                model.headPic = (String(show_presentIllegalMessage) + String(appVolumeTitle))
                //: model.nickname = "All Numbers".localized
                model.nickname = (String(k_favoriteDataName.prefix(6)) + constTollKey.replacingOccurrences(of: "sh", with: "er")).localized
                //: model.uid = "all"
                model.uid = (String(constServerPath))
    ...
    private func regulateAccessible() {
...
extension StageTag: UITableViewDelegate, UITableViewDataSource {
    ...
    public func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        let cell = tableView.dequeueReusableCell(withIdentifier: (String(data_sophisticatedValue) + String(show_exclusiveKey)), for: indexPath) as! ExclusiveWrap
        ...
        if recentmenuArray.count > 0 && indexPath.section == 0 {
            ...
            cellModel = recentmenuArray[indexPath.row] as! IndicatorMakeModel
            //: }else {
        } else {
            //: cellModel = menuArray[indexPath.row] as! TalkingChatRoomMemberModel
            cellModel = menuArray[indexPath.row] as! IndicatorMakeModel
        }
        //: cell.setDropMemberMenuCell(model: cellModel)
        cell.evolutionCircle(model: cellModel)
        //: return cell
        return cell
    }
    ...
    public func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        menuDelegate?.buttEnableMeasure(model: cellModel)
    }

    //: func tableView(_ tableView: UITableView, viewForHeaderInSection section: Int) -> UIView? {
    func tableView(_ tableView: UITableView, viewForHeaderInSection section: Int) -> UIView? {
        //: let headeView = UILabel.init(frame: CGRect(x: 0, y: 0, width: ScreenWidth, height: 23))
        let headeView = UILabel(frame: CGRect(x: 0, y: 0, width: notiExploreKey, height: 23))
        //: headeView.backgroundColor = UIColor.appBgColor()
        headeView.backgroundColor = UIColor.counterrupt()
        //: let titleLB = UILabel.init()
        let titleLB = UILabel()
        //: titleLB.frame = CGRect(x: 3, y: 5, width: 100, height: 14)
        titleLB.frame = CGRect(x: 3, y: 5, width: 100, height: 14)
        //: titleLB.font = .pingfangFont(type: .Medium, fontSize: 12)
        titleLB.font = .statinEssay(type: .Medium, fontSize: 12)
        //: titleLB.textColor = .appValueDetailColor()
        titleLB.textColor = .mainSpread()
        //: if recentmenuArray.count > 0 && section == 0 {
        if recentmenuArray.count > 0 && section == 0 {
            //: titleLB.text = "Recent Gifts".localized
            titleLB.text = (String(show_sSessionUrl.prefix(8)) + String(noti_recentUndertakeName)).localized
            //: }else {
        } else {
            //: titleLB.text = "All Numbers".localized
            titleLB.text = (String(k_favoriteDataName.prefix(6)) + constTollKey.replacingOccurrences(of: "sh", with: "er")).localized
        }
