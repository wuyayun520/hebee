...
public protocol ToolPer: NSObject {
    /// 文字内容for cell
...
open class CollectionMulti: UICollectionViewLayout {
    // 标签的内边距
//    open var tagInnerMargin: CGFloat = 25
    ...
    open var itemFont = UIFont.statinEssay(type: .Regular, fontSize: 15)

    //: open weak var delegate: TagLayoutDelegate?
    open weak var delegate: ToolPer?
//    var titles = [String]()
    /// tag 内容的对齐方式
    //: open var contentAlignment = TagContentAlignment.left
    open var contentAlignment = SpineBulletHidden.left
    ...
    override open func prepare() {
        ...
        for section in 0 ..< sections {
            ...
            let sectionHeadSize = delegate.voice(self, collectionView: collectionView, sizeForSupplementaryElementOfKind: UICollectionView.elementKindSectionHeader, at: section)
            
            //: let bottomSpace = delegate.tagLayout(self, collectionView: collectionView, heightBottomAt: section)
            let bottomSpace = delegate.under(self, collectionView: collectionView, heightBottomAt: section)

            //: let itemLeftSpace = delegate.tagLayout(self, collectionView: collectionView, leftSapceAt: section)
            let itemLeftSpace = delegate.accessible(self, collectionView: collectionView, leftSapceAt: section)
            ...
            for item in 0 ..< rows {
                ...
                let text = delegate.exercise(self, collectionView: collectionView, textForItemAt: indexPath)
                //: let tagInnerMargin = delegate.tagLayout(self, collectionView: collectionView, tagInnerMarginForItemAt: indexPath)
                let tagInnerMargin = delegate.qualityWeekly(self, collectionView: collectionView, tagInnerMarginForItemAt: indexPath)
                //: let tagWidth = textWidth(text) + tagInnerMargin
                let tagWidth = selected(text) + tagInnerMargin
...
//        return visibleLayoutAttributes
    ...
    func selected(_ text: String) -> CGFloat {
