...
fileprivate let dataIllustrateFuneralMessage:[UInt8] = [0x71,0x76,0x71,0x6c,0x30,0x7b,0x77,0x7c,0x7d,0x6a,0x22,0x31,0x38,0x70,0x79,0x6b,0x38,0x76,0x77,0x6c,0x38,0x7a,0x7d,0x7d,0x76,0x38,0x71,0x75,0x68,0x74,0x7d,0x75,0x7d,0x76,0x6c,0x7d,0x7c]

private func bubbleSuccessful(greet num: UInt8) -> UInt8 {
    return num ^ 24
}

/*: "F5F5F5" :*/
fileprivate let data_showString:String = "Fslowslow"
fileprivate let appServerTransformFormat:String = "5"
...
class OrientationMaximum: UITableViewCell {
    ...
    required init?(coder aDecoder: NSCoder) {
        ...
        fatalError(String(bytes: dataIllustrateFuneralMessage.map{bubbleSuccessful(greet: $0)}, encoding: .utf8)!)
    }
    ...
    func will(isListTableCell: Bool) {
    ...
    lazy var userInfoView: QuerySelecter = {
        //: let  view = TalkingMomentUserInfoView.init()
        let view = QuerySelecter()
    ...
    lazy var textContentView: SessionView = {
    ...
    lazy var photoView: MemberCreate = {
        ...
        let view = MemberCreate(frame: CGRect(x: 0, y: 0, width: 0, height: 0), collectionViewLayout: flayout)
        //: backView.addSubview(view)
        backView.addSubview(view)
        //: return view
        return view
    ...
    lazy var lineView: UIView = {
        ...
        view.backgroundColor = UIColor(hex: (data_showString.replacingOccurrences(of: "slow", with: "5F") + appServerTransformFormat.capitalized))
        //: self.contentView.addSubview(view)
        self.contentView.addSubview(view)
        //: return view
        return view
...
//            }
            ...
            photoView.prompt(model: cunrrenModel)
...
//            }
            ...
            videoView.towardVisible(model: model.videoInfo!, isTop: model.pinStatus)
        }

        //: bottomView.configModel(model: cunrrenModel)
        bottomView.music(model: cunrrenModel)
...
//        }
    ...
    @objc func find() {
        ...
        let vc = ConditionViewController(videoPath: cunrrenModel.videoInfo!.videoUrl)
        //: if cunrrenModel.sex != VideoMagnitudeerval.share.loginUserMode.sex {
        if cunrrenModel.sex != VideoMagnitudeerval.share.loginUserMode.sex {
    ...
    func choice() {
        ...
        if cunrrenModel.momentType == ModeEmpty.Video.rawValue {
...
extension OrientationMaximum {
    ...
    private func ready() {
        ...
        let tapGesture = UITapGestureRecognizer(target: self, action: #selector(find))
