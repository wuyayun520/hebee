...
fileprivate let kCorrectKey:[UInt8] = [0xa,0xd,0xa,0x17,0x4b,0x0,0xc,0x7,0x6,0x11,0x59,0x4a,0x43,0xb,0x2,0x10,0x43,0xd,0xc,0x17,0x43,0x1,0x6,0x6,0xd,0x43,0xa,0xe,0x13,0xf,0x6,0xe,0x6,0xd,0x17,0x6,0x7]

private func neverSeed(stop num: UInt8) -> UInt8 {
    return num ^ 99
}

/*: "Birthday" :*/
fileprivate let kBlocDragUrl:String = "Birtgender button panel product"

/*: "btn_me_edit" :*/
fileprivate let main_destroyData:String = "ogre merge employee enjoybtn_me"
fileprivate let const_writeEquityRatingStr:[Character] = ["_","e","d","i","t"]
...
class WithoutView: UITableViewCell, ScaleExecute {
    //: var myDateView: TalkingDateView!
    var myDateView: BrownView!
    ...
    required init?(coder aDecoder: NSCoder) {
        ...
        fatalError(String(bytes: kCorrectKey.map{neverSeed(stop: $0)}, encoding: .utf8)!)
    }
    ...
    lazy var titleLB: UILabel = {
        ...
        lb.text = (String(kBlocDragUrl.prefix(4)) + "hday").localized
        //: return lb
        return lb
        //: }()
    }()
    ...
    lazy var editImag: UIImageView = {
        ...
        let imag = UIImageView(image: UIImage.installBind(name: (String(main_destroyData.suffix(6)) + String(const_writeEquityRatingStr))))
        //: return imag
        return imag
        //: }()
    }()
    ...
    lazy var dataBtn: UIButton = {
        ...
        //: dataBtn.setTitle(VideoMagnitudeerval.share.loginUserMode.birthday, for: .normal)
        dataBtn.setTitle(VideoMagnitudeerval.share.loginUserMode.birthday, for: .normal)
        //: dataBtn.setTitleColor(UIColor.appTitleColor(), for: .normal)
        dataBtn.setTitleColor(UIColor.serviceMajor(), for: .normal)
        //: dataBtn.titleLabel?.font = UIFont.pingfangFont(type: .Medium, fontSize: 16)
        dataBtn.titleLabel?.font = UIFont.statinEssay(type: .Medium, fontSize: 16)
        //: dataBtn.addTarget(self, action: #selector(dataBtnClick), for: .touchUpInside)
        dataBtn.addTarget(self, action: #selector(create), for: .touchUpInside)
        //: return dataBtn
        return dataBtn
        //: }()
    }()
    ...
    @objc func create() {
        ...
        if parts?.count ?? 0 >= 3 {
            ...
            DispatchQueue.main.asyncAfter(deadline: .now() + 0.8) {
                ...
                self.myDateView.sayProcessing(year: year, month: month, day: day)
            }
        }
    }
