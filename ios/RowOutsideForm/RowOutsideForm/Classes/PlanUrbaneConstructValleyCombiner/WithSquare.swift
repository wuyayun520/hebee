...
/*: "cellID" :*/
fileprivate let const_entertainmentBoardStr:[Character] = ["c","e","l","l","I","D"]

/*: "init(coder:) has not been implemented" :*/
fileprivate let data_imageKey:[UInt8] = [0xd6,0xd1,0xd6,0xcb,0x97,0xdc,0xd0,0xdb,0xda,0xcd,0x85,0x96,0x9f,0xd7,0xde,0xcc,0x9f,0xd1,0xd0,0xcb,0x9f,0xdd,0xda,0xda,0xd1,0x9f,0xd6,0xd2,0xcf,0xd3,0xda,0xd2,0xda,0xd1,0xcb,0xda,0xdb]

private func spreadBind(merge num: UInt8) -> UInt8 {
    return num ^ 191
}

/*: "Video Call" :*/
fileprivate let data_sunOwnString:[Character] = ["V","i","d","e","o"," ","C","a","l","l"]

/*: "Video Call (%@ Points/Min)" :*/
fileprivate let app_modId:[Character] = ["V","i","d","e","o"," ","C","a","l","l"," ","(","%","@"," ","P","o","i"]
fileprivate let user_capAdditionMessage:[Character] = ["n","t","s","/","M","i","n",")"]

/*: "Voice Call" :*/
fileprivate let userGreatConversationTitle:String = "Voicetarget public star plat happy"
fileprivate let noti_radioFormat:[Character] = [" ","C","a","l","l"]

/*: "Voice Call (%@ Points/Min)" :*/
fileprivate let const_shareKey:[Character] = ["V","o","i","c","e"," ","C","a","l","l"," ","(","%"]
fileprivate let user_reliabilityData:String = "written jar original versus self@ Poi"

/*: "#F8F8F8" :*/
fileprivate let app_drawData:String = "others info identity#F8F8F8"

/*: "Cancel" :*/
fileprivate let appHideDecideMessage:String = "super lastCance"
fileprivate let kJustName:String = "bar"

/*: "#F5F5F5" :*/
fileprivate let dataCountroStr:[Character] = ["#","F","5","F","5"]
fileprivate let appHemeId:[Character] = ["F","5"]

/*: "icon_lounge" :*/
fileprivate let kSterolData:[Character] = ["i","c","o","n","_","l","o","u","n","g","e"]
...
class WithSquare: UIView {
    //: var munuBlock: MunuBlock?
    var munuBlock: MunuBlock?

    //: private var type: MunuPopType = .Unknown
    private var type: SkinInput = .Unknown // 当前展示类型
    ...
    var cellTitleColor = UIColor.serviceMajor()
    /// cell字体大小
    //: var cellTitleFont = UIFont.pingfangFont(type: .Regular, fontSize: 16)
    var cellTitleFont = UIFont.statinEssay(type: .Regular, fontSize: 16)
    /// 是否有"Cancel"菜单
    //: var hasCancel = true
    var hasCancel = true
    /// cancel字体颜色
    //: var cancelColor = UIColor.appValueDetailColor()
    var cancelColor = UIColor.mainSpread()
    /// cancel字体大小
    //: var cancelTitleFont = UIFont.pingfangFont(type: .Regular, fontSize: 16)
    var cancelTitleFont = UIFont.statinEssay(type: .Regular, fontSize: 16)
    /// 标题
    //: var title: String?
    var title: String?
    /// 标题颜色
    //: var titleColor = UIColor.appThemeColor()
    var titleColor = UIColor.tress()
    /// 标题字体大小
    //: var titleFont = UIFont.pingfangMediumFont(fontSize: 16)
    var titleFont = UIFont.theSearcher(fontSize: 16)

    //: var tableView = UITableView()
    var tableView = UITableView()
    //: let cellID = "cellID"
    let cellID = (String(const_entertainmentBoardStr))
    ...
    fileprivate var footerViewHight: CGFloat {
        ...
        return hasCancel ? (footerViewLineHight + 50 + constPublisherValue) : constPublisherValue
    }
    ...
    required init?(coder: NSCoder) {
        ...
        fatalError(String(bytes: data_imageKey.map{spreadBind(merge: $0)}, encoding: .utf8)!)
    }

    //: func initwithList(cellTitleList: [String]!) {
    func street(cellTitleList: [String]!) {
        // 初始化
    ...
    func straighten(type: SkinInput, videoPrice: String?, voicePrice: String?) {
        //: self.type = type
        self.type = type
        //: if self.type == .Call {
        if self.type == .Call { // 音视频通话
            ...
            if VideoMagnitudeerval.share.appStatus == PromiseTime.normal.rawValue {
                ...
                if VideoMagnitudeerval.share.loginUserMode.sex == ChannelDown.male.rawValue {
                    ...
                    let videoAttrStr = String.explorationPressure(VideoPrice: Int(videoPrice ?? "0") ?? 0)
                    //: let voiceAttrStr = String.setVoicePriceStr(VoicePrice: Int(voicePrice ?? "0") ?? 0)
                    let voiceAttrStr = String.spreadExist(VoicePrice: Int(voicePrice ?? "0") ?? 0)
                    ...
                    let attributes = [.foregroundColor: UIColor.tress(), .font: UIFont.statinEssay(type: .Regular, fontSize: 14)] as [NSAttributedString.Key: Any]
                    //: let VideoCall = "Video Call".localized
                    let VideoCall = (String(data_sunOwnString)).localized
                    //: let videoStr = "Video Call (%@ Points/Min)".localizedArguments(videoPrice ?? "0")
                    let videoStr = (String(app_modId) + String(user_capAdditionMessage)).noneRage(videoPrice ?? "0")
                    ...
                    let VoiceCall = (String(userGreatConversationTitle.prefix(5)) + String(noti_radioFormat)).localized
                    //: let voiceStr = "Voice Call (%@ Points/Min)".localizedArguments(voicePrice ?? "0")
                    let voiceStr = (String(const_shareKey) + String(user_reliabilityData.suffix(5)) + "nts/Min)").noneRage(voicePrice ?? "0")
                ...
                self.cellTitleList = [(String(data_sunOwnString)).localized, (String(userGreatConversationTitle.prefix(5)) + String(noti_radioFormat)).localized]
            }
        }
    ...
    public func sumervalEntity() {
        ...
        tableView = UITableView(frame: CGRect(x: 0, y: app_keepId - tableViewHight, width: notiExploreKey, height: tableViewHight), style: .plain)
        ...
        tableView.register(OvalView.self, forCellReuseIdentifier: cellID)
        ...
        if title == nil {
            ...
            tableView.boaEffect(width: notiExploreKey,
...
extension WithSquare: UITableViewDelegate, UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        var cell = tableView.dequeueReusableCell(withIdentifier: cellID) as? OvalView
        //: if cell == nil {
        if cell == nil {
            //: cell = TitleCell(style: .default, reuseIdentifier: cellID)
            cell = OvalView(style: .default, reuseIdentifier: cellID)
        }

        //: if VideoMagnitudeerval.share.loginUserMode.sex == Gender.male.rawValue,
        if VideoMagnitudeerval.share.loginUserMode.sex == ChannelDown.male.rawValue,
           //: VideoMagnitudeerval.share.loginUserMode.loungePlus == true,
           VideoMagnitudeerval.share.loginUserMode.loungePlus == true,
           //: type == .Call {
           type == .Call
        {
        ...
        cell!.titleLab.font = cellTitleFont
        //: if self.type == .Call && VideoMagnitudeerval.share.appStatus == AppSkinStatus.normal.rawValue {
        if self.type == .Call && VideoMagnitudeerval.share.appStatus == PromiseTime.normal.rawValue {
    ...
    func tableView(_ tableView: UITableView, viewForFooterInSection section: Int) -> UIView? {
        ...
        let footerView = UIView(frame: CGRect(x: 0, y: 0, width: notiExploreKey, height: footerViewHight))
        //: let lineView = UIView(frame: CGRect(x: 0, y: 0, width: ScreenWidth, height: footerViewLineHight))
        let lineView = UIView(frame: CGRect(x: 0, y: 0, width: notiExploreKey, height: footerViewLineHight))
        //: lineView.backgroundColor = UIColor(hex: "#F8F8F8")
        lineView.backgroundColor = UIColor(hex: (String(app_drawData.suffix(7))))
        ...
        btn.frame = CGRect(x: 0, y: footerViewLineHight, width: notiExploreKey, height: 50)
        //: footerView.addSubview(btn)
        footerView.addSubview(btn)
        //: btn.setTitle("Cancel".localized, for: .normal)
        btn.setTitle((String(appHideDecideMessage.suffix(5)) + kJustName.replacingOccurrences(of: "bar", with: "l")).localized, for: .normal)
        ...
        btn.addTarget(self, action: #selector(mightBelow), for: .touchUpInside)
        //: return footerView
        return footerView
    }
    ...
    func tableView(_ tableView: UITableView, viewForHeaderInSection section: Int) -> UIView? {
        ...
        let headerView = UIView(frame: CGRect(x: 0, y: 0, width: notiExploreKey, height: headerViewHight))

        //: let titleLab = UILabel(frame: CGRect(x: 15, y: 0, width: ScreenWidth-30, height: headerViewHight))
        let titleLab = UILabel(frame: CGRect(x: 15, y: 0, width: notiExploreKey - 30, height: headerViewHight))
        ...
        let lineView = UIView(frame: CGRect(x: 0, y: headerViewHight - 1, width: notiExploreKey, height: 1))
        //: lineView.backgroundColor = UIColor.init(hex: "#F5F5F5")
        lineView.backgroundColor = UIColor(hex: (String(dataCountroStr) + String(appHemeId)))
...
class OvalView: UITableViewCell {
    ...
    lazy var vipIcon: UIImageView = {
        ...
        img.image = UIImage.installBind(name: (String(kSterolData)))
        //: img.isHidden = true
        img.isHidden = true
        //: return img
        return img
    ...
    lazy var lineLab: UILabel = {
        ...
        lab.backgroundColor = UIColor(hex: (String(dataCountroStr) + String(appHemeId)))
        //: return lab
        return lab
        //: }()
    }()
