...
fileprivate let notiVoiceMusicStr:[UInt8] = [0xd3,0xd4,0xd3,0xce,0x92,0xd9,0xd5,0xde,0xdf,0xc8,0x80,0x93,0x9a,0xd2,0xdb,0xc9,0x9a,0xd4,0xd5,0xce,0x9a,0xd8,0xdf,0xdf,0xd4,0x9a,0xd3,0xd7,0xca,0xd6,0xdf,0xd7,0xdf,0xd4,0xce,0xdf,0xde]

private func lockVariable(beauty num: UInt8) -> UInt8 {
    return num ^ 186
}

/*: "#AAAAAA" :*/
fileprivate let showThroughKey:String = "unique close catch mobile#AAAAA"
fileprivate let dataSubjectKey:[Character] = ["A"]

/*: "btn_chatsettings_unchoice" :*/
fileprivate let notiCessValue:String = "angle give quarterback temporary viewbtn_c"
fileprivate let data_shadeTitle:String = "eaddin"
fileprivate let appDecisionTitle:String = "CHOI"
fileprivate let show_areaUrl:String = "cspine"

/*: "btn_chatsettings_choiced" :*/
fileprivate let constAssignmentMinimumMessage:String = "distance buildbtn_c"
fileprivate let main_flowBalanceMessage:String = "tings_typical ref mask"
fileprivate let noti_curveData:String = "CHOICED"

/*: "Top" :*/
fileprivate let app_protectionPartValue:String = "half exceptionTop"

/*: "Save" :*/
fileprivate let const_kitPath:String = "head permissionSave"

/*: "Cancel" :*/
fileprivate let user_againString:String = "later normally tensionCancel"
...
class CreateDark: UIView {
    ...
    required init?(coder: NSCoder) {
        ...
        fatalError(String(bytes: notiVoiceMusicStr.map{lockVariable(beauty: $0)}, encoding: .utf8)!)
    }
    ...
    private lazy var contentView: UIView = {
        ...
        let height = actualWidth(w: 475) + constPublisherValue
        //: let v = UIView(frame: CGRect(x: 0, y: ScreenHeight-height, width: ScreenWidth, height: height))
        let v = UIView(frame: CGRect(x: 0, y: app_keepId - height, width: notiExploreKey, height: height))
        //: v.backgroundColor = .white
        v.backgroundColor = .white
        //: v.Corner(width: v.width, height: v.height, corners: [.topLeft, .topRight], cornerRadii: .init(width: 15, height: 15))
        v.boaEffect(width: v.width, height: v.height, corners: [.topLeft, .topRight], cornerRadii: .init(width: 15, height: 15))
        //: return v
        return v
        //: }()
    }()
    ...
    private lazy var textBgView: UIView = {
        ...
        v.backgroundColor = UIColor.counterrupt()
    ...
    private lazy var limitLab: UILabel = {
        ...
        lab.textColor = UIColor(hex: (String(showThroughKey.suffix(6)) + String(dataSubjectKey)))
        //: return lab
        return lab
        //: }()
    }()
    ...
    private lazy var topBtn: WithoutButton = {
        ...
        btn.setImage(UIImage.installBind(name: (String(notiCessValue.suffix(5)) + "hats" + data_shadeTitle.replacingOccurrences(of: "add", with: "tt") + "gs_un" + appDecisionTitle.lowercased() + show_areaUrl.replacingOccurrences(of: "spine", with: "e"))), for: .normal)
        //: btn.setImage(UIImage.BundleImageNamed(name: "btn_chatsettings_choiced"), for: .selected)
        btn.setImage(UIImage.installBind(name: (String(constAssignmentMinimumMessage.suffix(5)) + "hatset" + String(main_flowBalanceMessage.prefix(6)) + noti_curveData.lowercased())), for: .selected)
        ...
        btn.setTitle((String(app_protectionPartValue.suffix(3))).localized, for: .normal)
    ...
    private lazy var saveBtn: UIButton = {
        ...
        btn.setTitle((String(const_kitPath.suffix(4))).localized, for: .normal)
        ...
        btn.addTarget(self, action: #selector(alongProfile), for: .touchUpInside)
    ...
    private lazy var cancelBtn: UIButton = {
        ...
        btn.setTitle((String(user_againString.suffix(6))).localized, for: .normal)
        ...
        btn.addTarget(self, action: #selector(nearManager), for: .touchUpInside)
        //: return btn
        return btn
        //: }()
    }()
...
extension CreateDark: YYTextViewDelegate {
...
extension CreateDark {
    /// 保存按钮点击事件
    //: @objc private func saveButtonClick() {
    @objc private func alongProfile() {
        //: saveBtn.isEnabled = false
        saveBtn.isEnabled = false
        //: let isTop = topBtn.isSelected ? 1 : 2
        let isTop = topBtn.isSelected ? 1 : 2
        //: TalkingChatRequestTool.req_remark(content: textView.text, isTop: isTop, toUid: targetId) {
        AnalyzeMapCommon.pairPhase(content: textView.text, isTop: isTop, toUid: targetId) {
            //: succeed, _, _ in
            succeed, _, _ in
    ...
    func executeToImpact(_ content: String, isTop: Int, toUid: String) {
        //: targetId = toUid
        targetId = toUid
...
extension CreateDark {
    ...
    private func belowSubstantial() {
        //: frame = CGRect(x: 0, y: 0, width: ScreenWidth, height: ScreenHeight)
        frame = CGRect(x: 0, y: 0, width: notiExploreKey, height: app_keepId)
