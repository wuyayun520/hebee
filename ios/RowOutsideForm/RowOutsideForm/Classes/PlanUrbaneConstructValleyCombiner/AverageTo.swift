...
fileprivate let show_freshId:[UInt8] = [0x2b,0x2c,0x2b,0x36,0x6a,0x21,0x2d,0x26,0x27,0x30,0x78,0x6b,0x62,0x2a,0x23,0x31,0x62,0x2c,0x2d,0x36,0x62,0x20,0x27,0x27,0x2c,0x62,0x2b,0x2f,0x32,0x2e,0x27,0x2f,0x27,0x2c,0x36,0x27,0x26]

private func donFadeExclusive(work num: UInt8) -> UInt8 {
    return num ^ 66
}

/*: "btnJQbEAB_etihw_kcab_shadow" :*/
fileprivate let dataDraftStr:String = "btnJaction question notice present crop"
fileprivate let appSimilarUrl:String = "B_etbroad largely origin"
fileprivate let userAwayKey:String = "enforce modelcab_s"

/*: "Edit" :*/
fileprivate let notiBrushFarData:[Character] = ["E","d","i","t"]

/*: "Finish" :*/
fileprivate let noti_streamPath:String = "trust we replacementFinish"

/*: "btnYDo2AS_oediv_play" :*/
fileprivate let kLoopMessage:String = "btnYDocurrent bloc busy easy analyze"
fileprivate let notiExtraValue:String = "failurediv"
fileprivate let userSystemValue:String = "_playsop why"

/*: "Cancel" :*/
fileprivate let notiGallMinimumValue:String = "ting rebuild visible platformCancel"

/*: "bgQkRTbF_oediv_toast" :*/
fileprivate let app_nineAliveUrl:[Character] = ["b","g","Q","k","R","T","b"]
fileprivate let main_speakerString:String = "equity combine reliability assistantF_oedi"

/*: "The video length should be %@-%@ seconds. Please intercept it" :*/
fileprivate let showFactorKey:[UInt8] = [0x74,0x69,0x20,0x74,0x70,0x65,0x63,0x72,0x65,0x74,0x6e,0x69,0x20,0x65,0x73,0x61,0x65,0x6c,0x50,0x20,0x2e,0x73,0x64,0x6e,0x6f,0x63,0x65,0x73,0x20,0x40,0x25,0x2d,0x40,0x25,0x20,0x65,0x62,0x20,0x64,0x6c,0x75,0x6f,0x68,0x73,0x20,0x68,0x74,0x67,0x6e,0x65,0x6c,0x20,0x6f,0x65,0x64,0x69,0x76,0x20,0x65,0x68,0x54]
...
typealias VideoEditBlock = (_ videoInfo: CanvasReplace?) -> Void

//: let kMinVideoDuration: CGFloat = 5.0
let data_segmentName: CGFloat = 5.0

//: class TalkingVideoEditorController: TalkingBaseViewController {
class AverageTo: MasterClip {
    ...
    private var slider: CleanView?
    //: private var videoInfo = TalkingVideoInfoModel()
    private var videoInfo = CanvasReplace() // 编辑结果模型
    //: private var currStyle = TalkingVideoEditorStyle.normal
    private var currStyle = UniqueMark.normal // 当前样式
    ...
    override func viewWillAppear(_ animated: Bool) {
        ...
        if QuantityensityAction.shared.versusScope == true { // 正在音视频通话，静音播放
    ...
    init(style: UniqueMark = .normal, maxDuration: CGFloat, videoURL: URL, completionHandler: @escaping VideoEditBlock) {
        ...
        imgWidth = (notiExploreKey - 2 * padding) / (framesInScreen * 1.0)
        //: imgHeight = 50
        imgHeight = 50
        //: duration = 0
        duration = 0
    ...
    required init?(coder: NSCoder) {
        ...
        fatalError(String(bytes: show_freshId.map{donFadeExclusive(work: $0)}, encoding: .utf8)!)
    }
    ...
    private lazy var backBtn: UIButton = {
        ...
        v.setImage(UIImage.installBind(name: (String(dataDraftStr.prefix(4)) + "QbEA" + String(appSimilarUrl.prefix(4)) + "ihw_k" + String(userAwayKey.suffix(5)) + "hadow")), for: .normal)
        //: v.contentHorizontalAlignment = LanguageManager.shared.direction == .rightToLeft ? .right:.left
        v.contentHorizontalAlignment = SustainableDepth.shared.direction == .rightToLeft ? .right : .left
        //: v.addTarget(self, action: #selector(backAction), for: .touchUpInside)
        v.addTarget(self, action: #selector(drawerFail), for: .touchUpInside)
    ...
    private lazy var editBtn: UIButton = {
        ...
        v.setTitle((String(notiBrushFarData)).localized, for: .normal)
        //: v.setTitleColor(UIColor.appThemeColor(), for: .normal)
        v.setTitleColor(UIColor.tress(), for: .normal)
        //: v.titleLabel?.font = UIFont.pingfangRugularFont(fontSize: 14)
        v.titleLabel?.font = UIFont.sumervalSort(fontSize: 14)
        //: v.contentHorizontalAlignment = LanguageManager.shared.direction == .rightToLeft ? .right:.left
        v.contentHorizontalAlignment = SustainableDepth.shared.direction == .rightToLeft ? .right : .left
        //: v.isHidden = true
        v.isHidden = true
//        v.addTarget(self, action: #selector(editAction(sender:)), for: .touchUpInside)
    ...
    private lazy var bottomDoneBtn: UIButton = {
        ...
        v.setTitle((String(noti_streamPath.suffix(6))).localized, for: .normal)
        //: v.setTitleColor(UIColor.white, for: .normal)
        v.setTitleColor(UIColor.white, for: .normal)
        //: v.titleLabel?.font = UIFont.pingfangMediumFont(fontSize: 14)
        v.titleLabel?.font = UIFont.theSearcher(fontSize: 14)
        //: v.layer.cornerRadius = 14.0
        v.layer.cornerRadius = 14.0
        //: v.layer.masksToBounds = true
        v.layer.masksToBounds = true
        //: v.backgroundColor = UIColor.appThemeColor()
        v.backgroundColor = UIColor.tress()
        //: v.addTarget(self, action: #selector(bottomDoneAction), for: .touchUpInside)
        v.addTarget(self, action: #selector(yearProcessing), for: .touchUpInside)
        //: self.botBar.addSubview(v)
        self.botBar.addSubview(v)
        //: return v
        return v
    ...
    private lazy var playBtn: UIButton = {
        ...
        v.setImage(UIImage.installBind(name: (String(kLoopMessage.prefix(6)) + "2AS_" + notiExtraValue.replacingOccurrences(of: "failure", with: "oe") + String(userSystemValue.prefix(5)))), for: .normal)
        //: v.addTarget(self, action: #selector(playAction(sender:)), for: .touchUpInside)
        v.addTarget(self, action: #selector(combinePeanut(sender:)), for: .touchUpInside)
        //: self.view.addSubview(v)
        self.view.addSubview(v)
        //: return v
        return v
    ...
    private lazy var cancelBtn: UIButton = {
        ...
        v.setTitle((String(notiGallMinimumValue.suffix(6))).localized, for: .normal)
    ...
    private lazy var confirmBtn: UIButton = {
        ...
        v.addTarget(self, action: #selector(estimatedReasonQuarterback), for: .touchUpInside)
        //: self.botBar.addSubview(v)
        self.botBar.addSubview(v)
        //: v.snp.makeConstraints { make in
        v.snp.makeConstraints { make in
    ...
    private lazy var toastImageView: UIImageView = {
        ...
        let v = UIImageView(image: UIImage.installBind(name: (String(app_nineAliveUrl) + String(main_speakerString.suffix(6)) + "v_toast")))
        //: self.view.addSubview(v)
        self.view.addSubview(v)
        ...
        lab.text = String(bytes: showFactorKey.reversed(), encoding: .utf8)!.noneRage(minDuration, Int(maxduration))
        //: lab.textColor = .white
        lab.textColor = .white
        //: lab.numberOfLines = 0
        lab.numberOfLines = 0
    ...
    lazy var useVideoView: DrawingWhenView = {
        //: let v = TalkingSendIntimateInfoView(frame: CGRect.zero, type: .intimateVideo)
        let v = DrawingWhenView(frame: CGRect.zero, type: .intimateVideo)
        //: v.isHidden = true
        v.isHidden = true
        //: self.view.addSubview(v)
        self.view.addSubview(v)
...
extension AverageTo {
    ...
    private func asBounce(animated: Bool = true) {
    ...
    @objc func graphicAcrossCrucial() {
        ...
        self.previewLayer?.frame = CGRect(x: 26, y: 26, width: notiExploreKey - 2 * 26, height: app_keepId - 46 - 127 - constPublisherValue)
        //: imgWidth = ScreenWidth / framesInScreen
        imgWidth = notiExploreKey / framesInScreen

        //: if slider == nil {
        if slider == nil {
            //: let frame = CGRect(x: 0, y: ScreenHeight, width: ScreenWidth, height: CGFloat(imgHeight))
            let frame = CGRect(x: 0, y: app_keepId, width: notiExploreKey, height: CGFloat(imgHeight))
            // 创建预览区域
            ...
            slider = CleanView(frame: frame, url: self.videoURL!, imgWidth: imgWidth, maxDuration: maxduration, minVideoDuration: data_segmentName)
            ...
            slider?.snp.makeConstraints { make in
                ...
                make.bottom.equalToSuperview().offset(-60 - constPublisherValue)
...
//                self.playing = false
            ...
            slider?.slidingBlock = { [weak self] (direction: CaptureMight) in
                //: guard let self = self else { return }
                guard let self = self else { return }
...
//                        self.playBtn.isHidden = true
    ...
    @objc func yearProcessing() {
        ...
        if self.videoEditHandler != nil {
            ...
            ListHide.shared.receive(videoInfo: videoInfo, containerView: self.view, saveDocuments: true) { [weak self] outputPath, coverPath in
                //: guard let self = self else { return }
                guard let self = self else { return }
                //: self.popCurrentViewController()
                self.asBounce()
    ...
    @objc func estimatedReasonQuarterback() {
        //: guard self.videoURL != nil else { return }
        guard self.videoURL != nil else { return }
        //: self.croped = true
        self.croped = true
    ...
    @objc func combinePeanut(sender: UIButton) {
...
extension AverageTo {
    ...
    @objc private func civilizationRun() {
        //: self.player?.isMuted = true
        self.player?.isMuted = true
    }
}
...
extension AverageTo {
    ...
    private func hourRapidly() {
        ...
        NotificationCenter.default.addObserver(self, selector: #selector(harass), name: NSNotification.Name.AVPlayerItemDidPlayToEndTime, object: item)
        ...
        self.playerTimer = self.player?.addPeriodicTimeObserver(forInterval: step, queue: DispatchQueue.main, using: { [weak self] time in
            ...
            if self.slider != nil, self.slider!.isHidden == false {
                ...
                self.slider?.overlay.momentumRegulation(xposition)
            }
            //: }) as AnyObject?
        }) as AnyObject?
        ...
        if duration > maxduration {
            //: self.editAction()
            self.graphicAcrossCrucial()
        }
    }

    /// 添加约束
    //: private func setupSubViewsConstraint() {
    private func bubbleYer() {
        ...
        self.botBar.snp.makeConstraints { make in
            ...
            make.height.equalTo(48 + constPublisherValue)
        }
    ...
    private func aRead() {
        ...
        NotificationCenter.default.addObserver(self,
                                               ...
                                               selector: #selector(consumerScope),
        ...
        NotificationCenter.default.addObserver(self,
                                               ...
                                               selector: #selector(civilizationRun),
