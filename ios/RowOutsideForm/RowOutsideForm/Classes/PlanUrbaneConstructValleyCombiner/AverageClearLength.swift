...
fileprivate let constChangeValue:[Character] = ["i","c","o","n","_","k","o","n","g","_","k","o","n"]
fileprivate let app_selectedPath:String = "skip"
fileprivate let app_flagName:String = "cycle resource user_default"

/*: "page" :*/
fileprivate let constToHeartHundredFormat:[UInt8] = [0xe6,0xf7,0xf1,0xf3]

private func actionAttract(every num: UInt8) -> UInt8 {
    return num ^ 150
}

/*: "uid" :*/
fileprivate let user_eagerMsg:[UInt8] = [0x61,0x7d,0x70]

/*: "list" :*/
fileprivate let data_allotTitle:String = "liskint"
...
class AverageClearLength: MasterClip {
    //: private var pageIndex = 0
    private var pageIndex = 0
    //: private var postDataArr = [TalkingUserInfoMomentItemModel]()
    private var postDataArr = [ZoneArchitecture]()
    ...
    private lazy var collectionView: UICollectionView = {
        ...
        collectionView.contentInset = UIEdgeInsets(top: 0, left: 0, bottom: showBulletStr, right: 0)
        //: collectionView.register(TalkingPostItemCell.self, forCellWithReuseIdentifier: TalkingPostItemCell.className())
        collectionView.register(OrientationIndex.self, forCellWithReuseIdentifier: OrientationIndex.className())
    ...
    private lazy var emptyView: UIView = {
        ...
        img.image = UIImage.installBind(name: (String(constChangeValue) + app_selectedPath.replacingOccurrences(of: "skip", with: "g") + String(app_flagName.suffix(8))))
...
extension AverageClearLength {
    //: private func headerRefresh() {
    private func contentNetwork() {
        //: pageIndex = 0
        pageIndex = 0
        //: load_listData()
        graphic()
    ...
    private func graphic() {
        ...
        let params = [String(bytes: constToHeartHundredFormat.map{actionAttract(every: $0)}, encoding: .utf8)!: pageIndex, String(bytes: user_eagerMsg.map{$0^20}, encoding: .utf8)!: userInfoModel.uid] as [String: Any]
        //: TalkingMomentRequestTool.req_UserMomentList(params: params) { [weak self] succeed, result, errorModel in
        QuitInsert.gem(params: params) { [weak self] succeed, result, errorModel in
            //: guard let self = self else { return }
            guard let self = self else { return }
            ...
            if let resultArr = resultDict[(data_allotTitle.replacingOccurrences(of: "skin", with: "s"))] as? [Any] {
                ...
                for dict in resultArr {
                    ...
                    if let model = ExerciseListen.deserialize(from: dict as? Dictionary) {
...
extension AverageClearLength: UICollectionViewDelegate, UICollectionViewDataSource, UICollectionViewDelegateFlowLayout {
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        ...
        let cell = collectionView.dequeueReusableCell(withReuseIdentifier: OrientationIndex.className(), for: indexPath) as! OrientationIndex
    ...
    func collectionView(_ collectionView: UICollectionView, didSelectItemAt indexPath: IndexPath) {
        ...
        let vc = BecomeSuccess(model: detailModel)
...
extension AverageClearLength: JXPagingViewListViewDelegate {
...
extension AverageClearLength {
    ...
    func butt(_ userModel: LineRate) {
