...
fileprivate let constNoticeUrl:[UInt8] = [0x64,0x65,0x74,0x6e,0x65,0x6d,0x65,0x6c,0x70,0x6d,0x69,0x20,0x6e,0x65,0x65,0x62,0x20,0x74,0x6f,0x6e,0x20,0x73,0x61,0x68,0x20,0x29,0x3a,0x72,0x65,0x64,0x6f,0x63,0x28,0x74,0x69,0x6e,0x69]

/*: "img_home_shadow" :*/
fileprivate let kCivilizationMessage:[Character] = ["i","m","g","_","h","o","m","e"]
fileprivate let main_holderEraseStr:[Character] = ["_","s","h","a","d","o","w"]

/*: "#9610FF" :*/
fileprivate let k_rageMsg:[Character] = ["#","9","6","1","0"]
fileprivate let const_appTitle:String = "numbernumber"

/*: "#8566FF" :*/
fileprivate let k_quarterbackUrl:String = "mystery"
fileprivate let show_failureString:[Character] = ["8","5","6","6","F","F"]

/*: "Random Video" :*/
fileprivate let user_applicationOldValue:[Character] = ["R","a","n","d","o","m"," "]
fileprivate let appOrientationBucketMsg:[Character] = ["V","i","d","e","o"]

/*: "icon_match_call_free" :*/
fileprivate let show_menuKingName:String = "ICON"
fileprivate let showTierRemindUrl:String = "_call_fpattern meet specie"
fileprivate let mainThanMessage:[Character] = ["r","e","e"]

/*: "Free" :*/
fileprivate let const_tilHiddenBodyStr:String = "stretch pad writeFree"

/*: "Scroll_upto_discove" :*/
fileprivate let const_incredibleName:String = "stranger bullet occupy fromScroll"
fileprivate let kSinkPath:String = "o_discovelevel err usual clean party"

/*: "icon_VectorStroke" :*/
fileprivate let const_equalUrl:String = "icon_Vaway soak rise"
fileprivate let appMoreGrateStr:String = "outer situation fire await weightrStroke"

/*: "get json error" :*/
fileprivate let main_positionMsg:String = "feature"
fileprivate let mainArrestValue:[Character] = ["e","t"," ","j","s","o","n"," ","e","r","r","o","r"]
...
class FindView: UIView {
    ...
    var maxRemoveDistance = notiExploreKey / 4.0 /// 最大移除距离(默认屏幕的1/4)
    //: var reusableCells = [TalkingClubCardViewCell]()
    var reusableCells = [SimilarStructure]() /// 重用卡片数组
    //: var isAlpha = true
    ...
    var dataSource: NameHandle? /// 数据源
    //: var delegate: TalkingClubCardViewDelegate?
    var delegate: CountensityBelow?
    ...
    required init?(coder: NSCoder) {
        ...
        fatalError(String(bytes: constNoticeUrl.reversed(), encoding: .utf8)!)
    }
    ...
    private lazy var leftbackImageView: UIImageView = {
        ...
        imag.image = UIImage.installBind(name: (String(kCivilizationMessage) + String(main_holderEraseStr)))
    ...
    private lazy var randomBtn: UIButton = {
        ...
        btn.setBackgroundImage(UIImage.airDecade(colors: [UIColor(hex: (String(k_rageMsg) + const_appTitle.replacingOccurrences(of: "number", with: "F")))!.cgColor, UIColor(hex: (k_quarterbackUrl.replacingOccurrences(of: "mystery", with: "#") + String(show_failureString)))!.cgColor], size: CGSize(width: notiExploreKey - 30, height: 54)), for: .normal)
        //: btn.layer.cornerRadius = 27
        btn.layer.cornerRadius = 27
        //: btn.layer.masksToBounds = true
        btn.layer.masksToBounds = true
        //: btn.setTitle("Random Video".localized, for: .normal)
        btn.setTitle((String(user_applicationOldValue) + String(appOrientationBucketMsg)).localized, for: .normal)
        //: btn.addTarget(self, action: #selector(randomBtnClick), for: .touchUpInside)
        btn.addTarget(self, action: #selector(yer), for: .touchUpInside)
        //: self.addSubview(btn)
        self.addSubview(btn)
    ...
    lazy var freeImag: UIButton = {
        ...
        img.setBackgroundImage(UIImage.installBind(name: (show_menuKingName.lowercased() + "_match" + String(showTierRemindUrl.prefix(7)) + String(mainThanMessage))), for: .normal)
        //: img.setTitle("Free".localized, for: .normal)
        img.setTitle((String(const_tilHiddenBodyStr.suffix(4))).localized, for: .normal)
...
extension FindView {
    ...
    private func explain() {
        ...
        image.image = UIImage.installBind(name: (String(const_incredibleName.suffix(6)) + "_upt" + String(kSinkPath.prefix(9))))
        ...
        image2.image = UIImage.installBind(name: (String(const_equalUrl.prefix(6)) + "ecto" + String(appMoreGrateStr.suffix(7))))
        ...
        chopErase(player: player)
    }

    //: private func likeplayer(player: SVGAPlayer) {
    private func chopErase(player: SVGAPlayer) {
        //: do {
        do {
            //: let url = SVGAEffectTool.default.getZipEffectPath(type: .Club_guidePage)
            let url = LimitIndicator.default.multiple(type: .Club_guidePage)
            ...
            printLog(message: (main_positionMsg.replacingOccurrences(of: "feature", with: "g") + String(mainArrestValue)))
        }
    }

    //: @objc private func randomBtnClick() {
    @objc private func yer() {
        //: self.delegate?.clubCardRandowBtnClick?()
        self.delegate?.asOwn?()
    }

    //: func reloadDataAnimated(anim: Bool) {
    func teenage(anim: Bool) {
        //: moveCount = 0
        moveCount = 0
        //: self.currentIndex = 0
        self.currentIndex = 0
        ...
        let maxCount = dataSource?.sumegrity(cardView: self) ?? 0
    ...
    func restriction(index: Int) {
        //: let cell = dataSource?.cardViewCellForRowAtIndex(cardView: self, index: index)
        let cell = dataSource?.anyCritical(cardView: self, index: index)
...
//        let showCount = Double(self.visibleCount - 1)
        ...
        containerView.insertSubview(cell ?? SimilarStructure(), at: 0)
    ...
    func nth(anim: Bool) {
        ...
        for i in 0 ... count - 1 {
            ...
            let cell: SimilarStructure = self.visibleCells[i] as! SimilarStructure
            ...
            if isVisbleLast {
                ...
                self.delegate?.bounceSign?(cardViewCell: cell, forRowAtIndex: self.currentIndex - i)
            }

            //: if anim {
            if anim {
                //: updateConstraintsCell(cell: cell, transform: transform)
                ointment(cell: cell, transform: transform)
                //: } else {
            } else {
    ...
    func ointment(cell: SimilarStructure, transform: CGAffineTransform) {
    ...
    func grateEvaluate(inde: Int) -> Int {
    ...
    func ever(cellClass: AnyClass, forellIdentifier: String) {
    ...
    func phase(identifier: String) -> SimilarStructure? {
        ...
        if self.cellClass != nil {
            ...
            let cellclass = self.cellClass as! PinSustainable.Type
            //: let cell: TalkingClubCardItemCell = cellclass.init(reuseIdentifier: identifier )
            let cell: PinSustainable = cellclass.init(reuseIdentifier: identifier)
...
extension FindView: HideOrientation {
    ...
    func specie(cell: SimilarStructure) {
        ...
        self.delegate?.counterval?(cardView: self, cardViewCell: cell, forRowAtIndex: self.currentFirstIndex - 1)

        //: let count = self.dataSource?.numberOfCountInCardView(cardView: self) ?? 0
        let count = self.dataSource?.sumegrity(cardView: self) ?? 0
        // 移除后的卡片是最后一张(没有更多)
        ...
        if self.visibleCells.count == 0 { // 只有最后一张卡片的时候
            ...
            self.delegate?.sterolDown?(cardViewCell: cell, forRowAtIndex: self.currentIndex)
            //: return
            return
        }
        // 当前数据源还有数据 继续创建cell
        ...
        if self.currentIndex < count - 1 {
            ...
            restriction(index: self.currentIndex + 1)
        }
        // 更新布局
    ...
    func backThan(cell: SimilarStructure, forMovePoint: CGPoint) {
        //: self.delegate?.didMoveCell?(cardView: self, cardViewCell: cell, forMovePoint: forMovePoint)
        self.delegate?.totalReward?(cardView: self, cardViewCell: cell, forMovePoint: forMovePoint)
    }
}
