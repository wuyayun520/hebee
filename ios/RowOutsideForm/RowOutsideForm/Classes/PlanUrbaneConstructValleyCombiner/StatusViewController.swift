...
/*: "Blacklist" :*/
fileprivate let kFormPointString:String = "Blaccolor search fraction"

/*: "page" :*/
fileprivate let main_transitionSponsorString:String = "pdepthge"

/*: "limit" :*/
fileprivate let constLemmaUrl:String = "lmightit"

/*: "You've got Blacklist yet." :*/
fileprivate let mainOccurNaturalMessage:[Character] = ["Y","o","u","\'","v","e"," ","g","o","t"," ","B"]
fileprivate let showTrickMsg:String = "mckl"
fileprivate let dataPersistString:String = "road graphyet."
...
class StatusViewController: MasterClip {
    //: var pageIndex = 0
    var pageIndex = 0
    //: var DataSource: [TalkingBlacklistModel] = []
    var DataSource: [OrientationFlow] = []
    ...
    override func viewDidLoad() {
        ...
        self.title = (String(kFormPointString.prefix(4)) + "klist").localized
        //: designView()
        go()
        //: reqData()
        within()
...
extension StatusViewController {
    ...
    func within() {
        ...
        var dict = [String: Any]()
        //: dict["page"] = pageIndex
        dict[(main_transitionSponsorString.replacingOccurrences(of: "depth", with: "a"))] = pageIndex
        //: dict["limit"] = "20"
        dict[(constLemmaUrl.replacingOccurrences(of: "might", with: "im"))] = "20"
        //: TalkingMeRequestTool.req_BlackList(params: dict) { succeed, result, errorModel in
        CombineWant.design(params: dict) { succeed, result, errorModel in
            ...
            if succeed {
                ...
                var dataArr: [OrientationFlow] = []

                //: if let datas = Array<TalkingBlacklistModel>.deserialize(from: array as? Array) {
                if let datas = Array<OrientationFlow>.deserialize(from: array as? Array) {
                    //: dataArr.append(contentsOf: (datas as? [TalkingBlacklistModel])!)
                    dataArr.append(contentsOf: (datas as? [OrientationFlow])!)
                }
                //: if self.pageIndex == 0 {
                if self.pageIndex == 0 {
    ...
    func record(index: Int) {
        ...
        let model: OrientationFlow = self.DataSource[index]
        //: TalkingUserRequestManager.func__pullBlackWithUserId(uid: model.uid, isBlack: false) { succeed, result, errorModel in
        ScreenScene.centerWith(uid: model.uid, isBlack: false) { succeed, result, errorModel in
            //: if succeed {
            if succeed {
                ...
                MediaSquare.wake(errorModel?.errorMsg)
            }
        }
    }
}
...
extension StatusViewController: UITableViewDelegate, UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        let identifier = FillView.className()
        //: var cell = tableView.dequeueReusableCell(withIdentifier: identifier) as? TalkingBlacklistCell
        var cell = tableView.dequeueReusableCell(withIdentifier: identifier) as? FillView
        //: if cell == nil {
        if cell == nil {
            //: cell = TalkingBlacklistCell(style: .default, reuseIdentifier: identifier)
            cell = FillView(style: .default, reuseIdentifier: identifier)
        }
        //: let model: TalkingBlacklistModel = self.DataSource[indexPath.row]
        let model: OrientationFlow = self.DataSource[indexPath.row]
        //: cell?.setCell(model: model, index: indexPath.row)
        cell?.deliveryBlack(model: model, index: indexPath.row)
...
extension StatusViewController {
    ...
    private func go() {
        ...
        style.TipsTitle = (String(mainOccurNaturalMessage) + showTrickMsg.replacingOccurrences(of: "m", with: "la") + "ist " + String(dataPersistString.suffix(4))).localized
        ...
        MainTable.register(FillView.self, forCellReuseIdentifier: FillView.className())
