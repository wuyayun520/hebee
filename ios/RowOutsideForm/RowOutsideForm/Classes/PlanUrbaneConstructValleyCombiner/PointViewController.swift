...
protocol ThresholdActive: AnyObject {
...
class PointViewController: UIViewController {
    ...
    weak var delegate: ThresholdActive?
    ...
    private lazy var controllerScrollView: UIScrollView = {
        ...
        let count = min(self.controllerItems.count, self.titleItems.count)
        //: var height = FormatDistribution.getScreenHeight() - (topDistance + kTitle_H)
        var height = FormatDistribution.towardDivideBold() - (topDistance + kTitle_H)
        //: if self.hasBttomTabBar {
        if self.hasBttomTabBar {
            //: height -= TabBarViewHeight
            height -= userOutputKey
        }
        ...
        //: let rect = CGRect(x: 0, y: topDistance + kTitle_H, width: FormatDistribution.getScreenWidth(), height: height)
        let rect = CGRect(x: 0, y: topDistance + kTitle_H, width: FormatDistribution.tillText(), height: height)
        //: let scrollView = UIScrollView(frame: rect)
        let scrollView = UIScrollView(frame: rect)
        //: scrollView.contentSize = CGSize(width: count * Int(FormatDistribution.getScreenWidth()), height: 0)
        scrollView.contentSize = CGSize(width: count * Int(FormatDistribution.tillText()), height: 0)
        ...
        scrollView.semanticContentAttribute = SustainableDepth.shared.direction == .leftToRight ? .forceLeftToRight:.forceRightToLeft
        //: return scrollView
        return scrollView
    //: }()
    }()
    ...
    private lazy var titleScrollView: UIScrollView = {
        //: var rect = CGRect(x: 15, y: topDistance, width: FormatDistribution.getScreenWidth()-rightSpace, height: kTitle_H)
        var rect = CGRect(x: 15, y: topDistance, width: FormatDistribution.tillText()-rightSpace, height: kTitle_H)
        //: if isBisect {
        if isBisect {
            //: rect = CGRect(x: 0, y: topDistance, width: FormatDistribution.getScreenWidth()-rightSpace, height: kTitle_H)
            rect = CGRect(x: 0, y: topDistance, width: FormatDistribution.tillText()-rightSpace, height: kTitle_H)
        }
        //: let scrollView = UIScrollView(frame: rect)
        let scrollView = UIScrollView(frame: rect)
        ...
        var originX: CGFloat = SustainableDepth.shared.direction == .leftToRight ? 0:25
        ...
        self.titleItems.enumerated().forEach { [weak self] index, obj in
            ...
            titleBtn.addTarget(self, action: #selector(sourceRoute), for: .touchDown)
            //: var titleWidth = 0.0
            var titleWidth = 0.0
            //: if isBisect {
            if isBisect {
                //: titleWidth = FormatDistribution.getScreenWidth() / Double(self.titleItems.count)
                titleWidth = FormatDistribution.tillText() / Double(self.titleItems.count)
            //: } else {
            } else {
                //: titleWidth = (titleBtn.titleLabel?.sizeThatFits(CGSize(width: CGFLOAT_MAX, height: kTitle_H)).width ?? 0) + 22
                titleWidth = (titleBtn.titleLabel?.sizeThatFits(CGSize(width: CGFLOAT_MAX, height: kTitle_H)).width ?? 0) + 22
            }
            //: let x = LanguageManager.shared.direction == .leftToRight ? originX:(FormatDistribution.getScreenWidth()-originX-titleWidth)
            let x = SustainableDepth.shared.direction == .leftToRight ? originX:(FormatDistribution.tillText()-originX-titleWidth)
...
extension PointViewController {
    ...
    @objc private func sourceRoute(_ sender: UIButton) {
        //: currentSelectedTitleButton(sender)
        focusToInfra(sender)
        //: let index = sender.tag - 1000
        let index = sender.tag - 1000
        //: let offset_x = CGFloat(index) * FormatDistribution.getScreenWidth()
        let offset_x = CGFloat(index) * FormatDistribution.tillText()
        //: controllerScrollView.contentOffset = CGPoint(x: offset_x, y: 0)
        controllerScrollView.contentOffset = CGPoint(x: offset_x, y: 0)
        //: addControllerView(withIndex: index)
        roundPee(withIndex: index)
        //: seletedViewController(withIndex: index)
        pageAcross(withIndex: index)
    }
    
    //: private func addControllerView(withIndex index: Int) {
    private func roundPee(withIndex index: Int) {
        //: if index >= children.count {
        if index >= children.count {
            //: return
            return
        }
        //: let origin_x = CGFloat(index) * FormatDistribution.getScreenWidth()
        let origin_x = CGFloat(index) * FormatDistribution.tillText()
        ...
        //: vc.view.frame = CGRect(x: origin_x, y: 0, width: FormatDistribution.getScreenWidth(), height: controllerScrollView.frame.size.height)
        vc.view.frame = CGRect(x: origin_x, y: 0, width: FormatDistribution.tillText(), height: controllerScrollView.frame.size.height)
        //: controllerScrollView.addSubview(vc.view)
        controllerScrollView.addSubview(vc.view)
    }
    
    //: private func currentSelectedTitleButton(_ button: UIButton) {
    private func focusToInfra(_ button: UIButton) {
    ...
    func rageMedia(_ sender: UIButton) {
        //: if titleScrollView.contentSize.width <= FormatDistribution.getScreenWidth() {
        if titleScrollView.contentSize.width <= FormatDistribution.tillText() {
            //: return
            return
        }
        //: var offset_x = sender.center.x - FormatDistribution.getScreenWidth() * 0.5
        var offset_x = sender.center.x - FormatDistribution.tillText() * 0.5
        //: if offset_x < 0 {
        if offset_x < 0 {
            //: offset_x = 0
            offset_x = 0
        }
        //: let maxOffset = titleScrollView.contentSize.width - FormatDistribution.getScreenWidth()
        let maxOffset = titleScrollView.contentSize.width - FormatDistribution.tillText()
...
extension PointViewController {
    ...
    func pageAcross(withIndex index: Int) {
        //: guard index < children.count else { return }
        guard index < children.count else { return }
        //: delegate?.selectedViewController(children[index], index: index)
        delegate?.recent(children[index], index: index)
    }
}

// MARK: - UIScrollViewDelegate
//: extension QYSlideNavigationViewController: UIScrollViewDelegate {
extension PointViewController: UIScrollViewDelegate {
    //: func scrollViewDidEndDecelerating(_ scrollView: UIScrollView) {
    func scrollViewDidEndDecelerating(_ scrollView: UIScrollView) {
        //: let index = Int(controllerScrollView.contentOffset.x / FormatDistribution.getScreenWidth())
        let index = Int(controllerScrollView.contentOffset.x / FormatDistribution.tillText())
        //: if let selectedBtn = titleScrollView.viewWithTag(index + 1000) as? UIButton {
        if let selectedBtn = titleScrollView.viewWithTag(index + 1000) as? UIButton {
            //: currentSelectedTitleButton(selectedBtn)
            focusToInfra(selectedBtn)
        }
        //: addControllerView(withIndex: index)
        roundPee(withIndex: index)
        //: seletedViewController(withIndex: index)
    ...
    func scrollViewDidScroll(_ scrollView: UIScrollView) {
        ...
        //: let index = Int(scrollView.contentOffset.x / FormatDistribution.getScreenWidth())
        let index = Int(scrollView.contentOffset.x / FormatDistribution.tillText())
        ...
        //: let scaleR = scrollView.contentOffset.x / FormatDistribution.getScreenWidth() - CGFloat(index)
        let scaleR = scrollView.contentOffset.x / FormatDistribution.tillText() - CGFloat(index)
        //: let scaleL = 1 - scaleR
        let scaleL = 1 - scaleR
