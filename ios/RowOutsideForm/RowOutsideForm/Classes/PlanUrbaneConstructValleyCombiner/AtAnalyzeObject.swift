...
/*: "mf/call/GetUserInfo" :*/
fileprivate let show_zoneMsg:String = "following enforce undertakemf/call"
fileprivate let const_weirdId:String = "remind admin enjoy gift char/GetU"

/*: "uid" :*/
fileprivate let userMediaRelateKey:[UInt8] = [0x22,0x3e,0x33]

private func correctFormal(income num: UInt8) -> UInt8 {
    return num ^ 87
}

/*: "TRTC 随机视频匹配中." :*/
fileprivate let noti_ovalId:String = "TRTC 随scientific attention"
fileprivate let appPointId:String = "\u{673a}\u{89c6}频匹配中."

/*: "TRTC  :*/
fileprivate let constHomeString:String = "TRTC hour eastern formal"

/*: 视频通话. logId: :*/
fileprivate let appPackTradeFormat:String = "视频通\u{8bdd}."
fileprivate let main_heartDecadeValue:[Character] = [" "]
fileprivate let kAloneName:String = "where install value invite storagelogId:"

/*: "TRTC 视频通话销毁. logId： :*/
fileprivate let showComprehensiveTaId:String = "TRTC 视onto project properly"
fileprivate let noti_homeName:[Character] = ["."," "]
fileprivate let noti_sterolFormat:[Character] = ["l","o","g","I","d","："]

/*: "TRTC 视频通话错误. errCode: :*/
fileprivate let appInsideFormat:String = "TRTC sustainable team graph robot"
fileprivate let app_itAdditionalFormat:String = "\u{9519}误. "
fileprivate let user_boardTwoString:[Character] = [":"]

/*: , errMsg: :*/
fileprivate let appDeviceAccessData:String = "transfer pair re sponsor foundation, errMsg:"

/*: "TRTC 视频通话SDK收到警告. warningCode: :*/
fileprivate let main_accelerateValue:String = "primary transformTRTC 视"
fileprivate let main_universalData:String = "vertical rank marginK收到警告. wa"
fileprivate let kEaseSubtleStr:[Character] = ["r","n","i","n","g","C","o","d","e",":"]

/*: , warningMsg: :*/
fileprivate let notiModelId:[Character] = [","," ","w","a","r","n"]
fileprivate let kPassingData:String = "ok format scan sophisticate tweeningMsg:"

/*: "TRTC 视频通话进入房间. result: :*/
fileprivate let showProvideId:[Character] = ["T","R","T","C"," ","视","频","\u{901a}","话","进","入","房","间","."," ","r","e"]
fileprivate let noti_medalAfterFailureFormat:String = "sult:element return impact sufficient"

/*: , logId： :*/
fileprivate let app_currentFormat:[Character] = [","]
fileprivate let user_womanPath:String = " logId：balance challenge consume that listen"

/*:  加入了房间" :*/
fileprivate let main_thenTitle:[Character] = [" ","加","入","\u{4e86}","房","间"]

/*:  离开了房间" :*/
fileprivate let noti_appropriateData:String = " 离tar房间"
...
class BlocLocation: NSObject {
    ...
    var remoteCameraStatus = WhiteControl.Normal // 对方摄像头状态
}

//: class TalkingVideoChatManager: NSObject {
class AtAnalyzeObject: NSObject {
    //: var infoModel: TalkingVideoChatModel?
    var infoModel: RespectiveModel? // 视频通话model
    //: var config = TalkingVideoChatConfig()
    var config = BlocLocation() // 视频渲染配置类
...
extension AtAnalyzeObject {
    /// 开始视频速配（男）
    /// - Parameters:
    ///   - uid: 接收者的id
    ///   - completion: 回调
    //: class func req_callGetUserInfo(uid: String, completion: @escaping FinishBlock) {
    class func sameOn(uid: String, completion: @escaping FinishBlock) {
        //: let reqModel = TalkingRequestModel.init()
        let reqModel = PopAdjust()
        //: reqModel.requestPath = "mf/call/GetUserInfo"
        reqModel.requestPath = (String(show_zoneMsg.suffix(7)) + String(const_weirdId.suffix(5)) + "serInfo")
        //: reqModel.requestType = .GET
        reqModel.requestType = .GET
        //: reqModel.params = ["uid": uid]
        reqModel.params = [String(bytes: userMediaRelateKey.map{correctFormal(income: $0)}, encoding: .utf8)!: uid]
...
extension AtAnalyzeObject {
    ...
    func enableMission(_ frontCamera: Bool) {
        ...
        self.policyWith(front: frontCamera)
    }

    /// 摄像头开启与关闭
    /// - Parameter openCamera: 是否开启
    //: func useCamera(_ openCamera: Bool) {
    func quoteMargin(_ openCamera: Bool) {
        ...
        self.drawerSample(isOpen: openCamera)
    }
    ...
    func direction() {
        ...
        DispatchQueue.main.async {
            ...
            self.meScale(smallWindow: self.infoModel?.renderPostion != .selfOnBg)
...
extension AtAnalyzeObject {
    ...
    private func policyWith(front: Bool) {
    ...
    private func drawerSample(isOpen: Bool) {
        //: if isOpen {
        if isOpen {
...
extension AtAnalyzeObject {
    ...
    func anDifficult() {
        ...
        NotificationCenter.default.addObserver(self,
                                               ...
                                               selector: #selector(isAmong(notif:)),
        ...
        NotificationCenter.default.addObserver(self,
                                               ...
                                               selector: #selector(dropDoingAgree(notif:)),
        ...
        NotificationCenter.default.addObserver(self,
                                               ...
                                               selector: #selector(rating(notif:)),
    ...
    @objc private func isAmong(notif: Notification?) {
        //: if notif == nil {
        if notif == nil {
            // 静音
    ...
    @objc private func dropDoingAgree(notif: Notification?) {
        //: if notif == nil {
    ...
    @objc private func rating(notif: Notification) {
    ...
    @objc func multi(notif: Notification) {
...
extension AtAnalyzeObject {
    ...
    func descriptionSecond(isMatch: Bool = false) {
        //: if isMatch {
        if isMatch {
            //: UploadLogTool.writeLog(msg: "TRTC 随机视频匹配中.")
            TrainGreat.bitRemove(msg: (String(noti_ovalId.prefix(6)) + appPointId))
            //: } else {
        } else {
            //: UploadLogTool.writeLog(msg: "TRTC \((infoModel?.videoStage == .Waiting) ? "发起":"收到")视频通话. logId:\(infoModel?.logId ?? 0).")
            TrainGreat.bitRemove(msg: (String(constHomeString.prefix(5))) + "\((infoModel?.videoStage == .Waiting) ? "发起" : "收到")" + (appPackTradeFormat + String(main_heartDecadeValue) + String(kAloneName.suffix(6))) + "\(infoModel?.logId ?? 0).")
        }
        //: if self.infoModel?.videoStage == .Waiting {
        if self.infoModel?.videoStage == .Waiting {
            //: startTRTCPreview(smallWindow: false)
            meScale(smallWindow: false)
    ...
    private func mechanismIndicator() {
    ...
    private func manualRate() {
        //: UploadLogTool.writeLog(msg: "TRTC 视频通话销毁. logId：\(infoModel?.logId ?? 0).")
        TrainGreat.bitRemove(msg: (String(showComprehensiveTaId.prefix(6)) + "频\u{901a}\u{8bdd}销\u{6bc1}" + String(noti_homeName) + String(noti_sterolFormat)) + "\(infoModel?.logId ?? 0).")
        ...
        //: if self.config.versusScope == true {
        if self.config.versusScope == true {
...
extension AtAnalyzeObject: TRTCCloudDelegate {
    //: func onError(_ errCode: TXLiteAVError, errMsg: String?, extInfo: [AnyHashable: Any]?) {
    func onError(_ errCode: TXLiteAVError, errMsg: String?, extInfo: [AnyHashable: Any]?) {
        //: UploadLogTool.writeLog(msg: "TRTC 视频通话错误. errCode:\(errCode), errMsg:\(String(describing: errMsg)).")
        TrainGreat.bitRemove(msg: (String(appInsideFormat.prefix(5)) + "视频通话" + app_itAdditionalFormat + "errCode" + String(user_boardTwoString)) + "\(errCode)" + (String(appDeviceAccessData.suffix(9))) + "\(String(describing: errMsg)).")
        //: if errMsg != nil {
        if errMsg != nil {
    ...
    func onWarning(_ warningCode: TXLiteAVWarning, warningMsg: String?, extInfo: [AnyHashable: Any]?) {
        ...
        TrainGreat.bitRemove(msg: (String(main_accelerateValue.suffix(6)) + "\u{9891}通话SD" + String(main_universalData.suffix(9)) + String(kEaseSubtleStr)) + "\(warningCode)" + (String(notiModelId) + String(kPassingData.suffix(7))) + "\(String(describing: warningMsg)).")
        ...
        if self.config.remoteCameraStatus == .Normal {
            ...
            NotificationCenter.default.post(name: mainMatchId, object: self)
        }
    }
    ...
    func onEnterRoom(_ result: Int) {
        ...
        TrainGreat.bitRemove(msg: (String(showProvideId) + String(noti_medalAfterFailureFormat.prefix(5))) + "\(result)" + (String(app_currentFormat) + String(user_womanPath.prefix(7))) + "\(infoModel?.logId ?? 0).")
        //: if result < 0 {
        if result < 0 { // 加入失败
    ...
    func onRemoteUserEnterRoom(_ userId: String) {
        ...
        printLog(message: "\(userId)" + (String(main_thenTitle)))
    }

    //: func onRemoteUserLeaveRoom(_ userId: String, reason: Int) {
    func onRemoteUserLeaveRoom(_ userId: String, reason: Int) {
        //: printLog(message: "\(userId) 离开了房间")
        printLog(message: "\(userId)" + (noti_appropriateData.replacingOccurrences(of: "tar", with: "开了")))
    }
    ...
    func onUserVideoAvailable(_ userId: String, available: Bool) {
        //: guard userId != VideoMagnitudeerval.share.loginUserMode.userID else { return }
        guard userId != VideoMagnitudeerval.share.loginUserMode.userID else { return }
