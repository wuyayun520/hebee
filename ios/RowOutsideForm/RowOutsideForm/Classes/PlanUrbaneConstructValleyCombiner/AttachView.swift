...
fileprivate let kConcreteValue:[UInt8] = [0x64,0x65,0x74,0x6e,0x65,0x6d,0x65,0x6c,0x70,0x6d,0x69,0x20,0x6e,0x65,0x65,0x62,0x20,0x74,0x6f,0x6e,0x20,0x73,0x61,0x68,0x20,0x29,0x3a,0x72,0x65,0x64,0x6f,0x63,0x28,0x74,0x69,0x6e,0x69]

/*: "icon_mini_voice_end" :*/
fileprivate let noti_takeName:[UInt8] = [0x64,0x6e,0x65,0x5f,0x65,0x63,0x69,0x6f,0x76,0x5f,0x69,0x6e,0x69,0x6d,0x5f,0x6e,0x6f,0x63,0x69]

/*: "icon_mini_voice_calling" :*/
fileprivate let data_trackJobData:String = "icon_instance shot shout"
fileprivate let k_youShapeStr:String = "refrigerator move hidden observe operateoice_cal"
fileprivate let showProvisionAtCommitValue:[Character] = ["l","i","n","g"]

/*: "#F95151" :*/
fileprivate let userTopicBlueFormat:String = "#Fenough object assistant create"
fileprivate let showBucketId:String = "95151"

/*: "#34C759" :*/
fileprivate let main_removeKey:[Character] = ["#","3","4","C","7"]
fileprivate let constReductionStr:[Character] = ["5","9"]

/*: "End of call" :*/
fileprivate let data_balanceSignData:String = "zz command face total varEnd o"
fileprivate let appRenderString:String = "visit factor re clubf call"
...
class AttachView: SensorView {
    ...
    required init?(coder: NSCoder) {
        ...
        fatalError(String(bytes: kConcreteValue.reversed(), encoding: .utf8)!)
    }
...
extension AttachView {
    ...
    static func computeMight() -> AttachView {
        //: let view = TalkingVoiceMiniView(frame: .zero)
        let view = AttachView(frame: .zero)
...
extension AttachView {
    ...
    private func connectionTension() {
        ...
        invite(status: .calling)
    }

    /// 刷新视图
    /// - Parameter status: 通话状态
    //: private func refreshVoiceView(status: MiniWindowStatus) {
    private func invite(status: HiddenWould) {
        //: self.currStatus = status
        self.currStatus = status
        //: let iconStr = (status == .end) ? "icon_mini_voice_end":"icon_mini_voice_calling"
        let iconStr = (status == .end) ? String(bytes: noti_takeName.reversed(), encoding: .utf8)! : (String(data_trackJobData.prefix(5)) + "mini_v" + String(k_youShapeStr.suffix(8)) + String(showProvisionAtCommitValue))
        //: self.statusIcon.image = UIImage.BundleImageNamed(name: iconStr)
        self.statusIcon.image = UIImage.installBind(name: iconStr)
        //: self.timeLab.textColor = (status == .end) ? UIColor(hex: "#F95151"):UIColor(hex: "#34C759")
        self.timeLab.textColor = (status == .end) ? UIColor(hex: (String(userTopicBlueFormat.prefix(2)) + showBucketId.capitalized)) : UIColor(hex: (String(main_removeKey) + String(constReductionStr)))
        //: if status == .end {
        if status == .end {
            //: self.timeLab.text = "End of call".localized
            self.timeLab.text = (String(data_balanceSignData.suffix(5)) + String(appRenderString.suffix(6))).localized
        }
    }
}
