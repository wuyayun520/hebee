...
struct ExerciseListen: HandyJSON {
    //: var uid: String?
    var uid: String?
    //: var sex: String?
    var sex: String?
...
struct NumberensityMark: HandyJSON {
    //: var thumbUrl: String?
    var thumbUrl: String?
    //: var imgUrl: String?
    var imgUrl: String?
...
struct PauseCountPresent: HandyJSON {
    //: var videoUrl = ""
    var videoUrl = ""
    //: var imgUrl = ""
    var imgUrl = ""
    ...
    var status: ListenRate?
}

///
//: enum MomentVideoStatus: String {
enum ListenRate: String {
    //: case NoPass = "-1"   // 未通过
    case NoPass = "-1" // 未通过
    //: case Audit = "0"     // 审核中
    case Audit = "0" // 审核中
...
extension ExerciseListen {
    //: mutating func caculateItemHeight(isPost: Bool = false) {
    mutating func edit(isPost: Bool = false) {
        ...
        if content!.count > 0 {
            ...
            textHeight = Int(DensityView.on(text: content!, size: CGSize(width: notiExploreKey - 54, height: CGFloat(MAXFLOAT)), font: UIFont.statinEssay(type: .Regular, fontSize: 17), lineNumber: lineNumber).height + (lineNumber == 0 ? 20 : 0))
        }

        //: if momentType == MomentType.Photo.rawValue || momentType == MomentType.Live.rawValue {
        if momentType == ModeEmpty.Photo.rawValue || momentType == ModeEmpty.Live.rawValue {
            //: if pic.count==1 {
            if pic.count == 1 {
                //: photesHeight = 212
                photesHeight = 212
