...
/*: "reply" :*/
fileprivate let main_cartoonKey:String = "rcornerly"
...
struct AlwaysImplement: HandyJSON {
    //: var cid: String?
    ...
    var comment = [ButtExitPersistModel]()
    ...
    mutating func broadcast() {
        ...
        if replyUser?.count ?? 0 > 0 {
            ...
            if SustainableDepth.shared.direction == .leftToRight {
                ...
                tempContent = (main_cartoonKey.replacingOccurrences(of: "corner", with: "ep")).localized + "\(replyUser!): \(content!)"
                //: } else {
            } else {
                //: tempContent = "\(content!) :\(replyUser!)" + "reply".localized
                tempContent = "\(content!) :\(replyUser!)" + (main_cartoonKey.replacingOccurrences(of: "corner", with: "ep")).localized
            }
        ...
        let sizeheight = Int(DensityView.on(text: tempContent, size: CGSize(width: notiExploreKey - 70, height: CGFloat(MAXFLOAT)), font: UIFont.statinEssay(type: .Regular, fontSize: 16), lineNumber: 0).height)
