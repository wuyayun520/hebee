...
fileprivate let data_descriptionName:[UInt8] = [0x73,0x74,0x73,0x6e,0x32,0x79,0x75,0x7e,0x7f,0x68,0x20,0x33,0x3a,0x72,0x7b,0x69,0x3a,0x74,0x75,0x6e,0x3a,0x78,0x7f,0x7f,0x74,0x3a,0x73,0x77,0x6a,0x76,0x7f,0x77,0x7f,0x74,0x6e,0x7f,0x7e]

private func extentInvite(reach num: UInt8) -> UInt8 {
    return num ^ 26
}

/*: "icon_userdetail_header_shadow" :*/
fileprivate let dataDeliveryData:String = "icon_selection sample to center drawer"
fileprivate let showDateId:String = "deteaseil"
fileprivate let const_databaseTitle:String = "der_spublic interval observe net passing"
fileprivate let appGreenSixKey:[Character] = ["h","a","d","o","w"]

/*: "icon_lounge" :*/
fileprivate let appHimObjectId:String = "icleading"

/*: "Official" :*/
fileprivate let userSinkId:String = "directly memory beforeOffi"

/*: "Reviewing" :*/
fileprivate let app_whenValue:[Character] = ["R","e","v","i","e"]
fileprivate let dataBesideId:String = "winnormally"

/*: "#FF506D" :*/
fileprivate let userCraftId:String = "#FF506Ddown it tag border back"

/*: "icon_user_detail_intimate_header" :*/
fileprivate let main_rapidlyKey:[UInt8] = [0x52,0x58,0x54,0x55,0x64,0x4e,0x48,0x5e,0x49,0x64,0x5f,0x5e,0x4f,0x5a,0x52,0x57,0x64,0x52,0x55,0x4f,0x52,0x56,0x5a,0x4f,0x5e,0x64,0x53,0x5e,0x5a,0x5f,0x5e,0x49]

private func frontUniform(column num: UInt8) -> UInt8 {
    return num ^ 59
}

/*: "?toUid= :*/
fileprivate let noti_sureMsg:String = "?toUidcomply possibly item pause recommend"
fileprivate let show_oddName:String = "="

/*: "WriteView" :*/
fileprivate let showInvisibleName:String = "let pressure commentTUser"
fileprivate let data_appropriateDecideUrl:String = "ilHeadcountry user gallery extension move"
fileprivate let mainHoneyValue:[Character] = ["e","r","I","t","e","m","C","e","l","l"]

/*: "#FFFFFF" :*/
fileprivate let notiLinkTitle:[Character] = ["#","F","F","F","F","F","F"]

/*: "#BDB8B5" :*/
fileprivate let kManValue:[Character] = ["#","B"]
fileprivate let mainUpMsg:[Character] = ["D","B","8","B","5"]

/*: "head_detail_bottom" :*/
fileprivate let dataGeneralName:[Character] = ["h","e","a","d","_","d","e","t","a"]
fileprivate let k_readyUrl:String = "similar quote nor drawer amil_bo"

/*: "#2DF2FF" :*/
fileprivate let dataZzString:[Character] = ["#","2","D","F","2","F"]
fileprivate let const_incomeData:String = "album"

/*: "Live" :*/
fileprivate let noti_marriedBasicValue:[Character] = ["L","i","v","e"]

/*: "#48FF1E" :*/
fileprivate let main_remotePleaseId:[Character] = ["#","4","8","F","F","1","E"]

/*: "Online" :*/
fileprivate let app_mayPath:String = "Onlineago procedure"

/*: "#FF4B7F" :*/
fileprivate let showSixHerKey:[Character] = ["#","F","F","4","B","7","F"]

/*: "busy" :*/
fileprivate let showWholeTailMessage:String = "reservesy"
...
let const_typicalTitle = (302.0 + show_uniformKey)

//: class TalkingUserDetailHeaderView: UIView {
class OriginEdit: UIView {
    //: var VideoCallBlock: (() -> Void)?
    var VideoCallBlock: (() -> Void)?
    //: private var userInfoModel = TalkingUserInfoModel()
    private var userInfoModel = LineRate()
    ...
    required init?(coder: NSCoder) {
        ...
        fatalError(String(bytes: data_descriptionName.map{extentInvite(reach: $0)}, encoding: .utf8)!)
    }
    ...
    private lazy var bannerView: JXBanner = {
        ...
        view.placeholderImgView.image = UIImage.versus()
    ...
    private lazy var shadowBgView: UIImageView = {
        ...
        let v = UIImageView(image: UIImage.installBind(name: (String(dataDeliveryData.prefix(5)) + "user" + showDateId.replacingOccurrences(of: "ease", with: "a") + "_hea" + String(const_databaseTitle.prefix(5)) + String(appGreenSixKey))))
        //: return v
        return v
        //: }()
    }()
    ...
    private lazy var vipBgView: UIImageView = {
        ...
        v.frame = CGRect(x: 0, y: const_typicalTitle - 15, width: notiExploreKey, height: 15)
        //: return v
        return v
        //: }()
    }()
    ...
    private lazy var nicknameLab: UILabel = {
        ...
        lab.font = UIFont.statinEssay(type: .Medium, fontSize: 19)
    ...
    private lazy var loungeImgV: UIImageView = {
        ...
        img.image = UIImage.installBind(name: (appHimObjectId.replacingOccurrences(of: "leading", with: "on") + "_lounge"))
        //: return img
        return img
        //: }()
    }()
    ...
    private lazy var officalImgV: UIButton = {
        ...
        img.setTitle((String(userSinkId.suffix(4)) + "cial").localized, for: .normal)
    ...
    private lazy var onlineImgV: WithoutButton = {
    ...
    private lazy var liveView: ExceptView = {
        ...
        v.tapGestureBlock = { [weak self] in
            ...
            ScopeEmbrace.share.nite(uid: self.userInfoModel.uid, enterType: .userDetail)
        }
        //: return v
        return v
        //: }()
    ...
    private lazy var reViewingLB: UILabel = {
        ...
        lb.text = (String(app_whenValue) + dataBesideId.replacingOccurrences(of: "normally", with: "g")).localized
        //: lb.textColor = .white
        lb.textColor = .white
        //: lb.backgroundColor = UIColor.init(hex: "#FF506D")
        lb.backgroundColor = UIColor(hex: (String(userCraftId.prefix(7))))
    ...
    lazy var lastLoginLab: UILabel = {
        ...
        lb.font = .statinEssay(type: .Medium, fontSize: 12)
        //: lb.textColor = .white
        lb.textColor = .white
        //: return lb
        return lb
    ...
    private lazy var intimateBtn: WithoutButton = {
        //: let btn = TalkingButton()
        let btn = WithoutButton()
        //: btn.setImage(UIImage.BundleImageNamed(name: "icon_user_detail_intimate_header"), for: .normal)
        btn.setImage(UIImage.installBind(name: String(bytes: main_rapidlyKey.map{frontUniform(column: $0)}, encoding: .utf8)!), for: .normal)
        //: btn.isHidden = true
        btn.isHidden = true
...
extension OriginEdit {
    /// 查看视频
    //: private func goToVideo() {
    private func reply() {
        //: let vc = TalkingMomentVideoVC.init(videoPath: self.userInfoModel.videoUrl)
        let vc = ConditionViewController(videoPath: self.userInfoModel.videoUrl)
        //: if VideoMagnitudeerval.share.loginUid != self.userInfoModel.uid {
        if VideoMagnitudeerval.share.loginUid != self.userInfoModel.uid {
    ...
    private func thumbRes(_ index: Int) {
        //: var tempArr: [TalkingMomentPhotoModel] = []
        var tempArr: [NumberensityMark] = []
        //: var model = TalkingMomentPhotoModel.init()
        var model = NumberensityMark()
        ...
        let vc = MagnitudeervalManagerFlex(momentModel: currenModel, index: index, type: .normal)
        //: self.currentViewController()?.navigationController?.pushViewController(vc, animated: true)
        self.congressionalFor()?.navigationController?.pushViewController(vc, animated: true)
    }
    ...
    @objc func minimize() {
        ...
        let url = MediaCharacteristic.outsideTrick(urlType: .Intimate) + (String(noti_sureMsg.prefix(6)) + show_oddName.capitalized) + "\(self.userInfoModel.uid)"
        //: TalkingAppPushManager.share.func__pushToWebVC(urlStr: url)
        ScopeEmbrace.share.painter(urlStr: url)
    }
}
...
extension OriginEdit: JXBannerDataSource {
    ...
    func jxBanner(_ banner: JXBannerType) -> (JXBannerCellRegister) {
        ...
        return JXBannerCellRegister(type: WriteView.self, reuseIdentifier: (String(showInvisibleName.suffix(5)) + "Deta" + String(data_appropriateDecideUrl.prefix(6)) + String(mainHoneyValue)))
    }

    //: func jxBanner(_ banner: JXBannerType, cellForItemAt index: Int, cell: UICollectionViewCell) -> UICollectionViewCell {
    func jxBanner(_ banner: JXBannerType, cellForItemAt index: Int, cell: UICollectionViewCell) -> UICollectionViewCell {
        //: let bannerCell = cell as! TUserDetailHeaderItemCell
        let bannerCell = cell as! WriteView
        //: let urlStr = self.dataArray[index]
        let urlStr = self.dataArray[index]

        //: if index == 0 && userInfoModel.sex == Gender.female.rawValue && userInfoModel.videoUrl.count>1 {
        if index == 0 && userInfoModel.sex == ChannelDown.female.rawValue && userInfoModel.videoUrl.count > 1 { // 视频
            //: bannerCell.setVideoView()
            bannerCell.decide()
            //: bannerCell.configModel(videoUrl: userInfoModel.videoUrl, imgUrl: userInfoModel.cover)
            bannerCell.lost(videoUrl: userInfoModel.videoUrl, imgUrl: userInfoModel.cover)
            ...
            bannerCell.bgImgView.spread(urlStr: urlStr, placeImg: UIImage.zoneManual())
        }
    ...
    func jxBanner(_ banner: JXBannerType, centerIndex: Int, centerCell: UICollectionViewCell) {
        //: guard userInfoModel.uid == VideoMagnitudeerval.share.loginUid else {
        guard userInfoModel.uid == VideoMagnitudeerval.share.loginUid else {
            //: return
            return
        }
    ...
    func jxBanner(pageControl banner: JXBannerType, numberOfPages: Int, coverView: UIView, builder: JXBannerPageControlBuilder) -> JXBannerPageControlBuilder {
        ...
        pageControl.activeColor = UIColor(hex: (String(notiLinkTitle)))!
        //: pageControl.inactiveColor = UIColor(hex: "#BDB8B5")!
        pageControl.inactiveColor = UIColor(hex: (String(kManValue) + String(mainUpMsg)))!
...
extension OriginEdit: JXBannerDelegate {
    //: public func jxBanner(_ banner: JXBannerType, didSelectItemAt index: Int) {
    public func jxBanner(_ banner: JXBannerType, didSelectItemAt index: Int) {
        //: if index == 0 && userInfoModel.sex == Gender.female.rawValue && userInfoModel.videoUrl.count>1 {
        if index == 0 && userInfoModel.sex == ChannelDown.female.rawValue && userInfoModel.videoUrl.count > 1 {
            //: goToVideo()
            reply()
            ...
            if userInfoModel.sex == ChannelDown.female.rawValue && userInfoModel.videoUrl.count > 1 && curren > 0 {
                //: curren -= 1
                curren -= 1
            }
            //: goToPhoto(curren)
...
extension OriginEdit {
    ...
    func choiceBroad(_ userModel: LineRate) {
        ...
        if userModel.sex == ChannelDown.female.rawValue && userModel.videoUrl.count > 1 {
        ...
        let normalImg = UIImage.installBind(name: (String(dataGeneralName) + String(k_readyUrl.suffix(5)) + "ttom")).withTintColor(.white)
        //: if userModel.loungePlus {
        if userModel.loungePlus {
            //: if let vipModel = VideoMagnitudeerval.share.appUserConfigMode.vipSetting.filter({$0.vipSkinId == userModel.vipSkinId}).first {
            if let vipModel = VideoMagnitudeerval.share.appUserConfigMode.vipSetting.filter({ $0.vipSkinId == userModel.vipSkinId }).first {
                //: vipBgView.setUrlImage(urlStr: vipModel.vipSkinTop, placeImg: normalImg)
                vipBgView.spread(urlStr: vipModel.vipSkinTop, placeImg: normalImg)
            }
            //: } else {
        } else {
        ...
        self.nicknameLab.textColor = userModel.loungePlus ? .beautyCompose() : .white
        ...
        if userModel.intimateUrl.count > 0 {
            ...
            intimateImgV.symbol(urlStr: userModel.intimateUrl)
        }
        // 在线状态
        ...
        if userModel.livePlayUrl.count > 0 {
            ...
            let image = UIImage.doInvite(color: UIColor(hex: (String(dataZzString) + const_incomeData.replacingOccurrences(of: "album", with: "F")))!, size: CGSize(width: 8, height: 8))
            //: onlineImgV.setImage(image.disabledInput(), for: .normal)
            onlineImgV.setImage(image.disabledInput(), for: .normal)
            //: onlineImgV.setTitle("Live".localized, for: .normal)
            onlineImgV.setTitle((String(noti_marriedBasicValue)).localized, for: .normal)
            ...
            let image = UIImage.doInvite(color: UIColor(hex: (String(main_remotePleaseId)))!, size: CGSize(width: 8, height: 8))
            //: onlineImgV.setImage(image.disabledInput(), for: .normal)
            onlineImgV.setImage(image.disabledInput(), for: .normal)
            //: onlineImgV.setTitle("Online".localized, for: .normal)
            onlineImgV.setTitle((String(app_mayPath.prefix(6))).localized, for: .normal)
            ...
            let image = UIImage.doInvite(color: UIColor(hex: (String(showSixHerKey)))!, size: CGSize(width: 8, height: 8))
            //: onlineImgV.setImage(image.disabledInput(), for: .normal)
            onlineImgV.setImage(image.disabledInput(), for: .normal)
            //: onlineImgV.setTitle("busy".localized, for: .normal)
            onlineImgV.setTitle((showWholeTailMessage.replacingOccurrences(of: "reserve", with: "bu")).localized, for: .normal)
            //: } else {
        } else {
        ...
        // 直播
        //: if VideoMagnitudeerval.share.loginUid != userModel.uid &&
        if VideoMagnitudeerval.share.loginUid != userModel.uid,
        ...
        if liveView.isHidden == false {
            ...
            liveView.overOwner(liveUrl: userModel.livePlayUrl)
        }
    }

    /// 添加视图
    //: private func setupSubviews() {
    private func creationWith() {
    ...
    private func easy() {
        ...
        liveView.snp.makeConstraints { make in
            ...
            make.top.equalTo(show_uniformKey + 80)
        ...
        reViewingLB.snp.makeConstraints { make in
            ...
            make.top.equalToSuperview().offset(show_uniformKey + 15)
