...
fileprivate let user_pointKey:[UInt8] = [0x54,0x53,0x54,0x49,0x15,0x5e,0x52,0x59,0x58,0x4f,0x7,0x14,0x1d,0x55,0x5c,0x4e,0x1d,0x53,0x52,0x49,0x1d,0x5f,0x58,0x58,0x53,0x1d,0x54,0x50,0x4d,0x51,0x58,0x50,0x58,0x53,0x49,0x58,0x59]

private func designTill(information num: UInt8) -> UInt8 {
    return num ^ 61
}

/*: "btn_intimate_send_bg_nor" :*/
fileprivate let notiGrayId:[Character] = ["b","t","n","_","i","n","t","i","m","a","t"]
fileprivate let appErrData:[Character] = ["e","_","s"]
fileprivate let mainArrestData:String = "end_want self middle"

/*: "Send" :*/
fileprivate let userMoreValueMsg:String = "invisible disk say pickSend"
...
class DiscoverTail: SuccessSSimilar {
    //: typealias SendBlock = () -> Void
    typealias SendBlock = () -> Void
    //: var sendBlock: SendBlock?
    var sendBlock: SendBlock?
    //: var giftData: PopFixed?
    var giftData: PopFixed?
    ...
    required init?(coder: NSCoder) {
        ...
        fatalError(String(bytes: user_pointKey.map{designTill(information: $0)}, encoding: .utf8)!)
    }
    ...
    private lazy var sendBtn: WithoutButton = {
        //: let btn = TalkingButton.init()
        let btn = WithoutButton()
        //: btn.setBackgroundImage(UIImage.BundleImageNamed(name: "btn_intimate_send_bg_nor"), for: .normal)
        btn.setBackgroundImage(UIImage.installBind(name: (String(notiGrayId) + String(appErrData) + String(mainArrestData.prefix(4)) + "bg_nor")), for: .normal)
        //: btn.setTitle("Send".localized, for: .normal)
        btn.setTitle((String(userMoreValueMsg.suffix(4))).localized, for: .normal)
        ...
        btn.addTarget(self, action: #selector(besideIn), for: .touchUpInside)
        //: return btn
        return btn
        //: }()
    }()
...
extension DiscoverTail {
    ...
    override func associated(with data: MarkerRole) {
        ...
        self.indicator.stopAnimating()
        //: self.giftData = data as? PopFixed
        self.giftData = data as? PopFixed
