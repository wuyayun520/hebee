...
fileprivate let mainUnlessStr:[UInt8] = [0xcf,0xd4,0xcf,0xda,0x8e,0xc9,0xd5,0xca,0xcb,0xd8,0xa0,0x8f,0x86,0xce,0xc7,0xd9,0x86,0xd4,0xd5,0xda,0x86,0xc8,0xcb,0xcb,0xd4,0x86,0xcf,0xd3,0xd6,0xd2,0xcb,0xd3,0xcb,0xd4,0xda,0xcb,0xca]

fileprivate func problemIncrease(the num: UInt8) -> UInt8 {
...
/*: "Not yet~" :*/
fileprivate let app_preciseTaName:[Character] = ["N","o","t"," ","y","e"]
fileprivate let appIconTitle:[Character] = ["t","~"]

/*: "#8C7AFF" :*/
fileprivate let dataThirdMissionTitle:[Character] = ["#","8","C","7","A","F","F"]
...
class ThanBorder: UIView {
    ...
    required init?(coder: NSCoder) {
        ...
        fatalError(String(bytes: mainUnlessStr.map{problemIncrease(the: $0)}, encoding: .utf8)!)
    }
    ...
    lazy var dataArray: [MomentumSubstantial] = [] {
        //: didSet {
        didSet {
    ...
    lazy var ranktableView = UITableView(frame: self.bounds, style: .plain).then {
        ...
        $0.register(SampleCanvasSound.self, forCellReuseIdentifier: SampleCanvasSound.className())
        //: $0.register(SocialRankListViewCell.self, forCellReuseIdentifier: SocialRankListViewCell.className())
        $0.register(SObserve.self, forCellReuseIdentifier: SObserve.className())
    }
    ...
    lazy var tableHeaderView = ClipGreat(frame: CGRect(x: 0, y: 0, width: notiExploreKey, height: actualHeight(h: 220 + 14))).then {
        //: $0.backgroundColor = .clear
        $0.backgroundColor = .clear
        //: $0.index = self.type
        $0.index = self.type
    ...
    lazy var tableBackgrourdView = VideoView().then {
        //: $0.backgroundColor = .white
        $0.backgroundColor = .white
        //: $0.desLab.text = "Not yet~".localized
        $0.desLab.text = (String(app_preciseTaName) + String(appIconTitle)).localized
        //: $0.desLab.textColor = UIColor(hex: "#8C7AFF")
        $0.desLab.textColor = UIColor(hex: (String(dataThirdMissionTitle)))
...
extension ThanBorder: UITableViewDataSource, UITableViewDelegate {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if indexPath.section == 0 {
            ...
            let cell = tableView.dequeueReusableCell(withIdentifier: SampleCanvasSound.className(), for: indexPath) as! SampleCanvasSound
            //: return cell
            return cell
        }
        ...
        let cell = tableView.dequeueReusableCell(withIdentifier: SObserve.className(), for: indexPath) as! SObserve
        //: let model = dataArray[indexPath.row+3]
        let model = dataArray[indexPath.row + 3]
        //: cell.updateRankCell(model: model, index: indexPath.row+3)
        cell.tabYield(model: model, index: indexPath.row + 3)
        //: cell.updateIcon(type: self.type)
        cell.nameure(type: self.type)
    ...
    func tableView(_ tableView: UITableView, heightForRowAt indexPath: IndexPath) -> CGFloat {
        ...
        if indexPath.section == 0 {
            ...
            let cell = tableView.dequeueReusableCell(withIdentifier: SampleCanvasSound.className()) as! SampleCanvasSound
...
extension ThanBorder: JXSegmentedListContainerViewListDelegate {
