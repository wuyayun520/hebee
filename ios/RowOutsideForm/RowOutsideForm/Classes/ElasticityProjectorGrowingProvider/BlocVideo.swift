...
fileprivate let show_displayMessage:[UInt8] = [0x16,0x11,0x16,0xb,0x57,0x1c,0x10,0x1b,0x1a,0xd,0x45,0x56,0x5f,0x17,0x1e,0xc,0x5f,0x11,0x10,0xb,0x5f,0x1d,0x1a,0x1a,0x11,0x5f,0x16,0x12,0xf,0x13,0x1a,0x12,0x1a,0x11,0xb,0x1a,0x1b]

private func requestHolder(largely num: UInt8) -> UInt8 {
    return num ^ 127
}

/*: "icon_lounge" :*/
fileprivate let notiToolId:[Character] = ["i","c","o","n","_"]
fileprivate let mainPreparePath:String = "loucleare"

/*: "#777777" :*/
fileprivate let data_takeContainerString:String = "#explainexplain7"
fileprivate let notiTimeCreateTitle:[Character] = ["7"]

/*: "UID: :*/
fileprivate let mainSecureUrl:String = "UID:accuse discount hung"
...
class BlocVideo: UITableViewCell {
    ...
    required init?(coder: NSCoder) {
        ...
        fatalError(String(bytes: show_displayMessage.map{requestHolder(largely: $0)}, encoding: .utf8)!)
    }
    ...
    private lazy var loungeImgV: UIImageView = {
        ...
        img.image = UIImage.installBind(name: (String(notiToolId) + mainPreparePath.replacingOccurrences(of: "clear", with: "ng")))
        //: return img
        return img
        //: }()
    }()
    ...
    private lazy var desLab: UILabel = {
        ...
        lab.font = UIFont.statinEssay(type: .Regular, fontSize: 15)
        //: lab.textColor = UIColor(hex: "#777777")
        lab.textColor = UIColor(hex: (data_takeContainerString.replacingOccurrences(of: "explain", with: "77") + String(notiTimeCreateTitle)))
        //: return lab
        return lab
        //: }()
    }()
...
extension BlocVideo {
    ...
    func topicDiscover(model: NumberegrityBelow) {
        //: headPicImgV.setUrlImage(urlStr: model.headPic)
        headPicImgV.symbol(urlStr: model.headPic)
        //: nicknameLab.textColor = model.loungePlus ? .userVipColor():.appTitleColor()
        nicknameLab.textColor = model.loungePlus ? .beautyCompose() : .serviceMajor()
        //: nicknameLab.text = model.nickname
        nicknameLab.text = model.nickname
        //: uidLab.text = "UID:\(model.uid)"
        uidLab.text = (String(mainSecureUrl.prefix(4))) + "\(model.uid)"
