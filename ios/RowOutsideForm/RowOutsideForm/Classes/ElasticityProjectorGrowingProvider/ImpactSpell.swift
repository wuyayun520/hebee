...
fileprivate let k_toleConclusionValue:[UInt8] = [0xd4,0xd3,0xd4,0xc9,0x95,0xde,0xd2,0xd9,0xd8,0xcf,0x87,0x94,0x9d,0xd5,0xdc,0xce,0x9d,0xd3,0xd2,0xc9,0x9d,0xdf,0xd8,0xd8,0xd3,0x9d,0xd4,0xd0,0xcd,0xd1,0xd8,0xd0,0xd8,0xd3,0xc9,0xd8,0xd9]

/*: "icon_chat_miss" :*/
fileprivate let dataGrainPath:[Character] = ["i","c","o","n","_","c","h","a","t","_","m","i","s","s"]

/*: "icon_chat_yes" :*/
fileprivate let data_refUrl:[Character] = ["i","c","o","n","_","c","h"]
fileprivate let notiCycleName:[Character] = ["a"]
fileprivate let user_originalId:String = "method opportunityt_yes"
...
class ImpactSpell: SuccessSSimilar {
//    var transBlock: (() -> Void)?

    //: var textData: ImageAngle?
    var textData: ImageAngle?
    ...
    override init(style: UITableViewCell.CellStyle, reuseIdentifier: String?) {
        ...
        //: designView()
        belowBlackAdvertising()
    }

    //: required init?(coder aDecoder: NSCoder) {
    required init?(coder aDecoder: NSCoder) {
        //: fatalError("init(coder:) has not been implemented")
        fatalError(String(bytes: k_toleConclusionValue.map{$0^189}, encoding: .utf8)!)
    }
...
//                self.messageData?.translateStatus != .MsgTranslate_succeed && self.messageData?.msgModel.msgInfo.callCmd.count ?? 0 <= 0 && temTextData.msgModel.msgInfo.messageType != .MessageTypeWelcome && "\(temTextData.msgModel.user.uid)" != FormatDistribution.getXiaoMiID()) { // 对方发的 && 开启了翻译功能 && 消息未翻译成功
...
//            }
            ...
            if temTextData.msgModel.quoteMsgInfo.willEqual() {
            ...
            if self.textData?.translateStatus == .MsgTranslate_translating {
                ...
                self.transStatusImagV.image = UIImage.installBind(name: (String(dataGrainPath)))
                ...
                self.transStatusImagV.image = UIImage.installBind(name: (String(data_refUrl) + String(notiCycleName) + String(user_originalId.suffix(5))))
            }
    ...
    func topFollow() {
        ...
        let containerTap = UITapGestureRecognizer(target: self, action: #selector(away(tap:)))
        ...
        self.container.addGestureRecognizer(containerTap)
        //: let isOfficial = (String(self.textData?.msgModel.user.uid ?? 0) == FormatDistribution.getXiaoMiID())
        let isOfficial = (String(self.textData?.msgModel.user.uid ?? 0) == FormatDistribution.raging())
        //: if isOfficial == false {
        if isOfficial == false {
            //: addLongTagGes()
            handleFit()
...
//            }
    ...
    func handleFit() {
        ...
        let longPressGr = UILongPressGestureRecognizer(target: self, action: #selector(head(tap:)))
        //: self.container.addGestureRecognizer(longPressGr)
        self.container.addGestureRecognizer(longPressGr)
    }

    //: @objc func TouchCellMessageAction(tap: UITapGestureRecognizer?) {
    @objc func away(tap: UITapGestureRecognizer?) {
    ...
    @objc func head(tap: UILongPressGestureRecognizer) {
        //: if tap.state == .ended {
        if tap.state == .ended {
            //: return
            return
...
//    }
    ...
    @objc func array() {
        ...
        self.delegate?.searched(cell: self)
    }
}
