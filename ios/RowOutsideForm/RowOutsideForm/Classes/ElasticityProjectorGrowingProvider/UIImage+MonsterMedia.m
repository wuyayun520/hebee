...
#import "UIImage+MonsterMedia.h"

//: @implementation UIImage (Compress)
@implementation UIImage (MonsterMedia)

//: - (NSData *)compressedImageData {
- (NSData *)spotDown {

    //: UIImage *image = self.wcTimelineCompress;
    UIImage *image = self.canvas;
...
- (UIImage *)bindGray {
    ...
    return [UIImage imageWithData:[self spotDown]];
}
...
- (UIImage *)stream {
    ...
    return [self magnitudeeraction:YES];
}

//: - (UIImage *)wcTimelineCompress {
- (UIImage *)canvas {
    //: return [self wcCompres:NO];
    return [self magnitudeeraction:NO];
}
...
- (UIImage *)magnitudeeraction:(Boolean)isSession {
    //: CGSize size = [self wxImageSize:isSession];
    CGSize size = [self modelPersist:isSession];
    //: UIImage *reImage = [self resizedImage:size];
    UIImage *reImage = [self shadowIn:size];
...
- (CGSize)modelPersist:(Boolean)isSession {
...
+ (NSData *)video:(UIImage *)image style:(NSInteger)fileSize{
    ...
    while (outIMGData.length>(fileSize*1024)) {
        ...
        thumIMG = [thumIMG format:scalesize minimum:NO];

        //: outIMGData = [thumIMG compressToFileSize:(fileSize*1024)];
        outIMGData = [thumIMG head:(fileSize*1024)];
    }
...
- (UIImage*)media:(CGSize)targetSize{
...
- (UIImage *)format:(CGSize)targetSize minimum:(BOOL)isScale{
...
- (NSData *)head:(NSInteger)fileSize{
...
- (UIImage *)shadowIn:(CGSize)newSize {
