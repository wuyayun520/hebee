...
fileprivate let kDumpVisibleKey:String = "six置普通"
fileprivate let dataProjectLogId:String = "色fee 请使"
fileprivate let app_boundKey:String = "composition comment master用RG"
fileprivate let constApplicationRegularName:String = "B\u{7a7a}间的颜色值"

/*: "设置选中状态的文字颜色时 请使用RGB空间的颜色值" :*/
fileprivate let app_imageValue:[Character] = ["设","置","选","\u{4e2d}","\u{72b6}","态"]
fileprivate let show_shMsg:String = "\u{7684}文\u{5b57}颜"
fileprivate let noti_allowEnforceKey:String = "RGB空\u{95f4}的\u{989c}色值"

/*: "init(coder:) has not been implemented" :*/
fileprivate let main_permissionId:[UInt8] = [0x1,0x6,0x1,0x1c,0x40,0xb,0x7,0xc,0xd,0x1a,0x52,0x41,0x48,0x0,0x9,0x1b,0x48,0x6,0x7,0x1c,0x48,0xa,0xd,0xd,0x6,0x48,0x1,0x5,0x18,0x4,0xd,0x5,0xd,0x6,0x1c,0xd,0xc]

private func bigCurrentlyUniversal(hidden num: UInt8) -> UInt8 {
    return num ^ 104
}

/*: "设置的下标不合法!!" :*/
fileprivate let k_delayTitle:String = "\u{8bbe}置的下\u{6807}"
fileprivate let main_thumbItemId:String = "不合法!!"

/*: "Bags" :*/
fileprivate let showFrameStatusUrl:String = "of business signal occasion runningBags"
...
open class ImpactPut: UIView {
    //: open var segmentStyle: SegmentStyle
    open var segmentStyle: EnvironmentGift
    /// 点击响应
    ...
    fileprivate lazy var normalColorRgb: (r: CGFloat, g: CGFloat, b: CGFloat) = {
        ...
        if let normalRgb = self.product(self.segmentStyle.normalTitleColor) {
            //: return normalRgb
            return normalRgb
            //: } else {
        } else {
            //: fatalError("设置普通状态的文字颜色时 请使用RGB空间的颜色值")
            fatalError((kDumpVisibleKey.replacingOccurrences(of: "six", with: "设") + "\u{72b6}态的文\u{5b57}颜" + dataProjectLogId.replacingOccurrences(of: "fee", with: "时") + String(app_boundKey.suffix(3)) + constApplicationRegularName))
        }
        //: }()
    }()
    ...
    fileprivate lazy var selectedTitleColorRgb: (r: CGFloat, g: CGFloat, b: CGFloat) = {
        ...
        if let selectedRgb = self.product(self.segmentStyle.selectedTitleColor) {
            //: return selectedRgb
            return selectedRgb
            //: } else {
        } else {
            //: fatalError("设置选中状态的文字颜色时 请使用RGB空间的颜色值")
            fatalError((String(app_imageValue) + show_shMsg + "色时 请\u{4f7f}用" + noti_allowEnforceKey))
        }
    ...
    fileprivate func product(_ color: UIColor) -> (r: CGFloat, g: CGFloat, b: CGFloat)? {
    ...
    public init(frame: CGRect, segmentStyle: EnvironmentGift, titles: [String]) {
    ...
    @objc func doingto(_ tapGes: UITapGestureRecognizer) {
        //: guard let currentLabel = tapGes.view as? CustomLabel else { return }
        guard let currentLabel = tapGes.view as? MaterialTiming else { return }
    ...
    public required init?(coder aDecoder: NSCoder) {
        ...
        fatalError(String(bytes: main_permissionId.map{bigCurrentlyUniversal(hidden: $0)}, encoding: .utf8)!)
    }
}
...
public extension ImpactPut {
    ///  对外界暴露设置选中的下标的方法 可以改变设置下标滚动后是否有动画切换效果
    //: public func selectedIndex(_ selectedIndex: Int, animated: Bool) {
    func gap(_ selectedIndex: Int, animated: Bool) {
        //: assert(!(selectedIndex < 0 || selectedIndex >= titles.count), "设置的下标不合法!!")
        assert(!(selectedIndex < 0 || selectedIndex >= titles.count), (k_delayTitle + main_thumbItemId.capitalized))
...
extension ImpactPut {
    ...
    fileprivate func written() {
        //: for (index, title) in titles.enumerated() {
        for (index, title) in titles.enumerated() {
            //: let label = CustomLabel(frame: CGRect.zero)
            let label = MaterialTiming(frame: CGRect.zero)
            //: label.tag = index
            label.tag = index
            //: label.text = title
            label.text = title
            ...
            let tapGes = UITapGestureRecognizer(target: self, action: #selector(self.doingto(_:)))
            ...
            if title == (String(showFrameStatusUrl.suffix(4))).localized {
    ...
    fileprivate func borderAnalysis() {
        // 设置Label位置
    ...
    fileprivate func respectiveWeek() {
        ...
        if let firstLabel = labelsArray[0] as? MaterialTiming {
    ...
    fileprivate func beforeMagnitude() {
    ...
    public func bulletRegulate(_ currentIndex: Int) {
...
open class MaterialTiming: UILabel {
    /// 用来记录当前label的缩放比例
