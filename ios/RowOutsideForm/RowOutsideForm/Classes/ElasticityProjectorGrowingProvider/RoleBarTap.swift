...
/*: "live_ended" :*/
fileprivate let showHemeValue:[Character] = ["l","i","v","e","_","e","n","d","e","d"]

/*: "The live broadcast has ended" :*/
fileprivate let appAssociateErrorString:[Character] = ["T","h","e"," ","l","i","v","e"," ","b","r"]
fileprivate let const_rainValue:[Character] = ["o","a","d","c","a","s","t"," ","h","a","s"," ","e","n","d","e","d"]

/*: "bth_live_male_phone_nor" :*/
fileprivate let mainCommitFormat:String = "bth_libuilder heap along explicit reminder"
fileprivate let data_pathStr:String = "e_phprojection bag scope rank"

/*: "floatingScreen" :*/
fileprivate let const_boldBitName:String = "freacha"
fileprivate let dataEnterKey:String = "EEN"

/*: "You have been muted" :*/
fileprivate let appVersusTunId:[Character] = ["Y","o","u"," ","h"]
fileprivate let showManModifyMessage:String = "provide situation since foundationave be"

/*: "You have entered the other party's blacklist" :*/
fileprivate let notiExplorationRibbonId:[UInt8] = [0xb8,0xce,0xd4,0x7f,0xc7,0xc0,0xd5,0xc4,0x7f,0xc4,0xcd,0xd3,0xc4,0xd1,0xc4,0xc3,0x7f,0xd3,0xc7,0xc4,0x7f,0xce,0xd3,0xc7,0xc4,0xd1,0x7f,0xcf,0xc0,0xd1,0xd3,0xd8,0x86,0xd2,0x7f,0xc1,0xcb,0xc0,0xc2,0xca,0xcb,0xc8,0xd2,0xd3]

fileprivate func dropLite(second num: UInt8) -> UInt8 {
...
fileprivate let mainWithinFormat:[UInt8] = [0xd8,0xfd,0xe2,0xf1,0xb4,0xe6,0xfb,0xfb,0xf9,0xb4,0xf1,0xec,0xf7,0xf1,0xe4,0xe0,0xfd,0xfb,0xfa,0xb8,0xb4,0xe4,0xf8,0xf1,0xf5,0xe7,0xf1,0xb4,0xe6,0xf1,0xe0,0xe1,0xe6,0xfa,0xb4,0xe0,0xfb,0xb4,0xe6,0xf1,0xe0,0xe6,0xed,0xb8,0xb4,0xfd,0xf2,0xb4,0xf9,0xe1,0xf8,0xe0,0xfd,0xe4,0xf8,0xf1,0xb4,0xf2,0xf5,0xfd,0xf8,0xe1,0xe6,0xf1,0xe7,0xb4,0xe4,0xf8,0xf1,0xf5,0xe7,0xf1,0xb4,0xf7,0xfb,0xfa,0xe0,0xf5,0xf7,0xe0,0xb4,0xe0,0xfc,0xf1,0xb4,0xe7,0xe0,0xf5,0xf2,0xf2]

private func onPan(exercise num: UInt8) -> UInt8 {
    return num ^ 148
}

/*: "isCalling" :*/
fileprivate let const_curveMsg:[Character] = ["i","s","C","a","l","l","i","n","g"]
...
//: enum LiveEnterType {
enum EnableChangeListenerReduce {
    //: case unknown
...
class RoleBarTap: MasterClip {
    //: var type = LiveEnterType.userDetail
    var type = EnableChangeListenerReduce.userDetail // 入口
    ...
    override func viewDidLoad() {
        ...
        YellowNumberensity.background().delegate = self
    ...
    private lazy var videoCallManager: CapDirect = {
        ...
        m.uid = String(RestoreChain.substantialAcrossBefore().liveRoomModel.streamerInfo.uid)
        //: return m
        return m
        //: }()
    }()
    ...
    private lazy var stopLiveBgView: UIView = {
        ...
        let icon = UIImageView(image: UIImage.installBind(name: (String(showHemeValue))))
        //: bg.addSubview(icon)
        bg.addSubview(icon)
        //: icon.snp.makeConstraints { make in
        icon.snp.makeConstraints { make in
            //: make.top.equalTo(StatusBarHeight+actualWidth(w: 205))
            make.top.equalTo(show_uniformKey + actualWidth(w: 205))
        ...
        lab.text = (String(appAssociateErrorString) + String(const_rainValue)).localized
        //: bg.addSubview(lab)
        bg.addSubview(lab)
    ...
    private lazy var bottomView: ChapterView = {
        //: let view = TalkingLiveRoomBottomView(frame: .zero)
        let view = ChapterView(frame: .zero)
        //: view.refreshBottomView(TalkingLiveManager.shared().liveRoomModel)
        view.blind(RestoreChain.substantialAcrossBefore().liveRoomModel)
        //: view.delegate = self
        view.delegate = self
        //: return view
        return view
    ...
    private lazy var commInputView: HandleThin = {
    ...
    private lazy var danmuView: ScientificView = {
        //: let view = TalkingDanmuMsgListTableView.init()
        let view = ScientificView()
        //: return view
        return view
        //: }()
    }()
    ...
    private lazy var callBtn: UIButton = {
        ...
        btn.setImage(UIImage.installBind(name: (String(mainCommitFormat.prefix(6)) + "ve_mal" + String(data_pathStr.prefix(4)) + "one_nor")), for: .normal)
        //: btn.addTarget(self, action: #selector(callButtonClick), for: .touchUpInside)
        btn.addTarget(self, action: #selector(cookieMovement), for: .touchUpInside)
    ...
    private lazy var giftEffectView: DivideView = {
        //: let effectView = TalkingPrivateChatAnimatView.init()
        let effectView = DivideView()
    ...
    private lazy var giftTrackView: PriorMultiple = {
        //: let trackView = TalkingGiftTrackView.init()
        let trackView = PriorMultiple()
    ...
    private lazy var topView: EagerView = {
        //: let v = TalkingLiveRoomTopView()
        let v = EagerView()
        //: v.weakVC = self
        v.weakVC = self
        //: v.refreshTopView(TalkingLiveManager.shared().liveRoomModel)
        v.itemThreshold(RestoreChain.substantialAcrossBefore().liveRoomModel)
    ...
    private lazy var floatScreenView: ElementBy = { // 飘屏
        //: let v = TalkingFloatScreenView()
        let v = ElementBy()
        //: v.isHidden = (VideoMagnitudeerval.share.appStatus == AppSkinStatus.special.rawValue)
        v.isHidden = (VideoMagnitudeerval.share.appStatus == PromiseTime.special.rawValue)
        //: return v
        return v
        //: }()
    }()
...
extension RoleBarTap {
    /// 播放直播内容
    //: @objc private func playLiveVideo() {
    @objc private func ridUnless() {
        //: needReconnect = false
        needReconnect = false
        ...
        self.player.startLivePlay(RestoreChain.substantialAcrossBefore().liveRoomModel.livePlayUrl, type: TX_Enum_PlayType.PLAY_TYPE_LIVE_FLV)
    }
    ...
    func estimated() {
        ...
        YellowNumberensity.by(groupId: RestoreChain.substantialAcrossBefore().liveRoomModel.chatGroupId, announcementMsg: RestoreChain.substantialAcrossBefore().liveRoomModel.notification)

        //: self.bottomView.toUid = String(TalkingLiveManager.shared().liveRoomModel.streamerInfo.uid)
        self.bottomView.toUid = String(RestoreChain.substantialAcrossBefore().liveRoomModel.streamerInfo.uid)
    }
    ...
    func variable() {
        ...
        V2TIMManager.sharedInstance().quitGroup(RestoreChain.substantialAcrossBefore().liveRoomModel.chatGroupId, succ: {
            //: }, fail: { code, str in
        }, fail: { code, str in
            //: })
        })
...
extension RoleBarTap {
    ...
    @objc func quantityry() {
        ...
        danmuView.snp.updateConstraints { make in
            ...
            make.bottom.equalTo(bottomView.snp.top).offset(-(app_keepId - constPutMsg - bottomView.height - constPublisherValue))
        }
    }

    //: @objc func danmuMoveBottom() {
    @objc func mutualButton() {
        //: var ishave = 0
        var ishave = 0
        //: for vc in self.children {
        for vc in self.children {
            //: if vc.isKind(of: TalkingPrivateChatController.self) || vc.isKind(of: TalkingChatListViewController.self) {
            if vc.isKind(of: BoundSmallThin.self) || vc.isKind(of: EmptyBelow.self) {
                //: ishave += 1
                ishave += 1
            }
        }
...
extension RoleBarTap {
    ...
    @objc private func cookieMovement() {
        //: var videoStr = TalkingLiveManager.shared().liveRoomModel.videoPrice
        var videoStr = RestoreChain.substantialAcrossBefore().liveRoomModel.videoPrice
        //: var voiceStr = TalkingLiveManager.shared().liveRoomModel.voicePrice
        var voiceStr = RestoreChain.substantialAcrossBefore().liveRoomModel.voicePrice
        //: if VideoMagnitudeerval.share.loginUserMode.loungePlus {
        if VideoMagnitudeerval.share.loginUserMode.loungePlus {
            //: videoStr = TalkingLiveManager.shared().liveRoomModel.videoVIPPrice
            videoStr = RestoreChain.substantialAcrossBefore().liveRoomModel.videoVIPPrice
            //: voiceStr = TalkingLiveManager.shared().liveRoomModel.voiceVIPPrice
            voiceStr = RestoreChain.substantialAcrossBefore().liveRoomModel.voiceVIPPrice
        }
        //: videoCallManager.showVideoCallMenu(videoPrice: "\(videoStr)", voicePrice: "\(voiceStr)")
        videoCallManager.aboveBy(videoPrice: "\(videoStr)", voicePrice: "\(voiceStr)")
        ...
        case .match:
            ...
            k_frameUrl.trainEssayDrownAccuseResMaxMessage(eventID: showMendReductionPath, toUid: String(RestoreChain.substantialAcrossBefore().liveRoomModel.streamerInfo.uid))
        ...
        case .userDetail:
            ...
            k_frameUrl.trainEssayDrownAccuseResMaxMessage(eventID: showFixedGreetName, toUid: String(RestoreChain.substantialAcrossBefore().liveRoomModel.streamerInfo.uid))
    ...
    func successTo(animated: Bool = true) {
        //: self.topView.stopTimer()
        self.topView.darkNorDisplay()
        //: req_quitGroup()
        variable() // 退出群聊
        //: TalkingLiveManager.destroy()
        RestoreChain.optionConsumption() // 销毁单例
        //: TalkingDanmuManager.shared().delegate = nil
        YellowNumberensity.background().delegate = nil
    ...
    @objc private func remain(notification: NSNotification) {
        //: let userinfo = notification.userInfo as! [String: AnyObject]
        let userinfo = notification.userInfo as! [String: AnyObject]
        //: let dict = userinfo["floatingScreen"]
        let dict = userinfo[(const_boldBitName.replacingOccurrences(of: "reach", with: "lo") + "tingScr" + dataEnterKey.lowercased())]
        //: if let model = TalkingFloatingScreenModel.deserialize(from: dict as? Dictionary) {
        if let model = StyleAll.deserialize(from: dict as? Dictionary) {
            //: self.floatScreenView.didReceive(model)
            self.floatScreenView.too(model)
        }
    }
}
...
extension RoleBarTap: SectionKeep {
    //: func func__commentBtnClick() {
    func containerEnhance() {
        //: if Int(Date().timeIntervalSince1970) < TalkingLiveManager.shared().liveRoomModel.muteExpireAt {
        if Int(Date().timeIntervalSince1970) < RestoreChain.substantialAcrossBefore().liveRoomModel.muteExpireAt {
            //: self.func__showStatusBarErrorMsg(showMsg: "You have been muted".localized)
            self.conclusion(showMsg: (String(appVersusTunId) + String(showManModifyMessage.suffix(6)) + "en muted").localized)
            //: return
            return
        }
        /// 拉起弹幕评论
...
extension RoleBarTap: ProvideArchitecture {
    ...
    func allGreen(Msg: CheckerUser) {
        ...
        danmuView.scientificWith(msgModel: Msg)
    }
    ...
    func bubbleOfChange(pushUid: String?) {
        ...
        let view = ForceView(frame: self.view.frame, uid: pushUid ?? "")
        //: view.delegate = self
        view.delegate = self
        //: view.show()
        view.betweenShadow()
    ...
    func kindPopular(nickName: String, atUid: String?) {
        ...
        commInputView.anyPure(toUid: atUid ?? "", nickName: nickName)
    }
...
extension RoleBarTap {
    ...
    func asBring(Msg: CheckerUser) {
        //: if giftTrackView.giftSendItem.count == 0 {
        if giftTrackView.giftSendItem.count == 0 {
            //: let arrM = NSMutableArray.init()
            let arrM = NSMutableArray()
            //: for model in TalkingChatGiftManager.share.getSendingItems() {
            for model in FlexibleTiming.share.waitServer() {
                //: arrM.add(model)
                arrM.add(model)
            }
            //: giftTrackView.giftSendItem = arrM as! [TalkingGiftNumArrModel]
            giftTrackView.giftSendItem = arrM as! [LikeCompose]
        }
        ...
        if giftModel != nil {
            ...
            giftTrackView.generateBy(model: giftModel!)
            //: giftEffectView.addGiftAnimatModelArr(modelArr: [giftModel!])
            giftEffectView.requireDelicate(modelArr: [giftModel!])
        }
    }
}
...
extension RoleBarTap: FragmentPurple {
    //: @objc func dismissClick() {
    @objc func pickAcrossNext() {
    ...
    func spineBy(msgStr: String, atUid: String?) {
        ...
        customerFun(msgStr: msgStr, toUid: atUid)
    }
    ...
    func script(heightToBottom: CGFloat) {
        ...
        danmuView.snp.updateConstraints { make in
            ...
            let height = heightToBottom == 0 ? 0 : -(heightToBottom - bottomView.height + commInputView.height - constPublisherValue)
    ...
    func customerFun(msgStr: String, toUid: String?) {
        ...
        YellowNumberensity.totalensityWith(groupId: RestoreChain.substantialAcrossBefore().liveRoomModel.chatGroupId, message: msgStr, toUid: toUid) { succeed, result, errorModel in
            //: if succeed {
            if succeed {}
        }
    }
...
extension RoleBarTap: V2TIMGroupListener {
    /// 被主播拉黑后，禁言退出房间
    //: func onMemberInfoChanged(_ groupID: String!, changeInfoList: [V2TIMGroupMemberChangeInfo]!) {
    func onMemberInfoChanged(_ groupID: String!, changeInfoList: [V2TIMGroupMemberChangeInfo]!) {
        //: if changeInfoList.first?.userID == VideoMagnitudeerval.share.loginUserMode.userID {
        if changeInfoList.first?.userID == VideoMagnitudeerval.share.loginUserMode.userID {
            //: let toastStr = "You have entered the other party's blacklist".localized
            let toastStr = String(bytes: notiExplorationRibbonId.map{dropLite(second: $0)}, encoding: .utf8)!.localized
...
extension RoleBarTap {
    ...
    @objc private func distributionPhase() {
...
extension RoleBarTap {
    ...
    @objc func sensorStructure(note: Notification) {
...
extension RoleBarTap: TXLivePlayListener {
    ...
    func onPlayEvent(_ evtID: Int32, withParam param: [AnyHashable: Any]!) {
        ...
        if evtID == PLAY_EVT_PLAY_END.rawValue { // 播放结束
            ...
            //: if VideoMagnitudeerval.share.networkStatus == .Unavailable {
            if VideoMagnitudeerval.share.networkStatus == .Unavailable {
                //: needReconnect = true
                needReconnect = true
                //: } else {
        ...
        { // 观众侧拉流失败，退出直播间
            ...
            self.conclusion(showMsg: String(bytes: mainWithinFormat.map{onPan(exercise: $0)}, encoding: .utf8)!.localized)
            //: popCurrentViewController()
            successTo()
        }
    }
...
extension RoleBarTap {
    ...
    private func belowBag() {
        //: self.view.backgroundColor = .appTitleColor()
        self.view.backgroundColor = .serviceMajor()
    ...
    private func tagAccessible() {
        ...
        danmuView.snp.makeConstraints { make in
            ...
            make.height.equalTo(data_transformKey)
            //: make.width.equalTo(MsgTableViewWidth)
            make.width.equalTo(noti_createTitle)
        }
        ...
        giftTrackView.snp.makeConstraints { make in
            ...
            make.bottom.equalTo(bottomView.snp.top).offset(-data_transformKey)
        }
    }
    ...
    func switche() {
        ...
        NotificationCenter.default.addObserver(self,
                                               ...
                                               selector: #selector(sensorStructure(note:)),
        ...
        NotificationCenter.default.addObserver(self,
                                               ...
                                               selector: #selector(distributionPhase),
        ...
        NotificationCenter.default.addObserver(self,
                                               ...
                                               selector: #selector(ridUnless),
                                               //: name: LIVE_RECIVE_STARTLIVE_NOTIFICATION,
                                               name: const_variableFormat,
                                               //: object: nil)
                                               object: nil)
        ...
        NotificationCenter.default.addObserver(self,
                                               ...
                                               selector: #selector(quantityry),
                                               //: name: LIVE_HALF_VIEW_SHOW,
                                               name: k_denseCompositionName,
                                               //: object: nil)
                                               object: nil)
        ...
        NotificationCenter.default.addObserver(self,
                                               ...
                                               selector: #selector(mutualButton),
                                               //: name: LIVE_HALF_VIEW_DISMISS,
                                               name: showAddSymbolName,
                                               //: object: nil)
                                               object: nil)
        // 飘屏
        //: NotificationCenter.default.addObserver(self,
        NotificationCenter.default.addObserver(self,
                                               ...
                                               selector: #selector(remain(notification:)),
                                               //: name: LIVE_RECIVE_FLOATSCREEN_NOTIFICATION,
                                               name: main_infoConversationTitle,
                                               //: object: nil)
                                               object: nil)

        // 音视频呼叫时，直播间静音
        //: TalkingSocketManager.shared.rx
        QuantityensityAction.shared.rx
            //: .observeWeakly(Bool.self, "isCalling")
            .observeWeakly(Bool.self, (String(const_curveMsg)))
        ...
        //: TalkingSocketManager.shared.rx
        QuantityensityAction.shared.rx
            .observeWeakly(Bool.self, "versusScope")
            //: .subscribe(onNext: { [weak self] (value) in
            .subscribe(onNext: { [weak self] value in
                //: guard let self = self else { return }
                guard let self = self else { return }
