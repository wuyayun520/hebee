...
fileprivate let main_dropName:[UInt8] = [0xaf,0xb4,0xaf,0xba,0x6e,0xa9,0xb5,0xaa,0xab,0xb8,0x80,0x6f,0x66,0xae,0xa7,0xb9,0x66,0xb4,0xb5,0xba,0x66,0xa8,0xab,0xab,0xb4,0x66,0xaf,0xb3,0xb6,0xb2,0xab,0xb3,0xab,0xb4,0xba,0xab,0xaa]

fileprivate func enterAcross(craft num: UInt8) -> UInt8 {
...
class ObjectViewController: MasterClip {
    //: var tranConfig: BasePresentTransition!
    var tranConfig: SizeSaveer!
    //: init() {
    init() {
        //: super.init(nibName: nil, bundle: nil)
        super.init(nibName: nil, bundle: nil)
        //: self.tranConfig = BasePresentTransition.init(target: self)
        self.tranConfig = SizeSaveer(target: self)
    }

    //: required init?(coder: NSCoder) {
    required init?(coder: NSCoder) {
        //: fatalError("init(coder:) has not been implemented")
        fatalError(String(bytes: main_dropName.map{enterAcross(craft: $0)}, encoding: .utf8)!)
    }
}

//: class BasePresentTransition: NSObject {
class SizeSaveer: NSObject {
    ...
    var frameOfPresentedView = CGRect(x: 0, y: 0, width: notiExploreKey, height: app_keepId * 0.6)
    ...
    func enableicial(corners: UIRectCorner, radius: CGFloat) {
...
extension SizeSaveer: UIViewControllerTransitioningDelegate {
    ...
    func presentationController(forPresented presented: UIViewController, presenting: UIViewController?, source: UIViewController) -> UIPresentationController? {
        ...
        let res = KeyController(transition: self, presentedViewController: presented, presentingViewController: presenting)
        //: return res
        return res
    }
}
...
extension SizeSaveer: UIViewControllerAnimatedTransitioning {
    ...
    func animateTransition(using transitionContext: UIViewControllerContextTransitioning) {
        ...
        if toVC == targetVC {
            ...
            bassOf(transitionContext: transitionContext)
            //: } else {
        } else {
            //: dismissTransition(transitionContext: transitionContext)
            architectureResponse(transitionContext: transitionContext)
        }
    }

    //: private func presentTransition(transitionContext: UIViewControllerContextTransitioning) {
    private func bassOf(transitionContext: UIViewControllerContextTransitioning) {
        //: let toVC = transitionContext.viewController(forKey: .to)
        let toVC = transitionContext.viewController(forKey: .to)
    ...
    private func architectureResponse(transitionContext: UIViewControllerContextTransitioning) {
...
class KeyController: UIPresentationController, UIGestureRecognizerDelegate {
    //: var transition: BasePresentTransition!
    var transition: SizeSaveer!

    //: init(transition: BasePresentTransition, presentedViewController: UIViewController, presentingViewController: UIViewController?) {
    init(transition: SizeSaveer, presentedViewController: UIViewController, presentingViewController: UIViewController?) {
    ...
    func lessFor() {
        ...
        let tap = UITapGestureRecognizer(target: self, action: #selector(drownWait))
        //: tap.delegate = self
        tap.delegate = self
    ...
    lazy var visualView: UIView = {
        ...
        view.frame = containerView?.bounds ?? CGRect(x: 0, y: 0, width: notiExploreKey, height: app_keepId)
