...
fileprivate let data_adSecurityPath:[UInt8] = [0x64,0x65,0x74,0x6e,0x65,0x6d,0x65,0x6c,0x70,0x6d,0x69,0x20,0x6e,0x65,0x65,0x62,0x20,0x74,0x6f,0x6e,0x20,0x73,0x61,0x68,0x20,0x29,0x3a,0x72,0x65,0x64,0x6f,0x63,0x28,0x74,0x69,0x6e,0x69]
...
let k_textAspectMsg = 80.0
//: let kProgressHUD_cornerRadius = 14.0
let constBelowName = 14.0
//: let kProgressHUD_alpha        = 0.9
let kBorderId = 0.9
//: let kBackgroundView_alpha     = 0.6
let kChapterCalculateFormat = 0.6
//: let kAnimationInterval        = 0.2
let show_qualityChallengeFormat = 0.2
//: let kTransformScale           = 0.9
let kManagerName = 0.9

//: open class ProgressHUD: UIView {
open class MediaSquare: UIView {
    //: required public init?(coder: NSCoder) {
    public required init?(coder: NSCoder) {
        //: fatalError("init(coder:) has not been implemented")
        fatalError(String(bytes: data_adSecurityPath.reversed(), encoding: .utf8)!)
    }

    //: static var shared = ProgressHUD()
    static var shared = MediaSquare()
    ...
    class func saveHead() {
        //: show(superView: nil)
        changeAndMedia(superView: nil)
    }

    //: class func show(superView: UIView?) {
    class func changeAndMedia(superView: UIView?) {
        //: if superView != nil {
        if superView != nil {
            //: DispatchQueue.main.async {
            DispatchQueue.main.async {
                //: ProgressHUD.shared.frame = superView!.bounds
                MediaSquare.shared.frame = superView!.bounds
                //: ProgressHUD.shared.activityIndicator.center = ProgressHUD.shared.center
                MediaSquare.shared.activityIndicator.center = MediaSquare.shared.center
                //: superView!.addSubview(ProgressHUD.shared)
                superView!.addSubview(MediaSquare.shared)
            }
            //: } else {
        } else {
            //: DispatchQueue.main.async {
            DispatchQueue.main.async {
                ...
                MediaSquare.shared.frame = UIScreen.main.bounds
                //: ProgressHUD.shared.activityIndicator.center = ProgressHUD.shared.center
                MediaSquare.shared.activityIndicator.center = MediaSquare.shared.center
                //: FormatDistribution.getWindow().addSubview(ProgressHUD.shared)
                FormatDistribution.group().addSubview(MediaSquare.shared)
            }
        }
        //: ProgressHUD.shared.hud_startAnimating()
        MediaSquare.shared.homeDownUnder()
    }

    //: class func dismiss() {
    class func spell() {
        //: ProgressHUD.shared.hud_stopAnimating()
        MediaSquare.shared.untilColor()
    }

    //: private func hud_startAnimating() {
    private func homeDownUnder() {
        //: DispatchQueue.main.async {
        DispatchQueue.main.async {
            //: self.backgroundColor = UIColor(white: 0, alpha: 0)
            self.backgroundColor = UIColor(white: 0, alpha: 0)
            //: self.activityIndicator.transform = CGAffineTransform(scaleX: kTransformScale, y: kTransformScale)
            self.activityIndicator.transform = CGAffineTransform(scaleX: kManagerName, y: kManagerName)
            //: self.activityIndicator.alpha = 0
            self.activityIndicator.alpha = 0
            //: UIView.animate(withDuration: kAnimationInterval) {
            UIView.animate(withDuration: show_qualityChallengeFormat) {
                //: self.backgroundColor = UIColor(white: 0, alpha: kBackgroundView_alpha)
                self.backgroundColor = UIColor(white: 0, alpha: kChapterCalculateFormat)
                //: self.activityIndicator.transform = CGAffineTransform(scaleX: 1, y: 1)
                self.activityIndicator.transform = CGAffineTransform(scaleX: 1, y: 1)
                //: self.activityIndicator.alpha = kProgressHUD_alpha
                self.activityIndicator.alpha = kBorderId
    ...
    private func untilColor() {
        ...
        DispatchQueue.main.async {
            ...
            UIView.animate(withDuration: show_qualityChallengeFormat) {
                ...
                MediaSquare.shared.removeFromSuperview()
            }
        }
    }
    ...
    private lazy var activityIndicator: UIActivityIndicatorView = {
        ...
        indicator.bounds = CGRect(x: 0, y: 0, width: k_textAspectMsg, height: k_textAspectMsg)
        //: indicator.center = self.center
        indicator.center = self.center
        //: indicator.backgroundColor = .black
        indicator.backgroundColor = .black
        //: indicator.layer.cornerRadius = kProgressHUD_cornerRadius
        indicator.layer.cornerRadius = constBelowName
...
extension MediaSquare {
    //: class func toast(_ str: String?) {
    class func wake(_ str: String?) {
        //: toast(str, showTime: 1)
        fileMore(str, showTime: 1)
    }

    //: class func toast(_ str: String?, showTime: CGFloat) {
    class func fileMore(_ str: String?, showTime: CGFloat) {
        ...
        titleLab.textColor = .white
        //: FormatDistribution.getWindow().addSubview(titleLab)
        FormatDistribution.group().addSubview(titleLab)
        //: let size = titleLab.sizeThatFits(CGSize(width: UIScreen.main.bounds.width - 40, height: CGFloat(MAXFLOAT)))
        let size = titleLab.sizeThatFits(CGSize(width: UIScreen.main.bounds.width - 40, height: CGFloat(MAXFLOAT)))
        //: titleLab.center = FormatDistribution.getWindow().center
        titleLab.center = FormatDistribution.group().center
