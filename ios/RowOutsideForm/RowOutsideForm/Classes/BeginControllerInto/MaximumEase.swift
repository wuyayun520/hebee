...
fileprivate let dataYerIdentityValue:[UInt8] = [0x64,0x65,0x74,0x6e,0x65,0x6d,0x65,0x6c,0x70,0x6d,0x69,0x20,0x6e,0x65,0x65,0x62,0x20,0x74,0x6f,0x6e,0x20,0x73,0x61,0x68,0x20,0x29,0x3a,0x72,0x65,0x64,0x6f,0x63,0x28,0x74,0x69,0x6e,0x69]

/*: "FFD855" :*/
fileprivate let kReplaceName:String = "comment"
fileprivate let const_professionalMsg:String = "FDtop55"
...
class MaximumEase: UIView {
    ...
    var defaultWidth: CGFloat = 0.0 {
        ...
        didSet {
            ...
            DispatchQueue.main.asyncAfter(deadline: DispatchTime.now() + 0.2) {
                ...
                let minX = self.leftSilder.frame.origin.x + self.defaultWidth - app_appearanceTitle
                //: self.rightSlider.frame = CGRect(x: minX,
                self.rightSlider.frame = CGRect(x: minX,
                                                //: y: self.rightSlider.frame.origin.y,
                                                y: self.rightSlider.frame.origin.y,
                                                //: width: kSliderWidth,
                                                width: app_appearanceTitle,
                ...
                self.survivalTotaleraction(startX: self.videoStartX)
    ...
    required init?(coder: NSCoder) {
        ...
        fatalError(String(bytes: dataYerIdentityValue.reversed(), encoding: .utf8)!)
    }
    ...
    private lazy var indicatorView: UIView = {
        ...
        indicator.backgroundColor = UIColor(hex: (kReplaceName.replacingOccurrences(of: "comment", with: "F") + const_professionalMsg.replacingOccurrences(of: "top", with: "8")))
        //    indicator.alpha = 0.5
    ...
    private lazy var leftSilder: UIView = {
        ...
        leftSilder.frame = CGRect(x: mainAgeScenarioData, y: 0, width: app_appearanceTitle, height: self.frame.size.height)
    ...
    private lazy var rightSlider: UIView = {
        ...
        rightSlider.frame = CGRect(x: self.frame.size.width - app_appearanceTitle, y: 0, width: app_appearanceTitle, height: self.frame.size.height)
    ...
    private lazy var leftCoverView: UIView = {
        ...
        leftCoverView.backgroundColor = UIColor.noticeSatisfy(0, 0, 0, 0.6)
        //: leftCoverView.frame = CGRect(x: 0, y: 0, width: LeftMargin, height: self.frame.size.height)
        leftCoverView.frame = CGRect(x: 0, y: 0, width: mainAgeScenarioData, height: self.frame.size.height)
    ...
    private lazy var rightCoverView: UIView = {
        ...
        rightCoverView.backgroundColor = UIColor.noticeSatisfy(0, 0, 0, 0.6)
...
extension MaximumEase {
    /// 重置
    //: func resetOriginalState() {
    func additionalDoingSpecify() {
        //: self.leftSilder.frame = CGRect(x: LeftMargin, y: 0, width: kSliderWidth, height: self.frame.size.height)
        self.leftSilder.frame = CGRect(x: mainAgeScenarioData, y: 0, width: app_appearanceTitle, height: self.frame.size.height)
        //: self.rightSlider.frame = CGRect(x: self.frame.size.width - kSliderWidth, y: 0, width: kSliderWidth, height: self.frame.size.height)
        self.rightSlider.frame = CGRect(x: self.frame.size.width - app_appearanceTitle, y: 0, width: app_appearanceTitle, height: self.frame.size.height)
        //: resetIndicator()
        counterruptTurn()
    }

    //: func needShowIndicator(isShow: Bool) {
    func raceAcross(isShow: Bool) {
        //: self.indicatorView.isHidden = !isShow
        self.indicatorView.isHidden = !isShow
    }

    //: func refreshSliderValidScope(startX: CGFloat) {
    func survivalTotaleraction(startX: CGFloat) {
        ...
        rightScope = CGVector(dx: leftSilder.frame.minX + minDurationGap - app_appearanceTitle,
                              //: dy: min(leftSilder.frame.minX + maxDurationGap, self.width) - kSliderWidth)
                              dy: min(leftSilder.frame.minX + maxDurationGap, self.width) - app_appearanceTitle)
    }
}
...
extension MaximumEase {
    ...
    @objc func increaseCouncil(gesture: UIPanGestureRecognizer) {
        ...
        case .ended:
            ...
            self.survivalTotaleraction(startX: videoStartX)
    ...
    @objc func withinEnvironment(gesture: UIPanGestureRecognizer) {
        //: let thisTimePoint = gesture.location(in: self)
        let thisTimePoint = gesture.location(in: self)
        //: let state = gesture.state
        let state = gesture.state
...
extension MaximumEase {
    ...
    private func opinionVisitor() {
        ...
        let leftPanGesture = UIPanGestureRecognizer(target: self, action: #selector(increaseCouncil(gesture:)))
        ...
        let rightPanGesture = UIPanGestureRecognizer(target: self, action: #selector(withinEnvironment(gesture:)))
    ...
    func momentumRegulation(_ xposition: CGFloat) {
