...
/*: "allowList" :*/
fileprivate let app_willingnessName:String = "allowLiwillingness rating too"
fileprivate let kWindowId:String = "sufficientt"

/*: "denyList" :*/
fileprivate let main_tweenName:String = "denyLihere step visitor resign scale"
fileprivate let main_additionalDrawData:[Character] = ["s","t"]

/*: "America" :*/
fileprivate let main_currencyDoingData:String = "Americaroad margin reliability"
...
class ThresholdCompare: NSObject {
    //: var reqSuccessBlock: ReqSuccessBlock!
    var reqSuccessBlock: ReqSuccessBlock!

    //: var areaArr = [TalkingAreaModel]()
    var areaArr = [ButtonStatusWrapModel]() /// 注册允许国家列表（白名单）
    //: var blackListAreaArr = [TalkingAreaModel]()
    var blackListAreaArr = [ButtonStatusWrapModel]() /// 注册不允许国家列表（黑名单）

    //: static let share = TalkingLocationTool()
    static let share = ThresholdCompare()
...
extension ThresholdCompare {
    ...
    private func jar() {
        ...
        NameExercise.persist { succeed, result, errorModel in
            //: ProgressHUD.dismiss()
            MediaSquare.spell()
            //: if succeed {
            if succeed {
                ...
                let allowArray: Array = dict[(String(app_willingnessName.prefix(7)) + kWindowId.replacingOccurrences(of: "sufficient", with: "s"))] as! [Any]
                //: let denyListArray: Array =  dict["denyList"] as! Array<Any>
                let denyListArray: Array = dict[(String(main_tweenName.prefix(6)) + String(main_additionalDrawData))] as! [Any]

                //: if let datas = Array<TalkingAreaModel>.deserialize(from: allowArray as? Array) {
                if let datas = Array<ButtonStatusWrapModel>.deserialize(from: allowArray as? Array) {
                    //: self.areaArr.append(contentsOf: (datas as? [TalkingAreaModel])!)
                    self.areaArr.append(contentsOf: (datas as? [ButtonStatusWrapModel])!)
                }
                //: if let blackdatas = Array<TalkingAreaModel>.deserialize(from: denyListArray as? Array) {
                if let blackdatas = Array<ButtonStatusWrapModel>.deserialize(from: denyListArray as? Array) {
                    //: self.blackListAreaArr.append(contentsOf: (blackdatas as? [TalkingAreaModel])!)
                    self.blackListAreaArr.append(contentsOf: (blackdatas as? [ButtonStatusWrapModel])!)
                }
    ...
    static func afterRise() -> Bool {
        //: var include = false
        var include = false
        //: let simInfo = UIDevice.getDeviceSimInfo()
        let simInfo = UIDevice.steerCharacter()
        //: let arr = TalkingLocationTool.share.blackListAreaArr
        let arr = ThresholdCompare.share.blackListAreaArr
        //: for model in arr {
        for model in arr {
    ...
    func cartoon() -> ButtonStatusWrapModel {
        //: let simInfo = UIDevice.getDeviceSimInfo()
        let simInfo = UIDevice.steerCharacter()
        ...
        return ButtonStatusWrapModel(areaCode: "1", areaName: (String(main_currencyDoingData.prefix(7))), countryCode: "US", url: "")
    }
}

//: class TalkingAreaModel: HandyJSON {
class ButtonStatusWrapModel: HandyJSON {
    //: var areaCode = ""
    var areaCode = ""
    //: var countryCode = ""
    var countryCode = ""
