...
fileprivate let constNoteValue:[UInt8] = [0x8b,0x90,0x8b,0x96,0x4a,0x85,0x91,0x86,0x87,0x94,0x5c,0x4b,0x42,0x8a,0x83,0x95,0x42,0x90,0x91,0x96,0x42,0x84,0x87,0x87,0x90,0x42,0x8b,0x8f,0x92,0x8e,0x87,0x8f,0x87,0x90,0x96,0x87,0x86]

fileprivate func nowRid(receive num: UInt8) -> UInt8 {
...
/*: "View read receipts?" :*/
fileprivate let user_edgeVideoId:[Character] = ["V","i","e","w"," ","r","e"]
fileprivate let showThreadMsg:[Character] = ["a","d"," ","r","e"]
fileprivate let data_includeSecondUrl:[Character] = ["c","e","i","p","t","s","?"]

/*: "text" :*/
fileprivate let user_orientationMessage:String = "statinexstatin"

/*: "color" :*/
fileprivate let k_fillTitle:[UInt8] = [0x72,0x6f,0x6c,0x6f,0x63]

/*: "<xml>%@</xml>" :*/
fileprivate let user_easeUrl:[Character] = ["<","x","m","l",">","%","@","<","/","x","m","l",">"]

/*: "href" :*/
fileprivate let data_fastLitMsg:[Character] = ["h","r","e","f"]

/*: "type" :*/
fileprivate let mainPeerPath:[UInt8] = [0xb2,0xbf,0xb6,0xa3]

private func tackScientific(always num: UInt8) -> UInt8 {
    return num ^ 198
}

/*: "txt" :*/
fileprivate let user_womanGoingString:[Character] = ["t","x","t"]

/*: "padding" :*/
fileprivate let k_formatMonthKey:String = "PADDIN"
fileprivate let constImpressionArchitectureString:String = "city"

/*: "size" :*/
fileprivate let mainColorEstateAlongsideString:String = "simedale"

/*: "maxFont" :*/
fileprivate let data_ageTaskName:[Character] = ["m"]
fileprivate let userGiveId:String = "axFontflexible fee make identify"

/*: "link" :*/
fileprivate let constWritingRenderStr:[Character] = ["l","i","n","k"]

/*: "PingFangSC-Regular" :*/
fileprivate let user_observeKey:String = "Pingdecide personal direction divide enable"
fileprivate let mainDumpPath:String = "genuine tap except down later-Regul"
fileprivate let showMagnitudeerestString:String = "again"

/*: "content" :*/
fileprivate let const_starValue:[UInt8] = [0x74,0x6e,0x65,0x74,0x6e,0x6f,0x63]
...
@objcMembers public class SecondDepthLike: AddProcessorCombine {
    ...
    required init?(coder: NSCoder) {
        ...
        fatalError(String(bytes: constNoteValue.map{nowRid(receive: $0)}, encoding: .utf8)!)
    }

    //: public override func heightOfWidth(_ width: CGFloat) ->CGFloat {
    override public func city(_ width: CGFloat) -> CGFloat {
        //: return contentSize().height+20
        return cancel().height + 20
    }
    ...
    override public func cancel() -> CGSize {
        ...
        if self.msgModel.tips.content == (String(user_edgeVideoId) + String(showThreadMsg) + String(data_includeSecondUrl)).localized {
            //: return CGSize(width: ScreenWidth, height: 30)
            return CGSize(width: notiExploreKey, height: 30)
        }
        ...
        //: let rect = self.attributedString.boundingRect(with: CGSize.init(width: FormatDistribution.getScreenWidth()-50, height: CGFloat(MAXFLOAT)), options: [.usesLineFragmentOrigin, .usesFontLeading], context: nil )
        let rect = self.attributedString.boundingRect(with: CGSize(width: FormatDistribution.tillText() - 50, height: CGFloat(MAXFLOAT)), options: [.usesLineFragmentOrigin, .usesFontLeading], context: nil)
    ...
    public lazy var attributedString: NSMutableAttributedString = {
        ...
        var attr = temporaryWrite(info: self.msgModel.tips)
        //: return attr
        return attr
        //: }()
    }()
...
extension SecondDepthLike {
    //: func formatMessageToStringM(info: AbTalkingChatMsgTipsModel) -> NSMutableAttributedString {
    func temporaryWrite(info: TriggerModel) -> NSMutableAttributedString {
...
//        let dic = NSDictionary(jsonString: extra ?? "")
        ...
        return purchase(tips: content)
    }

    //: func designTextFream(tips: String) -> NSMutableAttributedString {
    func purchase(tips: String) -> NSMutableAttributedString {
        ...
        _ = seek(msg: tips, info: attrbutedArray)
        //: self.yyLayout = designYYTextLayout(array: attrbutedArray, displayMaxWidth: FormatDistribution.getScreenWidth()-50, para: para)
        self.yyLayout = entity(array: attrbutedArray, displayMaxWidth: FormatDistribution.tillText() - 50, para: para)
    ...
    func seek(msg: String, info: NSMutableArray) -> String {
        ...
        let pure_msg = price(source: msg, attrbAM: contentAttrArray)
        //: for  textInfo in contentAttrArray {
        for textInfo in contentAttrArray {
            //: let textDic = textInfo as![String: Any]
            let textDic = textInfo as! [String: Any]
            //: let text: String = textDic["text"] as! String
            let text: String = textDic[(user_orientationMessage.replacingOccurrences(of: "statin", with: "t"))] as! String

            //: var textColor: UIColor?
            var textColor: UIColor?

            //: var obj = textDic["color"]
            var obj = textDic[String(bytes: k_fillTitle.reversed(), encoding: .utf8)!]
            //: if obj is UIColor {
            if obj is UIColor {
                //: obj = obj as? UIColor
                obj = obj as? UIColor
            ...
            contentDict.removeObjects(forKeys: [String(bytes: k_fillTitle.reversed(), encoding: .utf8)!, (user_orientationMessage.replacingOccurrences(of: "statin", with: "t"))])

            //: let attrDict = ctDictionaryForText(content: text, color: textColor)
            let attrDict = liteFrame(content: text, color: textColor)
    ...
    func price(source: String, attrbAM: NSMutableArray) -> String {
        ...
        do {
            ...
            let document = try ONOXMLDocument(string: String(format: (String(user_easeUrl)), newSource), encoding: CFStringConvertEncodingToNSStringEncoding(CFStringBuiltInEncodings.UTF8.rawValue))
            //: let element = document.rootElement
            let element = document.rootElement
            //: parseXMLElement(element: element ?? ONOXMLElement(), arrM: attrbAM, attrib: ["color": UIColor.white])
            mediaSegmentLostEnhance(element: element ?? ONOXMLElement(), arrM: attrbAM, attrib: [String(bytes: k_fillTitle.reversed(), encoding: .utf8)!: UIColor.white])
            ...
            attrbAM.add([(user_orientationMessage.replacingOccurrences(of: "statin", with: "t")): newSource, String(bytes: k_fillTitle.reversed(), encoding: .utf8)!: UIColor.white])
            //: return newSource
            return newSource
        }
    }
    ...
    func mediaSegmentLostEnhance(element: ONOXMLElement, arrM: NSMutableArray, attrib: NSDictionary) {
        ...
        if children.count > 0 {
            ...
            if element.tag == "u" { // 下滑线
                ...
                let url: String = element.attributes[(String(data_fastLitMsg))] as! String
                //: if url.count > 0 {
                if url.count > 0 {
                    //: parentInfo["href"] = url
                    parentInfo[(String(data_fastLitMsg))] = url
                }
            }
            //: for  subElement in children {
            for subElement in children {
                //: parseXMLElement(element: subElement as! ONOXMLElement, arrM: arrM, attrib: parentInfo)
                mediaSegmentLostEnhance(element: subElement as! ONOXMLElement, arrM: arrM, attrib: parentInfo)
            }
            ...
            if text != nil {
                ...
                //: info["text"] = text
                info[(user_orientationMessage.replacingOccurrences(of: "statin", with: "t"))] = text
                //: arrM.add(info)
                arrM.add(info)
            }
        }
    ...
    func entity(array: NSArray, displayMaxWidth: CGFloat, para: NSParagraphStyle?) -> YYTextLayout {
        //: let attrString = designAttribStr(array: array, paragraphStyle: para)
        let attrString = outsideAnswer(array: array, paragraphStyle: para)
    ...
    func outsideAnswer(array: NSArray, paragraphStyle: NSParagraphStyle?) -> NSAttributedString {
        ...
        array.enumerateObjects { obj, idx, stop in
            ...
            let type = newObj[String(bytes: mainPeerPath.map{tackScientific(always: $0)}, encoding: .utf8)!] as? String
            //: var tmp = NSAttributedString.init()
            var tmp = NSAttributedString()
            //: if type  == "txt" {
            if type == (String(user_womanGoingString)) {
                //: tmp = designAttribStrByTextDic(dict: newObj as NSDictionary)
                tmp = selecter(dict: newObj as NSDictionary)
                //: } else if type == "padding" {
            } else if type == (k_formatMonthKey.lowercased() + constImpressionArchitectureString.replacingOccurrences(of: "city", with: "g")) {
                //: let imageView = UIImageView.init()
                let imageView = UIImageView()
                //: imageView.size = (newObj["size"] as! NSValue).cgSizeValue
                imageView.size = (newObj[(mainColorEstateAlongsideString.replacingOccurrences(of: "medal", with: "z"))] as! NSValue).cgSizeValue
                ...
                tmp = NSMutableAttributedString.yy_attachmentString(withContent: imageView, contentMode: .center, attachmentSize: imageView.size, alignTo: newObj[(String(data_ageTaskName) + String(userGiveId.prefix(6)))] as! UIFont, alignment: .center)
            }
            ...
            if underline {
                ...
                if len > 0 {
                    ...
                    _ = YYTextDecoration(style: .single, width: 1.5, color: newObj[String(bytes: k_fillTitle.reversed(), encoding: .utf8)!] as? UIColor)
    ...
    func zone(dict: NSDictionary) -> NSAttributedString {
        //: let imageView = UIImageView.init()
        let imageView = UIImageView()
        //: imageView.size = (dict["size"]as! NSValue).cgSizeValue
        imageView.size = (dict[(mainColorEstateAlongsideString.replacingOccurrences(of: "medal", with: "z"))] as! NSValue).cgSizeValue
        ...
        let attachText = NSMutableAttributedString.yy_attachmentString(withContent: imageView, contentMode: .center, attachmentSize: imageView.size, alignTo: dict[(String(data_ageTaskName) + String(userGiveId.prefix(6)))] as! UIFont, alignment: .center)
        //: return attachText
        return attachText
    }

    //: func designAttribStrByTextDic(dict: NSDictionary) ->NSMutableAttributedString {
    func selecter(dict: NSDictionary) -> NSMutableAttributedString {
        //: let type = dict["type"] as! String
        let type = dict[String(bytes: mainPeerPath.map{tackScientific(always: $0)}, encoding: .utf8)!] as! String
        //: if type != "link" && type != "txt" {
        if type != (String(constWritingRenderStr)) && type != (String(user_womanGoingString)) {
            //: return NSMutableAttributedString.init()
            return NSMutableAttributedString()
        }
        //: let attributes = attributesWithConfig()
        let attributes = pressure()
        // set color
        //: let color = dict["color"] as? UIColor
        let color = dict[String(bytes: k_fillTitle.reversed(), encoding: .utf8)!] as? UIColor
        //: if color != nil {
        if color != nil {
        ...
        let fontSize = (dict[(mainColorEstateAlongsideString.replacingOccurrences(of: "medal", with: "z"))] as? NSNumber)?.floatValue
        //: if (fontSize ?? 0 > 0) {
        if fontSize ?? 0 > 0 {
            //: let fontRef = CTFontCreateWithName("PingFangSC-Regular" as CFString, CGFloat(fontSize!), nil)
            let fontRef = CTFontCreateWithName((String(user_observeKey.prefix(4)) + "FangSC" + String(mainDumpPath.suffix(6)) + showMagnitudeerestString.replacingOccurrences(of: "again", with: "ar")) as CFString, CGFloat(fontSize!), nil)
            //: attributes[kCTFontAttributeName] = fontRef
            attributes[kCTFontAttributeName] = fontRef
        }
        //: var content = (dict["content"] != nil) ? dict["content"] : ""
        var content = (dict[String(bytes: const_starValue.reversed(), encoding: .utf8)!] != nil) ? dict[String(bytes: const_starValue.reversed(), encoding: .utf8)!] : ""
    ...
    func pressure() -> NSMutableDictionary {
        //: let font = UIFont.pingfangRugularFont(fontSize: 15)
        let font = UIFont.sumervalSort(fontSize: 15)
    ...
    func liteFrame(content: String, color: UIColor?) -> NSDictionary {
        //: if content.count <= 0 {
        if content.count <= 0 {
        ...
        if color == nil {
            ...
            return recordLight(content: content)
        }
        //: return ["type": "txt",
        return [String(bytes: mainPeerPath.map{tackScientific(always: $0)}, encoding: .utf8)!: (String(user_womanGoingString)),
                //: "content": content,
                String(bytes: const_starValue.reversed(), encoding: .utf8)!: content,
                //: "color": color ?? .black]
                String(bytes: k_fillTitle.reversed(), encoding: .utf8)!: color ?? .black]
    }

    //: func ctDictionaryForText(content: String) ->NSDictionary {
    func recordLight(content: String) -> NSDictionary {
        //: if content.count <= 0 {
        if content.count <= 0 {
            //: return NSDictionary.init()
            return NSDictionary()
