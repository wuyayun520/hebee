...
fileprivate let data_featureBeatStr:[UInt8] = [0xdb,0xe0,0xdb,0xe6,0x9a,0xd5,0xe1,0xd6,0xd7,0xe4,0xac,0x9b,0x92,0xda,0xd3,0xe5,0x92,0xe0,0xe1,0xe6,0x92,0xd4,0xd7,0xd7,0xe0,0x92,0xdb,0xdf,0xe2,0xde,0xd7,0xdf,0xd7,0xe0,0xe6,0xd7,0xd6]

fileprivate func memorySire(tail num: UInt8) -> UInt8 {
...
/*: "move" :*/
fileprivate let dataExpressionKey:String = "movking"
...
class ElementBy: UIView {
    ...
    required init?(coder: NSCoder) {
        ...
        fatalError(String(bytes: data_featureBeatStr.map{memorySire(tail: $0)}, encoding: .utf8)!)
    }
    ...
    func too(_ danumuMsgModel: StyleAll?) {
    ...
    @objc func spineInLoopTask(_ tapGesture: UITapGestureRecognizer) {}
    ...
    lazy var tapGesture: UITapGestureRecognizer = {
        ...
        let t = UITapGestureRecognizer(target: self, action: #selector(spineInLoopTask(_:)))
        //: t.delegate = self
        t.delegate = self
        //: return t
        return t
...
extension ElementBy: UIGestureRecognizerDelegate {
    ...
    func isolate(for danumuMsgModel: StyleAll?, isFirst: Bool) {
        //: let itemView = TalkingFloatScreenItemView(model: danumuMsgModel)
        let itemView = MasterView(model: danumuMsgModel)
        ...
        if let anim = POPBasicAnimation(propertyNamed: kPOPViewFrame) {
            ...
            itemView.pop_add(anim, forKey: (dataExpressionKey.replacingOccurrences(of: "king", with: "e")))
    ...
    func secondOdd(_ isFirst: Bool) {
        ...
        if danumuMsgArray.count > 0 {
            ...
            if let model {
                ...
                isolate(for: model, isFirst: isFirst)
