...
fileprivate let dataExactlyString:[UInt8] = [0x6a,0x6f,0x6a,0x75,0x29,0x64,0x70,0x65,0x66,0x73,0x3b,0x2a,0x21,0x69,0x62,0x74,0x21,0x6f,0x70,0x75,0x21,0x63,0x66,0x66,0x6f,0x21,0x6a,0x6e,0x71,0x6d,0x66,0x6e,0x66,0x6f,0x75,0x66,0x65]

fileprivate func explorationHead(regulation num: UInt8) -> UInt8 {
...
/*: "#FFD13A" :*/
fileprivate let k_explainName:String = "grate beat roman digital#FFD13A"

/*: "gift_gb_umnad_1" :*/
fileprivate let app_customUrl:[Character] = ["g","i"]
fileprivate let main_exceptStr:String = "ft_gb_assistant contact"

/*: "icon_circle_pic_default" :*/
fileprivate let appBuildName:String = "ICON"
fileprivate let showRelatedMaleKey:[Character] = ["_","p","i","c","_","d","e","f","a","u","l","t"]

/*: "Open lucky blind-box" :*/
fileprivate let appMicPathKey:String = "Open levery offer"
fileprivate let userSpreadFormat:String = " bliinfo prompt automatic characteristic manual"

/*: "Sent " :*/
fileprivate let data_snapStr:[Character] = ["S","e","n","t"," "]

/*: "PingFangSC-Semibold" :*/
fileprivate let appCombineAvailableValue:String = "back any never ready partnerPing"
fileprivate let app_depthMessage:String = "nor bring bodyC-Sem"

/*: "Bombard" :*/
fileprivate let kReadingMessage:String = "Bombardreceiver subsequent salt"
...
class ExclusiveDismissView: UIView {
    //: var giftMsgModel = TalkingGiftMsgAnimatModel()
    var giftMsgModel = RunPerform()
    ...
    required init?(coder: NSCoder) {
        ...
        fatalError(String(bytes: dataExactlyString.map{explorationHead(regulation: $0)}, encoding: .utf8)!)
    }
    ...
    private lazy var nickLb: UILabel = {
        ...
        lb.font = .sumervalSort(fontSize: 14)
...
//        lb.numberOfLines = 1
        ...
        lb.textColor = UIColor(hex: (String(k_explainName.suffix(7))))
    ...
    private lazy var pluralTip: MobileMaterial = {
        //: let lab = TalkingGiftTrackItemLabel()
        let lab = MobileMaterial()
        //: lab.isHidden = true
        lab.isHidden = true
    ...
    private lazy var countLb: MobileMaterial = {
        //: let lab = TalkingGiftTrackItemLabel()
        let lab = MobileMaterial()
...
extension ExclusiveDismissView {
    //: func setGiftMsgModel(model: TalkingGiftMsgAnimatModel) {
    func replaceWithoutAfter(model: RunPerform) {
        ...
        self.bgImage.image = UIImage.installBind(name: (String(app_customUrl) + String(main_exceptStr.prefix(6)) + "umnad_1"))

        //: let placeholder = UIImage.BundleImageNamed(name: "icon_circle_pic_default")
        let placeholder = UIImage.installBind(name: (appBuildName.lowercased() + "_circle" + String(showRelatedMaleKey)))

        //: if (model.showType == ChatGiftAnimatType.myStery.rawValue ) {
        if model.showType == VisibleListen.myStery.rawValue {
            //: self.nickLb.text = "Open lucky blind-box".localized
            self.nickLb.text = (String(appMicPathKey.prefix(6)) + "ucky" + String(userSpreadFormat.prefix(4)) + "nd-box").localized
            //: }else {
        } else {
        ...
        self.avatarImage.spread(urlStr: giftMsgModel.fromHeadPic, placeImg: placeholder)
        //: self.giftImage.setUrlImage(urlStr: giftMsgModel.giftPic, placeImg: placeholder)
        self.giftImage.spread(urlStr: giftMsgModel.giftPic, placeImg: placeholder)

        //: self.giftName.text = "Sent ".localized + giftMsgModel.pname
        self.giftName.text = (String(data_snapStr)).localized + giftMsgModel.pname
        ...
        if sendNumber > 1 {
            ...
            var font1 = UIFont(name: (String(appCombineAvailableValue.suffix(4)) + "FangS" + String(app_depthMessage.suffix(5)) + "ibold"), size: 22)
            //: font1 = italicFontWith(angle: 15, fontSize: 18, fontName: font1!.fontName)
            font1 = bindSophisticated(angle: 15, fontSize: 18, fontName: font1!.fontName)
            //: var font2 = UIFont(name: "PingFangSC-Semibold", size: 26)
            var font2 = UIFont(name: (String(appCombineAvailableValue.suffix(4)) + "FangS" + String(app_depthMessage.suffix(5)) + "ibold"), size: 26)
            //: font2 = italicFontWith(angle: 15, fontSize: 22, fontName: font2!.fontName)
            font2 = bindSophisticated(angle: 15, fontSize: 22, fontName: font2!.fontName)
            ...
            numText = numText.allot(texts: strs, colors: colors, fonts: fonts)
        ...
        hiddenOf(giftCount: giftMsgModel.comboNum)
    }

    //: func refreshGiftCount(giftCount: Int) {
    func hiddenOf(giftCount: Int) {
        //: var font1 = UIFont(name: "PingFangSC-Semibold", size: 45)
        var font1 = UIFont(name: (String(appCombineAvailableValue.suffix(4)) + "FangS" + String(app_depthMessage.suffix(5)) + "ibold"), size: 45)
        //: font1 = italicFontWith(angle: 15, fontSize: 26, fontName: font1!.fontName)
        font1 = bindSophisticated(angle: 15, fontSize: 26, fontName: font1!.fontName)
        //: var font2 = UIFont(name: "PingFangSC-Semibold", size: 45)
        var font2 = UIFont(name: (String(appCombineAvailableValue.suffix(4)) + "FangS" + String(app_depthMessage.suffix(5)) + "ibold"), size: 45)
        //: font2 = italicFontWith(angle: 15, fontSize: 38, fontName: font2!.fontName)
        font2 = bindSophisticated(angle: 15, fontSize: 38, fontName: font2!.fontName)
        ...
        let colors = [UIColor.noticeSatisfy(150, 77, 20, 1), UIColor.white, UIColor.noticeSatisfy(150, 77, 20, 1)]
        ...
        attText = attText.allot(texts: strs, colors: colors, fonts: fonts)
        ...
        countLb.questionSeek(attributedText: attText)
        ...
        countLb.consumer(frame: CGRect(x: x, y: countLb.origin.y, width: width, height: countLb.size.height))
    }

    /// 连击
    //: func comboHitWith(giftMsgModel: TalkingGiftMsgAnimatModel) {
    func floorOf(giftMsgModel: RunPerform) {
        ...
        NSObject.cancelPreviousPerformRequests(withTarget: self, selector: #selector(weightAndTitle), object: nil)
        ...
        self.independentJoin()
        //: self.perform(#selector(animateHide), afterDelay: 3.5)
        self.perform(#selector(weightAndTitle), afterDelay: 3.5)
    }

    /// 展示
    //: func showGiftAnimate(model: TalkingGiftMsgAnimatModel, animate: Bool) {
    func validSophisticatedBothCapacityColorLocal(model: RunPerform, animate: Bool) {
    ...
    func independentJoin() {}

    //: func animationViewToTop(frame: CGRect) {
    func oddLikeCeiling(frame: CGRect) {
        //: UIView.animate(withDuration: 0.2, delay: 0.1, options: [.layoutSubviews]) {
        UIView.animate(withDuration: 0.2, delay: 0.1, options: [.layoutSubviews]) {
            //: self.frame = frame
            self.frame = frame
    ...
    @objc func weightAndTitle() {
        //: NSObject.cancelPreviousPerformRequests(withTarget: self, selector: #selector(animateHide), object: nil)
        NSObject.cancelPreviousPerformRequests(withTarget: self, selector: #selector(weightAndTitle), object: nil)
        //: giftMsgModel = TalkingGiftMsgAnimatModel()
        giftMsgModel = RunPerform()
...
extension ExclusiveDismissView {
    ...
    func bindSophisticated(angle: CGFloat, fontSize: CGFloat, fontName: String) -> (UIFont) {
...
extension ExclusiveDismissView {
    ...
    private func fragmentMount() {
        ...
        font = bindSophisticated(angle: 15, fontSize: 20, fontName: font.fontName)
        ...
        let attText = attributedString.allot(texts: [(String(kReadingMessage.prefix(7))).localized], colors: [UIColor.noticeSatisfy(255, 232, 72, 1)], fonts: [font])
        //: pluralTip.setFrame(frame: CGRect.init(x: giftImage.right, y: 14, width: attText.size().width + 2, height: height-20))
        pluralTip.consumer(frame: CGRect(x: giftImage.right, y: 14, width: attText.size().width + 2, height: height - 20))

        //: pluralTip.setAttributedText(attributedText: attText)
        pluralTip.questionSeek(attributedText: attText)
        //: contentView.addSubview(pluralTip)
        contentView.addSubview(pluralTip)

        //: countLb.setFrame(frame: CGRect.init(x: giftImage.right, y: 2, width: 70, height: height))
        countLb.consumer(frame: CGRect(x: giftImage.right, y: 2, width: 70, height: height))
