...
fileprivate let k_constructString:[Character] = ["b","t","n","_","s","h","o","w","_","a"]
fileprivate let app_hardValue:String = "lerprovider"
fileprivate let main_othersId:String = "ROUND"

/*: "btn_alertBackground_false" :*/
fileprivate let user_phoneData:[Character] = ["b","t","n","_","a","l","e","r","t","B","a","c","k","g","r","o","u","n","d"]
fileprivate let notiListFormat:String = "_"
fileprivate let userHandleValue:String = "fsecurelse"
...
public enum AnalyzeRemoveFactor {
    //: case color
    case color
    //: case effect
    case effect
...
public class TapBounce: NSObject {
    /// 通用回调
...
public extension TapBounce {
    /// 适配器回调,用于给适配器参数赋值
    //: public typealias ConfigAlert = ((_ config: ShowAlertConfig) -> Void)
    typealias ConfigAlert = (_ config: ImpactMobileContainer) -> Void
    ...
    internal class func notMovementSick(title: String? = nil,
                                        //: message: String?  = nil,
                                        message: String? = nil,
                                        ...
                                        config: ImpactMobileContainer? = nil)
    {
        //: customAlert(title: title,
        himLeave(title: title,
                 //: message: message,
                 message: message,
    ...
    @objc internal class func centerMission(title: String? = nil,
    ...
    class func himLeave(title: String? = nil,
                        ...
                        config: ImpactMobileContainer? = nil)
    {
        ...
        let model = config ?? ImpactMobileContainer()

        //: let alertView = TalkingAlertView(title: title,
        let alertView = SinkBounceView(title: title,
        ...
        //: FormatDistribution.getWindow().addSubview(alertView)
        FormatDistribution.group().addSubview(alertView)
    ...
    class func outConvert() {
        //: FormatDistribution.getWindow().subviews.forEach { (view) in
        FormatDistribution.group().subviews.forEach { view in
            //: if view.isKind(of: TalkingAlertView.self) {
            if view.isKind(of: SinkBounceView.self) {
    ...
    class func laughterPole(title: String, isTouch: Bool) {
        //: FormatDistribution.getWindow().subviews.forEach { (view) in
        FormatDistribution.group().subviews.forEach { view in
            //: if view.isKind(of: TalkingAlertView.self) {
            if view.isKind(of: SinkBounceView.self) {
                //: let cunrenView: TalkingAlertView = view as! TalkingAlertView
                let cunrenView: SinkBounceView = view as! SinkBounceView
                //: cunrenView.changeRightBtn(title: title, isTouch: isTouch)
                cunrenView.reporterFeedback(title: title, isTouch: isTouch)
            }
        }
    }
}
...
public class ImpactMobileContainer {
    /// 执行动画时间
    ...
    public var maskType: AnalyzeRemoveFactor = .color
    /// 阴影
    ...
    public var titleFont: UIFont = .statinEssay(type: .Medium, fontSize: 18)
    /// alert标题字体颜色
    //: public var titleColor  = UIColor.appTitleColor()
    public var titleColor = UIColor.serviceMajor()
    /// alert信息字体
    //: public var textFont: UIFont = .pingfangFont(type: .Medium, fontSize: 16)
    public var textFont: UIFont = .statinEssay(type: .Medium, fontSize: 16)
    //: public var textColor  = UIColor.appTitleColor()
    public var textColor = UIColor.serviceMajor()
    ...
    public var buttonFont: UIFont = .statinEssay(type: .Medium, fontSize: 16)
    /// alert按钮字体颜色
    //: public var leftColor  = UIColor.appThemeColor()
    public var leftColor = UIColor.tress()
    //: public var rightColor = UIColor.white
    public var rightColor = UIColor.white
    //: public var rightBackgroundImage = "btn_show_alertBackground"
    public var rightBackgroundImage = (String(k_constructString) + app_hardValue.replacingOccurrences(of: "provider", with: "t") + "Backg" + main_othersId.lowercased())

    //: public var rightBackgroundImagefalse = "btn_alertBackground_false"
    public var rightBackgroundImagefalse = (String(user_phoneData) + notiListFormat.capitalized + userHandleValue.replacingOccurrences(of: "secure", with: "a"))
    //: public var rightFont: UIFont = .pingfangFont(type: .Medium, fontSize: 16)
    public var rightFont: UIFont = .statinEssay(type: .Medium, fontSize: 16)
