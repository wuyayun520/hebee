...
/*: "recharge" :*/
fileprivate let showPeanutData:[Character] = ["r","e","c","h","a","r","g","e"]

/*: "recharge_ :*/
fileprivate let showAudienceMsg:String = "recchoice"
fileprivate let app_contrastName:[Character] = ["r","g","e","_"]

/*: ".png" :*/
fileprivate let dataSpaceMessage:String = "class safe standard.png"

/*: .png" :*/
fileprivate let appExaminationId:String = ".pnginfo remark"

/*: ".jpg" :*/
fileprivate let showInvisibleFarTitle:String = "minimize clearly right missing.jpg"

/*: .jpg" :*/
fileprivate let showSpeakerId:String = ".jpgconcrete maintain incredible square since"
...
class WithoutButton: UIButton {
    ...
    var imageAlignment: LimitClear = .left
...
extension UIButton {
    ...
    func notResource(_ interval: CGFloat) {
        //: verticalAlignImageAndTitle(interval: interval, topOffset: 0)
        convertOff(interval: interval, topOffset: 0)
    }
    ...
    func convertOff(interval: CGFloat, topOffset: CGFloat) {
    ...
    func originWithBy(interval: CGFloat, imageSize: CGSize, topOffset: CGFloat) {
...
extension UIButton {
    ...
    func marginFellow(color: UIColor, forState: UIControl.State) {
    ...
    func exceptHung(urlStr: String) {
        //: guard !urlStr.isEmptyString else { return }
        guard !urlStr.isEmptyString else { return }
        //: var newUrl = ""
        var newUrl = ""
        //: if urlStr.contains("recharge") {
        if urlStr.contains((String(showPeanutData))) {
            //: newUrl = urlStr.replacingOccurrences(of: "recharge", with: "recharge_\(PackageID)")
            newUrl = urlStr.replacingOccurrences(of: (String(showPeanutData)), with: (showAudienceMsg.replacingOccurrences(of: "choice", with: "ha") + String(app_contrastName)) + "\(user_oneData)")
            //: self.setUrlImage(urlStr: newUrl, placeImg: nil)
            self.rime(urlStr: newUrl, placeImg: nil)
            //: return
            return
                //: } else if urlStr.contains(".png") {
        } else if urlStr.contains((String(dataSpaceMessage.suffix(4)))) {
            //: newUrl = urlStr.replacingOccurrences(of: ".png", with: "_\(PackageID).png")
            newUrl = urlStr.replacingOccurrences(of: (String(dataSpaceMessage.suffix(4))), with: "_\(user_oneData)" + (String(appExaminationId.prefix(4))))

            //: } else if urlStr.contains(".jpg") {
        } else if urlStr.contains((String(showInvisibleFarTitle.suffix(4)))) {
            //: newUrl = urlStr.replacingOccurrences(of: ".jpg", with: "_\(PackageID).jpg")
            newUrl = urlStr.replacingOccurrences(of: (String(showInvisibleFarTitle.suffix(4))), with: "_\(user_oneData)" + (String(showSpeakerId.prefix(4))))
        }
        //: self.setUrlImage(urlStr: newUrl, placeImg: nil)
        self.rime(urlStr: newUrl, placeImg: nil)
    }
