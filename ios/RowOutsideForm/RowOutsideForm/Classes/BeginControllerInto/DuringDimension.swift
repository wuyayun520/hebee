...
fileprivate let dataRoundString:[UInt8] = [0x64,0x65,0x74,0x6e,0x65,0x6d,0x65,0x6c,0x70,0x6d,0x69,0x20,0x6e,0x65,0x65,0x62,0x20,0x74,0x6f,0x6e,0x20,0x73,0x61,0x68,0x20,0x29,0x3a,0x72,0x65,0x64,0x6f,0x63,0x28,0x74,0x69,0x6e,0x69]

/*: "#999999" :*/
fileprivate let showManualExistingString:[Character] = ["#","9","9","9","9","9","9"]

/*: "0/20" :*/
fileprivate let constFemaleUrl:[Character] = ["0","/","2","0"]

/*: "Nickname" :*/
fileprivate let app_emptySurvivalPath:[Character] = ["N","i","c","k","n"]
fileprivate let main_drawingPath:[Character] = ["a","m","e"]

/*: "enter your name" :*/
fileprivate let kDrawUrl:[Character] = ["e"]
fileprivate let dataReaderRomanMessage:[Character] = ["n","t","e","r"," ","y","o","u","r"," ","n","a","m","e"]
...
class DuringDimension: UITableViewCell {
    ...
    override init(style: UITableViewCell.CellStyle, reuseIdentifier: String?) {
        ...
        NotificationCenter.default.addObserver(self, selector: #selector(duringWith(notification:)), name: UIResponder.keyboardWillHideNotification, object: nil)
    }

    //: required init?(coder aDecoder: NSCoder) {
    required init?(coder aDecoder: NSCoder) {
        //: fatalError("init(coder:) has not been implemented")
        fatalError(String(bytes: dataRoundString.reversed(), encoding: .utf8)!)
    }
    ...
    lazy var numberLB: UILabel = {
        ...
        LB.textColor = UIColor(hex: (String(showManualExistingString)))
        //: LB.font = UIFont.pingfangFont(type: .Regular, fontSize: 14)
        LB.font = UIFont.statinEssay(type: .Regular, fontSize: 14)
        //: LB.text = "0/20"
        LB.text = (String(constFemaleUrl))
    ...
    lazy var titleLB: UILabel = {
        ...
        LB.text = (String(app_emptySurvivalPath) + String(main_drawingPath)).localized
    ...
    lazy var nameTF: UITextField = {
        ...
        tf.font = UIFont.statinEssay(type: .Medium, fontSize: 16)
        //: tf.textAlignment = .center
        tf.textAlignment = .center
        //: tf.attributedPlaceholder = NSAttributedString.init(string: "enter your name".localized, attributes: [
        tf.attributedPlaceholder = NSAttributedString(string: (String(kDrawUrl) + String(dataReaderRomanMessage)).localized, attributes: [
            //: NSAttributedString.Key.foregroundColor: UIColor.init(hex: "#999999")!])
            NSAttributedString.Key.foregroundColor: UIColor(hex: (String(showManualExistingString)))!])
        //: tf.delegate = self
        tf.delegate = self
        //: tf.returnKeyType = UIReturnKeyType.done
        tf.returnKeyType = UIReturnKeyType.done
        //: tf.text = VideoMagnitudeerval.share.loginUserMode.nickname
        tf.text = VideoMagnitudeerval.share.loginUserMode.nickname
        //: numberLB.text = "\(VideoMagnitudeerval.share.loginUserMode.nickname?.count ?? 0)/20"
        numberLB.text = "\(VideoMagnitudeerval.share.loginUserMode.nickname?.count ?? 0)/20"
        //: return tf
        return tf
        //: }()
...
extension DuringDimension {
    //: @objc func keyboardWillBeHidden(notification: NSNotification) {
    @objc func duringWith(notification: NSNotification) {
...
extension DuringDimension: UITextFieldDelegate {
