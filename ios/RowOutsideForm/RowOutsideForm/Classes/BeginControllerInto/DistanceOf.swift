...
fileprivate let app_explorationData:[UInt8] = [0x77,0x7c,0x77,0x82,0x36,0x71,0x7d,0x72,0x73,0x80,0x48,0x37,0x2e,0x76,0x6f,0x81,0x2e,0x7c,0x7d,0x82,0x2e,0x70,0x73,0x73,0x7c,0x2e,0x77,0x7b,0x7e,0x7a,0x73,0x7b,0x73,0x7c,0x82,0x73,0x72]

fileprivate func spellWhole(contrast num: UInt8) -> UInt8 {
...
/*: "#4FAAFF" :*/
fileprivate let main_tickPath:[Character] = ["#","4","F","A","A","F","F"]

/*: "reply" :*/
fileprivate let notiSString:String = "reminiy"

/*: "F5F5F5" :*/
fileprivate let userDeadlineKey:[Character] = ["F","5","F","5","F","5"]
...
class DistanceOf: UITableViewCell {
    ...
    required init?(coder aDecoder: NSCoder) {
        ...
        fatalError(String(bytes: app_explorationData.map{spellWhole(contrast: $0)}, encoding: .utf8)!)
    }
...
extension DistanceOf {
    ...
    func distinction(model: ButtExitPersistModel) {
        ...
        if model.allCountContent.count > 0 {
            ...
            let font = UIFont.sumervalSort(fontSize: 14)
            //: if model.replyUser.count>0 {
            if model.replyUser.count > 0 {
                //: strA = [model.nickname, " "+"reply".localized+" ", model.replyUser, "：\(model.content)"]
                strA = [model.nickname, " " + (notiSString.replacingOccurrences(of: "mini", with: "pl")).localized + " ", model.replyUser, "：\(model.content)"]
                //: colorA = [UIColor.appValueDetailColor(), UIColor.appTitleColor(), UIColor.appValueDetailColor(), UIColor.appTitleColor()]
                colorA = [UIColor.mainSpread(), UIColor.serviceMajor(), UIColor.mainSpread(), UIColor.serviceMajor()]
                ...
                colorA = [UIColor.mainSpread(), UIColor.serviceMajor()]
                //: fontA = [font, font]
                fontA = [font, font]
            }
    ...
    func countegration(cell: UITableViewCell, tableView: UITableView, indexPath: NSIndexPath, radius: CGFloat, height: CGFloat) {
        ...
        let bounds = CGRect(x: 51, y: 0, width: notiExploreKey - 51 - 15, height: cell.bounds.size.height)
        ...
        layer.fillColor = UIColor(hex: (String(userDeadlineKey)))?.cgColor
...
extension DistanceOf {
    ...
    private func draft() {
        ...
        let longtap = UILongPressGestureRecognizer(target: self, action: #selector(vid(sender:)))
