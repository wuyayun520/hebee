...
/*: "tab" :*/
fileprivate let show_peopleName:String = "tjar"

/*: "page" :*/
fileprivate let app_nobodyKey:String = "pfixedge"

/*: "uid" :*/
fileprivate let user_ointmentData:String = "streetid"

/*: "model" :*/
fileprivate let notiTapSterolMessage:String = "mtradeel"

/*: "There's no feed yet." :*/
fileprivate let user_guideData:[Character] = ["T","h","e","r","e","\'","s"," ","n","o"," "]
fileprivate let showDatePath:[Character] = ["f","e","e","d"," ","y","e","t","."]
...
class EagerTop: UIViewController {
    //: var pageIndex = 0
    var pageIndex = 0
    ...
    var seleteCell = OrientationMaximum()
    //: var lastVideoUrl = ""
    var lastVideoUrl = ""
    ...
    lazy var mainTableView: UITableView = {
        ...
        table.contentInset = UIEdgeInsets(top: 0, left: 0, bottom: userOutputKey, right: 0)
        //: return table
        return table
...
extension EagerTop {
    ...
    func less() {
        ...
        var dict = [String: Any]()
        //: dict["tab"] = 1
        dict[(show_peopleName.replacingOccurrences(of: "jar", with: "ab"))] = 1
        //: dict["page"] = pageIndex
        dict[(app_nobodyKey.replacingOccurrences(of: "fixed", with: "a"))] = pageIndex
        //: TalkingMomentRequestTool.req_MomentList(params: dict) { succeed, result, errorModel in
        QuitInsert.sensorBy(params: dict) { succeed, result, errorModel in
...
extension EagerTop {
    ...
    @objc func loop(notification: NSNotification) {
        //: let userinfo = notification.userInfo as![String: AnyObject]
        let userinfo = notification.userInfo as! [String: AnyObject]
        //: let uid: String = userinfo["uid"] as! String
        let uid: String = userinfo[(user_ointmentData.replacingOccurrences(of: "street", with: "u"))] as! String
    ...
    @objc func mediaEnd(notification: NSNotification) {
        //: let userinfo = notification.userInfo as![String: AnyObject]
        let userinfo = notification.userInfo as! [String: AnyObject]
        //: let model: TalkingMomentModel = userinfo["model"] as! TalkingMomentModel
        let model: ExerciseListen = userinfo[(notiTapSterolMessage.replacingOccurrences(of: "trade", with: "od"))] as! ExerciseListen
        //: for i in 0..<self.dataSourceArr.count {
        for i in 0 ..< self.dataSourceArr.count {
    ...
    @objc func position(notification: NSNotification) {
        //: let userinfo = notification.userInfo as![String: AnyObject]
        let userinfo = notification.userInfo as! [String: AnyObject]
        //: let model: TalkingMomentModel = userinfo["model"] as! TalkingMomentModel
        let model: ExerciseListen = userinfo[(notiTapSterolMessage.replacingOccurrences(of: "trade", with: "od"))] as! ExerciseListen
...
extension EagerTop: UITableViewDelegate, UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        let identifier = OrientationMaximum.className()
        //: var cell = tableView.dequeueReusableCell(withIdentifier: identifier) as? TalkingMomentItemCell
        var cell = tableView.dequeueReusableCell(withIdentifier: identifier) as? OrientationMaximum
        //: if cell == nil {
        if cell == nil {
            //: cell = TalkingMomentItemCell(style: .default, reuseIdentifier: identifier)
            cell = OrientationMaximum(style: .default, reuseIdentifier: identifier)
        }
    ...
    func tableView(_ tableView: UITableView, willDisplay cell: UITableViewCell, forRowAt indexPath: IndexPath) {
        ...
        if self.dataSourceArr.count > indexPath.row {
            ...
            if indexPath.row == 0 && self.isfirstvideoCell == true {
                ...
                if model.momentType == ModeEmpty.Video.rawValue {
                    //: guard let tempcell: TalkingMomentItemCell = cell as? TalkingMomentItemCell  else {
                    guard let tempcell: OrientationMaximum = cell as? OrientationMaximum else {
                        //: return
                        return
                    }
                    //: tempcell.playVideo()
...
extension EagerTop: UIScrollViewDelegate {
    //: func scrollViewDidEndDecelerating(_ scrollView: UIScrollView) {
    func scrollViewDidEndDecelerating(_ scrollView: UIScrollView) {
        //: scrollViewEndScrollPlayer(scrollView: scrollView)
        familyDark(scrollView: scrollView)
    }
    ...
    func familyDark(scrollView: UIScrollView) {
        ...
        if let middleIndexPath = mainTableView.indexPathForRow(at: CGPoint(x: 0, y: scrollView.contentOffset.y + mainTableView.frame.size.height / 2)) {
            ...
            if dataSourceArr.count > middleIndexPath.row {
                ...
                let cell: OrientationMaximum = mainTableView.cellForRow(at: middleIndexPath) as! OrientationMaximum
...
// MARK: - HiddenExclusive

//: extension TalkingPopularMomentVC {
extension EagerTop {
    //: @objc func PostSuccess() {
    @objc func sustainable() {
        //: mainTableView.mj_header?.beginRefreshing()
        mainTableView.mj_header?.beginRefreshing()
    }
}
...
extension EagerTop {
    ...
    private func graph() {
        ...
        style.TipsTitle = (String(user_guideData) + String(showDatePath)).localized
    ...
    private func constraintUniform() {
        ...
        NotificationCenter.default.addObserver(self, selector: #selector(mediaEnd(notification:)), name: kOldKey, object: nil)
        ...
        NotificationCenter.default.addObserver(self, selector: #selector(loop(notification:)), name: user_smartValue, object: nil)
        ...
        NotificationCenter.default.addObserver(self, selector: #selector(position(notification:)), name: userAwayStr, object: nil)
        ...
        NotificationCenter.default.addObserver(self, selector: #selector(sustainable), name: const_sessionDownUrl, object: nil)
    }
}
