...
fileprivate let k_disableLosePath:[Character] = ["C","a","c","h","e","G","i","f","t","E","f","f","e","c","t","C","a","c","h","e","K","e","y"]

/*: "下载动画文件" :*/
fileprivate let constSkipPath:String = "下载\u{52a8}画文件"

/*: "加载本地动画文件" :*/
fileprivate let show_secondFormat:String = "\u{52a0}\u{8f7d}本地\u{52a8}"
fileprivate let const_feedbackMsg:String = "画文\u{4ef6}"

/*: "Cache_GiftEffect/" :*/
fileprivate let app_flexFormat:[Character] = ["C","a","c","h","e","_","G","i","f","t","E","f","f","e","c","t","/"]

/*: "GJSGVA" :*/
fileprivate let dataCivilizationMarginStr:String = "GJSGarchitecture"

/*: "GJVap" :*/
fileprivate let user_composeSpecificallyValue:[Character] = ["G","J","V","a","p"]

/*: "config.json" :*/
fileprivate let showBuilderButtonTitle:String = "cobluefi"

/*: "app. :*/
fileprivate let kMaleId:String = "app.reduction large marker shape"

/*: .com" :*/
fileprivate let main_linkAngleString:String = ".comintegration lay"

/*: "key" :*/
fileprivate let data_boxData:String = "KEY"

/*: "zip" :*/
fileprivate let k_scripId:String = "zmatch"

/*: " fail to unzip file" :*/
fileprivate let dataGreatMsg:[Character] = [" ","f","a","i","l"," ","t","o"," ","u","n","z"]
fileprivate let show_gratePath:String = "consumption uniqueip file"

/*: "RSA 解密失败" :*/
fileprivate let notiReListenerClosedPath:String = "open norm toolRSA 解"
fileprivate let mainTargetSnapStr:String = "\u{5bc6}失败"

/*: "fail to unzip script file" :*/
fileprivate let userRemoteMedalPlaceKey:String = "fail trise spark more general"
fileprivate let show_accuracyMessage:String = "form choose conversion frame subp sc"
fileprivate let constQuitNumbName:String = "natural directly ago processing clipript file"

/*: "gift" :*/
fileprivate let main_ontoMessage:String = "githrought"

/*: "%@%@effect_%ld" :*/
fileprivate let dataSickStr:[Character] = ["%","@","%","@","e","f"]
fileprivate let const_scripRecordValue:String = "fitemct"

/*: "%@%@effect_%ld_unzipTest/" :*/
fileprivate let const_maxPath:[Character] = ["%","@","%","@","e","f","f","e","c","t","_","%","l","d","_","u","n","z","i","p","T","e","s","t","/"]

/*: "%@%@effect_%ld_unzip/" :*/
fileprivate let constCommitTitle:String = "reduce more input%@%@e"
fileprivate let dataStrokeAppearanceStr:String = "fun flowt_%ld_"
fileprivate let data_bounceKey:[Character] = ["p","/"]
...
typealias CompletionBlock = (_ success: Bool, _ effectModel: ComputeLike?) -> Void

//: typealias ErrorBlock = (_ error: NSError?) -> Void
typealias ErrorBlock = (_ error: NSError?) -> Void

//: class TalkingGiftAnimatTool: NSObject {
class TitleDescription: NSObject {
    //: private let AnimatCacheKey = "CacheGiftEffectCacheKey"
    private let AnimatCacheKey = (String(k_disableLosePath))

    //: @objc static let shared = TalkingGiftAnimatTool()
    @objc static let shared = TitleDescription()
    ...
    func clip(effectMsgModel: BeyondUsed, completion: @escaping CompletionBlock) {
        //: if checkNewVersionWithAnimatMsgModel(model: effectMsgModel) {
        if conversation(model: effectMsgModel) {
            //: printLog(message: "下载动画文件")
            printLog(message: (constSkipPath))
            //: downloadAnimat(effectMsgModel: effectMsgModel) { [weak self] error in
            sinceOwner(effectMsgModel: effectMsgModel) { [weak self] error in
                ...
                self.sumo(effectMsgModel: effectMsgModel, complete: completion)
            }
            //: } else {
        } else {
            //: printLog(message: "加载本地动画文件")
            printLog(message: (show_secondFormat + const_feedbackMsg))
            //: self.loadLocalAnimatConfig(effectMsgModel: effectMsgModel, complete: completion)
            self.sumo(effectMsgModel: effectMsgModel, complete: completion)
        }
    }
    ...
    func block(fileName: String, model: BeyondUsed) -> String {
        //: let scriptDirectory = fetchScriptDirectory(animatMsgModel: model)
        let scriptDirectory = originalMode(animatMsgModel: model)
    ...
    func red() -> String {
        ...
        let subPath = (String(app_flexFormat))
    ...
    func doinge(filePath: String) -> String {
    ...
    func level(model: BeyondUsed) -> Bool {
    ...
    func found(oldName: String) -> String {
        //: var newName = oldName
        var newName = oldName
        //: if newName.hasPrefix("GJSGVA") {
        if newName.hasPrefix((dataCivilizationMarginStr.replacingOccurrences(of: "architecture", with: "VA"))) {
            //: newName = String(describing: TalkingSGVAAnimationView.self)
            newName = String(describing: SourceSaveSpot.self)
            //: } else if newName.hasPrefix("GJVap") {
        } else if newName.hasPrefix((String(user_composeSpecificallyValue))) {
            //: newName = String(describing: TalkingVapAnimationView.self)
            newName = String(describing: EffectEdit.self)
        }
        //: return newName
        return newName
    }
...
extension TitleDescription {
    //: func loadLocalAnimatConfig(effectMsgModel: TalkingAnimatMsgModel, complete: @escaping CompletionBlock) {
    func sumo(effectMsgModel: BeyondUsed, complete: @escaping CompletionBlock) {
        //: let scriptDirectory = fetchScriptDirectory(animatMsgModel: effectMsgModel)
        let scriptDirectory = originalMode(animatMsgModel: effectMsgModel)
        //: let scriptPath = (scriptDirectory as NSString).appendingPathComponent("config.json")
        let scriptPath = (scriptDirectory as NSString).appendingPathComponent((showBuilderButtonTitle.replacingOccurrences(of: "blue", with: "n") + "g.json"))
        ...
        let dic = NSDictionary.magnitudeeractionMusic(withJsonString: configString!) ?? NSDictionary()
        //: let giftAmodel = TalkingGiftAnimatModel.init()
        let giftAmodel = ComputeLike()
        //: let effectModel = giftAmodel.setAnimatModek(dic: dic)
        let effectModel = giftAmodel.stage(dic: dic)
        ...
        className = found(oldName: className)
    ...
    func conversation(model: BeyondUsed) -> Bool {
    ...
    func sinceOwner(effectMsgModel: BeyondUsed, errorBlock: @escaping ErrorBlock) {
        ...
        AnalyzeMapCommon.rank(giftUrl: effectMsgModel.effectUrl, path: sample(model: effectMsgModel)) { [weak self] t in
            //: guard let self = self else {return}
            guard let self = self else { return }
            //: DispatchQueue.global().async {
            DispatchQueue.global().async {
                //: self.afterDownloadGiftAnimat(effectMsgModel: effectMsgModel, isSucess: t, errorBlock: errorBlock)
                self.hide(effectMsgModel: effectMsgModel, isSucess: t, errorBlock: errorBlock)
            }
        }
    }

    //: func afterDownloadGiftAnimat(effectMsgModel: TalkingAnimatMsgModel, isSucess: Bool, errorBlock: @escaping ErrorBlock) {
    func hide(effectMsgModel: BeyondUsed, isSucess: Bool, errorBlock: @escaping ErrorBlock) {
        //: if !isSucess {
        if !isSucess {
            //: errorBlock(NSError.init(domain: "app.\(ReplaceUrlDomain).com", code: GiftAnimatUpdateError.DownLoadFailed.rawValue))
            errorBlock(NSError(domain: (String(kMaleId.prefix(4))) + "\(user_basicString)" + (String(main_linkAngleString.prefix(4))), code: InfoMechanism.DownLoadFailed.rawValue))
            //: return
            return
            ...
            let downloadPath = sample(model: effectMsgModel)
            //: let unzipVerifyDir = unzipVerifyDirectory(model: effectMsgModel)
            let unzipVerifyDir = us(model: effectMsgModel)
            ...
            if verifyUnzipSucc {
                ...
                for filename in unzippedFiles {
                    //: if filename == "key" {
                    if filename == (data_boxData.lowercased()) {
                        // encrypted md5 file
                        //: keyFilePath = (unzipVerifyDir as NSString).appendingPathComponent(filename)
                        keyFilePath = (unzipVerifyDir as NSString).appendingPathComponent(filename)
                        //: } else if (filename as NSString).pathExtension == "zip" {
                    } else if (filename as NSString).pathExtension == (k_scripId.replacingOccurrences(of: "match", with: "ip")) {
                        // script file
                ...
                printLog(message: (String(dataGreatMsg) + String(show_gratePath.suffix(7))))
                //: isFailed = true
                isFailed = true
                //: errorBlock(NSError.init(domain: "app.\(ReplaceUrlDomain).com", code: GiftAnimatUpdateError.UnzipFailed.rawValue))
                errorBlock(NSError(domain: (String(kMaleId.prefix(4))) + "\(user_basicString)" + (String(main_linkAngleString.prefix(4))), code: InfoMechanism.UnzipFailed.rawValue))
            }
            ...
            if !isFailed {
                ...
                do {
                    ...
                    let md5Data: Data = FromStorage.centerWrap(NSData(contentsOfFile: keyFilePath) as Data?, density: mainTransitionMessage)
                    ...
                    let md5 = doinge(filePath: scriptZipFilePath)
                    ...
                    if decryptMD5 != md5, FileManager.default.fileExists(atPath: keyFilePath) {
                        ...
                        errorBlock(NSError(domain: (String(kMaleId.prefix(4))) + "\(user_basicString)" + (String(main_linkAngleString.prefix(4))), code: InfoMechanism.VerifyFailed.rawValue))
                    }
                    ...
                    printLog(message: (String(notiReListenerClosedPath.suffix(5)) + mainTargetSnapStr))
                    //: isFailed = true
                    isFailed = true
                    //: errorBlock(NSError.init(domain: "app.\(ReplaceUrlDomain).com", code: GiftAnimatUpdateError.VerifyFailed.rawValue))
                    errorBlock(NSError(domain: (String(kMaleId.prefix(4))) + "\(user_basicString)" + (String(main_linkAngleString.prefix(4))), code: InfoMechanism.VerifyFailed.rawValue))
            ...
            if !isFailed {
                ...
                let unzipTmpDir = chainDaily(model: effectMsgModel)
                ...
                //: else {
                    ...
                    printLog(message: (String(userRemoteMedalPlaceKey.prefix(6)) + "o unzi" + String(show_accuracyMessage.suffix(4)) + String(constQuitNumbName.suffix(9))))
                    //: isFailed = true
                    isFailed = true
                    //: errorBlock(NSError.init(domain: "app.\(ReplaceUrlDomain).com", code: GiftAnimatUpdateError.UnzipFailed.rawValue))
                    errorBlock(NSError(domain: (String(kMaleId.prefix(4))) + "\(user_basicString)" + (String(main_linkAngleString.prefix(4))), code: InfoMechanism.UnzipFailed.rawValue))
    ...
    func scanLikeSpellGroup(path: String) -> Bool {
        //: if cleanFolder(path: path) {
        if sufficientStroke(path: path) {
    ...
    func sufficientStroke(path: String) -> Bool {
    ...
    func attachMust(type: AmongResource) -> String {
        //: var tmpIdentify = ""
        var tmpIdentify = ""
        //: switch type {
        switch type {
        //: case .Gift:     tmpIdentify =  "gift"
        case .Gift: tmpIdentify = (main_ontoMessage.replacingOccurrences(of: "through", with: "f"))
        //: break
        //: default:
        default:
            //: break
    ...
    func sample(model: BeyondUsed) -> String {
        ...
        let downloadTmpPath = String(format: (String(dataSickStr) + const_scripRecordValue.replacingOccurrences(of: "item", with: "e") + "_%ld"), NSTemporaryDirectory(), attachMust(type: model.effectType), model.eid)
        //: return downloadTmpPath
        return downloadTmpPath
    }

    //: func unzipVerifyDirectory(model: TalkingAnimatMsgModel)->String {
    func us(model: BeyondUsed) -> String {
        ...
        let unzipVerifyDirectory = String(format: (String(const_maxPath)), NSTemporaryDirectory(), attachMust(type: model.effectType), model.eid)
        //: return unzipVerifyDirectory
        return unzipVerifyDirectory
    }

    //: func unzipTmpDirectory(model: TalkingAnimatMsgModel)->String {
    func chainDaily(model: BeyondUsed) -> String {
        ...
        let unzipTmpDirectory = String(format: (String(constCommitTitle.suffix(5)) + "ffec" + String(dataStrokeAppearanceStr.suffix(6)) + "unzi" + String(data_bounceKey)), NSTemporaryDirectory(), attachMust(type: model.effectType), model.eid)
