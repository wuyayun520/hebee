...
/*: "Select a date" :*/
fileprivate let data_yerDetailedKey:[Character] = ["S","e","l","e","c","t"," ","a"]
fileprivate let const_receiverValue:[Character] = [" ","d","a","t","e"]
...
protocol ScaleExecute: NSObjectProtocol {
    //: func pickDateView(year: Int, month: Int, day: Int)
    func popSource(year: Int, month: Int, day: Int)
}

//: class TalkingDateView: TalkingBasePickView, UIPickerViewDataSource, UIPickerViewDelegate {
class BrownView: PanReloadThin, UIPickerViewDataSource, UIPickerViewDelegate {
    ...
    weak var delegate: ScaleExecute?
    ...
    override func executeGlobal() {
        ...
        titleString = (String(data_yerDetailedKey) + String(const_receiverValue)).localized
        ...
        sayProcessing(year: defaultYear!, month: defaultMonth!, day: defaultDay!)
    }

    //: func setDefaultDate(year: Int, month: Int, day: Int) {
    func sayProcessing(year: Int, month: Int, day: Int) {
        //: currentYear = year
        currentYear = year
        //: if year == defaultYear {
        if year == defaultYear {
...
//        }
    ...
    override func violation(button: UIButton) {
        ...
        if let day = selectDay {
            ...
            delegate?.popSource(year: selectYear!, month: selectMonth!, day: day)
            //: } else {
        } else {
            //: delegate?.pickDateView(year: selectYear!, month: selectMonth!, day: 0)
            delegate?.popSource(year: selectYear!, month: selectMonth!, day: 0)
        }
    }
        ...
        if isShowDay {
            ...
            if component == 0 {
                ...
                if currentYear == yearSum + minShowYear - 1 {
                    ...
                    return queryionWeird(year: year, month: month)
                }
                //: } else {
            } else {
                //: return self.yearSum
    ...
    func queryionWeird(year: Int, month: Int) -> Int {
