...
/*: "call_bgm" :*/
fileprivate let kAudienceData:[Character] = ["c","a","l","l","_","b","g","m"]

/*: "call_response_bgm" :*/
fileprivate let noti_streamDiscoverFormat:[Character] = ["c","a","l","l","_","r","e","s"]
fileprivate let dataHelloBetterId:[Character] = ["p","o","n","s"]
fileprivate let mainSoundMsg:[Character] = ["e","_","b","g","m"]

/*: "invitePop_call_bgm" :*/
fileprivate let constComplaintValue:String = "merge wed never subtleinvitePo"
fileprivate let const_talkPassingValue:String = "p_cafire literal efficiency asset"
...
class PerSmart: NSObject {
    //: var type = CallBGMPlayerType.Call
    var type = CeilingOval.Call
    //: var player = AVPlayer()
    var player = AVPlayer()
    //: var filePath = ""
    var filePath = ""
    ...
    class func complete(type: CeilingOval) -> PerSmart {
        //: let player = TalkingCallBGMPlayer()
        let player = PerSmart()
        //: player.type = type
        player.type = type
        //: player.getFilePath()
        player.prepareConsumer()
        //: return player
        return player
    }
...
extension PerSmart {
    ...
    private func techniqueMultiple() {
    ...
    private func appearBy(notify: Notification) {
    ...
    private func sophisticated(notify: Notification) {
    ...
    private func prepareConsumer() {
        //: var fileName = "call_bgm"
        var fileName = (String(kAudienceData))
        //: if self.type == .Response {
        if self.type == .Response {
            //: fileName = "call_response_bgm"
            fileName = (String(noti_streamDiscoverFormat) + String(dataHelloBetterId) + String(mainSoundMsg))
            //: } else if self.type == .invitePop {
        } else if self.type == .invitePop {
            //: fileName = "invitePop_call_bgm"
            fileName = (String(constComplaintValue.suffix(8)) + String(const_talkPassingValue.prefix(4)) + "ll_bgm")
        }
        // 收到通话推送要播放mp3声音，不能放到.zip中
        //: let path = SVGAEffectTool.default.getMp3Path(name: fileName)
        let path = LimitIndicator.default.failureObject(name: fileName)
        //: self.filePath = path
        self.filePath = path
    }
}
...
extension PerSmart: TXVodPlayListener {
