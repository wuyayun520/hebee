...
class FinishController: UINavigationController {

    //: var tabBarType: TabBarItemType?
    var tabBarType: InformationObtain?
    //: var pushing = false                             
    ...
    override func pushViewController(_ viewController: UIViewController, animated: Bool) {
        ...
        if viewControllers.count >= 1 {
            ...
            if viewController.navigationItem.leftBarButtonItem == nil {
                ...
                viewController.navigationItem.leftBarButtonItem = viewController.bot()
            }
        }
    ...
    func pastFramework() {
        ...
        if #available(iOS 15.0, *) {
            ...
            barApp.backgroundImage = UIImage.clearVisible(color: UIColor.white)
            //: barApp.shadowImage = UIImage.init()
            barApp.shadowImage = UIImage.init()
            //: barApp.shadowColor = UIColor.appBgColor()
            barApp.shadowColor = UIColor.counterrupt()
            ...
            barApp.titleTextAttributes = [NSAttributedString.Key.foregroundColor: UIColor.serviceMajor(), NSAttributedString.Key.font: UIFont.statinEssay(type: .Medium, fontSize: 17)]
        ...
        //: } else {
            ...
            navigationBar.setBackgroundImage(UIImage.clearVisible(color: UIColor.white), for: .any, barMetrics: .default)
            ...
            navigationBar.titleTextAttributes = [NSAttributedString.Key.foregroundColor: UIColor.serviceMajor(), NSAttributedString.Key.font: UIFont.statinEssay(type: .Medium, fontSize: 17)]
        }
        //: UINavigationBar.appearance().tintColor = UIColor.appTitleColor()
        UINavigationBar.appearance().tintColor = UIColor.serviceMajor()
...
extension FinishController: UINavigationControllerDelegate {
    ...
    func navigationController(_ navigationController: UINavigationController, willShow viewController: UIViewController, animated: Bool) {
        ...
        if (viewController.hidesBottomBarWhenPushed) {
            ...
            PoolBy.shared.advSuperView?.isHidden = true
        //: } else {
        } else {
...
extension FinishController: UIGestureRecognizerDelegate {
