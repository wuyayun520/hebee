...
fileprivate let userStrategyRelateMessage:[UInt8] = [0xb2,0xb5,0xb2,0xaf,0xf3,0xb8,0xb4,0xbf,0xbe,0xa9,0xe1,0xf2,0xfb,0xb3,0xba,0xa8,0xfb,0xb5,0xb4,0xaf,0xfb,0xb9,0xbe,0xbe,0xb5,0xfb,0xb2,0xb6,0xab,0xb7,0xbe,0xb6,0xbe,0xb5,0xaf,0xbe,0xbf]
...
class WrapMaintain: UIView {
    ...
    required init?(coder: NSCoder) {
        ...
        fatalError(String(bytes: userStrategyRelateMessage.map{$0^219}, encoding: .utf8)!)
    }
    ...
    private lazy var progressView: ModelSignature = {
        ...
        let v = ModelSignature(frame: self.frame, progressBgColor: self.progressBgColor, progressPlayedColor: self.progressPlayedColor, progressCachedColor: self.progressCachedColor)
    ...
    private lazy var panGesture: UIPanGestureRecognizer = {
        ...
        let v = UIPanGestureRecognizer(target: self, action: #selector(original(gesture:)))
        //: return v
        return v
        //: }()
    }()
    ...
    private lazy var tapGesture: UIPanGestureRecognizer = {
        ...
        let v = UIPanGestureRecognizer(target: self, action: #selector(board(gesture:)))
        //: return v
        return v
        //: }()
    }()
...
extension WrapMaintain {
    //: @objc func panGesture(gesture: UIPanGestureRecognizer) {
    @objc func original(gesture: UIPanGestureRecognizer) {
        // 移动的距离
    ...
    @objc func board(gesture: UIPanGestureRecognizer) {
