...
struct FrequencyModel: HandyJSON {
    //: var uid: String?
    var uid: String?
    //: var sex: String?
    var sex: String?
...
struct CurrentScenario: HandyJSON {
    //: var thumbUrl: String?
    var thumbUrl: String?
    //: var imgUrl: String?
    var imgUrl: String?
...
enum ManagerSavePressure: String {
    //: case nones  = "0"
    case nones = "0"
    //: case Photo = "1"
    case Photo = "1"
...
struct InvisibleBlack: HandyJSON {
    //: var videoUrl = ""
    var videoUrl = ""
    //: var imgUrl = ""
    var imgUrl = ""
...
extension FrequencyModel {
    ...
    mutating func recent(isPost: Bool = false) {
        ...
        if content!.count > 0 {
            ...
            textHeight = Int(PublishView.sourceTier(text: content!, size: CGSize(width: mainCellMsg - 54, height: CGFloat(MAXFLOAT)), font: UIFont.stepTransition(type: .Regular, fontSize: 17), lineNumber: lineNumber).height + (lineNumber == 0 ? 20 : 0))
        }

        //: if momentType == MomentType.Photo.rawValue || momentType == MomentType.Live.rawValue {
        if momentType == ManagerSavePressure.Photo.rawValue || momentType == ManagerSavePressure.Live.rawValue {
            //: if pic.count==1 {
            if pic.count == 1 {
                //: photesHeight = 212
                photesHeight = 212
