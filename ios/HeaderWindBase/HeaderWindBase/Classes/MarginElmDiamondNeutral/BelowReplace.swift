...
fileprivate let user_cellYetOthersFormat:[UInt8] = [0x48,0x4f,0x48,0x55,0x9,0x42,0x4e,0x45,0x44,0x53,0x1b,0x8,0x1,0x49,0x40,0x52,0x1,0x4f,0x4e,0x55,0x1,0x43,0x44,0x44,0x4f,0x1,0x48,0x4c,0x51,0x4d,0x44,0x4c,0x44,0x4f,0x55,0x44,0x45]

/*: "momentId" :*/
fileprivate let userSequenceTitle:[UInt8] = [0xce,0xd0,0xce,0xc6,0xcf,0xd5,0xaa,0xc5]

fileprivate func addressAccuracy(terms num: UInt8) -> UInt8 {
...
/*: "lastId" :*/
fileprivate let k_aroundSufficientString:[Character] = ["l","a","s","t","I"]
fileprivate let data_attachKey:String = "lose"

/*: "limit" :*/
fileprivate let noti_rowAgainKey:[Character] = ["l","i","m","i","t"]

/*: "list" :*/
fileprivate let data_monsterUrl:[Character] = ["l","i","s","t"]

/*: "more>>" :*/
fileprivate let userTagData:[Character] = ["m","o","r","e",">",">"]

/*: "Comment on success" :*/
fileprivate let show_squareYouKeyValue:[Character] = ["C","o","m","m","e","n","t"," ","o","n"," ","s","u","c"]
fileprivate let appAfterPath:String = "cemembermember"

/*: "replyId" :*/
fileprivate let const_squareName:[Character] = ["r","e","p","l","y"]
fileprivate let userScreenObjectKey:[Character] = ["I","d"]

/*: "type" :*/
fileprivate let noti_rePassValue:[UInt8] = [0xc1,0xcc,0xc5,0xd0]

private func alwaysBucket(despite num: UInt8) -> UInt8 {
    return num ^ 181
}

/*: "comment" :*/
fileprivate let constStartStageData:String = "cobroadcastbroadcastent"

/*: "number" :*/
fileprivate let app_withProduceId:[UInt8] = [0x60,0x67,0x5f,0x54,0x57,0x64]

fileprivate func fragmentNotice(sound num: UInt8) -> UInt8 {
...
/*: "Delete Post" :*/
fileprivate let app_bassLocateString:[Character] = ["D","e","l","e","t","e"," ","P"]
fileprivate let showQuickData:String = "announcementt"

/*: "model" :*/
fileprivate let const_chartPath:[UInt8] = [0x3b,0x39,0x32,0x33,0x3a]

private func personAddress(two num: UInt8) -> UInt8 {
    return num ^ 86
}

/*: "The content cannot be empty!" :*/
fileprivate let app_pathAboveFormat:String = "net deny running automaticThe "
fileprivate let main_wayTitle:String = "state single explaint can"
fileprivate let data_denyAskAppointedMsg:String = "not beimage long pack analyze finish"

/*: "content" :*/
fileprivate let show_overMergeKey:String = "coextrateextrat"

/*: "Delete" :*/
fileprivate let app_kindMightKey:[Character] = ["D","e","l","e","t","e"]

/*: "Delete Comment?" :*/
fileprivate let notiBringId:String = "Deleteretain design manage listener"
fileprivate let mainPerformString:[Character] = ["e","n","t","?"]

/*: "Cancel" :*/
fileprivate let main_stretchData:[Character] = ["C","a","n","c","e","l"]

/*: "Reply" :*/
fileprivate let kRangePath:String = "tier can minute window noticeReply"

/*: "Details" :*/
fileprivate let dataObserverOperationPath:String = "current reserve duringDetails"

/*: "icon_moment_nor_report" :*/
fileprivate let main_flowFormat:String = "icbridgen"
fileprivate let kTaskPlainPath:String = "ent_regular gift threshold"
fileprivate let k_donStateString:[Character] = ["n","o","r","_"]
fileprivate let notiDomainMsg:[Character] = ["r","e","p","o","r","t"]

/*: "Comment" :*/
fileprivate let kOfKitMsg:String = "Commentshade rate welcome today three"
...
class BelowReplace: PetViewController {
    ...
    required init?(coder: NSCoder) {
        ...
        fatalError(String(bytes: user_cellYetOthersFormat.map{$0^33}, encoding: .utf8)!)
    }
    ...
    override func viewWillDisappear(_ animated: Bool) {
        ...
        (MechanismObserve.shared as! MechanismObserve).cookieTail(selectorString: appShapePostMessage)
    }

    //: lazy var mainTableView: UITableView = {
    lazy var mainTableView: UITableView = {
        //: let table = UITableView(frame: CGRect(x: 0, y: 0, width: ScreenWidth, height: ScreenHeight), style: UITableView.Style.grouped)
        let table = UITableView(frame: CGRect(x: 0, y: 0, width: mainCellMsg, height: kPressEasePublishString), style: UITableView.Style.grouped)
        ...
        table.register(ScreenView.self, forCellReuseIdentifier: ScreenView.className())
    ...
    lazy var comInputView: CompositionFrom = {
        //: let text = TalkingCommentInputView.init()
        let text = CompositionFrom()
        //: text.delegate = self
        text.delegate = self
        //: return text
        return text
    ...
    lazy var backBtn: UIButton = {
        ...
        btn.crop(color: .clear, forState: .normal)
        //: btn.addTarget( self, action: #selector(dismissClick), for: .touchUpInside)
        btn.addTarget(self, action: #selector(kitDimension), for: .touchUpInside)
...
extension BelowReplace {
    //: func requestMomentDetail() {
    func sameIn() {
        //: var dict = Dictionary<String, Any>()
        var dict = [String: Any]()
        //: dict["momentId"] = momentModel.mid
        dict[String(bytes: userSequenceTitle.map{addressAccuracy(terms: $0)}, encoding: .utf8)!] = momentModel.mid

        //: TalkingMomentRequestTool.req_MomentDetail(params: dict) { succeed, result, errorModel in
        SmallFinishTemp.light(params: dict) { succeed, result, errorModel in

            //: if succeed {
            if succeed {
                //: if var model = JSONDeserializer<TalkingMomentModel>.deserializeFrom(dict: result as? Dictionary<String, Any>, designatedPath: nil) {
                if var model = JSONDeserializer<FrequencyModel>.deserializeFrom(dict: result as? [String: Any], designatedPath: nil) {
                    ...
                    (MechanismObserve.shared as! MechanismObserve).offto(uid: model.uid ?? "", videoUrl: model.videoUrl, headPic: model.headPic ?? "")
                }
                //: } else {
            } else {
    ...
    func overEstimated() {
        ...
        dict[String(bytes: userSequenceTitle.map{addressAccuracy(terms: $0)}, encoding: .utf8)!] = momentModel.mid
        //: dict["lastId"] = lastId
        dict[(String(k_aroundSufficientString) + data_attachKey.replacingOccurrences(of: "lose", with: "d"))] = lastId
        //: dict["limit"] = limit
        dict[(String(noti_rowAgainKey))] = limit
        //: ProgressHUD.show()
        LevelBoxIndex.naturalBy()

        //: TalkingMomentRequestTool.req_CommentList(params: dict) { succeed, result, errorModel in
        SmallFinishTemp.channel(params: dict) { succeed, result, errorModel in
            //: ProgressHUD.dismiss()
            ...
            if succeed {
                ...
                let array: Array = dict[(String(data_monsterUrl))] as! [Any]
                ...
                for i in 0 ..< array.count {
                    ...
                    if var model = JSONDeserializer<ExceptionRefresh>.deserializeFrom(dict: array[i] as? [String: Any], designatedPath: nil) {
                        //: model.caculateCommentItemHeight()
                        model.afterRun()
                        //: var replyArray: [TalkingCommentReplyModel] = []
                        var replyArray: [MarginComponentSecond] = []
                        //: for j in 0..<model.comment.count {
                        for j in 0 ..< model.comment.count {
                            //: var replymodel = model.comment[j]
                            var replymodel = model.comment[j]
                            //: replymodel.caculateReplyItemHeight()
                            replymodel.streamStage()
                        ...
                        if model.totalReply > 3 {
                            ...
                            var replymodel = MarginComponentSecond()
                            //: replymodel.allCountContent = String(format: "more>>")
                            replymodel.allCountContent = String(format: (String(userTagData)))
                            //: replymodel.caculateReplyItemHeight()
                            replymodel.streamStage()
                            //: replyArray.append(replymodel)
                            replyArray.append(replymodel)
    ...
    func written(params: [String: Any], index: Int) {
        //: ProgressHUD.show()
        LevelBoxIndex.naturalBy()

        //: TalkingMomentRequestTool.req_SendComment(params: params) { [self] succeed, result, errorModel in
        SmallFinishTemp.distributionTag(params: params) { [self] succeed, result, errorModel in
            ...
            if succeed {
                ...
                self.notFade(showMsg: (String(show_squareYouKeyValue) + appAfterPath.replacingOccurrences(of: "member", with: "s")).localized)
                //: if params["replyId"] != nil {
                if params[(String(const_squareName) + String(userScreenObjectKey))] != nil {
                    //: var model = TalkingCommentModel()
                    var model = ExceptionRefresh()
                    ...
                    if model.uid!.count > 0 {
                        ...
                        if var replyModel = JSONDeserializer<MarginComponentSecond>.deserializeFrom(dict: result as? [String: Any], designatedPath: nil) {
                            //: model.totalReply += 1
                            model.totalReply += 1
...
//                            if array.count>3 {
//                                array.removeLast()
//                                var tempModel = MarginComponentSecond.init()
...
//                            }
                    ...
                    if var model = JSONDeserializer<ExceptionRefresh>.deserializeFrom(dict: result as? [String: Any], designatedPath: nil) {
                        //: model.caculateCommentItemHeight()
                        model.afterRun()
                        //: if model.uid == MountCustom.share.loginUserMode.userID {
                        if model.uid == MountCustom.share.loginUserMode.userID {
                            //: model.headPicFrame = MountCustom.share.loginUserMode.headPicFrame
                            model.headPicFrame = MountCustom.share.loginUserMode.headPicFrame
                        }
                ...
                NotificationCenter.default.post(name: mainWaitPath, object: self, userInfo: [String(bytes: noti_rePassValue.map{alwaysBucket(despite: $0)}, encoding: .utf8)!: (constStartStageData.replacingOccurrences(of: "broadcast", with: "m")).localized, String(bytes: app_withProduceId.map{fragmentNotice(sound: $0)}, encoding: .utf8)!: (self.momentModel.replyNum) ?? 0, String(bytes: userSequenceTitle.map{addressAccuracy(terms: $0)}, encoding: .utf8)!: self.momentModel.mid ?? ""])
            }
        }
    }
    ...
    func disappearChapter(cid: String, lastId: String, limit: Int, index: IndexPath) {
        //: ProgressHUD.show()
        LevelBoxIndex.naturalBy()
        //: ProgressHUD.dismiss()
        LevelBoxIndex.gestureOff()
        ...
        var dict = [String: Any]()
        //: dict["replyId"] = cid
        dict[(String(const_squareName) + String(userScreenObjectKey))] = cid
        //: dict["lastId"] = lastId
        dict[(String(k_aroundSufficientString) + data_attachKey.replacingOccurrences(of: "lose", with: "d"))] = lastId
        //: dict["limit"] = limit
        dict[(String(noti_rowAgainKey))] = limit
        ...
        SmallFinishTemp.provisionSumerval(params: dict) { succeed, result, errorModel in
            //: ProgressHUD.dismiss()
            LevelBoxIndex.gestureOff()
            //: if succeed {
            if succeed {
                ...
                for i in 0 ..< array.count {
                    ...
                    if var model = JSONDeserializer<MarginComponentSecond>.deserializeFrom(dict: array[i] as? [String: Any], designatedPath: nil) {
...
extension BelowReplace {
    ...
    func beyondStrategyLabel() {
        ...
        DispatchQueue.main.asyncAfter(deadline: DispatchTime.now() + 0.3) {
            ...
            self.mainTableView.scrollRectToVisible(CGRect(x: 0, y: self.momentModel.itemHeight ?? 56, width: mainCellMsg, height: kPressEasePublishString - CGFloat(self.inputViewHeight) - noti_stageFormat - mainItemFormat), animated: true)
        }
    }

    //: @objc func BlockBtnClick() {
    @objc func announcement() {
        //: if self.momentModel.uid == MountCustom.share.loginUserMode.userID {
        if self.momentModel.uid == MountCustom.share.loginUserMode.userID {
            //: let vc = TalkingMunuPopView.init(frame: self.view.frame)
            let vc = BrownGroup(frame: self.view.frame)
            //: vc.initwithList(cellTitleList: ["Delete Post".localized])
            vc.isolateThroughDistribution(cellTitleList: [(String(app_bassLocateString) + showQuickData.replacingOccurrences(of: "announcement", with: "os")).localized])
            ...
            vc.munuBlock = { [weak self] index, str in
                ...
                SmallFinishTemp.jumpTo(params: dict) { succeed, result, errorModel in
                    ...
                    if succeed {
                        ...
                        NotificationCenter.default.post(name: app_processString, object: self, userInfo: [String(bytes: const_chartPath.map{personAddress(two: $0)}, encoding: .utf8)!: self.momentModel])
                    }
                }
            }
            ...
            let reportView = SortSectionStep(frame: UIScreen.main.bounds, type: .reportUserType, rId: momentModel.uid ?? "")
    ...
    @objc func kitDimension() {
        //: comInputView.inputTextView.text = ""
        comInputView.inputTextView.text = ""
        //: comInputView.resignkeyBoard()
        comInputView.respectiveReveal()
    }
}

// MARK: - TierExceptPhase

//: extension TalkingMomentDetailVC: CommentInputViewDelegate {
extension BelowReplace: TierExceptPhase {
    //: func func__replyTextMsg(msgStr: String, row: IndexPath) {
    func mustSubstantial(msgStr: String, row: IndexPath) {
        //: if msgStr.count == 0 {
        if msgStr.count == 0 {
            //: self.func__showStatusBarErrorMsg(showMsg: "The content cannot be empty!".localized)
            self.signWhen(showMsg: (String(app_pathAboveFormat.suffix(4)) + "conten" + String(main_wayTitle.suffix(5)) + String(data_denyAskAppointedMsg.prefix(6)) + " empty!").localized)
        }
        ...
        dict[String(bytes: userSequenceTitle.map{addressAccuracy(terms: $0)}, encoding: .utf8)!] = momentModel.mid
        //: dict["content"] = msgStr
        dict[(show_overMergeKey.replacingOccurrences(of: "extra", with: "n"))] = msgStr
        ...
        dict[(String(const_squareName) + String(userScreenObjectKey))] = replymodel.cid
        //: requestCommentWithParams(params: dict, index: row.section)
        written(params: dict, index: row.section)
    }

    //: func func__sendTextMsg(msgStr: String, index: Int) {
    func against(msgStr: String, index: Int) {
        //: if msgStr.count == 0 {
        if msgStr.count == 0 {
            //: self.func__showStatusBarErrorMsg(showMsg: "The content cannot be empty!".localized)
            self.signWhen(showMsg: (String(app_pathAboveFormat.suffix(4)) + "conten" + String(main_wayTitle.suffix(5)) + String(data_denyAskAppointedMsg.prefix(6)) + " empty!").localized)
        ...
        if index >= 0 {
            ...
            dict[(String(const_squareName) + String(userScreenObjectKey))] = model.cid
            //: } else if index == quickCommentReplyIndex {
        } else if index == quickCommentReplyIndex {}

        //: requestCommentWithParams(params: dict, index: index)
        written(params: dict, index: index)
    }

    //: func heightToBottomChanged(heightToBottom: CGFloat) {
    func music(heightToBottom: CGFloat) {
        ...
        if heightToBottom > 100 {
            ...
            backBtn.frame = CGRect(x: 0, y: 0, width: mainCellMsg, height: self.view.frame.size.height - heightToBottom - inputViewHeight)
    ...
    func dragWithOrigin(height: CGFloat) {
        //: comInputView.snp.updateConstraints { make in
        comInputView.snp.updateConstraints { make in
        ...
        backBtn.frame = CGRect(x: 0, y: 0, width: mainCellMsg, height: backBtn.frame.size.height - height)
        //: backBtn.isHidden = false
        backBtn.isHidden = false
    ...
    func star(cid: String) {
        //: if self.momentModel.uid == MountCustom.share.loginUserMode.userID {
        if self.momentModel.uid == MountCustom.share.loginUserMode.userID {
            //: let vc = TalkingMunuPopView.init(frame: self.view.frame)
            let vc = BrownGroup(frame: self.view.frame)
            //: vc.initwithList(cellTitleList: ["Delete".localized])
            vc.isolateThroughDistribution(cellTitleList: [(String(app_kindMightKey)).localized])
            ...
            vc.munuBlock = { [weak self] index, str in
                ...
                MainTranslate.galleryWith(title: (String(notiBringId.prefix(6)) + " Comm" + String(mainPerformString)).localized, attributedMessage: NSMutableAttributedString(), leftBtnTitle: (String(main_stretchData)).localized, rightBtnTitle: (String(app_kindMightKey)).localized) {
                    ...
                    SmallFinishTemp.agree(params: dict) { succeed, result, errorModel in
                        //: ProgressHUD.dismiss()
                        LevelBoxIndex.gestureOff()
                        //: if succeed {
                        if succeed {
...
extension BelowReplace: UITableViewDelegate, UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        let identifier = ScreenView.className()
        //: var cell = tableView.dequeueReusableCell(withIdentifier: identifier) as? TalkingCommentReplyItemCell
        var cell = tableView.dequeueReusableCell(withIdentifier: identifier) as? ScreenView
        //: if cell == nil {
        if cell == nil {
            //: cell = TalkingCommentReplyItemCell(style: .default, reuseIdentifier: identifier)
            cell = ScreenView(style: .default, reuseIdentifier: identifier)
        }
        //: let model = self.commentList[indexPath.section]
        let model = self.commentList[indexPath.section]
        ...
        if model.comment.count > 0 {
            ...
            cell?.pressure(model: replymodel)
        }
        //: return cell!
        return cell!
    }
    ...
    func tableView(_ tableView: UITableView, willDisplay cell: UITableViewCell, forRowAt indexPath: IndexPath) {
        ...
        let tcell: ScreenView = cell as! ScreenView
        //: tcell.shearTableViewSection(cell: cell, tableView: tableView, indexPath: indexPath as NSIndexPath, radius: 6, height: height)
        tcell.pair(cell: cell, tableView: tableView, indexPath: indexPath as NSIndexPath, radius: 6, height: height)
    }
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        if replymodel.allCountContent == (String(userTagData)) {
            ...
            disappearChapter(cid: model.cid!, lastId: lastmodel.cid ?? "", limit: 5, index: indexPath)
            //: } else {
        } else {
            //: self.comInputView.updatePlaceholder(holder: "Reply".localized + "\(replymodel.nickname)" + ":", index: 0, row: indexPath, type: 2)
            self.comInputView.noneKeep(holder: (String(kRangePath.suffix(5))).localized + "\(replymodel.nickname)" + ":", index: 0, row: indexPath, type: 2)
        }
    }
    ...
    func tableView(_ tableView: UITableView, viewForHeaderInSection section: Int) -> UIView? {
        ...
        let headeView = WithinEnable(style: .default, reuseIdentifier: nil)
        //: let model = self.commentList[section]
        let model = self.commentList[section]
        //: headeView.configCell(model: model)
        headeView.randomDown(model: model)
        ...
        headeView.commentReplyBlock = { [weak self] nickname in
            ...
            self.comInputView.noneKeep(holder: (String(kRangePath.suffix(5))).localized + " \(nickname)" + ":", index: section, row: IndexPath(), type: 1)
        }
...
extension BelowReplace {
    ...
    private func lost() {
        ...
        self.title = (String(dataObserverOperationPath.suffix(7))).localized
        ...
        if self.momentModel.uid == MountCustom.share.loginUserMode.userID {
            ...
            btn.setTitle((String(app_kindMightKey)).localized, for: .normal)
            //: btn.frame = CGRect(x: 0, y: 0, width: 60, height: 20)
            btn.frame = CGRect(x: 0, y: 0, width: 60, height: 20)
            //: btn.setTitleColor(.appTitleColor(), for: .normal)
            btn.setTitleColor(.labelMomentum(), for: .normal)
            ...
            btn.setImage(UIImage.evolutionary(name: (main_flowFormat.replacingOccurrences(of: "bridge", with: "o") + "_mom" + String(kTaskPlainPath.prefix(4)) + String(k_donStateString) + String(notiDomainMsg))), for: .normal)
        }
    ...
    func doDelay() {
        ...
        let cell = BoxView(style: .default, reuseIdentifier: nil)
        //: cell.frame = CGRect(x: 0, y: 0, width: ScreenWidth, height: momentModel.itemHeight ?? 56 - 12 )
        cell.frame = CGRect(x: 0, y: 0, width: mainCellMsg, height: momentModel.itemHeight ?? 56 - 12)
        //: cell.initwith(isListTableCell: false)
        cell.towardResign(isListTableCell: false)
        //: cell.configCell(model: momentModel)
        cell.switchd(model: momentModel)
        //: cell.playVideo()
        cell.pickEnable()
        ...
        if momentModel.replyNum! > 0 {
            ...
            commentNumberLabel.text = String(format: (String(kOfKitMsg.prefix(7))).localized + " \(momentModel.replyNum!)")
            ...
            commentNumberLabel.snp.makeConstraints { make in
                ...
                make.size.equalTo(CGSize(width: mainCellMsg, height: 22))
            }
