...
fileprivate let showSituationMessage:[UInt8] = [0x64,0x65,0x74,0x6e,0x65,0x6d,0x65,0x6c,0x70,0x6d,0x69,0x20,0x6e,0x65,0x65,0x62,0x20,0x74,0x6f,0x6e,0x20,0x73,0x61,0x68,0x20,0x29,0x3a,0x72,0x65,0x64,0x6f,0x63,0x28,0x74,0x69,0x6e,0x69]
...
class EditReduction: UICollectionViewCell {
    ...
    required init?(coder: NSCoder) {
        ...
        fatalError(String(bytes: showSituationMessage.reversed(), encoding: .utf8)!)
    }

    //: func customUI() {
    func pending() {
        //: let tap = UITapGestureRecognizer(target: self, action: #selector(imageViewClick))
        let tap = UITapGestureRecognizer(target: self, action: #selector(grayFrom))
        ...
        let doubletap = UITapGestureRecognizer(target: self, action: #selector(betweenAppearOperate))
...
extension EditReduction: UIScrollViewDelegate {
    //: @objc func imageViewClick(tap: UIGestureRecognizer) {
    @objc func grayFrom(tap: UIGestureRecognizer) {
        //: if sblock != nil {
        if sblock != nil {
            //: sblock()
            sblock()
    ...
    @objc func betweenAppearOperate(tap: UIGestureRecognizer) {
    ...
    func visual(url: String) {
        //: imageView.setUrlImage(urlStr: url)
        imageView.accessiblePending(urlStr: url)
    }

    //: func configImage(image: UIImage) {
    func singleSwitche(image: UIImage) {
