...
fileprivate let kTickAssetFormat:[UInt8] = [0x9b,0x9c,0x9b,0x86,0xda,0x91,0x9d,0x96,0x97,0x80,0xc8,0xdb,0xd2,0x9a,0x93,0x81,0xd2,0x9c,0x9d,0x86,0xd2,0x90,0x97,0x97,0x9c,0xd2,0x9b,0x9f,0x82,0x9e,0x97,0x9f,0x97,0x9c,0x86,0x97,0x96]

private func taskRaw(hi num: UInt8) -> UInt8 {
    return num ^ 242
}

/*: "iosSvgaEffect" :*/
fileprivate let show_judgeValue:String = "iosSvgaEsay upper request accept"
fileprivate let appSceneEraseFormat:[Character] = ["f","f","e","c","t"]

/*: "iosEffect" :*/
fileprivate let kRidUrl:String = "iosEffless schedule appear man"
fileprivate let constPriorityVideoPath:[Character] = ["e","c","t"]

/*: "entityId" :*/
fileprivate let main_oldConfirmData:[Character] = ["e"]
fileprivate let userBitId:String = "space heavy absolute question dimensionntityId"
...
class MissionSkirtShape: UIView {
    //: var currentEffectMsgModel: TalkingAnimatMsgModel?
    var currentEffectMsgModel: ExamineModel?
    ...
    var animatedView: ConsumptionMay?

    //: open weak var delegate: AnimatViewStepProtocol?
    open weak var delegate: AddSample?
    ...
    required init?(coder: NSCoder) {
        ...
        fatalError(String(bytes: kTickAssetFormat.map{taskRaw(hi: $0)}, encoding: .utf8)!)
    }
...
extension MissionSkirtShape {
    //: func playAnimation(infoDic: Dictionary<String, Any>) {
    func evaluateAcross(infoDic: [String: Any]) {
        //: let dict = infoDic
        let dict = infoDic
        //: if dict.keys.isEmpty == false {
        if dict.keys.isEmpty == false {
            //: var iosEffect: String = dict["iosSvgaEffect"] as! String
            var iosEffect: String = dict[(String(show_judgeValue.prefix(8)) + String(appSceneEraseFormat))] as! String
            //: if iosEffect.isEmptyString {
            if iosEffect.isEmptyString {
                //: iosEffect = dict["iosEffect"] as! String
                iosEffect = dict[(String(kRidUrl.prefix(6)) + String(constPriorityVideoPath))] as! String
            }

            //: let entityId = dict["entityId"] as! Int
            let entityId = dict[(String(main_oldConfirmData) + String(userBitId.suffix(7)))] as! Int
            ...
            if iosEffect.isEmptyString == false {
                ...
                let successs = live(msgModel: effectMsgModel)
            }
        }
    }
    ...
    func live(msgModel: Any?) -> Bool {
        ...
        if msgModel is ExamineModel {
            ...
            if giftMsgModel.animationTimes > 1 {
                ...
                tmpGiftMsgModel.pastM(giftNum: 1) // 礼物个数设置为1，每次动效显示
                ...
                var repeatEfftect = ExamineModel()
            ...
            if giftMsgModel.showType == CustomHiddenPet.myStery.rawValue || giftMsgModel.showType == CustomHiddenPet.king.rawValue {
        ...
        if self.currentEffectMsgModel != nil || paused {
            ...
            if effectMsgModel?.top ?? false && self.effectMsgModelArray.count > 0 {
                ...
                for i in 0 ..< self.effectMsgModelArray.count {
                    ...
                    let aModel: ExamineModel = self.effectMsgModelArray[i] as! ExamineModel
                    //: if aModel.top == false {
                    if aModel.top == false {
                        //: index = i
                        index = i
                ...
                if index == -1 {
                    ...
                    self.effectMsgModelArray.add(effectMsgModel ?? ExamineModel())
                    //: } else {
                } else {
                    //: self.effectMsgModelArray.insert(effectMsgModel ?? TalkingAnimatMsgModel.init(), at: index)
                    self.effectMsgModelArray.insert(effectMsgModel ?? ExamineModel(), at: index)
                }
            ...
            enableFront(effectMsgModel: effectMsgModel!)
        }
...
extension MissionSkirtShape {
    ...
    func bag() {
        ...
        self.animatedView?.speedUserShare()
    }
    ...
    func compare() {
        ...
        if self.currentEffectMsgModel == nil {
            ...
            if self.effectMsgModelArray.count > 0 {
                ...
                guard let removeModel = towardVideo() else { return }
                //: firstShow(effectMsgModel: removeModel)
                enableFront(effectMsgModel: removeModel)
            }
            //: } else {
        } else {
            //: self.animatedView?.resumeAnimation()
            self.animatedView?.afterConsume()
        }
    ...
    func towardVideo() -> ExamineModel? {
        ...
        if effectMsgModel is ExamineModel {
            //: let lastEffectMsgModel = effectMsgModel as! TalkingAnimatMsgModel
            let lastEffectMsgModel = effectMsgModel as! ExamineModel
        ...
        return effectMsgModel as? ExamineModel
    }
}
...
extension MissionSkirtShape {
    ...
    func enableFront(effectMsgModel: ExamineModel) {
        ...
        self.delegate?.remoteArc(effectView: self, obj: effectMsgModel)

        //: TalkingGiftAnimatTool.shared.loadEffectData(effectMsgModel: effectMsgModel, completion: { [weak self] success, giftModel in
        WorkDocument.shared.ignoreFind(effectMsgModel: effectMsgModel, completion: { [weak self] success, giftModel in
            ...
            self.speed(effectMsgModel: effectMsgModel, success: success, effectModel: giftModel ?? ValueModel())
            //: })
        })
    }
    ...
    func speed(effectMsgModel: ExamineModel, success: Bool, effectModel: ValueModel) {
        // 资源下载完成
        //: self.delegate?.animatViewWithDidEndLoadingResource(effectView: self, obj: effectMsgModel, success: success)
        self.delegate?.situationPull(effectView: self, obj: effectMsgModel, success: success)
        ...
        if success {
            ...
            if self.paused {
                ...
                if self.currentEffectMsgModel?.eid == effectMsgModel.eid, self.currentEffectMsgModel?.effectUrl == effectMsgModel.effectUrl {
                    ...
                    mend(effectMsgModel: effectMsgModel, model: effectModel)
                    //: return
                    return
                }
            }
    ...
    func atBirthday(effectMsgModel: ExamineModel, effectModel: ValueModel) {
        //: if self.animatedView != nil {
        if self.animatedView != nil {
            //: self.animatedView?.stopAnimating()
            self.animatedView?.op()
            //: self.animatedView?.cleanAnimating()
            self.animatedView?.finishAcrossObtain()
        ...
        className = WorkDocument.shared.conversationOrScale(oldName: className ?? "")
        //: let clsName = PodspecName + "." + className!
        let clsName = appTweenKey + "." + className!
        ...
        let realClass = AnimatedViewClass as! ConsumptionMay.Type
...
extension MissionSkirtShape: FindTell {
    ...
    func offd(effectItemView: ConsumptionMay, success: Bool) {
        ...
        self.delegate?.sparkHi(effectView: self, obj: effectItemView.effectMsgModel ?? ExamineModel(), success: success)
    }
    ...
    func albumHidden(effectItemView: ConsumptionMay) {
        ...
        self.delegate?.remainBrushOrange(effectView: self, obj: effectMsgModel!)
    }
}
...
extension MissionSkirtShape {
    ...
    func aroundDevice() {
        ...
        if animatedView != nil {
            //: self.animatedView?.stopAnimating()
            self.animatedView?.op() // stop animation
            //: self.animatedView?.cleanAnimating()
            self.animatedView?.finishAcrossObtain() // clean animation data
    ...
    func agileCommon(success: Bool) {
        ...
        if self.effectMsgModelArray.count > 0, self.needStopAll == false {
            ...
            let effectMsgModel = towardVideo()
