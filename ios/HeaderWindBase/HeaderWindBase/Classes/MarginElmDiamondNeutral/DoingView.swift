...
fileprivate let show_welcomeFormat:[UInt8] = [0x4f,0x48,0x4f,0x52,0xe,0x45,0x49,0x42,0x43,0x54,0x1c,0xf,0x6,0x4e,0x47,0x55,0x6,0x48,0x49,0x52,0x6,0x44,0x43,0x43,0x48,0x6,0x4f,0x4b,0x56,0x4a,0x43,0x4b,0x43,0x48,0x52,0x43,0x42]

/*: "move" :*/
fileprivate let mainEdgeValue:String = "momini"
...
class DoingView: UIView {
    ...
    required init?(coder: NSCoder) {
        ...
        fatalError(String(bytes: show_welcomeFormat.map{$0^38}, encoding: .utf8)!)
    }
    ...
    func doingVisit(_ danumuMsgModel: ImmediateGlobal?) {
    ...
    @objc func consume(_ tapGesture: UITapGestureRecognizer) {}
    ...
    lazy var tapGesture: UITapGestureRecognizer = {
        ...
        let t = UITapGestureRecognizer(target: self, action: #selector(consume(_:)))
        //: t.delegate = self
        t.delegate = self
        //: return t
        return t
...
extension DoingView: UIGestureRecognizerDelegate {
    ...
    func roundBridge(for danumuMsgModel: ImmediateGlobal?, isFirst: Bool) {
        //: let itemView = TalkingGameFloatingScreenItemView(model: danumuMsgModel)
        let itemView = SegmentCreate(model: danumuMsgModel)
        ...
        var frame = CGRect(x: mainCellMsg, y: isFirst ? 7 : 50, width: itemView.widthValue, height: 24)
        //: var endFrame = frame
        var endFrame = frame
        ...
        if let anim = POPBasicAnimation(propertyNamed: kPOPViewFrame) {
            ...
            itemView.pop_add(anim, forKey: (mainEdgeValue.replacingOccurrences(of: "mini", with: "ve")))
...
//    }
    ...
    func tensionGallery(_ isFirst: Bool) {
        ...
        if danumuMsgArray.count > 0 {
            ...
            if let model {
                ...
                roundBridge(for: model, isFirst: isFirst)
