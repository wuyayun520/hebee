...
/*: "gifts" :*/
fileprivate let k_groupName:String = "gwhyts"

/*: "vipGifts" :*/
fileprivate let app_globalUrl:String = "vipGiftsduring uniform mark"

/*: "packGifts" :*/
fileprivate let kAccountLocationPath:String = "pcurk"

/*: "giftSendItem" :*/
fileprivate let const_calculationStr:String = "drag"
fileprivate let showCommentKey:String = "injury global diskftSen"
...
class EdgeCell: NSObject {
    ...
    private var cacheDict = [String: [SimilarLarge: [Any]]]()
    // 数据锁
    ...
    private var giftType = FilePriority.normal

    //: static let share = TalkingChatGiftManager()
    static let share = EdgeCell()
...
extension EdgeCell {
    ...
    func sumeraction(type: FilePriority = .normal, isRefresh: Bool = false, completion: @escaping () -> Void) {
        //: TalkingChatGiftManager.share.giftType = type
        EdgeCell.share.giftType = type
        //: if isRefresh == false {
        if isRefresh == false {
            //: let dataDict = cacheDict[type.rawValue]
            let dataDict = cacheDict[type.rawValue]
            //: let hotArr = dataDict?[GiftDataType.Hot] ?? [Any]()
            let hotArr = dataDict?[SimilarLarge.Hot] ?? [Any]()
        ...
        if type == .groupChat {
            ...
            EdgeCell.share.versusUs(completion: { isSucceed in
            ...
            EdgeCell.share.mediumRid(completion: { isSucceed in
                //: guard isSucceed else { return }
                guard isSucceed else { return }
                //: let dataDict = self.cacheDict[type.rawValue]
                let dataDict = self.cacheDict[type.rawValue]
            ...
            EdgeCell.share.anyFollowing(completion: { isSucceed in
                //: guard isSucceed else { return }
                guard isSucceed else { return }
                //: let dataDict = self.cacheDict[type.rawValue]
                let dataDict = self.cacheDict[type.rawValue]
    ...
    private func anyFollowing(completion: @escaping (_ isSucceed: Bool) -> Void) {
        //: let type = TalkingChatGiftManager.share.giftType
        let type = EdgeCell.share.giftType
        //: TalkingChatRequestTool.req_ChatGiftList(type: type.rawValue) { succeed, result, errorModel in
        VariableCombine.zoneFor(type: type.rawValue) { succeed, result, errorModel in
            //: if succeed {
            if succeed {
                //: let data: Dictionary = result as! Dictionary<String, Any>
                let data: Dictionary = result as! [String: Any]
                //: var dataDict = [GiftDataType: [Any]]()
                var dataDict = [SimilarLarge: [Any]]()
                // hot礼物
                //: if let array: Array = data["gifts"] as? Array<Any> {
                if let array: Array = data[(k_groupName.replacingOccurrences(of: "why", with: "if"))] as? [Any] {
                    //: var hotArr = [Any]()
                    var hotArr = [Any]()
                    //: if let datas = Array<TalkingRoomGiftModel>.deserialize(from: array as? Array) {
                    if let datas = Array<PublishZone>.deserialize(from: array as? Array) {
                        //: hotArr.append(contentsOf: datas as [Any])
                        hotArr.append(contentsOf: datas as [Any])
                    }
                    //: dataDict[GiftDataType.Hot] = hotArr
                    dataDict[SimilarLarge.Hot] = hotArr
                }

                // vip礼物
                //: if let array: Array = data["vipGifts"] as? Array<Any> {
                if let array: Array = data[(String(app_globalUrl.prefix(8)))] as? [Any] {
                    //: var vipArr = [Any]()
                    var vipArr = [Any]()
                    //: if let datas = Array<TalkingRoomGiftModel>.deserialize(from: array as? Array) {
                    if let datas = Array<PublishZone>.deserialize(from: array as? Array) {
                    ...
                    dataDict[SimilarLarge.Vip] = vipArr
                }

                // Bags礼物
                //: if let array: Array = data["packGifts"] as? Array<Any> {
                if let array: Array = data[(kAccountLocationPath.replacingOccurrences(of: "cur", with: "ac") + "Gifts")] as? [Any] {
                    //: var bagsArr = [Any]()
                    var bagsArr = [Any]()
                    //: if let datas = Array<TalkingRoomGiftModel>.deserialize(from: array as? Array) {
                    if let datas = Array<PublishZone>.deserialize(from: array as? Array) {
                    ...
                    dataDict[SimilarLarge.Bags] = bagsArr
                }
                ...
                let sendItem: Array = data[(const_calculationStr.replacingOccurrences(of: "drag", with: "gi") + String(showCommentKey.suffix(5)) + "dItem")] as! [Any]
                ...
                for i in 0 ..< sendItem.count {
                    ...
                    if let configModel = JSONDeserializer<AreaModel>.deserializeFrom(dict: sendItem[i] as? [String: Any], designatedPath: nil) {
    ...
    private func versusUs(completion: @escaping (_ isSucceed: Bool) -> Void) {
        //: let type = TalkingChatGiftManager.share.giftType
        let type = EdgeCell.share.giftType
        //: TalkingChatRequestTool.req_ChatRoomGiftList(type: type.rawValue) { succeed, result, errorModel in
        VariableCombine.pastPainter(type: type.rawValue) { succeed, result, errorModel in
            //: if succeed {
            if succeed {
                //: let data: Dictionary = result as! Dictionary<String, Any>
                let data: Dictionary = result as! [String: Any]
    ...
    func refresh(params: [String: Any], completion: @escaping (_ data: [Any]?) -> Void) {
        //: TalkingChatRequestTool.req_getChatRoomMembers(params: params, completion: { succeed, result, errorModel in
        VariableCombine.tillCur(params: params, completion: { succeed, result, errorModel in
            //: if succeed {
            if succeed {
                ...
                for dict in data {
                    ...
                    if let memberModel = JSONDeserializer<StateDifficult>.deserializeFrom(dict: dict as? [String: Any], designatedPath: nil) {
    ...
    private func mediumRid(completion: @escaping (_ isSucceed: Bool) -> Void) {
        //: let type = GiftViewStyle.live
        let type = FilePriority.live
        //: TalkingChatRequestTool.req_LiveGiftList(type: type.rawValue) { succeed, result, errorModel in
        VariableCombine.theYearMission(type: type.rawValue) { succeed, result, errorModel in
            //: if succeed {
            if succeed {
                //: let data: Dictionary = result as! Dictionary<String, Any>
                let data: Dictionary = result as! [String: Any]
    ...
    func enterTheme(perPageCount: Int, dataType: SimilarLarge) -> Int {
    ...
    func basicWithoutFeedback(dataType: SimilarLarge) -> [Any] {
        //: cacheLock.lock()
        cacheLock.lock()
        //: defer {
        defer {
    ...
    func roundExclusiveScale(dataType: SimilarLarge, dataArr: [Any]) {
        //: cacheLock.lock()
        cacheLock.lock()
        //: defer {
        defer {
...
extension EdgeCell {
    ...
    private func outRecognize() {
        // 刷新礼物背包
        //: NotificationCenter.default.addObserver(self,
        NotificationCenter.default.addObserver(self,
                                               //: selector: #selector(receiveNewGiftPackage),
                                               selector: #selector(typicalRestore),
    ...
    @objc private func typicalRestore() {
        //: gift_clearMemoryCache(showRed: true)
        tagBold(showRed: true)
    }
}
...
extension EdgeCell {
    ...
    func tagBold(showRed: Bool = false) {
        //: TalkingChatGiftManager.share.showBagsRed = showRed
        EdgeCell.share.showBagsRed = showRed
        //: cacheDict.removeAll()
        cacheDict.removeAll()
    }
}
