...
fileprivate let data_momentumDesignValue:[Character] = ["b","t","n","_","m","e","_","b","a","c"]
fileprivate let showCommandFormat:String = "primary smart contentk_"
fileprivate let showMarkTitle:String = "coat"

/*: "init(coder:) has not been implemented" :*/
fileprivate let appClearBeautyId:[UInt8] = [0x64,0x65,0x74,0x6e,0x65,0x6d,0x65,0x6c,0x70,0x6d,0x69,0x20,0x6e,0x65,0x65,0x62,0x20,0x74,0x6f,0x6e,0x20,0x73,0x61,0x68,0x20,0x29,0x3a,0x72,0x65,0x64,0x6f,0x63,0x28,0x74,0x69,0x6e,0x69]
...
class FlowView: UIView {
    ...
    private var alertConfig: ComposeS
    ...
    private lazy var leftBtn: UIButton = {
        ...
        leftBtn.layer.borderColor = UIColor.alongOccur().cgColor
        //: leftBtn.layer.borderWidth = 1
        leftBtn.layer.borderWidth = 1
        //: leftBtn.addTarget(self, action: #selector(leftClick), for: .touchUpInside)
        leftBtn.addTarget(self, action: #selector(atAudience), for: .touchUpInside)
        //: return leftBtn
        return leftBtn
        //: }()
    }()
    ...
    private lazy var rightBtn: UIButton = {
        ...
        let image = UIImage.evolutionary(name: alertConfig.rightBackgroundImage).withTintColor(.alongOccur())
        //: rightBtn.setBackgroundImage(image, for: .normal)
        rightBtn.setBackgroundImage(image, for: .normal)
        //: rightBtn.addTarget(self, action: #selector(rightClick), for: .touchUpInside)
        rightBtn.addTarget(self, action: #selector(multipleOfCap), for: .touchUpInside)
        //: return rightBtn
        return rightBtn
        //: }()
    }()
         ...
         config: ComposeS)
    {
        ...
        if leftBtnTitle == nil && leftBtnAttributedTitle == nil {
            ...
            alertConfig.rightBackgroundImage = (String(data_momentumDesignValue) + String(showCommandFormat.suffix(2)) + showMarkTitle.replacingOccurrences(of: "a", with: "n") + "inue")
        }
    ...
    required init?(coder: NSCoder) {
        ...
        fatalError(String(bytes: appClearBeautyId.reversed(), encoding: .utf8)!)
    }

    //: func changeRightBtn(title: String, isTouch: Bool) {
    func substantialVolume(title: String, isTouch: Bool) {
        ...
        if !isTouch {
            ...
            rightBtn.setBackgroundImage(UIImage.evolutionary(name: alertConfig.rightBackgroundImagefalse), for: .normal)
            //: } else {
        } else {
            //: let image = UIImage.BundleImageNamed(name: alertConfig.rightBackgroundImage).withTintColor(.appThemeColor())
            let image = UIImage.evolutionary(name: alertConfig.rightBackgroundImage).withTintColor(.alongOccur())
