...
fileprivate let showUsString:[UInt8] = [0x5b,0x5c,0x5b,0x46,0x1a,0x51,0x5d,0x56,0x57,0x40,0x8,0x1b,0x12,0x5a,0x53,0x41,0x12,0x5c,0x5d,0x46,0x12,0x50,0x57,0x57,0x5c,0x12,0x5b,0x5f,0x42,0x5e,0x57,0x5f,0x57,0x5c,0x46,0x57,0x56]

private func symbolPositionOnicial(inside num: UInt8) -> UInt8 {
    return num ^ 50
}

/*: "%@.mp4" :*/
fileprivate let appRecentUrl:[Character] = ["%","@",".","m","p","4"]

/*: "http" :*/
fileprivate let const_shapeUrl:[Character] = ["h","t","t","p"]
...
class PanView: ConsumptionMay {
    ...
    required init?(coder: NSCoder) {
        ...
        fatalError(String(bytes: showUsString.map{symbolPositionOnicial(inside: $0)}, encoding: .utf8)!)
    }
    ...
    public lazy var vapPlayerView: UIView? = {
        ...
        let tap = UITapGestureRecognizer(target: self, action: #selector(graphOn))
    ...
    override func storage() {
        ...
        let path = WorkDocument.shared.emptyDuring(fileName: self.giftEffectModel?.mainFile ?? "", model: self.effectMsgModel ?? ExamineModel())

        //: if  path.count > 0 {
        if path.count > 0 {
            //: let mp4Path = String.init(format: "%@.mp4", path)
            let mp4Path = String(format: (String(appRecentUrl)), path)
...
extension PanView: HWDMP4PlayDelegate {
    ...
    func loadVapImage(withURL urlStr: String!, context: [AnyHashable: Any]!, completion completionBlock: VAPImageCompletionBlock!) {
        ...
        DispatchQueue.main.async {
            ...
            if urlStr.hasPrefix((String(const_shapeUrl))) {
