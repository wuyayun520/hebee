...
fileprivate let constHeadValue:[UInt8] = [0x6a,0x6d,0x6a,0x77,0x2b,0x60,0x6c,0x67,0x66,0x71,0x39,0x2a,0x23,0x6b,0x62,0x70,0x23,0x6d,0x6c,0x77,0x23,0x61,0x66,0x66,0x6d,0x23,0x6a,0x6e,0x73,0x6f,0x66,0x6e,0x66,0x6d,0x77,0x66,0x67]

private func snapCrucial(zz num: UInt8) -> UInt8 {
    return num ^ 3
}

/*: "icon_chat_miss" :*/
fileprivate let main_discoverUsString:[Character] = ["i"]
fileprivate let userBalanceViaFormat:[Character] = ["c","o","n","_","c","h","a","t","_","m","i","s","s"]

/*: "icon_chat_yes" :*/
fileprivate let main_easeKey:String = "icon_cpresentation object"
fileprivate let app_momentMessage:[Character] = ["s"]
...
class ValueView: UniformSkip {
//    var transBlock: (() -> Void)?

    //: var textData: PropertyReadAngle?
    var textData: PropertyReadAngle?
    ...
    required init?(coder aDecoder: NSCoder) {
        ...
        fatalError(String(bytes: constHeadValue.map{snapCrucial(zz: $0)}, encoding: .utf8)!)
    }
    ...
    lazy var quoteView: EmbraceMovementVariable = {
        //: let quoteView = TalkingMsgQuoteView()
        let quoteView = EmbraceMovementVariable()
        //: quoteView.isHidden = false
        quoteView.isHidden = false
        //: quoteView.addTarget(self, action: #selector(quoteBtnClick), for: .touchUpInside)
        quoteView.addTarget(self, action: #selector(listenWhite), for: .touchUpInside)
        //: return quoteView
        return quoteView
        //: }()
    }()
...
extension ValueView {
    ...
    override func creationCharacter(with data: EventColor) {
        ...
        //: if let temTextData = data as? PropertyReadAngle {
        if let temTextData = data as? PropertyReadAngle {
...
//                self.messageData?.translateStatus != .MsgTranslate_succeed && self.messageData?.msgModel.msgInfo.callCmd.count ?? 0 <= 0 && temTextData.msgModel.msgInfo.messageType != .MessageTypeWelcome && "\(temTextData.msgModel.user.uid)" != StopBrush.getXiaoMiID()) { // 对方发的 && 开启了翻译功能 && 消息未翻译成功
...
//            }
            ...
            if temTextData.msgModel.quoteMsgInfo.gem() {
            ...
            if self.textData?.translateStatus == .MsgTranslate_translating {
                ...
                self.transStatusImagV.image = UIImage.evolutionary(name: (String(main_discoverUsString) + String(userBalanceViaFormat)))
                ...
                self.transStatusImagV.image = UIImage.evolutionary(name: (String(main_easeKey.prefix(6)) + "hat_ye" + String(app_momentMessage)))
            }
    ...
    func head() {
        ...
        let containerTap = UITapGestureRecognizer(target: self, action: #selector(doEachPage(tap:)))
        ...
        self.container.addGestureRecognizer(containerTap)
        //: let isOfficial = (String(self.textData?.msgModel.user.uid ?? 0) == StopBrush.getXiaoMiID())
        let isOfficial = (String(self.textData?.msgModel.user.uid ?? 0) == StopBrush.pinBy())
        //: if isOfficial == false {
        if isOfficial == false {
            //: addLongTagGes()
            sayCheckion()
...
//            }
    ...
    func sayCheckion() {
        ...
        let longPressGr = UILongPressGestureRecognizer(target: self, action: #selector(mediaFor(tap:)))
        //: self.container.addGestureRecognizer(longPressGr)
        self.container.addGestureRecognizer(longPressGr)
    }

    //: @objc func TouchCellMessageAction(tap: UITapGestureRecognizer?) {
    @objc func doEachPage(tap: UITapGestureRecognizer?) {
    ...
    @objc func mediaFor(tap: UILongPressGestureRecognizer) {
        //: if tap.state == .ended {
        if tap.state == .ended {
            //: return
            return
...
//    }
    ...
    @objc func listenWhite() {
        //: self.delegate?.onReplyMessage(cell: self)
        self.delegate?.serverByConfirm(cell: self)
    }
}
