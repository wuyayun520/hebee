...
fileprivate let dataFarMsg:[UInt8] = [0x64,0x65,0x74,0x6e,0x65,0x6d,0x65,0x6c,0x70,0x6d,0x69,0x20,0x6e,0x65,0x65,0x62,0x20,0x74,0x6f,0x6e,0x20,0x73,0x61,0x68,0x20,0x29,0x3a,0x72,0x65,0x64,0x6f,0x63,0x28,0x74,0x69,0x6e,0x69]

/*: "FFD855" :*/
fileprivate let main_turnMsg:String = "FFDicon5"
...
class ItemRequest: UIView {
    ...
    var defaultWidth: CGFloat = 0.0 {
        ...
        didSet {
            ...
            DispatchQueue.main.asyncAfter(deadline: DispatchTime.now() + 0.2) {
                ...
                let minX = self.leftSilder.frame.origin.x + self.defaultWidth - user_promiseName
                //: self.rightSlider.frame = CGRect(x: minX,
                self.rightSlider.frame = CGRect(x: minX,
                                                //: y: self.rightSlider.frame.origin.y,
                                                y: self.rightSlider.frame.origin.y,
                                                //: width: kSliderWidth,
                                                width: user_promiseName,
                ...
                self.choiceWeekRoute()
                //: self.refreshSliderValidScope(startX: self.videoStartX)
                self.audienceFollow(startX: self.videoStartX)
    ...
    required init?(coder: NSCoder) {
        ...
        fatalError(String(bytes: dataFarMsg.reversed(), encoding: .utf8)!)
    }
    ...
    private lazy var indicatorView: UIView = {
        ...
        indicator.backgroundColor = UIColor(hex: (main_turnMsg.replacingOccurrences(of: "icon", with: "85")))
        //    indicator.alpha = 0.5
    ...
    private lazy var leftSilder: UIView = {
        ...
        leftSilder.frame = CGRect(x: constBecomeStr, y: 0, width: user_promiseName, height: self.frame.size.height)
    ...
    private lazy var rightSlider: UIView = {
        ...
        rightSlider.frame = CGRect(x: self.frame.size.width - user_promiseName, y: 0, width: user_promiseName, height: self.frame.size.height)
    ...
    private lazy var leftCoverView: UIView = {
        ...
        leftCoverView.backgroundColor = UIColor.roundItem(0, 0, 0, 0.6)
        //: leftCoverView.frame = CGRect(x: 0, y: 0, width: LeftMargin, height: self.frame.size.height)
        leftCoverView.frame = CGRect(x: 0, y: 0, width: constBecomeStr, height: self.frame.size.height)
    ...
    private lazy var rightCoverView: UIView = {
        ...
        rightCoverView.backgroundColor = UIColor.roundItem(0, 0, 0, 0.6)
...
extension ItemRequest {
    ...
    func levelBar() {
        ...
        self.leftSilder.frame = CGRect(x: constBecomeStr, y: 0, width: user_promiseName, height: self.frame.size.height)
        //: self.rightSlider.frame = CGRect(x: self.frame.size.width - kSliderWidth, y: 0, width: kSliderWidth, height: self.frame.size.height)
        self.rightSlider.frame = CGRect(x: self.frame.size.width - user_promiseName, y: 0, width: user_promiseName, height: self.frame.size.height)
        //: resetIndicator()
        album()
    }

    //: func needShowIndicator(isShow: Bool) {
    func occasionBelow(isShow: Bool) {
        //: self.indicatorView.isHidden = !isShow
        self.indicatorView.isHidden = !isShow
    }

    //: func refreshSliderValidScope(startX: CGFloat) {
    func audienceFollow(startX: CGFloat) {
        ...
        rightScope = CGVector(dx: leftSilder.frame.minX + minDurationGap - user_promiseName,
                              //: dy: min(leftSilder.frame.minX + maxDurationGap, self.width) - kSliderWidth)
                              dy: min(leftSilder.frame.minX + maxDurationGap, self.width) - user_promiseName)
    }
}
...
extension ItemRequest {
    ...
    @objc func noTransition(gesture: UIPanGestureRecognizer) {
        ...
        case .ended:
            ...
            self.audienceFollow(startX: videoStartX)
    ...
    @objc func someVariable(gesture: UIPanGestureRecognizer) {
        //: let thisTimePoint = gesture.location(in: self)
        let thisTimePoint = gesture.location(in: self)
        //: let state = gesture.state
        let state = gesture.state
...
extension ItemRequest {
    ...
    private func modeBecome() {
        ...
        let leftPanGesture = UIPanGestureRecognizer(target: self, action: #selector(noTransition(gesture:)))
        ...
        let rightPanGesture = UIPanGestureRecognizer(target: self, action: #selector(someVariable(gesture:)))
    ...
    func satisfyResolve(_ xposition: CGFloat) {
