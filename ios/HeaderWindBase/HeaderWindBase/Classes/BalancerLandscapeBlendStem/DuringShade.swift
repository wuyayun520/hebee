...
fileprivate let mainParaId:[UInt8] = [0x59,0x5e,0x59,0x44,0x18,0x53,0x5f,0x54,0x55,0x42,0xa,0x19,0x10,0x58,0x51,0x43,0x10,0x5e,0x5f,0x44,0x10,0x52,0x55,0x55,0x5e,0x10,0x59,0x5d,0x40,0x5c,0x55,0x5d,0x55,0x5e,0x44,0x55,0x54]

private func buttBot(environment num: UInt8) -> UInt8 {
    return num ^ 48
}
...
typealias SlidingHandler = (_ direction: EditIndex) -> Void
...
class DuringShade: UIView {
    //: var cropStart: CGFloat {
    var cropStart: CGFloat {
        //: let offset = overlay.startOffset - (LeftMargin - self.contentView.contentOffset.x)
        let offset = overlay.startOffset - (constBecomeStr - self.contentView.contentOffset.x)
    ...
    required init?(coder: NSCoder) {
        ...
        fatalError(String(bytes: mainParaId.map{buttBot(environment: $0)}, encoding: .utf8)!)
    }
    ...
    private lazy var contentView: UIScrollView = {
        ...
        while time <= self.duration {
            ...
            let image = self.tillCap(point: time)
            //: imv.image = image
            imv.image = image
    ...
    lazy var overlay: ItemRequest = {
        ...
        let overlay = ItemRequest(frame: self.bounds, maxCropLen: maxWidth, minCropLen: minWidth)
...
extension DuringShade {
    ...
    private func tillCap(point: CGFloat) -> UIImage {
...
extension DuringShade: UIScrollViewDelegate {
    //: func scrollViewDidScroll(_ scrollView: UIScrollView) {
    func scrollViewDidScroll(_ scrollView: UIScrollView) {
        //: var offset = scrollView.contentOffset
        var offset = scrollView.contentOffset
        //: let minx = LeftMargin - overlay.startOffset
        let minx = constBecomeStr - overlay.startOffset
    ...
    func scrollViewWillBeginDragging(_ scrollView: UIScrollView) {
        ...
        self.overlay.occasionBelow(isShow: false)
    }
    ...
    func scrollViewDidEndDragging(_ scrollView: UIScrollView, willDecelerate decelerate: Bool) {
        ...
        self.overlay.occasionBelow(isShow: true)
    }
...
extension DuringShade {
    ...
    private func noLayoff() {
        ...
        self.contentView.snp.makeConstraints { make in
            ...
            make.leading.equalToSuperview().offset(constBecomeStr)
