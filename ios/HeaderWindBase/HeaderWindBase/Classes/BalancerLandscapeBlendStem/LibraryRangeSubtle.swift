...
fileprivate let dataAnimaAttachMsg:[Character] = ["\u{8bbe}","置","普","\u{901a}","状"]
fileprivate let main_executeId:String = "\u{6001}的文"
fileprivate let noti_complaintTitle:[Character] = ["字","颜","色","时"," ","请","使","用","R","G","B","空","间","的","颜","色","值"]

/*: "设置选中状态的文字颜色时 请使用RGB空间的颜色值" :*/
fileprivate let show_environmentData:String = "\u{8bbe}\u{7f6e}选\u{4e2d}状"
fileprivate let kDenseMonthKey:String = "时 请people"

/*: "init(coder:) has not been implemented" :*/
fileprivate let notiBodString:[UInt8] = [0x64,0x65,0x74,0x6e,0x65,0x6d,0x65,0x6c,0x70,0x6d,0x69,0x20,0x6e,0x65,0x65,0x62,0x20,0x74,0x6f,0x6e,0x20,0x73,0x61,0x68,0x20,0x29,0x3a,0x72,0x65,0x64,0x6f,0x63,0x28,0x74,0x69,0x6e,0x69]

/*: "设置的下标不合法!!" :*/
fileprivate let dataGraphicName:String = "设置\u{7684}下标不合"
fileprivate let constNorKey:String = "produce!!"

/*: "Bags" :*/
fileprivate let const_mixStr:String = "button loopBags"
...
open class LibraryRangeSubtle: UIView {
    //: open var segmentStyle: SegmentStyle
    open var segmentStyle: TweenHeap
    /// 点击响应
    ...
    fileprivate lazy var normalColorRgb: (r: CGFloat, g: CGFloat, b: CGFloat) = {
        ...
        if let normalRgb = self.embrace(self.segmentStyle.normalTitleColor) {
            //: return normalRgb
            return normalRgb
            //: } else {
        } else {
            //: fatalError("设置普通状态的文字颜色时 请使用RGB空间的颜色值")
            fatalError((String(dataAnimaAttachMsg) + main_executeId + String(noti_complaintTitle)))
        }
        //: }()
    }()
    ...
    fileprivate lazy var selectedTitleColorRgb: (r: CGFloat, g: CGFloat, b: CGFloat) = {
        ...
        if let selectedRgb = self.embrace(self.segmentStyle.selectedTitleColor) {
            //: return selectedRgb
            return selectedRgb
            //: } else {
        } else {
            //: fatalError("设置选中状态的文字颜色时 请使用RGB空间的颜色值")
            fatalError((show_environmentData + "态\u{7684}文字颜色" + kDenseMonthKey.replacingOccurrences(of: "people", with: "使用") + "RGB空\u{95f4}的\u{989c}色\u{503c}"))
        }
    ...
    fileprivate func embrace(_ color: UIColor) -> (r: CGFloat, g: CGFloat, b: CGFloat)? {
    ...
    public init(frame: CGRect, segmentStyle: TweenHeap, titles: [String]) {
    ...
    @objc func lab(_ tapGes: UITapGestureRecognizer) {
        //: guard let currentLabel = tapGes.view as? CustomLabel else { return }
        guard let currentLabel = tapGes.view as? RedView else { return }
    ...
    public required init?(coder aDecoder: NSCoder) {
        ...
        fatalError(String(bytes: notiBodString.reversed(), encoding: .utf8)!)
    }
}
...
public extension LibraryRangeSubtle {
    ///  对外界暴露设置选中的下标的方法 可以改变设置下标滚动后是否有动画切换效果
    //: public func selectedIndex(_ selectedIndex: Int, animated: Bool) {
    func exceptText(_ selectedIndex: Int, animated: Bool) {
        //: assert(!(selectedIndex < 0 || selectedIndex >= titles.count), "设置的下标不合法!!")
        assert(!(selectedIndex < 0 || selectedIndex >= titles.count), (dataGraphicName + constNorKey.replacingOccurrences(of: "produce", with: "法")))
...
//: extension ScrollSegmentView {
extension LibraryRangeSubtle {
    // 根据Titles添加相应的控件
    //: fileprivate func setupTitles() {
    fileprivate func original() {
        //: for (index, title) in titles.enumerated() {
        for (index, title) in titles.enumerated() {
            //: let label = CustomLabel(frame: CGRect.zero)
            let label = RedView(frame: CGRect.zero)
            //: label.tag = index
            label.tag = index
            //: label.text = title
            label.text = title
            ...
            let tapGes = UITapGestureRecognizer(target: self, action: #selector(self.lab(_:)))
            ...
            if title == (String(const_mixStr.suffix(4))).localized {
    ...
    fileprivate func context() {
        ...
        if let firstLabel = labelsArray[0] as? RedView {
    ...
    public func stopSkip(_ currentIndex: Int) {
