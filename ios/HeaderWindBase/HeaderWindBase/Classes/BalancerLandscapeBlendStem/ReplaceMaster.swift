...
fileprivate let dataLeastString:[UInt8] = [0x55,0x52,0x55,0x48,0x14,0x5f,0x53,0x58,0x59,0x4e,0x6,0x15,0x1c,0x54,0x5d,0x4f,0x1c,0x52,0x53,0x48,0x1c,0x5e,0x59,0x59,0x52,0x1c,0x55,0x51,0x4c,0x50,0x59,0x51,0x59,0x52,0x48,0x59,0x58]

private func bodyKit(display num: UInt8) -> UInt8 {
    return num ^ 60
}
...
class ReplaceMaster: PetViewController {
    //: var tranConfig: BasePresentTransition!
    var tranConfig: DistinctionVisitor!
    //: init() {
    init() {
        //: super.init(nibName: nil, bundle: nil)
        super.init(nibName: nil, bundle: nil)
        //: self.tranConfig = BasePresentTransition.init(target: self)
        self.tranConfig = DistinctionVisitor(target: self)
    }

    //: required init?(coder: NSCoder) {
    required init?(coder: NSCoder) {
        //: fatalError("init(coder:) has not been implemented")
        fatalError(String(bytes: dataLeastString.map{bodyKit(display: $0)}, encoding: .utf8)!)
    }
}

//: class BasePresentTransition: NSObject {
class DistinctionVisitor: NSObject {
    ...
    var frameOfPresentedView = CGRect(x: 0, y: 0, width: mainCellMsg, height: kPressEasePublishString * 0.6)
    ...
    func highlightLike(corners: UIRectCorner, radius: CGFloat) {
...
extension DistinctionVisitor: UIViewControllerTransitioningDelegate {
    ...
    func presentationController(forPresented presented: UIViewController, presenting: UIViewController?, source: UIViewController) -> UIPresentationController? {
        ...
        let res = MaxCharacteristic(transition: self, presentedViewController: presented, presentingViewController: presenting)
        //: return res
        return res
    }
}
...
extension DistinctionVisitor: UIViewControllerAnimatedTransitioning {
    ...
    func animateTransition(using transitionContext: UIViewControllerContextTransitioning) {
        ...
        if toVC == targetVC {
            ...
            outsideFieldLeave(transitionContext: transitionContext)
            //: } else {
        } else {
            //: dismissTransition(transitionContext: transitionContext)
            start(transitionContext: transitionContext)
        }
    }

    //: private func presentTransition(transitionContext: UIViewControllerContextTransitioning) {
    private func outsideFieldLeave(transitionContext: UIViewControllerContextTransitioning) {
        //: let toVC = transitionContext.viewController(forKey: .to)
        let toVC = transitionContext.viewController(forKey: .to)
    ...
    private func start(transitionContext: UIViewControllerContextTransitioning) {
...
class MaxCharacteristic: UIPresentationController, UIGestureRecognizerDelegate {
    //: var transition: BasePresentTransition!
    var transition: DistinctionVisitor!

    //: init(transition: BasePresentTransition, presentedViewController: UIViewController, presentingViewController: UIViewController?) {
    init(transition: DistinctionVisitor, presentedViewController: UIViewController, presentingViewController: UIViewController?) {
    ...
    func con() {
        ...
        let tap = UITapGestureRecognizer(target: self, action: #selector(frequencyPurchase))
        //: tap.delegate = self
        tap.delegate = self
    ...
    lazy var visualView: UIView = {
        ...
        view.frame = containerView?.bounds ?? CGRect(x: 0, y: 0, width: mainCellMsg, height: kPressEasePublishString)
