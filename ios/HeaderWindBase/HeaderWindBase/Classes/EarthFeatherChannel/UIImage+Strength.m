...
#import "UIImage+Strength.h"

//: @implementation UIImage (Compress)
@implementation UIImage (Strength)

//: - (NSData *)compressedImageData {
- (NSData *)from {

    //: UIImage *image = self.wcTimelineCompress;
    UIImage *image = self.dismissAssociated;
...
- (UIImage *)odd {
    ...
    return [UIImage imageWithData:[self from]];
}
...
- (UIImage *)share {
    ...
    return [self info:YES];
}

//: - (UIImage *)wcTimelineCompress {
- (UIImage *)dismissAssociated {
...
- (UIImage *)info:(Boolean)isSession {
    //: CGSize size = [self wxImageSize:isSession];
    CGSize size = [self observe:isSession];
    //: UIImage *reImage = [self resizedImage:size];
    UIImage *reImage = [self instruction:size];
...
- (CGSize)observe:(Boolean)isSession {
...
+ (NSData *)quantity:(UIImage *)image master:(NSInteger)fileSize{
    ...
    while (outIMGData.length>(fileSize*1024)) {
        ...
        thumIMG = [thumIMG array:scalesize lockGraph:NO];

        //: outIMGData = [thumIMG compressToFileSize:(fileSize*1024)];
        outIMGData = [thumIMG row:(fileSize*1024)];
    }
...
- (UIImage*)bridge:(CGSize)targetSize{
...
- (UIImage *)array:(CGSize)targetSize lockGraph:(BOOL)isScale{
...
- (UIImage *)instruction:(CGSize)newSize {
