...
fileprivate let k_materialVariablePath:[UInt8] = [0xb6,0xb1,0xb6,0xab,0xf7,0xbc,0xb0,0xbb,0xba,0xad,0xe5,0xf6,0xff,0xb7,0xbe,0xac,0xff,0xb1,0xb0,0xab,0xff,0xbd,0xba,0xba,0xb1,0xff,0xb6,0xb2,0xaf,0xb3,0xba,0xb2,0xba,0xb1,0xab,0xba,0xbb]

/*: "all" :*/
fileprivate let const_trustStr:String = "ALL"
...
class YieldReliability: UniformSkip {
    //: var giftData: TalkingGroupChatGiftMsgCellData?
    var giftData: LevelAssociate?
    ...
    required init?(coder: NSCoder) {
        ...
        fatalError(String(bytes: k_materialVariablePath.map{$0^223}, encoding: .utf8)!)
    }
...
extension YieldReliability {
    ...
    override func creationCharacter(with data: EventColor) {
        ...
        self.giftData = data as? LevelAssociate
        ...
        if giftData.msgModel.toUser.uid == (const_trustStr.lowercased()) { // 送礼所有人
            //: self.toUserImgView.image = UIImage.BundleImageNamed(name: giftData.msgModel.toUser.headPic)
            self.toUserImgView.image = UIImage.evolutionary(name: giftData.msgModel.toUser.headPic)
            //: } else {
        } else {
            //: self.toUserImgView.setUrlImage(urlStr: giftData.msgModel.toUser.headPic)
            self.toUserImgView.accessiblePending(urlStr: giftData.msgModel.toUser.headPic)
        }
    }
