...
class NextView: UIView {
    ...
    convenience init(placeholder: String?, placeholderColor: UIColor?, frame: CGRect) {
        ...
        sumBloc(placeholder: placeholder, placeholderColor: placeholderColor)
    ...
    required init?(coder aDecoder: NSCoder) {
        ...
        sumBloc(placeholder: nil, placeholderColor: nil)
    }
}
...
extension NextView {
    ...
    private func sumBloc(placeholder: String?, placeholderColor: UIColor?) {
        ...
        plaleLabel.snp.remakeConstraints { make in
            ...
            make.leading.equalTo(SelectCommand().x)
            //: make.top.equalTo(PlaceholderLabelOrigin().y)
            make.top.equalTo(SelectCommand().y)
            //: make.trailing.equalTo(-PlaceholderLabelOrigin().x)
            make.trailing.equalTo(-SelectCommand().x)
        }
        ...
        placeholdertextView.textContainerInset = UIEdgeInsets(top: ProMessage().top, left: ProMessage().left, bottom: ProMessage().bottom, right: ProMessage().right)
...
extension NextView: UITextViewDelegate {
...
extension NextView {
    ...
    private func followAndNorInstead() {
