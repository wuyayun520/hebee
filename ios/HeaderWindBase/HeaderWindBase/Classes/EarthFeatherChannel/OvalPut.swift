...
fileprivate let appReadingAcceptId:[UInt8] = [0x55,0x52,0x55,0x48,0x14,0x5f,0x53,0x58,0x59,0x4e,0x6,0x15,0x1c,0x54,0x5d,0x4f,0x1c,0x52,0x53,0x48,0x1c,0x5e,0x59,0x59,0x52,0x1c,0x55,0x51,0x4c,0x50,0x59,0x51,0x59,0x52,0x48,0x59,0x58]

/*: "SHBDropMenuCell" :*/
fileprivate let main_darkUrl:String = "SlanguageD"
fileprivate let main_evolveValue:String = "uCellthat respective"

/*: "title" :*/
fileprivate let main_skinKey:String = "graphicigraphicle"
...
public class OvalPut: UIView {
    //: var menuDelegate: DropDownMenuViewDelegate?
    var menuDelegate: AlwaysKind?
    ...
    var menuBgColor: UIColor? {
        ...
        //: }
    } // = Config.PhaseLocal.menuBgColor
    // 线条颜色
    //: var lineColor = Config.MenuCellConfig.lineColor
    var lineColor = Config.DarkColor.lineColor
    // cell高度
    //: public var menuCellHeight = Config.MenuCellConfig.menuCellHeight
    public var menuCellHeight = Config.DarkColor.menuCellHeight
    /**
     *  table最大高度限制
     *  默认：5 * cellHeight
     */
    //: var menuMaxHeight = Config.MenuCellConfig.menuMaxHeight
    var menuMaxHeight = Config.DarkColor.menuMaxHeight
    /**
     *  小三角高度
     *  45°等腰三角形
     */
    ...
    public var menuStyle = Config.initialMenuStyle ?? GrainTrain.MenuDarkStyle {
        //: didSet {
        didSet {
            //: switch menuStyle {
            switch menuStyle {
    ...
    public class func outGreen(anchorView: UIView, titleArray: [String], imageArray: [UIImage]) -> OvalPut {
        ...
        let menuView = fragmentLocal(anchorView: anchorView, menuArray: menuArray)
        //: return menuView
        return menuView
    }

    //: public class func pullDropDrownMenu(anchorView: UIView, menuArray: [DropMenuModel]) -> DropDownMenuView {
    public class func fragmentLocal(anchorView: UIView, menuArray: [FactoryMechanismCountervalModel]) -> OvalPut {
        //: let window = StopBrush.getWindow()
        let window = StopBrush.correct()
        //: let menuView = DropDownMenuView()
        let menuView = OvalPut()
...
//        menuView.refreshUI()
    ...
    required init?(coder: NSCoder) {
        ...
        fatalError(String(bytes: appReadingAcceptId.map{$0^60}, encoding: .utf8)!)
    }

    //: private func makeContentView() -> UIView {
    private func lite() -> UIView {
        //: let view  = UIView()
        let view = UIView()
        //: return view
        return view
    ...
    private func produceing() -> UITableView {
    ...
    private func beTrigger() {
        //: addSubview(contentView)
        addSubview(contentView)
        //: mTable.register(DropMenuCell.self, forCellReuseIdentifier: "SHBDropMenuCell")
        mTable.register(CornerCoordinator.self, forCellReuseIdentifier: (main_darkUrl.replacingOccurrences(of: "language", with: "HB") + "ropMen" + String(main_evolveValue.prefix(5))))
        //: contentView.addSubview(mTable)
        contentView.addSubview(mTable)
    }

    //: private func refreshUI() {
    private func byToday() {
        //: drawmTableFrame()
        associateAccessible()
        //: mTable.reloadData()
        mTable.reloadData()
    ...
    private func missionThird() -> CGFloat {
    ...
    private func associateAccessible() {
...
extension OvalPut: UITableViewDelegate, UITableViewDataSource {
    ...
    public func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        let cell = tableView.dequeueReusableCell(withIdentifier: (main_darkUrl.replacingOccurrences(of: "language", with: "HB") + "ropMen" + String(main_evolveValue.prefix(5))), for: indexPath) as! CornerCoordinator
    ...
    public func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        selectionAction?(indexPath.row, cellModel.title ?? (main_skinKey.replacingOccurrences(of: "graphic", with: "t")))
        //: animateRemoveView()
        impression()

        //: menuDelegate?.didClickSelectedRow(index: indexPath.row, title: cellModel.title ?? "")
        menuDelegate?.policyContainer(index: indexPath.row, title: cellModel.title ?? "")
    }
}
