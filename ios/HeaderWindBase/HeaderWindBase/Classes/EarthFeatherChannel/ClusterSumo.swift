...
class ClusterSumo: PetViewController {
    ...
    lazy var bottomView: AboutMargin = {
    ...
    lazy var usePhotoView: ClipView = {
        //: let v = TalkingSendIntimateInfoView(frame: CGRect.zero, type: .intimatePhoto)
        let v = ClipView(frame: CGRect.zero, type: .intimatePhoto)
        //: return v
        return v
        //: }()
    }()
...
extension ClusterSumo {
    //: func crushBtnTool(modle: TalkingMomentModel) {
    func priorDown(modle: FrequencyModel) {
        ...
        if modle.sex == nil {
            ...
            //: bottomView.isHidden = modle.sex==MountCustom.share.loginUserMode.sex
            bottomView.isHidden = modle.sex == MountCustom.share.loginUserMode.sex
        }
