...
/*: "add_header_cookies" :*/
fileprivate let showGravityPath:[Character] = ["a","d","d","_","h","e"]
fileprivate let data_groupUrl:[Character] = ["a","d","e","r","_","c","o"]
fileprivate let data_edgeValue:String = "okiehappy"
...
public extension UIImageView {
    ...
    @objc func accessiblePending(urlStr: String,
    ...
    internal func containRetain(urlStr: String,
                                //: placeImg: UIImage = UIImage.placeImgSquare(),
                                placeImg: UIImage = UIImage.smartThreshold(),
    ...
    @objc func basicReflect(urlStr: String) {
        //: let imgUrl = urlStr.replacingOccurrences(of: UrlDomin.urlAes256Decrypt(), with: ReplaceUrlDomain)
        let imgUrl = urlStr.replacingOccurrences(of: appProvideSectionPath.mentionAccount(), with: dataServiceName)
        //: self.kf.setImage(with: URL(string: imgUrl), placeholder: UIImage.giftPlaceHolderImage())
        self.kf.setImage(with: URL(string: imgUrl), placeholder: UIImage.sophisticatedSwitcher())
    }
    ...
    @objc func tapCounterval(urlStr: String, completion: ((_ image: UIImage?) -> Void)? = nil) {
        // 允许请求携带cookie
        ...
        if var urlComponents = URLComponents(string: urlStr),
           ...
           let currURL = URL(string: PutDirection.share.serverUrlStr)
        {
        ...
        let downloader = ImageDownloader(name: (String(showGravityPath) + String(data_groupUrl) + data_edgeValue.replacingOccurrences(of: "happy", with: "s")))
        ...
        self.kf.setImage(with: URL(string: newImgUrl), placeholder: UIImage.smartThreshold(), options: optionsInfo) { result in
            //: if completion != nil {
            if completion != nil {
                //: let image = try? result.get().image
                let image = try? result.get().image
