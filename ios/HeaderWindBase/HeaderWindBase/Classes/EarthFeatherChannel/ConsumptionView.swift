...
/*: "Select a date" :*/
fileprivate let data_typicalGeneralName:String = "middle class destination possible inviteSelect"
...
protocol CurveProcessor: NSObjectProtocol {
    //: func pickDateView(year: Int, month: Int, day: Int)
    func logWithReveal(year: Int, month: Int, day: Int)
}

//: class TalkingDateView: TalkingBasePickView, UIPickerViewDataSource, UIPickerViewDelegate {
class ConsumptionView: RadioBrush, UIPickerViewDataSource, UIPickerViewDelegate {
    ...
    weak var delegate: CurveProcessor?
    ...
    override func timingNow() {
        ...
        titleString = (String(data_typicalGeneralName.suffix(6)) + " a date").localized
        ...
        outsideList(year: defaultYear!, month: defaultMonth!, day: defaultDay!)
    }

    //: func setDefaultDate(year: Int, month: Int, day: Int) {
    func outsideList(year: Int, month: Int, day: Int) {
        //: currentYear = year
        currentYear = year
        //: if year == defaultYear {
        if year == defaultYear {
...
//        }
    ...
    override func pin(button: UIButton) {
        ...
        if let day = selectDay {
            ...
            delegate?.logWithReveal(year: selectYear!, month: selectMonth!, day: day)
            //: } else {
        } else {
            //: delegate?.pickDateView(year: selectYear!, month: selectMonth!, day: 0)
            delegate?.logWithReveal(year: selectYear!, month: selectMonth!, day: 0)
        }
    }
        ...
        if isShowDay {
            ...
            if component == 0 {
                ...
                if currentYear == yearSum + minShowYear - 1 {
                    ...
                    return errAsk(year: year, month: month)
                }
                //: } else {
            } else {
                //: return self.yearSum
    ...
    func errAsk(year: Int, month: Int) -> Int {
