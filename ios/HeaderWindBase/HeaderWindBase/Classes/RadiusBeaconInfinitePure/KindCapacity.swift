...
fileprivate let userMobileUrl:[UInt8] = [0x8d,0x8a,0x8d,0x90,0xcc,0x87,0x8b,0x80,0x81,0x96,0xde,0xcd,0xc4,0x8c,0x85,0x97,0xc4,0x8a,0x8b,0x90,0xc4,0x86,0x81,0x81,0x8a,0xc4,0x8d,0x89,0x94,0x88,0x81,0x89,0x81,0x8a,0x90,0x81,0x80]

/*: "icon_coinxxx" :*/
fileprivate let userGoldPath:String = "too manage natural socialicon_coi"
fileprivate let mainDocumentBoundId:String = "nsnapsnapsnap"

/*: "icon_gift_num_bg" :*/
fileprivate let mainFastValue:String = "menu spot detail complaint activeicon_"
fileprivate let constShadowKey:String = "um_bgpossible half"
...
class KindCapacity: UICollectionViewCell {
    //: var model = TalkingRoomGiftModel()
    var model = PublishZone()
    //: let checked = false
    let checked = false
    //: let giftNum = 0
    let giftNum = 0
    ...
    required init?(coder aDecoder: NSCoder) {
        ...
        fatalError(String(bytes: userMobileUrl.map{$0^228}, encoding: .utf8)!)
    }
    ...
    lazy var nameLB: UILabel = {
        ...
        lb.font = UIFont.stepTransition(type: .Regular, fontSize: 10)
        //: lb.textColor = .white
        lb.textColor = .white
    ...
    lazy var priceBtn: MethodButton = {
        //: let btn = TalkingButton.init()
        let btn = MethodButton()
        //: btn.setTitleColor(UIColor.white, for: .normal)
        btn.setTitleColor(UIColor.white, for: .normal)
        //: btn.titleLabel?.font = UIFont.pingfangFont(type: .Regular, fontSize: 10)
        btn.titleLabel?.font = UIFont.stepTransition(type: .Regular, fontSize: 10)
        //: btn.setImage(UIImage.BundleImageNamed(name: "icon_coinxxx"), for: .normal)
        btn.setImage(UIImage.evolutionary(name: (String(userGoldPath.suffix(8)) + mainDocumentBoundId.replacingOccurrences(of: "snap", with: "x"))), for: .normal)
    ...
    lazy var numBgView: UIImageView = {
        ...
        let imag = UIImage.evolutionary(name: (String(mainFastValue.suffix(5)) + "gift_n" + String(constShadowKey.prefix(5))))
        //: let sizex = imag.size
        let sizex = imag.size
    ...
    lazy var numLb: UILabel = {
        ...
        lb.font = UIFont.stepTransition(type: .Regular, fontSize: 9)
        //: return lb
        return lb
        //: }()
    }()
...
extension KindCapacity {
    /// 更改选中状态
    //: func setChecked(checked: Bool) {
    func labelDown(checked: Bool) {
        ...
        if checked {
            ...
            bgView.layer.borderColor = UIColor.alongOccur().cgColor
    ...
    private func descriptionTolerance() {
    ...
    func disabledNetwork(currenmodel: PublishZone, giftType: SimilarLarge) {
        //: model = currenmodel
        model = currenmodel
        //: giftIcon.setUrlImage(urlStr: model.imgPreview ?? "")
        giftIcon.accessiblePending(urlStr: model.imgPreview ?? "")
        //: priceBtn.setTitle("\(model.price)", for: .normal)
        priceBtn.setTitle("\(model.price)", for: .normal)
        //: selectedImageView.setUrlImage(urlStr: model.cornerMark, placeImg: nil)
        selectedImageView.accessiblePending(urlStr: model.cornerMark, placeImg: nil)
        ...
        let attributes = [NSAttributedString.Key.font: UIFont.stepTransition(type: .Regular, fontSize: 11),
...
extension KindCapacity {
    ...
    func eager() {
        ...
        bgView.backgroundColor = UIColor.noneSufficient()
