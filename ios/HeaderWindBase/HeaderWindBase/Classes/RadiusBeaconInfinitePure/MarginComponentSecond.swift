...
/*: "%@ reply %@：%@" :*/
fileprivate let showQuantityTickValue:String = "%@ reprobot conversion jump sample"
fileprivate let dataLowerUrl:String = "@：%@"
...
struct MarginComponentSecond: HandyJSON {
    //: var cid: String?
...
extension MarginComponentSecond {
    //: mutating func caculateReplyItemHeight() {
    mutating func streamStage() {
        //: var tempContent = ""
        var tempContent = ""
        //: if replyUser.count>0 {
        if replyUser.count > 0 {
            //: tempContent = String(format: "%@ reply %@：%@", nickname, replyUser, content)
            tempContent = String(format: (String(showQuantityTickValue.prefix(6)) + "ly %" + dataLowerUrl.capitalized), nickname, replyUser, content)
            //: } else {
        } else {
        ...
        let sizeheight = Int(PublishView.sourceTier(text: tempContent, size: CGSize(width: mainCellMsg - 82, height: CGFloat(MAXFLOAT)), font: UIFont.stepTransition(type: .Regular, fontSize: 14), lineNumber: 0).height)
