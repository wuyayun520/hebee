...
fileprivate let k_poolStr:[UInt8] = [0x64,0x65,0x74,0x6e,0x65,0x6d,0x65,0x6c,0x70,0x6d,0x69,0x20,0x6e,0x65,0x65,0x62,0x20,0x74,0x6f,0x6e,0x20,0x73,0x61,0x68,0x20,0x29,0x3a,0x72,0x65,0x64,0x6f,0x63,0x28,0x74,0x69,0x6e,0x69]

/*: "img_loading_lose" :*/
fileprivate let notiMixQueryMessage:[Character] = ["i","m","g","_","l","o","a","d","i","n","g","_","l","o","s","e"]
...
class OwnerSection: UICollectionViewCell {
    ...
    required init?(coder: NSCoder) {
        ...
        fatalError(String(bytes: k_poolStr.reversed(), encoding: .utf8)!)
    }
...
extension OwnerSection {
    //: private func bindInteraction() {
    private func hold() {
        //: let singleTap = UITapGestureRecognizer(target: self, action: #selector(singleTapAction))
        let singleTap = UITapGestureRecognizer(target: self, action: #selector(become))

        //: doubleTap = UITapGestureRecognizer(target: self, action: #selector(doubleTapAction)).then {
        doubleTap = UITapGestureRecognizer(target: self, action: #selector(reading)).then {
...
extension OwnerSection {
    ...
    func image(imgModel: PrimaryModel) {
        ...
        if imgModel.imagePic != nil {
            ...
            imgView.kf.setImage(with: URL(string: imgModel.url), placeholder: UIImage(named: (String(notiMixQueryMessage))), options: [.keepCurrentImageWhileLoading]) { result in
                //: self.recoverSubviews()
                self.tapPress()
            }
        }
    ...
    func smart(imgUrl: String) {
        ...
        imgView.kf.setImage(with: URL(string: imgUrl), placeholder: UIImage(named: (String(notiMixQueryMessage))), options: [.keepCurrentImageWhileLoading]) { result in
            //: self.recoverSubviews()
            self.tapPress()
        }
    }
...
extension OwnerSection: UIScrollViewDelegate, UIGestureRecognizerDelegate {
...
extension OwnerSection {
    ...
    func tapPress() {
        ...
        var newW = mainCellMsg
        //: var newH = ScreenWidth * tempImg!.size.height / tempImg!.size.width
        var newH = mainCellMsg * tempImg!.size.height / tempImg!.size.width

        //: if newH > ScreenHeight {
        if newH > kPressEasePublishString {
            //: newH = ScreenHeight
            newH = kPressEasePublishString
            //: newW = tempImg!.size.width * ScreenHeight / tempImg!.size.height
            newW = tempImg!.size.width * kPressEasePublishString / tempImg!.size.height
        }
        ...
        self.imgContainerView.origin = CGPoint(x: (mainCellMsg - newW) / 2.0, y: (kPressEasePublishString - newH) / 2)
