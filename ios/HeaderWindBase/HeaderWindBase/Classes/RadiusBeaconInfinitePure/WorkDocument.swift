...
fileprivate let appWithinPath:[Character] = ["C","a","c","h","e","G","i","f","t","E","f","f","e","c","t","C","a","c","h","e"]
fileprivate let showSituationString:[Character] = ["K","e","y"]

/*: "下载动画文件" :*/
fileprivate let mainTitleureFormValue:[Character] = ["下","载","动","画","文","件"]

/*: "加载本地动画文件" :*/
fileprivate let k_informationMessage:String = "加载本地动画"
fileprivate let constMedalStr:String = "文件"

/*: "Cache_GiftEffect/" :*/
fileprivate let data_reserveWeekTitle:String = "extension characteristic stretchCach"
fileprivate let noti_allDownStr:String = "temp tap produce typicaltEffect/"

/*: "GJSGVA" :*/
fileprivate let k_finishTitle:[Character] = ["G","J","S","G","V","A"]

/*: "GJVap" :*/
fileprivate let dataIconValue:String = "equipment specify springGJVap"

/*: "config.json" :*/
fileprivate let constDatePossibleFormat:[Character] = ["c","o","n","f","i","g",".","j","s","o","n"]

/*: "app. :*/
fileprivate let constAngleFormat:[Character] = ["a","p","p","."]

/*: .com" :*/
fileprivate let show_reflectTitle:String = "every invisible king entry unique.com"

/*: "key" :*/
fileprivate let main_businessUrl:String = "kcontinuing"

/*: "zip" :*/
fileprivate let data_occasionString:String = "ZIP"

/*: " fail to unzip file" :*/
fileprivate let data_invitationHoneyTitle:String = " failneed below try special dinner"
fileprivate let noti_foundPath:String = "unzicombine"
fileprivate let show_phaseStr:String = "alter awake file"

/*: "RSA 解密失败" :*/
fileprivate let dataStageMessage:String = "RSA 解密running everyone partner"
fileprivate let show_creationStr:[Character] = ["\u{5931}","\u{8d25}"]

/*: "fail to unzip script file" :*/
fileprivate let mainRegulateKey:String = "fyesl"
fileprivate let k_warnConsumeMsg:String = "ya restriction purchase outernzip s"
fileprivate let constRemarkResponseString:String = "count discover create sibling keept file"

/*: "gift" :*/
fileprivate let notiSayString:[Character] = ["g","i","f","t"]

/*: "%@%@effect_%ld" :*/
fileprivate let userDangerousOperationString:String = "%@%@efthrough progress quick intensity rebuild"
fileprivate let userLargeName:String = "bloc"

/*: "%@%@effect_%ld_unzipTest/" :*/
fileprivate let app_forceKey:[Character] = ["%","@","%","@","e","f","f","e","c","t","_","%"]
fileprivate let data_enjoyData:[Character] = ["l","d","_","u","n","z","i","p"]
fileprivate let show_remainingString:String = "Test/disappear used owner certain"

/*: "%@%@effect_%ld_unzip/" :*/
fileprivate let dataFireString:String = "gender@gender@"
fileprivate let dataClearUrl:String = "_%ld_feedback explicit"
fileprivate let show_tooSuccessfullyName:String = "yellow"
...
typealias CompletionBlock = (_ success: Bool, _ effectModel: ValueModel?) -> Void

//: typealias ErrorBlock = (_ error: NSError?) -> Void
typealias ErrorBlock = (_ error: NSError?) -> Void

//: class TalkingGiftAnimatTool: NSObject {
class WorkDocument: NSObject {
    //: let CacheKey  = "CacheGiftEffectCacheKey"
    let CacheKey = (String(appWithinPath) + String(showSituationString))

    //: @objc static let shared = TalkingGiftAnimatTool()
    @objc static let shared = WorkDocument()
    ...
    func ignoreFind(effectMsgModel: ExamineModel, completion: @escaping CompletionBlock) {
        //: if checkNewVersionWithAnimatMsgModel(model: effectMsgModel) {
        if independentOn(model: effectMsgModel) {
            //: printLog(message: "下载动画文件")
            printLog(message: (String(mainTitleureFormValue)))
            //: downloadAnimat(effectMsgModel: effectMsgModel) { [weak self] error in
            theOrVia(effectMsgModel: effectMsgModel) { [weak self] error in
                ...
                self.sensor(effectMsgModel: effectMsgModel, complete: completion)
            }
            //: } else {
        } else {
            //: printLog(message: "加载本地动画文件")
            printLog(message: (k_informationMessage.capitalized + constMedalStr.capitalized))
            //: self.loadLocalAnimatConfig(effectMsgModel: effectMsgModel, complete: completion)
            self.sensor(effectMsgModel: effectMsgModel, complete: completion)
        }
    }
    ...
    func emptyDuring(fileName: String, model: ExamineModel) -> String {
        //: let scriptDirectory = fetchScriptDirectory(animatMsgModel: model)
        let scriptDirectory = familyTask(animatMsgModel: model)
    ...
    func volumeInfo() {
        ...
        minimizeCon(path: doSucceedEach())
    }

    // 动效资源本地的缓存目录路径
    //: func fetchScriptDirectory(animatMsgModel: TalkingAnimatMsgModel) -> String {
    func familyTask(animatMsgModel: ExamineModel) -> String {
        //: if animatMsgModel.eid == 0 {
        if animatMsgModel.eid == 0 {
            //: return String.init(format: "%@/%@/", giftEffectDirectory(), animatMsgModel.effectUrl.md5()!)
            return String(format: "%@/%@/", doSucceedEach(), animatMsgModel.effectUrl.md5()!)
            //: } else {
        } else {
            //: return String.init(format: "%@/%zd/", giftEffectDirectory(), animatMsgModel.eid)
            return String(format: "%@/%zd/", doSucceedEach(), animatMsgModel.eid)
        }
    }

    //: func giftEffectDirectory() -> String {
    func doSucceedEach() -> String {
        //: let subPath = "Cache_GiftEffect/"
        let subPath = (String(data_reserveWeekTitle.suffix(4)) + "e_Gif" + String(noti_allDownStr.suffix(8)))
    ...
    func stack(filePath: String) -> String {
    ...
    func service(model: ExamineModel) -> Bool {
        //: return !checkNewVersionWithAnimatMsgModel(model: model)
        return !independentOn(model: model)
    }

    //: func getNewAnimatClassName(oldName: String) -> String {
    func conversationOrScale(oldName: String) -> String {
        //: var newName = oldName
        var newName = oldName
        //: if newName.hasPrefix("GJSGVA") {
        if newName.hasPrefix((String(k_finishTitle))) {
            //: newName = String(describing: TalkingSGVAAnimationView.self)
            newName = String(describing: BesideLocateFactorView.self)
            //: } else if newName.hasPrefix("GJVap") {
        } else if newName.hasPrefix((String(dataIconValue.suffix(5)))) {
            //: newName = String(describing: TalkingVapAnimationView.self)
            newName = String(describing: PanView.self)
        }
        //: return newName
        return newName
    }
...
extension WorkDocument {
    //: func loadLocalAnimatConfig(effectMsgModel: TalkingAnimatMsgModel, complete: @escaping CompletionBlock) {
    func sensor(effectMsgModel: ExamineModel, complete: @escaping CompletionBlock) {
        //: let scriptDirectory = fetchScriptDirectory(animatMsgModel: effectMsgModel)
        let scriptDirectory = familyTask(animatMsgModel: effectMsgModel)
        //: let scriptPath = (scriptDirectory as NSString).appendingPathComponent("config.json")
        let scriptPath = (scriptDirectory as NSString).appendingPathComponent((String(constDatePossibleFormat)))
        ...
        let dic = NSDictionary.exerciseDoing(withJsonString: configString!) ?? NSDictionary()
        //: let giftAmodel = TalkingGiftAnimatModel.init()
        let giftAmodel = ValueModel()
        //: let effectModel = giftAmodel.setAnimatModek(dic: dic)
        let effectModel = giftAmodel.delayInsideMiss(dic: dic)
        ...
        className = conversationOrScale(oldName: className)
    ...
    func independentOn(model: ExamineModel) -> Bool {
        //: let giftPlist = UserDefaults.standard.object(forKey: CacheKey) as? Dictionary<String, Any> ?? Dictionary<String, Any>.init()
        let giftPlist = UserDefaults.standard.object(forKey: CacheKey) as? [String: Any] ?? Dictionary<String, Any>.init()
    ...
    func theOrVia(effectMsgModel: ExamineModel, errorBlock: @escaping ErrorBlock) {
        ...
        VariableCombine.modelSquare(giftUrl: effectMsgModel.effectUrl, path: triggerSegment(model: effectMsgModel)) { [weak self] t in
            //: guard let self = self else {return}
            guard let self = self else { return }
            //: DispatchQueue.global().async {
            DispatchQueue.global().async {
                //: self.afterDownloadGiftAnimat(effectMsgModel: effectMsgModel, isSucess: t, errorBlock: errorBlock)
                self.despite(effectMsgModel: effectMsgModel, isSucess: t, errorBlock: errorBlock)
            }
        }
    }

    //: func afterDownloadGiftAnimat(effectMsgModel: TalkingAnimatMsgModel, isSucess: Bool, errorBlock: @escaping ErrorBlock) {
    func despite(effectMsgModel: ExamineModel, isSucess: Bool, errorBlock: @escaping ErrorBlock) {
        //: if !isSucess {
        if !isSucess {
            //: errorBlock(NSError.init(domain: "app.\(ReplaceUrlDomain).com", code: GiftAnimatUpdateError.DownLoadFailed.rawValue))
            errorBlock(NSError(domain: (String(constAngleFormat)) + "\(dataServiceName)" + (String(show_reflectTitle.suffix(4))), code: RoundingChannel.DownLoadFailed.rawValue))
            //: return
            return
            ...
            let downloadPath = triggerSegment(model: effectMsgModel)
            //: let unzipVerifyDir = unzipVerifyDirectory(model: effectMsgModel)
            let unzipVerifyDir = drawerWithinError(model: effectMsgModel)
            ...
            if verifyUnzipSucc {
                ...
                for filename in unzippedFiles {
                    //: if filename == "key" {
                    if filename == (main_businessUrl.replacingOccurrences(of: "continuing", with: "ey")) {
                        // encrypted md5 file
                        //: keyFilePath = (unzipVerifyDir as NSString).appendingPathComponent(filename)
                        keyFilePath = (unzipVerifyDir as NSString).appendingPathComponent(filename)
                        //: } else if (filename as NSString).pathExtension == "zip" {
                    } else if (filename as NSString).pathExtension == (data_occasionString.lowercased()) {
                        // script file
                ...
                printLog(message: (String(data_invitationHoneyTitle.prefix(5)) + " to " + noti_foundPath.replacingOccurrences(of: "combine", with: "p") + String(show_phaseStr.suffix(5))))
                //: isFailed = true
                isFailed = true
                //: errorBlock(NSError.init(domain: "app.\(ReplaceUrlDomain).com", code: GiftAnimatUpdateError.UnzipFailed.rawValue))
                errorBlock(NSError(domain: (String(constAngleFormat)) + "\(dataServiceName)" + (String(show_reflectTitle.suffix(4))), code: RoundingChannel.UnzipFailed.rawValue))
            }
            ...
            if !isFailed {
                ...
                do {
                    ...
                    let md5Data: Data = ToolMeasure.rebuildOrMendOccasion(NSData(contentsOfFile: keyFilePath) as Data?, head: show_marginName)
                    ...
                    let md5 = stack(filePath: scriptZipFilePath)
                    ...
                    if decryptMD5 != md5, FileManager.default.fileExists(atPath: keyFilePath) {
                        ...
                        errorBlock(NSError(domain: (String(constAngleFormat)) + "\(dataServiceName)" + (String(show_reflectTitle.suffix(4))), code: RoundingChannel.VerifyFailed.rawValue))
                    }
                    ...
                    printLog(message: (String(dataStageMessage.prefix(6)) + String(show_creationStr)))
                    //: isFailed = true
                    isFailed = true
                    //: errorBlock(NSError.init(domain: "app.\(ReplaceUrlDomain).com", code: GiftAnimatUpdateError.VerifyFailed.rawValue))
                    errorBlock(NSError(domain: (String(constAngleFormat)) + "\(dataServiceName)" + (String(show_reflectTitle.suffix(4))), code: RoundingChannel.VerifyFailed.rawValue))
            ...
            if !isFailed {
                ...
                let unzipTmpDir = calledDown(model: effectMsgModel)
                ...
                //: else {
                    ...
                    printLog(message: (mainRegulateKey.replacingOccurrences(of: "yes", with: "ai") + " to u" + String(k_warnConsumeMsg.suffix(6)) + "crip" + String(constRemarkResponseString.suffix(6))))
                    //: isFailed = true
                    isFailed = true
                    //: errorBlock(NSError.init(domain: "app.\(ReplaceUrlDomain).com", code: GiftAnimatUpdateError.UnzipFailed.rawValue))
                    errorBlock(NSError(domain: (String(constAngleFormat)) + "\(dataServiceName)" + (String(show_reflectTitle.suffix(4))), code: RoundingChannel.UnzipFailed.rawValue))
    ...
    func minimizeCon(path: String) -> Bool {
    ...
    func saveer(type: ExecuteFilter) -> String {
        //: var tmpIdentify = ""
        var tmpIdentify = ""
        //: switch type {
        switch type {
        //: case .Gift:     tmpIdentify =  "gift"
        case .Gift: tmpIdentify = (String(notiSayString))
        //: break
        //: default:
        default:
            //: break
    ...
    func triggerSegment(model: ExamineModel) -> String {
        ...
        let downloadTmpPath = String(format: (String(userDangerousOperationString.prefix(6)) + "fect_%l" + userLargeName.replacingOccurrences(of: "bloc", with: "d")), NSTemporaryDirectory(), saveer(type: model.effectType), model.eid)
        //: return downloadTmpPath
        return downloadTmpPath
    }

    //: func unzipVerifyDirectory(model: TalkingAnimatMsgModel)->String {
    func drawerWithinError(model: ExamineModel) -> String {
        ...
        let unzipVerifyDirectory = String(format: (String(app_forceKey) + String(data_enjoyData) + String(show_remainingString.prefix(5))), NSTemporaryDirectory(), saveer(type: model.effectType), model.eid)
        //: return unzipVerifyDirectory
        return unzipVerifyDirectory
    }

    //: func unzipTmpDirectory(model: TalkingAnimatMsgModel)->String {
    func calledDown(model: ExamineModel) -> String {
        ...
        let unzipTmpDirectory = String(format: (dataFireString.replacingOccurrences(of: "gender", with: "%") + "effect" + String(dataClearUrl.prefix(5)) + "unzip" + show_tooSuccessfullyName.replacingOccurrences(of: "yellow", with: "/")), NSTemporaryDirectory(), saveer(type: model.effectType), model.eid)
