...
fileprivate let app_versusDistinctionFormat:[Character] = ["i","c","o","n","_","k","o","n","g","_"]
fileprivate let notiSeekKey:String = "kong_pose blue self"

/*: "page" :*/
fileprivate let appAlwaysMsg:[UInt8] = [0xe,0xff,0x5,0x3]

fileprivate func itEh(problem num: UInt8) -> UInt8 {
...
/*: "uid" :*/
fileprivate let user_mightName:[UInt8] = [0x64,0x69,0x75]

/*: "list" :*/
fileprivate let const_efficiencyName:String = "limechanism"
...
// __CLOSE_PRINT__
//
//  ListenReloadViewController.swift
//  AbroadTalking
//
...
class ListenReloadViewController: PetViewController {
    //: private var pageIndex = 0
    private var pageIndex = 0
    //: private var postDataArr = [TalkingUserInfoMomentItemModel]()
    private var postDataArr = [AroundEvaluate]()
    ...
    private lazy var collectionView: UICollectionView = {
        ...
        collectionView.contentInset = UIEdgeInsets(top: 0, left: 0, bottom: user_workFormat, right: 0)
        //: collectionView.register(TalkingPostItemCell.self, forCellWithReuseIdentifier: TalkingPostItemCell.className())
        collectionView.register(ScaleView.self, forCellWithReuseIdentifier: ScaleView.className())
    ...
    private lazy var emptyView: UIView = {
        ...
        img.image = UIImage.evolutionary(name: (String(app_versusDistinctionFormat) + String(notiSeekKey.prefix(5)) + "default"))
...
extension ListenReloadViewController {
    //: private func headerRefresh() {
    private func command() {
        //: pageIndex = 0
        pageIndex = 0
        //: load_listData()
        pressureSuccessfully()
    }

    //: private func footerRefresh() {
    private func byPresentation() {
        //: pageIndex += 1
        pageIndex += 1
        //: load_listData()
        pressureSuccessfully()
    ...
    private func pressureSuccessfully() {
        //: let params = ["page": pageIndex, "uid": userInfoModel.uid] as [String: Any]
        let params = [String(bytes: appAlwaysMsg.map{itEh(problem: $0)}, encoding: .utf8)!: pageIndex, String(bytes: user_mightName.reversed(), encoding: .utf8)!: userInfoModel.uid] as [String: Any]
        //: TalkingMomentRequestTool.req_UserMomentList(params: params) { [weak self] succeed, result, errorModel in
        SmallFinishTemp.independentMovement(params: params) { [weak self] succeed, result, errorModel in
            //: guard let self = self else { return }
            guard let self = self else { return }
            ...
            if let resultArr = resultDict[(const_efficiencyName.replacingOccurrences(of: "mechanism", with: "st"))] as? [Any] {
                ...
                for dict in resultArr {
                    ...
                    if let model = FrequencyModel.deserialize(from: dict as? Dictionary) {
...
extension ListenReloadViewController: UICollectionViewDelegate, UICollectionViewDataSource, UICollectionViewDelegateFlowLayout {
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        ...
        let cell = collectionView.dequeueReusableCell(withReuseIdentifier: ScaleView.className(), for: indexPath) as! ScaleView
        //: if indexPath.row < self.postDataArr.count {
        if indexPath.row < self.postDataArr.count {
            //: let model = self.postDataArr[indexPath.row]
            let model = self.postDataArr[indexPath.row]
            //: cell.refreshItem(model)
            cell.permissionThroughResource(model)
        }
    ...
    func collectionView(_ collectionView: UICollectionView, didSelectItemAt indexPath: IndexPath) {
        ...
        let vc = BelowReplace(model: detailModel)
...
extension ListenReloadViewController: JXPagingViewListViewDelegate {
...
extension ListenReloadViewController {
    ...
    func go(_ userModel: EstimatedCharacteristic) {
