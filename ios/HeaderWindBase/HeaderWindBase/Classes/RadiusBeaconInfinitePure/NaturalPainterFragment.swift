...
fileprivate let show_interactionValue:[UInt8] = [0x14,0x13,0x14,0x9,0x55,0x1e,0x12,0x19,0x18,0xf,0x47,0x54,0x5d,0x15,0x1c,0xe,0x5d,0x13,0x12,0x9,0x5d,0x1f,0x18,0x18,0x13,0x5d,0x14,0x10,0xd,0x11,0x18,0x10,0x18,0x13,0x9,0x18,0x19]

private func expressionMustAll(infrastructure num: UInt8) -> UInt8 {
    return num ^ 125
}
...
class NaturalPainterFragment: TransformView {
    // 播放结束Block
    ...
    required init?(coder: NSCoder) {
        ...
        fatalError(String(bytes: show_interactionValue.map{expressionMustAll(infrastructure: $0)}, encoding: .utf8)!)
    }
...
extension NaturalPainterFragment {
    /// 开始播放直播
    //: func setupPlayer(liveUrl: String) {
    func checkedCommit(liveUrl: String) {
...
extension NaturalPainterFragment: TXLivePlayListener {
