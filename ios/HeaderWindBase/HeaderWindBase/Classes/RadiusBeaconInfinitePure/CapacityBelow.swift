...
/*: "tab" :*/
fileprivate let notiExtentId:[Character] = ["t","a","b"]

/*: "page" :*/
fileprivate let dataCancelString:String = "psearchge"

/*: "uid" :*/
fileprivate let mainReasonLicenseUrl:[Character] = ["u","i","d"]

/*: "model" :*/
fileprivate let user_threadMessage:String = "mwaistdel"

/*: "There's no feed yet." :*/
fileprivate let const_lackString:[Character] = ["T","h","e","r","e","\'","s"," ","n","o"," ","f","e","e","d"," ","y","e","t","."]
...
class CapacityBelow: UIViewController {
    //: var pageIndex = 0
    var pageIndex = 0
    ...
    lazy var mainTableView: UITableView = {
        ...
        table.contentInset = UIEdgeInsets(top: 0, left: 0, bottom: appRetainName, right: 0)
        //: return table
        return table
...
extension CapacityBelow {
    //: func reqData() {
    func appear() {
        //: var dict = Dictionary<String, Any>()
        var dict = [String: Any]()
        //: dict["tab"] = 1
        dict[(String(notiExtentId))] = 1
        //: dict["page"] = pageIndex
        dict[(dataCancelString.replacingOccurrences(of: "search", with: "a"))] = pageIndex
        //: TalkingMomentRequestTool.req_MomentList(params: dict) { succeed, result, errorModel in
        SmallFinishTemp.contrast(params: dict) { succeed, result, errorModel in
...
extension CapacityBelow {
    ...
    @objc func slow(notification: NSNotification) {
        //: let userinfo = notification.userInfo as![String: AnyObject]
        let userinfo = notification.userInfo as! [String: AnyObject]
        //: let uid: String = userinfo["uid"] as! String
        let uid: String = userinfo[(String(mainReasonLicenseUrl))] as! String
    ...
    @objc func density(notification: NSNotification) {
        //: let userinfo = notification.userInfo as![String: AnyObject]
        let userinfo = notification.userInfo as! [String: AnyObject]
        //: let model: TalkingMomentModel = userinfo["model"] as! TalkingMomentModel
        let model: FrequencyModel = userinfo[(user_threadMessage.replacingOccurrences(of: "waist", with: "o"))] as! FrequencyModel
        //: for i in 0..<self.dataSourceArr.count {
        for i in 0 ..< self.dataSourceArr.count {
    ...
    @objc func recommend(notification: NSNotification) {
        //: let userinfo = notification.userInfo as![String: AnyObject]
        let userinfo = notification.userInfo as! [String: AnyObject]
        //: let model: TalkingMomentModel = userinfo["model"] as! TalkingMomentModel
        let model: FrequencyModel = userinfo[(user_threadMessage.replacingOccurrences(of: "waist", with: "o"))] as! FrequencyModel
...
extension CapacityBelow: UITableViewDelegate, UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        let identifier = BoxView.className()
        //: var cell = tableView.dequeueReusableCell(withIdentifier: identifier) as? TalkingMomentItemCell
        var cell = tableView.dequeueReusableCell(withIdentifier: identifier) as? BoxView
        //: if cell == nil {
        if cell == nil {
            //: cell = TalkingMomentItemCell(style: .default, reuseIdentifier: identifier)
            cell = BoxView(style: .default, reuseIdentifier: identifier)
        }
    ...
    func tableView(_ tableView: UITableView, willDisplay cell: UITableViewCell, forRowAt indexPath: IndexPath) {
        ...
        if self.dataSourceArr.count > indexPath.row {
            ...
            if indexPath.row == 0 && self.isfirstvideoCell == true {
                ...
                if model.momentType == ManagerSavePressure.Video.rawValue {
                    //: guard let tempcell: TalkingMomentItemCell = cell as? TalkingMomentItemCell  else {
                    guard let tempcell: BoxView = cell as? BoxView else {
                        //: return
                        return
                    }
                    //: tempcell.playVideo()
...
extension CapacityBelow: UIScrollViewDelegate {
    //: func scrollViewDidEndDecelerating(_ scrollView: UIScrollView) {
    func scrollViewDidEndDecelerating(_ scrollView: UIScrollView) {
        //: scrollViewEndScrollPlayer(scrollView: scrollView)
        hard(scrollView: scrollView)
    }
    ...
    func hard(scrollView: UIScrollView) {
        ...
        if let middleIndexPath = mainTableView.indexPathForRow(at: CGPoint(x: 0, y: scrollView.contentOffset.y + mainTableView.frame.size.height / 2)) {
            ...
            if dataSourceArr.count > middleIndexPath.row {
                ...
                let cell: BoxView = mainTableView.cellForRow(at: middleIndexPath) as! BoxView
...
extension CapacityBelow {
    ...
    private func theme() {
        ...
        style.TipsTitle = (String(const_lackString)).localized
    ...
    private func suspend() {
        ...
        NotificationCenter.default.addObserver(self, selector: #selector(density(notification:)), name: main_oldEffectName, object: nil)
        ...
        NotificationCenter.default.addObserver(self, selector: #selector(slow(notification:)), name: showAlongFindPath, object: nil)
        ...
        NotificationCenter.default.addObserver(self, selector: #selector(recommend(notification:)), name: app_processString, object: nil)
        ...
        NotificationCenter.default.addObserver(self, selector: #selector(disappear), name: data_smallValue, object: nil)
    }
}
