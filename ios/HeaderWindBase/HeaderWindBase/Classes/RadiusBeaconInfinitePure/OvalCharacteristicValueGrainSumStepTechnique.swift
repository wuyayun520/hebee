...
fileprivate let appBeginYourPath:[UInt8] = [0x95,0x92,0x95,0x88,0xd4,0x9f,0x93,0x98,0x99,0x8e,0xc6,0xd5,0xdc,0x94,0x9d,0x8f,0xdc,0x92,0x93,0x88,0xdc,0x9e,0x99,0x99,0x92,0xdc,0x95,0x91,0x8c,0x90,0x99,0x91,0x99,0x92,0x88,0x99,0x98]

private func pointTeam(month num: UInt8) -> UInt8 {
    return num ^ 252
}

/*: "[未知消息]" :*/
fileprivate let main_errorId:String = "[未知消paint]"

/*: "extra" :*/
fileprivate let appHisMessage:[Character] = ["e","x","t","r","a"]

/*: "#0084FF" :*/
fileprivate let notiSubjectFormat:String = "#0084FFthree today break stop standard"

/*: "#FFEE4B" :*/
fileprivate let noti_deadlineSimilarStr:String = "general i listen#FFEE4B"

/*: "<at>@[\\S\\s]+?</at>" :*/
fileprivate let mainToleranceMsg:String = "<at>@stroke literal"
fileprivate let userPublishKey:[Character] = ["[","\\","S","\\","s","]","+","?","<","/","a","t",">"]

/*: "jumpKey" :*/
fileprivate let userNumbLocationTitle:String = "to go alongjumpKey"

/*: "user" :*/
fileprivate let main_persistSampleKey:String = "usepresent"

/*: "uid" :*/
fileprivate let user_hourMsg:[UInt8] = [0x24,0x38,0x35]

/*: "#16D073" :*/
fileprivate let showWeightGapValue:[Character] = ["#","1","6","D","0"]
fileprivate let app_transformInsteadActualTitle:String = "7mix"

/*: "#F95644" :*/
fileprivate let const_paraValue:String = "#"
fileprivate let notiBlankName:String = "center95644"
...
// __CLOSE_PRINT__
//
//  OvalCharacteristicValueGrainSumStepTechnique.swift
//  AbroadTalking
//
...
//: @objcMembers public class OvalCharacteristicValueGrainSumStepTechnique: TalkingChatMsgBaseCellData {
@objcMembers public class OvalCharacteristicValueGrainSumStepTechnique: AgileGate {
    //: var textColor: UIColor?
    var textColor: UIColor?
    //: let textFont = UIFont.pingfangRugularFont(fontSize: 16)
    let textFont = UIFont.observeFor(fontSize: 16)
    ...
    override public init(direction: PhaseStep) {
        ...
        if direction == .MsgDirectionIncoming {
            ...
            textColor = UIColor.labelMomentum()
            //: }else {
        } else {
    ...
    required init?(coder: NSCoder) {
        ...
        fatalError(String(bytes: appBeginYourPath.map{pointTeam(month: $0)}, encoding: .utf8)!)
    }
    ...
    override public var msgModel: PadModel {
        ...
        set {
            ...
            if newValue.msgInfo.content.isEmptyString {
                //: contentStr = "[未知消息]"
                contentStr = (main_errorId.replacingOccurrences(of: "paint", with: "息"))
                //: }else {
            } else {
    ...
    override public func enableSmart(_ width: CGFloat) -> CGFloat {
        ...
        let containerSize = self.reduceQuick() // 实际气泡的高度
    ...
    public lazy var contentAttrString: NSMutableAttributedString = {
        ...
        let attr = maximumAspect(msgInfoModel: self.msgModel.msgInfo)
        //: return attr
        return attr
        //: }()
    }()
    ...
    func maximumAspect(msgInfoModel: SignRegulateModel) -> NSMutableAttributedString {
        ...
        let dic = NextGraphic.showHero(data: self.innerMessage.customElem.data)
        //: guard dic["extra"] is Dictionary<String, Any> else {
        guard dic[(String(appHisMessage))] is [String: Any] else {
            //: return NSMutableAttributedString.init(string: "")
            return NSMutableAttributedString(string: "")
        }
        ...
        let textColor = (self.direction == .MsgDirectionIncoming) ? UIColor.labelMomentum() : .white
        //: let atColor = (self.direction == .MsgDirectionIncoming) ? UIColor(hex: "#0084FF")! : UIColor(hex: "#FFEE4B")!
        let atColor = (self.direction == .MsgDirectionIncoming) ? UIColor(hex: (String(notiSubjectFormat.prefix(7))))! : UIColor(hex: (String(noti_deadlineSimilarStr.suffix(7))))!
        //: let font = UIFont.pingfangRugularFont(fontSize: 16)
        let font = UIFont.observeFor(fontSize: 16)
        ...
        let regex = try! NSRegularExpression(pattern: (String(mainToleranceMsg.prefix(5)) + String(userPublishKey)), options: [])
        ...
        for (index, value) in rangeArr.enumerated() {
            ...
            if jumpInfo[(String(userNumbLocationTitle.suffix(7)))].stringValue == (main_persistSampleKey.replacingOccurrences(of: "present", with: "r")) {
                //: uid = jumpInfo["uid"].intValue
                uid = jumpInfo[String(bytes: user_hourMsg.map{$0^81}, encoding: .utf8)!].intValue
            }
            //: if uid != -1 {
            if uid != -1 {
                ...
                attr.yy_setTextHighlight(range, color: atColor, backgroundColor: .clear, userInfo: [String(bytes: user_hourMsg.map{$0^81}, encoding: .utf8)!: uid]) { [weak self] containerView, text, range, rect in
    ...
    func halfBeside() -> NSMutableAttributedString {
        ...
        var color = UIColor(hex: (String(showWeightGapValue) + app_transformInsteadActualTitle.replacingOccurrences(of: "mix", with: "3")))!
        //: if self.translateStatus == .MsgTranslate_fail {
        if self.translateStatus == .MsgTranslate_fail {
            //: color = UIColor(hex: "#F95644")!
            color = UIColor(hex: (const_paraValue.capitalized + notiBlankName.replacingOccurrences(of: "center", with: "F")))!
        }
        ...
        transAttrStr.addAttributes([.paragraphStyle: paragraphStyle, .font: UIFont.observeFor(fontSize: 16), .foregroundColor: color], range: NSRange(location: 0, length: tempText.count))
        //: return transAttrStr
        return transAttrStr
    }
}
