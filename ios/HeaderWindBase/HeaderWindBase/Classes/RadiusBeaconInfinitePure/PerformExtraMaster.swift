...
/*: "call_bgm" :*/
fileprivate let data_independentString:String = "just social pet limit formatcall_b"
fileprivate let user_norTitle:[Character] = ["g","m"]

/*: "call_response_bgm" :*/
fileprivate let const_noteUpMsg:String = "caimplement"
fileprivate let const_fireString:String = "sibling mineonse_"
fileprivate let main_carrierSineString:String = "bginstruction"

/*: "invitePop_call_bgm" :*/
fileprivate let notiSearcherString:[Character] = ["i","n","v","i","t","e","P","o","p","_","c"]
fileprivate let user_exitPath:String = "all_bgmforce create others"
...
class PerformExtraMaster: NSObject {
    //: var type = CallBGMPlayerType.Call
    var type = DescriptionBind.Call
    //: var player = AVPlayer()
    var player = AVPlayer()
    //: var filePath = ""
    var filePath = ""
    ...
    class func magnitudeerest(type: DescriptionBind) -> PerformExtraMaster {
        //: let player = TalkingCallBGMPlayer()
        let player = PerformExtraMaster()
        //: player.type = type
        player.type = type
        //: player.getFilePath()
        player.purchaseSingle()
        //: return player
        return player
    }
...
extension PerformExtraMaster {
    //: func play() {
    func impressionForce() {
    ...
    private func sumOf(notify: Notification) {
    ...
    private func purchaseSingle() {
        //: var fileName = "call_bgm"
        var fileName = (String(data_independentString.suffix(6)) + String(user_norTitle))
        //: if self.type == .Response {
        if self.type == .Response {
            //: fileName = "call_response_bgm"
            fileName = (const_noteUpMsg.replacingOccurrences(of: "implement", with: "ll") + "_resp" + String(const_fireString.suffix(5)) + main_carrierSineString.replacingOccurrences(of: "instruction", with: "m"))
            //: } else if self.type == .invitePop {
        } else if self.type == .invitePop {
            //: fileName = "invitePop_call_bgm"
            fileName = (String(notiSearcherString) + String(user_exitPath.prefix(7)))
        }
        // 收到通话推送要播放mp3声音，不能放到.zip中
        //: let path = SVGAEffectTool.default.getMp3Path(name: fileName)
        let path = PhoneAccelerate.default.current(name: fileName)
        //: self.filePath = path
        self.filePath = path
    }
}
...
extension PerformExtraMaster: TXVodPlayListener {
