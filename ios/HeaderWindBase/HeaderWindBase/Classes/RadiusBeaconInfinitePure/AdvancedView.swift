...
fileprivate let mainPortraitTitle:[UInt8] = [0x76,0x71,0x76,0x6b,0x37,0x7c,0x70,0x7b,0x7a,0x6d,0x25,0x36,0x3f,0x77,0x7e,0x6c,0x3f,0x71,0x70,0x6b,0x3f,0x7d,0x7a,0x7a,0x71,0x3f,0x76,0x72,0x6f,0x73,0x7a,0x72,0x7a,0x71,0x6b,0x7a,0x7b]

private func petSame(reader num: UInt8) -> UInt8 {
    return num ^ 31
}

/*: "gift_bg_dm" :*/
fileprivate let kAssetKey:String = "giup"
fileprivate let kLiteralArcUrl:String = "t_bg_dmpending refuse very move"

/*: "#FFF4B5" :*/
fileprivate let show_mustFormat:String = "#FFF4must bottom request"
fileprivate let main_cornerName:String = "nothing"

/*: "won " :*/
fileprivate let dataYesQuickMsg:String = "won remove method mirror"

/*: " coins in " :*/
fileprivate let k_reflectReadRankFormat:[Character] = [" ","c","o","i","n","s"," ","i"]
fileprivate let const_defineFormat:String = "n speaker destroy"

/*: "just sent" :*/
fileprivate let appDetailedTitle:String = "jupeople"
fileprivate let show_wedMountName:String = "t sentinterest background currently"

/*: "#4120C1" :*/
fileprivate let kTermsScanId:String = "#4120C1foundation define choice"
...
class AdvancedView: UIView {
    ...
    private var giftModel = TimingIconSchedule()
    ...
    required init?(coder: NSCoder) {
        ...
        fatalError(String(bytes: mainPortraitTitle.map{petSame(reader: $0)}, encoding: .utf8)!)
    }
    ...
    private lazy var bgView: UIImageView = {
        ...
        v.image = UIImage.evolutionary(name: (kAssetKey.replacingOccurrences(of: "up", with: "f") + String(kLiteralArcUrl.prefix(7))))
    ...
    private lazy var icon: UIImageView = {
        ...
        v.layer.borderColor = UIColor(hex: (String(show_mustFormat.prefix(5)) + main_cornerName.replacingOccurrences(of: "nothing", with: "B5")))?.cgColor
    ...
    private lazy var nicknameLab: UILabel = {
        ...
        lab.textColor = UIColor(hex: (String(show_mustFormat.prefix(5)) + main_cornerName.replacingOccurrences(of: "nothing", with: "B5")))
...
extension AdvancedView {
    ...
    func enhanceClear(_ model: TimingIconSchedule) {
        //: giftModel = model
        giftModel = model
        //: icon.setUrlImage(urlStr: giftModel.headPic)
        icon.accessiblePending(urlStr: giftModel.headPic)
        ...
        if giftModel.broadStatus == 1 { // 中奖
            ...
            contentLab.text = (String(dataYesQuickMsg.prefix(4))).localized + "\(giftModel.totalCoin)" + (String(k_reflectReadRankFormat) + String(const_defineFormat.prefix(2))).localized + giftModel.gameName
            ...
            giftImgV.accessiblePending(urlStr: giftModel.img)
            //: giftNumLab.isHidden = false
            giftNumLab.isHidden = false
            //: giftNumLab.text = " x\(giftModel.num)"
            giftNumLab.text = " x\(giftModel.num)"
            //: let rangeText = "just sent".localized
            let rangeText = (appDetailedTitle.replacingOccurrences(of: "people", with: "s") + String(show_wedMountName.prefix(6))).localized
            //: let giftName = changeGiftName(giftmodel: giftModel)
            let giftName = elementDecision(giftmodel: giftModel)
            ...
            if let range = fullText.range(of: rangeText) {
                ...
                attributedString.addAttribute(.foregroundColor, value: UIColor(hex: (String(kTermsScanId.prefix(7))))!, range: nsRange)
            }
    ...
    private func elementDecision(giftmodel: TimingIconSchedule) -> String {
...
extension AdvancedView {
    ...
    private func couldPath() {
        //: self.frame = CGRect(x: 0, y: StatusBarNavigationBarHeight, width: ScreenWidth, height: 60)
        self.frame = CGRect(x: 0, y: mainItemFormat, width: mainCellMsg, height: 60)
