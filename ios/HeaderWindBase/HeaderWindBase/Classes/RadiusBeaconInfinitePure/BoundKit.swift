...
fileprivate let constOrangeId:[UInt8] = [0x9a,0x9f,0x9a,0xa5,0x59,0x94,0xa0,0x95,0x96,0xa3,0x6b,0x5a,0x51,0x99,0x92,0xa4,0x51,0x9f,0xa0,0xa5,0x51,0x93,0x96,0x96,0x9f,0x51,0x9a,0x9e,0xa1,0x9d,0x96,0x9e,0x96,0x9f,0xa5,0x96,0x95]

fileprivate func balanceRem(reply num: UInt8) -> UInt8 {
...
/*: "icon_chat_start" :*/
fileprivate let appIncludeString:String = "span business birthday lost screenicon_ch"
fileprivate let kAmPath:String = "AT"
fileprivate let notiExMsg:String = "_starthim note graphic"

/*: "icon_chat_lock" :*/
fileprivate let user_strokeMsg:String = "momentum ovalicon_c"
fileprivate let app_targetMessage:String = "HAT"
fileprivate let app_fatalFormat:String = "_lockcomply migration feature union compute"

/*: "icon_chat_burned" :*/
fileprivate let dataSubjectGreatString:[Character] = ["i","c","o","n","_","c","h","a","t","_","b","u","r"]
fileprivate let dataMovieBoundTitle:[Character] = ["n","e","d"]

/*: "icon_chat_burned_shan" :*/
fileprivate let show_windData:String = "missing month localicon_c"
fileprivate let k_towardAttachMsg:String = "hat_broute tween alter"
fileprivate let appBadBroadPath:String = "SHAN"

/*: "Already burned" :*/
fileprivate let data_hiScanFormat:[Character] = ["A","l","r","e","a","d","y"," ","b","u","r"]
fileprivate let noti_goTableKey:[Character] = ["n","e","d"]
...
class BoundKit: UniformSkip {
    //: var videoData: LitePage?
    var videoData: LitePage?
    ...
    required init?(coder: NSCoder) {
        ...
        fatalError(String(bytes: constOrangeId.map{balanceRem(reply: $0)}, encoding: .utf8)!)
    }
    ...
    private lazy var playImgView: UIImageView = {
        ...
        imgV.image = UIImage.evolutionary(name: (String(appIncludeString.suffix(7)) + kAmPath.lowercased() + String(notiExMsg.prefix(6))))
        //: return imgV
        return imgV
        //: }()
    }()
    ...
    private lazy var lockImgView: UIImageView = {
        ...
        imgV.image = UIImage.evolutionary(name: (String(user_strokeMsg.suffix(6)) + app_targetMessage.lowercased() + String(app_fatalFormat.prefix(5))))
        //: return imgV
        return imgV
        //: }()
    }()
    ...
    private lazy var burnedImgView: MethodButton = {
        ...
        imgV.setBackgroundImage(UIImage.evolutionary(name: (String(dataSubjectGreatString) + String(dataMovieBoundTitle))), for: .normal)
        //: imgV.setImage(UIImage.BundleImageNamed(name: "icon_chat_burned_shan"), for: .normal)
        imgV.setImage(UIImage.evolutionary(name: (String(show_windData.suffix(6)) + String(k_towardAttachMsg.prefix(5)) + "urned_" + appBadBroadPath.lowercased())), for: .normal)
        //: imgV.imageAlignment = .top
        imgV.imageAlignment = .top
        //: imgV.setTitle("Already burned".localized, for: .normal)
        imgV.setTitle((String(data_hiScanFormat) + String(noti_goTableKey)).localized, for: .normal)
        //: imgV.setTitleColor(.white, for: .normal)
        imgV.setTitleColor(.white, for: .normal)
        //: imgV.titleLabel?.font = UIFont.pingfangRugularFont(fontSize: 16)
        imgV.titleLabel?.font = UIFont.observeFor(fontSize: 16)
        //: return imgV
        return imgV
        //: }()
    }()
...
extension BoundKit {
    ...
    override func creationCharacter(with data: EventColor) {
        ...
        //: self.videoData = data as? LitePage
        self.videoData = data as? LitePage
        ...
        self.dutationLab.text = NSDate.promiseReady(totalTime: duration)
        ...
        self.dragSignificant(urlStr: videoModel.coverImg, isMosaic: imgIsMosaic)
    }
    ...
    func dragSignificant(urlStr: String, isMosaic: Bool) {
        //: guard let newData = self.videoData else { return }
        guard let newData = self.videoData else { return }
        ...
        if !coverImgPath.isEmptyString, isExit { // 加载本地
            ...
            //: if isMosaic {
            if isMosaic {
                //: self.picImgView.image = UIImage.become(in: image, lengthConcrete: 90)
                self.picImgView.image = UIImage.become(in: image, lengthConcrete: 90)
                //: } else {
            } else {
            ...
            self.picImgView.tapCounterval(urlStr: urlStr) { [weak self] image in
