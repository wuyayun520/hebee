...
fileprivate let kBehaviorStr:[UInt8] = [0x64,0x65,0x74,0x6e,0x65,0x6d,0x65,0x6c,0x70,0x6d,0x69,0x20,0x6e,0x65,0x65,0x62,0x20,0x74,0x6f,0x6e,0x20,0x73,0x61,0x68,0x20,0x29,0x3a,0x72,0x65,0x64,0x6f,0x63,0x28,0x74,0x69,0x6e,0x69]

/*: "View read receipts?" :*/
fileprivate let kMutualMenuUrl:[Character] = ["V","i","e","w"," ","r","e","a","d"]
fileprivate let k_flagName:String = "least presentation recei"

/*: "text" :*/
fileprivate let userCurrentProtectionData:String = "tfinishxt"

/*: "color" :*/
fileprivate let app_extentName:[UInt8] = [0x72,0x6f,0x6c,0x6f,0x63]

/*: "<xml>%@</xml>" :*/
fileprivate let dataBorderOriginPath:String = "array accuracy<xml>%@"
fileprivate let dataStatusId:String = "</xml>cover origin"

/*: "href" :*/
fileprivate let data_assistantUrl:String = "hrelayoff"

/*: "type" :*/
fileprivate let showNoteChildStr:[UInt8] = [0x69,0x64,0x6d,0x78]

private func coordinateNumb(shall num: UInt8) -> UInt8 {
    return num ^ 29
}

/*: "txt" :*/
fileprivate let k_seaName:String = "translatextranslate"

/*: "padding" :*/
fileprivate let notiGoldValue:[Character] = ["p","a","d","d","i","n","g"]

/*: "size" :*/
fileprivate let appImplementationStr:String = "srete"

/*: "maxFont" :*/
fileprivate let appFoundationName:[Character] = ["m","a"]
fileprivate let notiPrepareStr:String = "xFontprocessing rank point"

/*: "link" :*/
fileprivate let app_transactionName:String = "linrecommend"

/*: "PingFangSC-Regular" :*/
fileprivate let const_gemPath:[Character] = ["P","i","n","g","F","a","n","g"]
fileprivate let dataAnalysisFormat:[Character] = ["S","C","-","R","e","g","u","l","a","r"]

/*: "content" :*/
fileprivate let showLayUrl:[UInt8] = [0x37,0x3b,0x3a,0x20,0x31,0x3a,0x20]
...
@objcMembers public class RevealAway: AgileGate {
    ...
    override public func reduceQuick() -> CGSize {
        ...
        if self.msgModel.tips.content == (String(kMutualMenuUrl) + String(k_flagName.suffix(6)) + "pts?").localized {
            //: return CGSize(width: ScreenWidth, height: 30)
            return CGSize(width: mainCellMsg, height: 30)
        }
        ...
        //: let rect = self.attributedString.boundingRect(with: CGSize.init(width: StopBrush.getScreenWidth()-50, height: CGFloat(MAXFLOAT)), options: [.usesLineFragmentOrigin, .usesFontLeading], context: nil )
        let rect = self.attributedString.boundingRect(with: CGSize(width: StopBrush.randomAwake() - 50, height: CGFloat(MAXFLOAT)), options: [.usesLineFragmentOrigin, .usesFontLeading], context: nil)
    ...
    public lazy var attributedString: NSMutableAttributedString = {
        ...
        var attr = attribute(info: self.msgModel.tips)
        //: return attr
        return attr
        //: }()
    }()
...
//        let dic = NSDictionary(jsonString: extra ?? "")
        ...
        return process(tips: content)
    }

    //: func designTextFream(tips: String) -> NSMutableAttributedString {
    func process(tips: String) -> NSMutableAttributedString {
        ...
        _ = feedbackQuantityeraction(msg: tips, info: attrbutedArray)
        //: self.yyLayout = designYYTextLayout(array: attrbutedArray, displayMaxWidth: StopBrush.getScreenWidth()-50, para: para)
        self.yyLayout = launch(array: attrbutedArray, displayMaxWidth: StopBrush.randomAwake() - 50, para: para)
    ...
    func feedbackQuantityeraction(msg: String, info: NSMutableArray) -> String {
        ...
        let pure_msg = burnStatus(source: msg, attrbAM: contentAttrArray)
        //: for  textInfo in contentAttrArray {
        for textInfo in contentAttrArray {
            //: let textDic = textInfo as![String: Any]
            let textDic = textInfo as! [String: Any]
            //: let text: String = textDic["text"] as! String
            let text: String = textDic[(userCurrentProtectionData.replacingOccurrences(of: "finish", with: "e"))] as! String

            //: var textColor: UIColor?
            var textColor: UIColor?

            //: var obj = textDic["color"]
            var obj = textDic[String(bytes: app_extentName.reversed(), encoding: .utf8)!]
            //: if obj is UIColor {
            if obj is UIColor {
                //: obj = obj as? UIColor
                obj = obj as? UIColor
            ...
            contentDict.removeObjects(forKeys: [String(bytes: app_extentName.reversed(), encoding: .utf8)!, (userCurrentProtectionData.replacingOccurrences(of: "finish", with: "e"))])

            //: let attrDict = ctDictionaryForText(content: text, color: textColor)
            let attrDict = easy(content: text, color: textColor)
    ...
    func burnStatus(source: String, attrbAM: NSMutableArray) -> String {
        ...
        do {
            ...
            let document = try ONOXMLDocument(string: String(format: (String(dataBorderOriginPath.suffix(7)) + String(dataStatusId.prefix(6))), newSource), encoding: CFStringConvertEncodingToNSStringEncoding(CFStringBuiltInEncodings.UTF8.rawValue))
            //: let element = document.rootElement
            let element = document.rootElement
            //: parseXMLElement(element: element ?? ONOXMLElement(), arrM: attrbAM, attrib: ["color": UIColor.white])
            generateing(element: element ?? ONOXMLElement(), arrM: attrbAM, attrib: [String(bytes: app_extentName.reversed(), encoding: .utf8)!: UIColor.white])
            ...
            attrbAM.add([(userCurrentProtectionData.replacingOccurrences(of: "finish", with: "e")): newSource, String(bytes: app_extentName.reversed(), encoding: .utf8)!: UIColor.white])
            //: return newSource
            return newSource
        }
    }
    ...
    func generateing(element: ONOXMLElement, arrM: NSMutableArray, attrib: NSDictionary) {
        ...
        if children.count > 0 {
            ...
            if element.tag == "u" { // 下滑线
                ...
                let url: String = element.attributes[(data_assistantUrl.replacingOccurrences(of: "layoff", with: "f"))] as! String
                //: if url.count > 0 {
                if url.count > 0 {
                    //: parentInfo["href"] = url
                    parentInfo[(data_assistantUrl.replacingOccurrences(of: "layoff", with: "f"))] = url
                }
            }
            //: for  subElement in children {
            for subElement in children {
                //: parseXMLElement(element: subElement as! ONOXMLElement, arrM: arrM, attrib: parentInfo)
                generateing(element: subElement as! ONOXMLElement, arrM: arrM, attrib: parentInfo)
            }
            ...
            if text != nil {
                ...
                //: info["text"] = text
                info[(userCurrentProtectionData.replacingOccurrences(of: "finish", with: "e"))] = text
                //: arrM.add(info)
                arrM.add(info)
            }
        }
    ...
    func launch(array: NSArray, displayMaxWidth: CGFloat, para: NSParagraphStyle?) -> YYTextLayout {
        //: let attrString = designAttribStr(array: array, paragraphStyle: para)
        let attrString = formatReason(array: array, paragraphStyle: para)
    ...
    func formatReason(array: NSArray, paragraphStyle: NSParagraphStyle?) -> NSAttributedString {
        ...
        array.enumerateObjects { obj, idx, stop in
            ...
            let type = newObj[String(bytes: showNoteChildStr.map{coordinateNumb(shall: $0)}, encoding: .utf8)!] as? String
            //: var tmp = NSAttributedString.init()
            var tmp = NSAttributedString()
            //: if type  == "txt" {
            if type == (k_seaName.replacingOccurrences(of: "translate", with: "t")) {
                //: tmp = designAttribStrByTextDic(dict: newObj as NSDictionary)
                tmp = spark(dict: newObj as NSDictionary)
                //: } else if type == "padding" {
            } else if type == (String(notiGoldValue)) {
                //: let imageView = UIImageView.init()
                let imageView = UIImageView()
                //: imageView.size = (newObj["size"] as! NSValue).cgSizeValue
                imageView.size = (newObj[(appImplementationStr.replacingOccurrences(of: "ret", with: "iz"))] as! NSValue).cgSizeValue
                ...
                tmp = NSMutableAttributedString.yy_attachmentString(withContent: imageView, contentMode: .center, attachmentSize: imageView.size, alignTo: newObj[(String(appFoundationName) + String(notiPrepareStr.prefix(5)))] as! UIFont, alignment: .center)
            }
            ...
            if underline {
                ...
                if len > 0 {
                    ...
                    _ = YYTextDecoration(style: .single, width: 1.5, color: newObj[String(bytes: app_extentName.reversed(), encoding: .utf8)!] as? UIColor)
    ...
    func byConnectRandomGalleryTheme(dict: NSDictionary) -> NSAttributedString {
        //: let imageView = UIImageView.init()
        let imageView = UIImageView()
        //: imageView.size = (dict["size"]as! NSValue).cgSizeValue
        imageView.size = (dict[(appImplementationStr.replacingOccurrences(of: "ret", with: "iz"))] as! NSValue).cgSizeValue
        ...
        let attachText = NSMutableAttributedString.yy_attachmentString(withContent: imageView, contentMode: .center, attachmentSize: imageView.size, alignTo: dict[(String(appFoundationName) + String(notiPrepareStr.prefix(5)))] as! UIFont, alignment: .center)
        //: return attachText
        return attachText
    }

    //: func designAttribStrByTextDic(dict: NSDictionary) ->NSMutableAttributedString {
    func spark(dict: NSDictionary) -> NSMutableAttributedString {
        //: let type = dict["type"] as! String
        let type = dict[String(bytes: showNoteChildStr.map{coordinateNumb(shall: $0)}, encoding: .utf8)!] as! String
        //: if type != "link" && type != "txt" {
        if type != (app_transactionName.replacingOccurrences(of: "recommend", with: "k")) && type != (k_seaName.replacingOccurrences(of: "translate", with: "t")) {
            //: return NSMutableAttributedString.init()
            return NSMutableAttributedString()
        }
        //: let attributes = attributesWithConfig()
        let attributes = aInRemark()
        // set color
        //: let color = dict["color"] as? UIColor
        let color = dict[String(bytes: app_extentName.reversed(), encoding: .utf8)!] as? UIColor
        //: if color != nil {
        if color != nil {
        ...
        let fontSize = (dict[(appImplementationStr.replacingOccurrences(of: "ret", with: "iz"))] as? NSNumber)?.floatValue
        //: if (fontSize ?? 0 > 0) {
        if fontSize ?? 0 > 0 {
            //: let fontRef = CTFontCreateWithName("PingFangSC-Regular" as CFString, CGFloat(fontSize!), nil)
            let fontRef = CTFontCreateWithName((String(const_gemPath) + String(dataAnalysisFormat)) as CFString, CGFloat(fontSize!), nil)
            //: attributes[kCTFontAttributeName] = fontRef
            attributes[kCTFontAttributeName] = fontRef
        }
        //: var content = (dict["content"] != nil) ? dict["content"] : ""
        var content = (dict[String(bytes: showLayUrl.map{$0^84}, encoding: .utf8)!] != nil) ? dict[String(bytes: showLayUrl.map{$0^84}, encoding: .utf8)!] : ""
    ...
    func aInRemark() -> NSMutableDictionary {
        //: let font = UIFont.pingfangRugularFont(fontSize: 15)
        let font = UIFont.observeFor(fontSize: 15)
    ...
    func easy(content: String, color: UIColor?) -> NSDictionary {
        //: if content.count <= 0 {
        if content.count <= 0 {
        ...
        if color == nil {
            ...
            return evaluation(content: content)
        }
        //: return ["type": "txt",
        return [String(bytes: showNoteChildStr.map{coordinateNumb(shall: $0)}, encoding: .utf8)!: (k_seaName.replacingOccurrences(of: "translate", with: "t")),
                //: "content": content,
                String(bytes: showLayUrl.map{$0^84}, encoding: .utf8)!: content,
                //: "color": color ?? .black]
                String(bytes: app_extentName.reversed(), encoding: .utf8)!: color ?? .black]
    }

    //: func ctDictionaryForText(content: String) ->NSDictionary {
    func evaluation(content: String) -> NSDictionary {
        //: if content.count <= 0 {
        if content.count <= 0 {
            //: return NSDictionary.init()
            return NSDictionary()
