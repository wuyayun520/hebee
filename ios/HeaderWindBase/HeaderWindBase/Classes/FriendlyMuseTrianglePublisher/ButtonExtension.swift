...
/*: "recharge" :*/
fileprivate let notiRecognizeValue:String = "reequivalent"
fileprivate let user_tradeInputSmartKey:[Character] = ["h","a","r","g","e"]

/*: "recharge_ :*/
fileprivate let constValidUrl:[Character] = ["r","e","c"]
fileprivate let mainFormalId:String = "harge_middle state peer clearly"

/*: ".png" :*/
fileprivate let show_weString:[Character] = [".","p","n","g"]

/*: .png" :*/
fileprivate let user_divideMsg:[Character] = [".","p","n","g"]

/*: ".jpg" :*/
fileprivate let dataEventTitle:String = ".jpgyour outer communicate daily"

/*: .jpg" :*/
fileprivate let app_naturalName:String = ".jpgdynamic evolution"
...
class MethodButton: UIButton {
    ...
    var imageAlignment: RemoveSpace = .left
...
extension UIButton {
    ...
    func under(_ interval: CGFloat) {
        //: verticalAlignImageAndTitle(interval: interval, topOffset: 0)
        cookieTo(interval: interval, topOffset: 0)
    }
    ...
    func cookieTo(interval: CGFloat, topOffset: CGFloat) {
    ...
    func enterTension(interval: CGFloat, imageSize: CGSize, topOffset: CGFloat) {
...
extension UIButton {
    ...
    func crop(color: UIColor, forState: UIControl.State) {
    ...
    func lengthEnableto(urlStr: String,
                        //: placeImg: UIImage? = UIImage.placeImgSquare(),
                        placeImg: UIImage? = UIImage.smartThreshold(),
                        ...
                        loadFinish: ((_ isSucceed: Bool) -> Void)? = nil)
    {
        ...
        let imgUrl = urlStr.replacingOccurrences(of: appProvideSectionPath.mentionAccount(), with: dataServiceName)
    ...
    func pendingRemote(urlStr: String) {
        //: guard !urlStr.isEmptyString else { return }
        guard !urlStr.isEmptyString else { return }
        //: var newUrl = ""
        var newUrl = ""
        //: if urlStr.contains("recharge") {
        if urlStr.contains((notiRecognizeValue.replacingOccurrences(of: "equivalent", with: "c") + String(user_tradeInputSmartKey))) {
            //: newUrl = urlStr.replacingOccurrences(of: "recharge", with: "recharge_\(PackageID)")
            newUrl = urlStr.replacingOccurrences(of: (notiRecognizeValue.replacingOccurrences(of: "equivalent", with: "c") + String(user_tradeInputSmartKey)), with: (String(constValidUrl) + String(mainFormalId.prefix(6))) + "\(data_scientificComposeFormat)")
            //: self.setUrlImage(urlStr: newUrl, placeImg: nil)
            self.lengthEnableto(urlStr: newUrl, placeImg: nil)
            //: return
            return
                //: } else if urlStr.contains(".png") {
        } else if urlStr.contains((String(show_weString))) {
            //: newUrl = urlStr.replacingOccurrences(of: ".png", with: "_\(PackageID).png")
            newUrl = urlStr.replacingOccurrences(of: (String(show_weString)), with: "_\(data_scientificComposeFormat)" + (String(user_divideMsg)))

            //: } else if urlStr.contains(".jpg") {
        } else if urlStr.contains((String(dataEventTitle.prefix(4)))) {
            //: newUrl = urlStr.replacingOccurrences(of: ".jpg", with: "_\(PackageID).jpg")
            newUrl = urlStr.replacingOccurrences(of: (String(dataEventTitle.prefix(4))), with: "_\(data_scientificComposeFormat)" + (String(app_naturalName.prefix(4))))
        }
        //: self.setUrlImage(urlStr: newUrl, placeImg: nil)
        self.lengthEnableto(urlStr: newUrl, placeImg: nil)
    }
