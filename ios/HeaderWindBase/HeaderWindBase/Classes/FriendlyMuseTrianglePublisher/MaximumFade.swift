...
fileprivate let user_entityName:[UInt8] = [0x64,0x65,0x74,0x6e,0x65,0x6d,0x65,0x6c,0x70,0x6d,0x69,0x20,0x6e,0x65,0x65,0x62,0x20,0x74,0x6f,0x6e,0x20,0x73,0x61,0x68,0x20,0x29,0x3a,0x72,0x65,0x64,0x6f,0x63,0x28,0x74,0x69,0x6e,0x69]

/*: "extra" :*/
fileprivate let notiMarginData:String = "euniformtra"

/*: "msgInfo" :*/
fileprivate let mainResultTitle:[Character] = ["m"]
fileprivate let noti_remindLaunchFormat:[Character] = ["s","g","I","n","f","o"]

/*: "icon_talk_left_voive_3" :*/
fileprivate let main_personValue:[Character] = ["i","c","o","n","_","t","a","l","k","_","l","e","f","t","_","v","o","i","v"]
fileprivate let noti_changeId:[Character] = ["e","_","3"]

/*: "icon_talk_right_voive_3" :*/
fileprivate let userSmallValue:String = "notice recordingicon_"
fileprivate let const_kindKey:[Character] = ["t","a","l","k","_","r","i","g","h","t","_","v","o","i","v","e","_","3"]

/*: "audioLength" :*/
fileprivate let noti_leaveName:String = "audioLewhy bullet property interval now"
fileprivate let showDimensionTitle:[Character] = ["n","g","t","h"]

/*: "isPlayingStatus" :*/
fileprivate let noti_situationFormat:String = "trust zz discover hideisPlay"
fileprivate let user_loseValueMessage:[Character] = ["i","n","g","S","t","a","t","u","s"]

/*: "activityShowStatus" :*/
fileprivate let notiSnapName:String = "acscenevi"
fileprivate let appWrapMsg:[Character] = ["w","S","t","a","t","u","s"]

/*: "FF506D" :*/
fileprivate let app_veryKey:[Character] = ["F","F","5","0","6","D"]

/*: "icon_talk_left_voive_1" :*/
fileprivate let main_choiceLiteId:[Character] = ["i","c","o","n","_","t","a","l","k","_","l","e","f","t","_","v","o","i","v","e","_","1"]

/*: "icon_talk_left_voive_2" :*/
fileprivate let show_intervalKey:[Character] = ["i","c","o","n","_","t","a","l","k","_","l","e","f","t","_","v"]
fileprivate let user_platFormat:String = "ta tailoive_2"

/*: "icon_talk_right_voive_1" :*/
fileprivate let k_partyProviderString:[Character] = ["i","c","o","n","_","t","a","l","k","_","r","i","g","h","t","_","v","o","i","v","e","_","1"]

/*: "icon_talk_right_voive_2" :*/
fileprivate let user_skipMsg:String = "IC"
fileprivate let user_speakerPremiumValue:[Character] = ["o","n","_","t","a","l","k","_","r","i","g","h","t","_","v","o","i","v","e","_","2"]
...
class MaximumFade: UniformSkip {
    //: var audioData: EnhanceTier?
    var audioData: EnhanceTier?
    ...
    required init?(coder aDecoder: NSCoder) {
        ...
        fatalError(String(bytes: user_entityName.reversed(), encoding: .utf8)!)
    }
    ...
    override func creationCharacter(with data: EventColor) {
        ...
        if let extra = String(data: newData.innerMessage.customElem.data, encoding: .utf8) {
            ...
            let conentDic = extraDic[(notiMarginData.replacingOccurrences(of: "uniform", with: "x"))]
            //: let msgInfo = conentDic["msgInfo"]
            let msgInfo = conentDic[(String(mainResultTitle) + String(noti_remindLaunchFormat))]
            //: let voiceModel = newData.voiceModel
            let voiceModel = newData.voiceModel

            //: let isVoiceStyle = TUIMsgChatManager.checkVoiceJsonMsg(msginfo: msgInfo.dictionaryValue)
            let isVoiceStyle = NextGraphic.scientific(msginfo: msgInfo.dictionaryValue)

            //: if newData.direction == .MsgDirectionIncoming {
            if newData.direction == .MsgDirectionIncoming {
                //: self.lenLB.textColor = UIColor.appTitleColor()
                self.lenLB.textColor = UIColor.labelMomentum()
                //: self.voiceImageV.image = UIImage.BundleImageNamed(name: "icon_talk_left_voive_3")
                self.voiceImageV.image = UIImage.evolutionary(name: (String(main_personValue) + String(noti_changeId)))
                //: self.voiceImageV.animationImages = leftVoiceAnimImagToCoverColor()
                self.voiceImageV.animationImages = minimum()
                //: } else {
            } else {
                //: self.lenLB.textColor = .white
                self.lenLB.textColor = .white
                //: self.voiceImageV.image = UIImage.BundleImageNamed(name: "icon_talk_right_voive_3")
                self.voiceImageV.image = UIImage.evolutionary(name: (String(userSmallValue.suffix(5)) + String(const_kindKey)))
                //: self.voiceImageV.animationImages = rightVoiceAnimImagToCoverColor()
                self.voiceImageV.animationImages = switchce()
            }
            //: if isVoiceStyle {
            if isVoiceStyle {
                //: self.lenLB.text = String.init(format: "%ld″", msgInfo["audioLength"].intValue)
                self.lenLB.text = String(format: "%ld″", msgInfo[(String(noti_leaveName.prefix(7)) + String(showDimensionTitle))].intValue)
                //: newData.voiceModel.audioLength = msgInfo["audioLength"].intValue
                newData.voiceModel.audioLength = msgInfo[(String(noti_leaveName.prefix(7)) + String(showDimensionTitle))].intValue

                //: voiceModel.rx.observeWeakly(Int.self, "isPlayingStatus").subscribe(onNext: { [weak self] (value) in
                voiceModel.rx.observeWeakly(Int.self, (String(noti_situationFormat.suffix(6)) + String(user_loseValueMessage))).subscribe(onNext: { [weak self] value in
                ...
                voiceModel.rx.observeWeakly(Int.self, (notiSnapName.replacingOccurrences(of: "scene", with: "ti") + "tySho" + String(appWrapMsg))).subscribe(onNext: { [weak self] value in
                    //: guard let self = self else { return }
                    guard let self = self else { return }
                    //: DispatchQueue.main.async {
                    DispatchQueue.main.async {
    ...
    lazy var voiceTrackPoint: UIImageView = {
        ...
        imag.backgroundColor = UIColor(hex: (String(app_veryKey)))
        //: imag.isHidden = true
        imag.isHidden = true
...
extension MaximumFade {
    ...
    func minimum() -> [UIImage] {
        ...
        return [UIImage.evolutionary(name: (String(main_choiceLiteId))), UIImage.evolutionary(name: (String(show_intervalKey) + String(user_platFormat.suffix(6)))), UIImage.evolutionary(name: (String(main_personValue) + String(noti_changeId)))]
    }

    //: func rightVoiceAnimImagToCoverColor() -> [UIImage] {
    func switchce() -> [UIImage] {
        ...
        return [UIImage.evolutionary(name: (String(k_partyProviderString))), UIImage.evolutionary(name: (user_skipMsg.lowercased() + String(user_speakerPremiumValue))), UIImage.evolutionary(name: (String(userSmallValue.suffix(5)) + String(const_kindKey)))]
    }
}
...
extension MaximumFade {
    ...
    override public func updateConstraints() {
        ...
        if newData.direction == .MsgDirectionIncoming {
            ...
            lenLB.textAlignment = ThanTrain.shared.direction == .rightToLeft ? .left : .right
            //: lenLB.snp.remakeConstraints { (make) in
            lenLB.snp.remakeConstraints { make in
