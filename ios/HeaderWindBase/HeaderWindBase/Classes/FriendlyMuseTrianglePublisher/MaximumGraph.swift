...
fileprivate let k_remGrainTrustTitle:[UInt8] = [0x1c,0x1b,0x1c,0x1,0x5d,0x16,0x1a,0x11,0x10,0x7,0x4f,0x5c,0x55,0x1d,0x14,0x6,0x55,0x1b,0x1a,0x1,0x55,0x17,0x10,0x10,0x1b,0x55,0x1c,0x18,0x5,0x19,0x10,0x18,0x10,0x1b,0x1,0x10,0x11]

private func givingTing(round num: UInt8) -> UInt8 {
    return num ^ 117
}

/*: "#30D00B" :*/
fileprivate let showTapDifficultFormat:[Character] = ["#","3","0","D","0","0","B"]

/*: "icon_receivedcalls" :*/
fileprivate let dataSufficientTitle:[Character] = ["i","c","o","n","_","r","e","c","e"]
fileprivate let show_sendPath:String = "ilessed"
fileprivate let notiWaistName:[Character] = ["c","a","l","l","s"]

/*: "Received calls" :*/
fileprivate let userLabelAudienceFormat:String = "Receborder the production"
fileprivate let notiAlongsideId:String = "allta"

/*: "icon_dialedcalls" :*/
fileprivate let k_lowerData:[Character] = ["i","c","o","n","_","d"]
fileprivate let dataUnknownTitle:[Character] = ["i","a","l","e","d","c","a","l","l","s"]

/*: "Dialed calls" :*/
fileprivate let appSunSubstantialString:String = "valid route reliabilityDiale"
...
class MaximumGraph: UITableViewCell {
    //: var currenModel = TalkingVideoCallRecordModel()
    var currenModel = AgainstInstruction()
    ...
    required init?(coder aDecoder: NSCoder) {
        ...
        fatalError(String(bytes: k_remGrainTrustTitle.map{givingTing(round: $0)}, encoding: .utf8)!)
    }
    ...
    lazy var iconBtn: UIButton = {
        ...
        btn.addTarget(self, action: #selector(nearSince), for: .touchUpInside)
        //: return btn
        return btn
        //: }()
    }()
    ...
    lazy var durationLB: UILabel = {
        ...
        label.font = .stepTransition(type: .Medium, fontSize: 10)
        //: label.textColor = .appValueDetailColor()
        label.textColor = .capBy()
    ...
    lazy var onlineView: UIView = {
        ...
        view.backgroundColor = UIColor(hex: (String(showTapDifficultFormat)))
...
extension MaximumGraph {
    //: func setVideoCallCell(model: TalkingVideoCallRecordModel) {
    func promptFor(model: AgainstInstruction) {
        //: currenModel = model
        currenModel = model
        ...
        if !model.headPicFrame.isEmptyString {
            ...
            iconBorderBtn.pendingRemote(urlStr: model.headPicFrame)
            //: } else {
        } else {
        ...
        if model.callStatus == 0 {
            ...
            rcordImg.image = UIImage.evolutionary(name: (String(dataSufficientTitle) + show_sendPath.replacingOccurrences(of: "less", with: "v") + String(notiWaistName)))
            //: rcordLB.text = "Received calls".localized
            rcordLB.text = (String(userLabelAudienceFormat.prefix(4)) + "ived c" + notiAlongsideId.replacingOccurrences(of: "ta", with: "s")).localized
            //: } else {
        } else {
            //: rcordImg.image = UIImage.BundleImageNamed(name: "icon_dialedcalls")
            rcordImg.image = UIImage.evolutionary(name: (String(k_lowerData) + String(dataUnknownTitle)))
            //: rcordLB.text = "Dialed calls".localized
            rcordLB.text = (String(appSunSubstantialString.suffix(5)) + "d calls").localized
        }
