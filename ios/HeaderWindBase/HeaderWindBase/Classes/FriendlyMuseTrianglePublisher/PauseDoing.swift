...
/*: "num" :*/
fileprivate let notiSquareString:String = "NUM"
...
@objc public enum CustomHiddenPet: Int {
    //: case normal  = 1
    case normal = 1 // 普通礼物
    //: case myStery = 2
    case myStery = 2 // 盲盒礼物
...
struct PauseDoing: HandyJSON {
    ...
    var animationTimes = 0
    //: var showType = 0
    var showType = 0 /// CustomHiddenPet
    ...
    mutating func pastM(giftNum: Int) {
        //: self.num = giftNum
        self.num = giftNum
    }
}
...
struct ProgressTimingInput: HandyJSON {
    //: init() {
    init() {}
    ...
    mutating func mapping(mapper: HelpingMapper) {
        ...
        mapper <<<
            //: self.giftNum <-- "num"
            self.giftNum <-- (notiSquareString.lowercased())
    }
}
