...
fileprivate let mainRefreshStr:[UInt8] = [0x7,0xc,0x7,0x12,0xc6,0x1,0xd,0x2,0x3,0x10,0xd8,0xc7,0xbe,0x6,0xff,0x11,0xbe,0xc,0xd,0x12,0xbe,0x0,0x3,0x3,0xc,0xbe,0x7,0xb,0xe,0xa,0x3,0xb,0x3,0xc,0x12,0x3,0x2]

fileprivate func activeBeat(meeting num: UInt8) -> UInt8 {
...
/*: "Post" :*/
fileprivate let k_magnitudeMsg:String = "train sessionPost"

/*: "Profile" :*/
fileprivate let dataWindowData:String = "signal farProfi"
fileprivate let const_aboveMaxMsg:[Character] = ["l","e"]

/*: "bth_me_follow_nor" :*/
fileprivate let const_projectionData:String = "bth_mexplain toward current"
fileprivate let noti_promptTermsMsg:String = "painter peoplee_fo"
fileprivate let kProperlyAgainstPath:String = "_nornumber resume"

/*: "removeAttentionUid" :*/
fileprivate let show_sufficientAboutData:String = "rpado"
fileprivate let userNatureValue:String = "skipenskipio"

/*: "attentionUid" :*/
fileprivate let appHealthyCalendarMsg:[Character] = ["a","t","t","e","n","t","i","o","n","U","i","d"]

/*: "bth_me_follow_pre" :*/
fileprivate let app_averageMessage:[UInt8] = [0x65,0x72,0x70,0x5f,0x77,0x6f,0x6c,0x6c,0x6f,0x66,0x5f,0x65,0x6d,0x5f,0x68,0x74,0x62]
...
class DrawImplementation: PetViewController {
    //: var uid: String = ""
    var uid: String = ""
    ...
    private var giftsArr = [ValueTranslate]() // 收到的礼物
    ...
    required init?(coder: NSCoder) {
        ...
        fatalError(String(bytes: mainRefreshStr.map{activeBeat(meeting: $0)}, encoding: .utf8)!)
    }
    ...
    private lazy var navView: ChannelView = { // 导航
    ...
    private lazy var headerView: YellowTool = { // banner
        //: let frame = CGRect(x: 0, y: 0, width: ScreenWidth, height: UserDetailHeaderView_H)
        let frame = CGRect(x: 0, y: 0, width: mainCellMsg, height: k_arrayTitle)
        //: let v = TalkingUserDetailHeaderView(frame: frame)
        let v = YellowTool(frame: frame)
    ...
    private lazy var bottomView: PersistLostAdjust = { // bottom
        //: let v = TalkingUserDetailBottomView(frame: .zero)
        let v = PersistLostAdjust(frame: .zero)
        //: return v
        return v
        //: }()
    }()
    ...
    private lazy var segmentedView: JXSegmentedView = {
        ...
        let segmentedView = JXSegmentedView(frame: CGRect(x: 0, y: data_visitorPath, width: mainCellMsg, height: 44))
        ...
        segmentedDataSource.titleNormalColor = UIColor.capBy()
        //: segmentedDataSource.titleSelectedColor = UIColor.appTitleColor()
        segmentedDataSource.titleSelectedColor = UIColor.labelMomentum()
        ...
        indicator.indicatorColor = UIColor.labelMomentum()
    ...
    private lazy var pagerView: JXPagingListRefreshView = {
        ...
        view.pinSectionHeaderVerticalOffset = Int(mainItemFormat)
        //: return view
        return view
        //: }()
    }()
    ...
    private lazy var titleArray: [String] = //: return ["Post".localized, "Profile".localized]
        [(String(k_magnitudeMsg.suffix(4))).localized, (String(dataWindowData.suffix(5)) + String(const_aboveMaxMsg)).localized]
    //: }()

    //: private lazy var dataVC: [TalkingBaseViewController] = {
    private lazy var dataVC: [PetViewController] = //: return [postVC, profileVC]
        [postVC, profileVC]
    //: }()

    //: private lazy var postVC: TalkingUserDetailPostViewController = {
    private lazy var postVC: ListenReloadViewController = {
        //: let vc = TalkingUserDetailPostViewController()
        let vc = ListenReloadViewController()
        //: return vc
        return vc
        //: }()
    }()
    ...
    private lazy var profileVC: QuantityStart = {
        //: let vc = TalkingUserDetailProfileViewController()
        let vc = QuantityStart()
        //: return vc
        return vc
        //: }()
    }()
    ...
    private lazy var followBtn: UIImageView = {
        ...
        img.image = UIImage.evolutionary(name: (String(const_projectionData.prefix(5)) + String(noti_promptTermsMsg.suffix(4)) + "llow" + String(kProperlyAgainstPath.prefix(4))))
...
extension DrawImplementation {
    ...
    private func rating() {
        ...
        if userInfoModel.isAttention {
            ...
            dict[(show_sufficientAboutData.replacingOccurrences(of: "pad", with: "em") + "veAt" + userNatureValue.replacingOccurrences(of: "skip", with: "t") + "nUid")] = userInfoModel.uid
            //: isattion = false
            isattion = false
            //: } else {
        } else {
            //: dict["attentionUid"] = userInfoModel.uid
            dict[(String(appHealthyCalendarMsg))] = userInfoModel.uid
        }
        //: TalkingChatRequestTool.req_atationTool(isAttention: isattion, params: dict) { succeed, result, errorModel in
        VariableCombine.slideIn(isAttention: isattion, params: dict) { succeed, result, errorModel in
    ...
    func deal() {
        ...
        queue.async(group: group) {
            ...
            PrepareReplace.remove(uid: self.uid) { succeed, result, errorModel in
                //: self.setNoremarlView()
                self.mutual()
                //: if self.uid == MountCustom.share.loginUid {
                if self.uid == MountCustom.share.loginUid {
                    //: self.bottomView.showEditBtn()
                    self.bottomView.reportByChapter()
                }
                //: if succeed == true {
                if succeed == true {
                    ...
                    if let userModel = JSONDeserializer<EstimatedCharacteristic>.deserializeFrom(dict: result as? [String: Any], designatedPath: nil) {
                        ...
                        self.navView.objectBegin(userModel)
                        //: self.headerView.refreshView(userModel)
                        self.headerView.oval(userModel)
                        //: self.bottomView.refreshView(userModel)
                        self.bottomView.chartSuccessfully(userModel)
                        //: self.postVC.refreshPostViewController(userModel)
                        self.postVC.go(userModel)
                        // 场景三：男性用户邀请通话弹窗监听
                        ...
                        (MechanismObserve.shared as! MechanismObserve).offto(uid: userModel.uid, videoUrl: userModel.videoUrl, headPic: userModel.headPic)
                    }
                }
                //: group.leave()
                group.leave()
        ...
        queue.async(group: group) {
            ...
            PrepareReplace.normAcross(uid: self.uid) { succeed, result, errorModel in
                //: if succeed == true {
                if succeed == true {
                    ...
                    for dic in result as! [[String: Any]] {
                        ...
                        if let model = ValueTranslate.deserialize(from: dic as? Dictionary) {
    ...
    private func behindSpecifyLibrary() {
        //: refreshfollowButtonStatus()
        evolution()

        // profile分区数据
        //: var profileSectionArr = [TUserDetailCellType.profileType]
        var profileSectionArr = [MultiStatus.profileType]
        ...
        self.profileVC.frequency(giftsArr, profileSectionArr, userInfoModel)
    }
}
...
extension DrawImplementation {
    ...
    private func evolution() {
        //: self.followBtn.isHidden = (userInfoModel.uid == MountCustom.share.loginUid)
        self.followBtn.isHidden = (userInfoModel.uid == MountCustom.share.loginUid)
        //: let followImgStr = self.userInfoModel.isAttention ? "bth_me_follow_pre":"bth_me_follow_nor"
        let followImgStr = self.userInfoModel.isAttention ? String(bytes: app_averageMessage.reversed(), encoding: .utf8)! : (String(const_projectionData.prefix(5)) + String(noti_promptTermsMsg.suffix(4)) + "llow" + String(kProperlyAgainstPath.prefix(4)))
        //: self.followBtn.image = UIImage.BundleImageNamed(name: followImgStr)
        self.followBtn.image = UIImage.evolutionary(name: followImgStr)
    }

    /// 展示VIP皮肤视图
    //: private func setupVIPSkinView() {
    private func matchTell() {
        //: guard userInfoModel.loungePlus else { return }
        guard userInfoModel.loungePlus else { return }
        //: guard let vipModel = MountCustom.share.appUserConfigMode.vipSetting.filter({$0.vipSkinId == userInfoModel.vipSkinId}).first else { return }
        guard let vipModel = MountCustom.share.appUserConfigMode.vipSetting.filter({ $0.vipSkinId == userInfoModel.vipSkinId }).first else { return }
        ...
        vipCenterView.accessiblePending(urlStr: vipModel.vipSkinCenter, placeImg: nil)
        //: vipBottomView.setUrlImage(urlStr: vipModel.vipSkinBottom, placeImg: nil)
        vipBottomView.accessiblePending(urlStr: vipModel.vipSkinBottom, placeImg: nil)
    }
}
...
extension DrawImplementation: JXSegmentedViewDelegate {
...
extension DrawImplementation: JXPagingViewDelegate {
...
extension DrawImplementation {
    ...
    private func mutual() {
        ...
        navView.snp.makeConstraints { make in
            ...
            make.height.equalTo(mainItemFormat)
        }
        ...
        bottomView.snp.makeConstraints { make in
            ...
            make.height.equalTo(user_workFormat)
        }
    }
    ...
    private func exception() {
        ...
        pagerView.frame = CGRect(x: 0, y: 0, width: mainCellMsg, height: kPressEasePublishString)
