...
/*: "#ffffff" :*/
fileprivate let constSoundStr:[Character] = ["#","f","f","f","f","f","f"]

/*: "/dist/loungePlus/index.html" :*/
fileprivate let mainCalculationId:[Character] = ["/","d","i","s","t","/","l","o","u","n","g","e","P","l","u","s","/","i","n","d","e","x",".","h","t","m","l"]

/*: "https://" :*/
fileprivate let show_automaticallyData:String = "keyt"
fileprivate let constExplainUrl:[Character] = ["t","p","s",":","/","/"]

/*: "http://" :*/
fileprivate let mainGraphAcceptString:String = "http:/single occasion voice panel"
fileprivate let mainWhyUrl:[Character] = ["/"]
...
class PushExcept: UIView {
    ...
    let Broadcast_TextColor = (String(constSoundStr))
    //: let Broadcast_ContentFont: UIFont = UIFont.pingfangMediumFont(fontSize: 12)
    let Broadcast_ContentFont: UIFont = .monsterColumn(fontSize: 12)
    ...
    convenience init(model: StorageCurrentModel?) {
        //: self.init()
        self.init()
    ...
    private func platformAcross() {
        ...
        if danumuMsgModel.content.count > 0 {
            ...
            var mutableAtt = parse.stepObserve(withFontSize: 12, insert: "", atIndex: 0)
            // 限制字符长度
        ...
        self.backgroundImageView.containRetain(urlStr: url) { [weak self] result, image in
            ...
            if let image {
                ...
                if image.size.height > self.Broadcast_HeightOfBackgroundImageView {
                    ...
                    newImage = image.arrayBy(with: CGSize(width: targetWidth, height: self.Broadcast_HeightOfBackgroundImageView))
                }
            }
    ...
    var danumuMsgModel: StorageCurrentModel? {
        //: didSet {
        didSet {
            //: configModel()
            platformAcross()
    ...
    var totalTime: CGFloat {
        ...
        return (mainCellMsg + widthValue) * Broadcast_SpendTime / mainCellMsg
    }

    //: var totalTimeOfLine: CGFloat {
    var totalTimeOfLine: CGFloat {
        //: return widthValue * Broadcast_LineSpendTime / ScreenWidth
        return widthValue * Broadcast_LineSpendTime / mainCellMsg
    }

    //: var hide2ShowTime: CGFloat {
    var hide2ShowTime: CGFloat {
        //: return widthValue * Broadcast_SpendTime / ScreenWidth
        return widthValue * Broadcast_SpendTime / mainCellMsg
    }
    ...
    lazy var tapGesture: UITapGestureRecognizer = {
        ...
        let t = UITapGestureRecognizer(target: self, action: #selector(soundInserting(_:)))
        //: return t
        return t
        //: }()
    }()
...
extension PushExcept {
    /// 弹幕点击跳转事件
    //: @objc func tapAction(_ tapGesture: UITapGestureRecognizer) {
    @objc func soundInserting(_ tapGesture: UITapGestureRecognizer) {
        ...
        //: case "1": // 个人中心
        case "1": // 个人中心
            //: RetainProcess.share.func__pushToUserDetailVC(uid: self.danumuMsgModel?.jumpKey)
            RetainProcess.share.videoCustom(uid: self.danumuMsgModel?.jumpKey)

        //: case "2": // 直播间
        case "2": // 直播间
            //: if let uid = self.danumuMsgModel?.jumpKey {
            if let uid = self.danumuMsgModel?.jumpKey {
                //: RetainProcess.share.func_audiencePushToLiveRoomVC(uid: uid, enterType: .unknown)
                RetainProcess.share.isSuspend(uid: uid, enterType: .unknown)
            }
        ...
        case "3": // H5页面
            ...
            if let url = self.danumuMsgModel?.jumpKey {
                ...
                if url.contains((String(mainCalculationId))) {
                    //: RetainProcess.share.func__pushToSubscribePageWebVC()
                    RetainProcess.share.userQuestion()
                    //: return
                    return
                }
                //: let httpsUrl = MountCustom.share.appConfigMode.urlH5Domain+url
                let httpsUrl = MountCustom.share.appConfigMode.urlH5Domain + url
                //: let httpUrl = httpsUrl.replacingOccurrences(of: "https://", with: "http://")
                let httpUrl = httpsUrl.replacingOccurrences(of: (show_automaticallyData.replacingOccurrences(of: "key", with: "h") + String(constExplainUrl)), with: (String(mainGraphAcceptString.prefix(6)) + String(mainWhyUrl)))
                //: RetainProcess.share.func__pushToWebVC(urlStr: httpUrl)
                RetainProcess.share.mpCurrency(urlStr: httpUrl)
            }
