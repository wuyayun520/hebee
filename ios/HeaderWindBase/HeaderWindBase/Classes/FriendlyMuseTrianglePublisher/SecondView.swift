...
fileprivate let showSimilarStr:String = "multi suspend explicit notTSys"
fileprivate let showReplyMsg:String = "sslabge"
fileprivate let data_insideMsg:String = "Cell_deploy button content other"

/*: "init(coder:) has not been implemented" :*/
fileprivate let show_fireStr:[UInt8] = [0xc3,0xc4,0xc3,0xde,0x82,0xc9,0xc5,0xce,0xcf,0xd8,0x90,0x83,0x8a,0xc2,0xcb,0xd9,0x8a,0xc4,0xc5,0xde,0x8a,0xc8,0xcf,0xcf,0xc4,0x8a,0xc3,0xc7,0xda,0xc6,0xcf,0xc7,0xcf,0xc4,0xde,0xcf,0xce]

private func bulletResult(production num: UInt8) -> UInt8 {
    return num ^ 170
}

/*: "icon_talk_notic" :*/
fileprivate let constDepthPath:[Character] = ["i","c","o","n","_","t","a"]
fileprivate let data_nextSheMessage:String = "lstructure"
fileprivate let constAboutName:String = "signature respective await camera i_notic"
...
@objc public protocol EagerWait: NSObjectProtocol {
    /**
     *  长按消息回调
     ...
     *  @param cell 委托者，消息单元
     */
    //: func onLongPressMessage(cell: SecondView)
    func securePlay(cell: SecondView)
     ...
     *  @param cell 委托者，消息单元
     */
    //: func onRetryMessage(cell: SecondView)
    func architecture(cell: SecondView)
     ...
     *  @param cell 委托者，消息单元
     */
    //: func onSelectMessage(cell: SecondView)
    func skin(cell: SecondView)
     ...
     *  @param cell 委托者，消息单元
     */
    //: func onSelectMessageAvatar(cell: SecondView)
    func commentSensor(cell: SecondView)

    /**
     *  点击消息单元中引用消息
     *  @param cell 委托者，消息单元
     */
    //: func onReplyMessage(cell: SecondView)
    func serverByConfirm(cell: SecondView)
}

//: @objcMembers public class SecondView: TCommonTableViewCell {
@objcMembers public class SecondView: BoxDescription {
    /**
     *  信息数据类。
     *  存储了该massageCell中所需的信息。包括发送者 ID，发送者头像、信息发送状态、信息气泡图标等。
     *  messageData 详细信息请参考：Section\Chat\CellData\NextAgainst.h
     */
    //: public var messageData: NextAgainst?
    public var messageData: NextAgainst?

    /// 复用cell标识符
    //: @objc public static let TSystemMessageCell_ReuseId = "TSystemMessageCell_ReuseId"
    public static let TSystemMessageCell_ReuseId = (String(showSimilarStr.suffix(4)) + "temMe" + showReplyMsg.replacingOccurrences(of: "lab", with: "a") + String(data_insideMsg.prefix(5)) + "ReuseId")

    /**
     *  协议委托
     *  负责实现 EagerWait 协议中的功能。
     */
    //: public weak var delegate: TMessageCellDelegate?
    public weak var delegate: EagerWait?
    ...
    required init?(coder: NSCoder) {
        ...
        fatalError(String(bytes: show_fireStr.map{bulletResult(production: $0)}, encoding: .utf8)!)
    }
    ...
    override public func creationCharacter(with data: EventColor) {
        //: super.fill(with: data)
        super.creationCharacter(with: data)
        //: let newData: NextAgainst = data as! NextAgainst
        let newData: NextAgainst = data as! NextAgainst
    ...
    override public func updateConstraints() {
        ...
        let csize = self.messageData?.reduceQuick()
    ...
    lazy var avatarView: UIImageView = {
        ...
        let tap1 = UITapGestureRecognizer(target: self, action: #selector(packageSubtle(recognizer:)))
    ...
    lazy var nameLabel: UILabel = {
        ...
        lb.font = UIFont.monsterColumn(fontSize: 13)
        //: lb.textColor = .gray
        lb.textColor = .gray
    ...
    public lazy var container: UIView = {
        ...
        let tap = UITapGestureRecognizer(target: self, action: #selector(exceptEven(recognizer:)))
        ...
        let longPress = UILongPressGestureRecognizer(target: self, action: #selector(ridAppear(recognizer:)))
    ...
    lazy var retryView: UIImageView = {
        ...
        imagV.image = UIImage.evolutionary(name: (String(constDepthPath) + data_nextSheMessage.replacingOccurrences(of: "structure", with: "k") + String(constAboutName.suffix(6))))
        ...
        let resendTap = UITapGestureRecognizer(target: self, action: #selector(talkBy(recognizer:)))
...
extension SecondView {
    ...
    @objc func packageSubtle(recognizer: UIGestureRecognizer) {
        //: self.delegate?.onSelectMessageAvatar(cell: self)
        self.delegate?.commentSensor(cell: self)
    }

    //: @objc func onLongPress(recognizer: UIGestureRecognizer) {
    @objc func ridAppear(recognizer: UIGestureRecognizer) {
