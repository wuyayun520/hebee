...
/*: "/TXCache" :*/
fileprivate let main_activeRelatedMessage:String = "strip own robot/TXCac"
fileprivate let show_diskData:String = "henable"
...
class FeatureShared: NSObject {
    ...
    open weak var delegate: EventTake?
    //: var playStatus: VideoPlayerStatus?
    var playStatus: BlockBackground?
    ...
    lazy var player: TXVodPlayer = {
        ...
        let cachePath = NSString(format: "%@%@", NSSearchPathForDirectoriesInDomains(.documentDirectory, .userDomainMask, true).first!, (String(main_activeRelatedMessage.suffix(6)) + show_diskData.replacingOccurrences(of: "enable", with: "e"))) as String
    ...
    lazy var timer: Timer = {
        ...
        let timer = Timer.scheduledTimer(timeInterval: 0.25, target: self, selector: #selector(characteristic), userInfo: nil, repeats: true)
...
extension FeatureShared {
    //: func playerWithUrlAndVideoView(url: String, view: UIView) {
    func property(url: String, view: UIView) {
        //: let newUrl = url.replacingOccurrences(of: UrlDomin.urlAes256Decrypt(), with: ReplaceUrlDomain)
        let newUrl = url.replacingOccurrences(of: appProvideSectionPath.mentionAccount(), with: dataServiceName)
    ...
    @objc func characteristic() {
        ...
        if duration() > 0 {
            ...
            self.delegate?.interval(player: self, progress: CGFloat(currentTime() / duration()))
        }
    }

    //: func setRenderMode(renderMode: EnumTypeRenderMode) {
    func sequence(renderMode: ComprehensiveMaintain) {
    ...
    func frame(bEnable: Bool) {
        //: var isMute = bEnable
        var isMute = bEnable
        //: if TalkingSocketManager.shared.usage == true {
        if PanCurrentStatus.shared.usage == true { // 音视频通话时静音
            //: isMute = true
            isMute = true
        }
    ...
    func immediate() {
        ...
        if self.playStatus != .Pausing {
            ...
            self.delegate?.accelerate(player: self, status: self.playStatus!)
        }
    }
...
extension FeatureShared: TXVodPlayListener {
    ...
    func onPlayEvent(_ player: TXVodPlayer!, event EvtID: Int32, withParam param: [AnyHashable: Any]!) {
        ...
        case PLAY_EVT_PLAY_PROGRESS.rawValue:
            ...
            self.delegate?.mp(player: self, duration: Int(duration), currentTime: Int(currentTime))
