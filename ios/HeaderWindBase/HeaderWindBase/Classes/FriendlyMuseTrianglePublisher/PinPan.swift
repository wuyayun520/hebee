...
protocol KeyShadeSpecify: AnyObject {
...
class PinPan: UIViewController {
    ...
    weak var delegate: KeyShadeSpecify?
    ...
    private lazy var controllerScrollView: UIScrollView = {
        ...
        let count = min(self.controllerItems.count, self.titleItems.count)
        //: var height = StopBrush.getScreenHeight() - (topDistance + kTitle_H)
        var height = StopBrush.failureWith() - (topDistance + kTitle_H)
        //: if self.hasBttomTabBar {
        if self.hasBttomTabBar {
            //: height -= TabBarViewHeight
            height -= appRetainName
        }
        ...
        //: let rect = CGRect(x: 0, y: topDistance + kTitle_H, width: StopBrush.getScreenWidth(), height: height)
        let rect = CGRect(x: 0, y: topDistance + kTitle_H, width: StopBrush.randomAwake(), height: height)
        //: let scrollView = UIScrollView(frame: rect)
        let scrollView = UIScrollView(frame: rect)
        //: scrollView.contentSize = CGSize(width: count * Int(StopBrush.getScreenWidth()), height: 0)
        scrollView.contentSize = CGSize(width: count * Int(StopBrush.randomAwake()), height: 0)
        ...
        scrollView.semanticContentAttribute = ThanTrain.shared.direction == .leftToRight ? .forceLeftToRight:.forceRightToLeft
        //: return scrollView
        return scrollView
    //: }()
    }()
    ...
    private lazy var titleScrollView: UIScrollView = {
        //: var rect = CGRect(x: 15, y: topDistance, width: StopBrush.getScreenWidth()-rightSpace, height: kTitle_H)
        var rect = CGRect(x: 15, y: topDistance, width: StopBrush.randomAwake()-rightSpace, height: kTitle_H)
        //: if isBisect {
        if isBisect {
            //: rect = CGRect(x: 0, y: topDistance, width: StopBrush.getScreenWidth()-rightSpace, height: kTitle_H)
            rect = CGRect(x: 0, y: topDistance, width: StopBrush.randomAwake()-rightSpace, height: kTitle_H)
        }
        //: let scrollView = UIScrollView(frame: rect)
        let scrollView = UIScrollView(frame: rect)
        ...
        var originX: CGFloat = ThanTrain.shared.direction == .leftToRight ? 0:25
        ...
        self.titleItems.enumerated().forEach { [weak self] index, obj in
            ...
            titleBtn.addTarget(self, action: #selector(crucialBy), for: .touchDown)
            //: var titleWidth = 0.0
            var titleWidth = 0.0
            //: if isBisect {
            if isBisect {
                //: titleWidth = StopBrush.getScreenWidth() / Double(self.titleItems.count)
                titleWidth = StopBrush.randomAwake() / Double(self.titleItems.count)
            //: } else {
            } else {
                //: titleWidth = (titleBtn.titleLabel?.sizeThatFits(CGSize(width: CGFLOAT_MAX, height: kTitle_H)).width ?? 0) + 22
                titleWidth = (titleBtn.titleLabel?.sizeThatFits(CGSize(width: CGFLOAT_MAX, height: kTitle_H)).width ?? 0) + 22
            }
            //: let x = LanguageManager.shared.direction == .leftToRight ? originX:(StopBrush.getScreenWidth()-originX-titleWidth)
            let x = ThanTrain.shared.direction == .leftToRight ? originX:(StopBrush.randomAwake()-originX-titleWidth)
...
extension PinPan {
    ...
    @objc private func crucialBy(_ sender: UIButton) {
        //: currentSelectedTitleButton(sender)
        clusterSecond(sender)
        //: let index = sender.tag - 1000
        let index = sender.tag - 1000
        //: let offset_x = CGFloat(index) * StopBrush.getScreenWidth()
        let offset_x = CGFloat(index) * StopBrush.randomAwake()
        //: controllerScrollView.contentOffset = CGPoint(x: offset_x, y: 0)
        controllerScrollView.contentOffset = CGPoint(x: offset_x, y: 0)
        //: addControllerView(withIndex: index)
        distinctionAcross(withIndex: index)
        //: seletedViewController(withIndex: index)
        commentOriginal(withIndex: index)
    }
    
    //: private func addControllerView(withIndex index: Int) {
    private func distinctionAcross(withIndex index: Int) {
        //: if index >= children.count {
        if index >= children.count {
            //: return
            return
        }
        //: let origin_x = CGFloat(index) * StopBrush.getScreenWidth()
        let origin_x = CGFloat(index) * StopBrush.randomAwake()
        ...
        //: vc.view.frame = CGRect(x: origin_x, y: 0, width: StopBrush.getScreenWidth(), height: controllerScrollView.frame.size.height)
        vc.view.frame = CGRect(x: origin_x, y: 0, width: StopBrush.randomAwake(), height: controllerScrollView.frame.size.height)
        //: controllerScrollView.addSubview(vc.view)
        controllerScrollView.addSubview(vc.view)
    }
    
    //: private func currentSelectedTitleButton(_ button: UIButton) {
    private func clusterSecond(_ button: UIButton) {
    ...
    func loopFlexible(_ sender: UIButton) {
        //: if titleScrollView.contentSize.width <= StopBrush.getScreenWidth() {
        if titleScrollView.contentSize.width <= StopBrush.randomAwake() {
            //: return
            return
        }
        //: var offset_x = sender.center.x - StopBrush.getScreenWidth() * 0.5
        var offset_x = sender.center.x - StopBrush.randomAwake() * 0.5
        //: if offset_x < 0 {
        if offset_x < 0 {
            //: offset_x = 0
            offset_x = 0
        }
        //: let maxOffset = titleScrollView.contentSize.width - StopBrush.getScreenWidth()
        let maxOffset = titleScrollView.contentSize.width - StopBrush.randomAwake()
...
extension PinPan {
    ...
    func commentOriginal(withIndex index: Int) {
        //: guard index < children.count else { return }
        guard index < children.count else { return }
        //: delegate?.selectedViewController(children[index], index: index)
        delegate?.to(children[index], index: index)
    }
}

// MARK: - UIScrollViewDelegate
//: extension QYSlideNavigationViewController: UIScrollViewDelegate {
extension PinPan: UIScrollViewDelegate {
    //: func scrollViewDidEndDecelerating(_ scrollView: UIScrollView) {
    func scrollViewDidEndDecelerating(_ scrollView: UIScrollView) {
        //: let index = Int(controllerScrollView.contentOffset.x / StopBrush.getScreenWidth())
        let index = Int(controllerScrollView.contentOffset.x / StopBrush.randomAwake())
        //: if let selectedBtn = titleScrollView.viewWithTag(index + 1000) as? UIButton {
        if let selectedBtn = titleScrollView.viewWithTag(index + 1000) as? UIButton {
            //: currentSelectedTitleButton(selectedBtn)
            clusterSecond(selectedBtn)
        }
        //: addControllerView(withIndex: index)
        distinctionAcross(withIndex: index)
        //: seletedViewController(withIndex: index)
    ...
    func scrollViewDidScroll(_ scrollView: UIScrollView) {
        ...
        //: let index = Int(scrollView.contentOffset.x / StopBrush.getScreenWidth())
        let index = Int(scrollView.contentOffset.x / StopBrush.randomAwake())
        ...
        //: let scaleR = scrollView.contentOffset.x / StopBrush.getScreenWidth() - CGFloat(index)
        let scaleR = scrollView.contentOffset.x / StopBrush.randomAwake() - CGFloat(index)
        //: let scaleL = 1 - scaleR
        let scaleL = 1 - scaleR
