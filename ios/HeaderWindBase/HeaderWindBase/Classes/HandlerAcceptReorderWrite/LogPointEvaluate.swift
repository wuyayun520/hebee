...
/*: "icon_randow_user_bg" :*/
fileprivate let appWritingUrl:String = "icon_rabe ya aspect impression provision"
fileprivate let main_currencyLeadingName:String = "would game decidendow_u"
...
class LogPointEvaluate: UIView {
    // MARK: - UI Components

    //: private let leftAvatarView = AvatarView()
    private let leftAvatarView = SharedPageEdge()
    //: private let centerAvatarView = AvatarView()
    private let centerAvatarView = SharedPageEdge()
    //: private let rightAvatarView = AvatarView()
    private let rightAvatarView = SharedPageEdge()
    ...
    private func captureWould() {
        ...
        centerBgView.image = UIImage.evolutionary(name: (String(appWritingUrl.prefix(7)) + String(main_currencyLeadingName.suffix(6)) + "ser_bg"))
    }
...
extension LogPointEvaluate {
    // MARK: - Public Methods

    //: func configure(avatars: [String]) {
    func maleBrown(avatars: [String]) {
...
class SharedPageEdge: UIView {
    ...
    private func mergeWhite() {
        //: imageView.setUrlImage(urlStr: self.imageurl ?? "")
        imageView.accessiblePending(urlStr: self.imageurl ?? "")
    }
}
