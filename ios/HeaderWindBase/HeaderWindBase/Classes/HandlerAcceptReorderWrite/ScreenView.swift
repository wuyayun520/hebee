...
fileprivate let mainManagePopularName:[UInt8] = [0x89,0x8e,0x89,0x94,0xc8,0x83,0x8f,0x84,0x85,0x92,0xda,0xc9,0xc0,0x88,0x81,0x93,0xc0,0x8e,0x8f,0x94,0xc0,0x82,0x85,0x85,0x8e,0xc0,0x89,0x8d,0x90,0x8c,0x85,0x8d,0x85,0x8e,0x94,0x85,0x84]

private func spellComment(over num: UInt8) -> UInt8 {
    return num ^ 224
}

/*: "#4FAAFF" :*/
fileprivate let notiIncreaseValue:String = "reward team calendar gem permanent#4FAA"
fileprivate let const_farWhichStr:String = "resolutionresolution"

/*: "reply" :*/
fileprivate let constHearingUsData:[Character] = ["r","e","p","l","y"]

/*: "F5F5F5" :*/
fileprivate let userUnexpectedId:String = "FofFofF"
fileprivate let show_tempValue:String = "5"
...
class ScreenView: UITableViewCell {
    ...
    required init?(coder aDecoder: NSCoder) {
        ...
        fatalError(String(bytes: mainManagePopularName.map{spellComment(over: $0)}, encoding: .utf8)!)
    }
...
extension ScreenView {
    ...
    @objc func gray(sender: UIGestureRecognizer) {}
}

//: extension TalkingCommentReplyItemCell {
extension ScreenView {
    //: func configCell(model: TalkingCommentReplyModel) {
    func pressure(model: MarginComponentSecond) {
        //: if model.allCountContent.count>0 {
        if model.allCountContent.count > 0 {
            //: contentLB.textColor = UIColor.init(hex: "#4FAAFF")
            contentLB.textColor = UIColor(hex: (String(notiIncreaseValue.suffix(5)) + const_farWhichStr.replacingOccurrences(of: "resolution", with: "F")))
            ...
            let font = UIFont.observeFor(fontSize: 14)
            //: if model.replyUser.count>0 {
            if model.replyUser.count > 0 {
                //: strA = [model.nickname, " "+"reply".localized+" ", model.replyUser, "：\(model.content)"]
                strA = [model.nickname, " " + (String(constHearingUsData)).localized + " ", model.replyUser, "：\(model.content)"]
                //: colorA = [UIColor.appValueDetailColor(), UIColor.appTitleColor(), UIColor.appValueDetailColor(), UIColor.appTitleColor()]
                colorA = [UIColor.capBy(), UIColor.labelMomentum(), UIColor.capBy(), UIColor.labelMomentum()]
                ...
                colorA = [UIColor.capBy(), UIColor.labelMomentum()]
                //: fontA = [font, font]
                fontA = [font, font]
            }
    ...
    func pair(cell: UITableViewCell, tableView: UITableView, indexPath: NSIndexPath, radius: CGFloat, height: CGFloat) {
        ...
        let bounds = CGRect(x: 51, y: 0, width: mainCellMsg - 51 - 15, height: cell.bounds.size.height)
        ...
        layer.fillColor = UIColor(hex: (userUnexpectedId.replacingOccurrences(of: "of", with: "5") + show_tempValue.capitalized))?.cgColor
...
extension ScreenView {
    ...
    private func partner() {
        ...
        let longtap = UILongPressGestureRecognizer(target: self, action: #selector(gray(sender:)))
