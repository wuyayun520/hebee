...
fileprivate let data_mergeName:[UInt8] = [0x64,0x65,0x74,0x6e,0x65,0x6d,0x65,0x6c,0x70,0x6d,0x69,0x20,0x6e,0x65,0x65,0x62,0x20,0x74,0x6f,0x6e,0x20,0x73,0x61,0x68,0x20,0x29,0x3a,0x72,0x65,0x64,0x6f,0x63,0x28,0x74,0x69,0x6e,0x69]

/*: "img_home_shadow" :*/
fileprivate let constDecreaseUrl:String = "img_number area advertising refuse bad"
fileprivate let dataSuccessItemThinData:String = "hablindow"

/*: "#9610FF" :*/
fileprivate let showOnRegularData:String = "#9610FFfull require restore"

/*: "#8566FF" :*/
fileprivate let show_assistantUrl:[Character] = ["#","8","5","6","6","F","F"]

/*: "Random Video" :*/
fileprivate let main_invitationStr:[Character] = ["R","a","n","d","o","m"," ","V","i","d","e","o"]

/*: "icon_match_call_free" :*/
fileprivate let mainPurchaseGreatAgreeData:String = "icon_addition brown fast translation"
fileprivate let userRelationDocumentUrl:String = "maintainll"
fileprivate let mainSameNextPath:String = "example construct bot m_free"

/*: "Free" :*/
fileprivate let main_makerKey:String = "Freemaintain distinction publisher size hearing"

/*: "Scroll_upto_discove" :*/
fileprivate let k_helpName:[Character] = ["S","c","r","o","l","l"]
fileprivate let dataRemarkData:String = "properly sufficient consumer_upto"

/*: "icon_VectorStroke" :*/
fileprivate let constReceiverPublisherMsg:String = "act listen terms dateicon_V"
fileprivate let notiYieldUrl:String = "rStrokerounding gallery custom"

/*: "get json error" :*/
fileprivate let const_computeAdditionalString:String = "circle go accuracyget "
fileprivate let user_hungStr:String = "eadviceadviceo"
fileprivate let app_oddId:String = "particle"
...
class ListLine: UIView {
    ...
    var maxRemoveDistance = mainCellMsg / 4.0 /// 最大移除距离(默认屏幕的1/4)
    //: var reusableCells = [TalkingClubCardViewCell]()
    var reusableCells = [CancelSmart]() /// 重用卡片数组
    //: var isAlpha = true
    ...
    var dataSource: ServiceResult? /// 数据源
    //: var delegate: TalkingClubCardViewDelegate?
    var delegate: PageDuring?
    ...
    required init?(coder: NSCoder) {
        ...
        fatalError(String(bytes: data_mergeName.reversed(), encoding: .utf8)!)
    }
    ...
    private lazy var leftbackImageView: UIImageView = {
        ...
        imag.image = UIImage.evolutionary(name: (String(constDecreaseUrl.prefix(4)) + "home_s" + dataSuccessItemThinData.replacingOccurrences(of: "blind", with: "d")))
    ...
    private lazy var randomBtn: UIButton = {
        ...
        btn.setBackgroundImage(UIImage.untilFactory(colors: [UIColor(hex: (String(showOnRegularData.prefix(7))))!.cgColor, UIColor(hex: (String(show_assistantUrl)))!.cgColor], size: CGSize(width: mainCellMsg - 30, height: 54)), for: .normal)
        //: btn.layer.cornerRadius = 27
        btn.layer.cornerRadius = 27
        //: btn.layer.masksToBounds = true
        btn.layer.masksToBounds = true
        //: btn.setTitle("Random Video".localized, for: .normal)
        btn.setTitle((String(main_invitationStr)).localized, for: .normal)
        //: btn.addTarget(self, action: #selector(randomBtnClick), for: .touchUpInside)
        btn.addTarget(self, action: #selector(fillTranslate), for: .touchUpInside)
        //: self.addSubview(btn)
        self.addSubview(btn)
    ...
    lazy var freeImag: UIButton = {
        ...
        img.setBackgroundImage(UIImage.evolutionary(name: (String(mainPurchaseGreatAgreeData.prefix(5)) + "match_" + userRelationDocumentUrl.replacingOccurrences(of: "maintain", with: "ca") + String(mainSameNextPath.suffix(5)))), for: .normal)
        //: img.setTitle("Free".localized, for: .normal)
        img.setTitle((String(main_makerKey.prefix(4))).localized, for: .normal)
...
extension ListLine {
    ...
    private func feedCan() {
        ...
        image.image = UIImage.evolutionary(name: (String(k_helpName) + String(dataRemarkData.suffix(5)) + "_discove"))
        ...
        image2.image = UIImage.evolutionary(name: (String(constReceiverPublisherMsg.suffix(6)) + "ecto" + String(notiYieldUrl.prefix(7))))
        ...
        opByTa(player: player)
    }

    //: private func likeplayer(player: SVGAPlayer) {
    private func opByTa(player: SVGAPlayer) {
        //: do {
        do {
            //: let url = SVGAEffectTool.default.getZipEffectPath(type: .Club_guidePage)
            let url = PhoneAccelerate.default.memberStructure(type: .Club_guidePage)
            ...
            printLog(message: (String(const_computeAdditionalString.suffix(4)) + "json " + user_hungStr.replacingOccurrences(of: "advice", with: "r") + app_oddId.replacingOccurrences(of: "particle", with: "r")))
        }
    }

    //: @objc private func randomBtnClick() {
    @objc private func fillTranslate() {
        //: self.delegate?.clubCardRandowBtnClick?()
        self.delegate?.untilPhase?()
    }

    //: func reloadDataAnimated(anim: Bool) {
    func replyRecord(anim: Bool) {
        //: moveCount = 0
        moveCount = 0
        //: self.currentIndex = 0
        self.currentIndex = 0
        ...
        let maxCount = dataSource?.signBorder(cardView: self) ?? 0
        ...
        switchicial(anim: anim)
    }

    /** 创建新的cell */
    //: func createCardViewCell(index: Int) {
    func areaForPersist(index: Int) {
        //: let cell = dataSource?.cardViewCellForRowAtIndex(cardView: self, index: index)
        let cell = dataSource?.divideInsideOffer(cardView: self, index: index)
...
//        let showCount = Double(self.visibleCount - 1)
        ...
        containerView.insertSubview(cell ?? CancelSmart(), at: 0)
    ...
    func switchicial(anim: Bool) {
        ...
        for i in 0 ... count - 1 {
            ...
            let cell: CancelSmart = self.visibleCells[i] as! CancelSmart
            ...
            if isVisbleLast {
                ...
                self.delegate?.agileCounterval?(cardViewCell: cell, forRowAtIndex: self.currentIndex - i)
            }

            //: if anim {
            if anim {
                //: updateConstraintsCell(cell: cell, transform: transform)
                chapterInsideReveal(cell: cell, transform: transform)
                //: } else {
            } else {
    ...
    func chapterInsideReveal(cell: CancelSmart, transform: CGAffineTransform) {
    ...
    func blocIn(cellClass: AnyClass, forellIdentifier: String) {
    ...
    func popularLoad(identifier: String) -> CancelSmart? {
        ...
        if self.cellClass != nil {
            ...
            let cellclass = self.cellClass as! ComposeView.Type
            //: let cell: TalkingClubCardItemCell = cellclass.init(reuseIdentifier: identifier )
            let cell: ComposeView = cellclass.init(reuseIdentifier: identifier)
...
extension ListLine: PrimaryUpThin {
    ...
    func curWithBorder(cell: CancelSmart) {
        ...
        self.delegate?.sineRoman?(cardView: self, cardViewCell: cell, forRowAtIndex: self.currentFirstIndex - 1)

        //: let count = self.dataSource?.numberOfCountInCardView(cardView: self) ?? 0
        let count = self.dataSource?.signBorder(cardView: self) ?? 0
        // 移除后的卡片是最后一张(没有更多)
        ...
        if self.visibleCells.count == 0 { // 只有最后一张卡片的时候
            ...
            self.delegate?.magnitudeervalWithoutVisitor?(cardViewCell: cell, forRowAtIndex: self.currentIndex)
            //: return
            return
        }
        // 当前数据源还有数据 继续创建cell
        ...
        if self.currentIndex < count - 1 {
            ...
            areaForPersist(index: self.currentIndex + 1)
        }
        // 更新布局
        //: updateLayoutVisibleCells(anim: true)
        switchicial(anim: true)
    }
    ...
    func daily(cell: CancelSmart, forMovePoint: CGPoint) {
        ...
        self.delegate?.processorTotal?(cardView: self, cardViewCell: cell, forMovePoint: forMovePoint)
    }
}
