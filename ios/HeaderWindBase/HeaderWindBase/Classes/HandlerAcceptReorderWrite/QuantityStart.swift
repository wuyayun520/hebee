...
fileprivate let show_easePauseKey:String = "UserDeat strategy"
fileprivate let notiThatValue:String = "tailTainfrastructure some"

/*: "UserDetailTagInterestCellID" :*/
fileprivate let appPortraitUrl:String = "Userpainter run"
fileprivate let app_visualMsg:String = "TagIntedouble permanent"
fileprivate let main_customKey:String = "restCecontain than object minute tick"

/*: "AIHobbyCellID" :*/
fileprivate let userMatchTitle:[Character] = ["A","I","H","o","b","b","y","C","e","l","l","I","D"]
...
class QuantityStart: PetViewController {
    ...
    private var tableHeightModel = YellowModel() // 记录分区高度
    ...
    private lazy var tableView: UITableView = {
        ...
        tableView.contentInset = UIEdgeInsets(top: 0, left: 0, bottom: user_workFormat, right: 0)
        ...
        tableView.register(MarginError.self, forCellReuseIdentifier: MarginError.className())
        //: tableView.register(TalkingUserDetailTagCell.self, forCellReuseIdentifier: "UserDetailTagMeCellID")
        tableView.register(PropertyFilter.self, forCellReuseIdentifier: (String(show_easePauseKey.prefix(6)) + String(notiThatValue.prefix(6)) + "gMeCellID"))
        //: tableView.register(TalkingUserDetailTagCell.self, forCellReuseIdentifier: "UserDetailTagInterestCellID")
        tableView.register(PropertyFilter.self, forCellReuseIdentifier: (String(appPortraitUrl.prefix(4)) + "Detail" + String(app_visualMsg.prefix(7)) + String(main_customKey.prefix(6)) + "llID"))
        //: tableView.register(TalkingUserDetailGiftCell.self, forCellReuseIdentifier: TalkingUserDetailGiftCell.className())
        tableView.register(InfoProcessor.self, forCellReuseIdentifier: InfoProcessor.className())
        //: tableView.register(TalkingUserDetailTagCell.self, forCellReuseIdentifier: "AIHobbyCellID")
        tableView.register(PropertyFilter.self, forCellReuseIdentifier: (String(userMatchTitle)))
        //: return tableView
        return tableView
        //: }()
    }()
...
extension QuantityStart: UITableViewDelegate, UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        let cellType: MultiStatus = self.sectionArr[indexPath.section]
        //: switch cellType {
        switch cellType {
        //: case .profileType:
        case .profileType:
            ...
            let cell = tableView.dequeueReusableCell(withIdentifier: MarginError.className(), for: indexPath) as! MarginError
            //: cell.configWithDataModel(userModel: userInfoModel)
            cell.lengthSpend(userModel: userInfoModel)
            //: self.setProfileCellHeight(signAttribute: cell.signLab.attributedText ?? NSAttributedString.init(string: ""))
            self.medium(signAttribute: cell.signLab.attributedText ?? NSAttributedString(string: ""))
            //: return cell
            return cell
        ...
        case .aboutMeType:
            ...
            let cell = tableView.dequeueReusableCell(withIdentifier: (String(show_easePauseKey.prefix(6)) + String(notiThatValue.prefix(6)) + "gMeCellID"), for: indexPath) as! PropertyFilter
            //: cell.configTagCellWithDataModel(type: TUserDetailCellType.aboutMeType, userModel: userInfoModel)
            cell.platformNetwork(type: MultiStatus.aboutMeType, userModel: userInfoModel)
        ...
        case .intersetsType:
            ...
            let cell = tableView.dequeueReusableCell(withIdentifier: (String(appPortraitUrl.prefix(4)) + "Detail" + String(app_visualMsg.prefix(7)) + String(main_customKey.prefix(6)) + "llID"), for: indexPath) as! PropertyFilter
            //: cell.configTagCellWithDataModel(type: TUserDetailCellType.intersetsType, userModel: userInfoModel)
            cell.platformNetwork(type: MultiStatus.intersetsType, userModel: userInfoModel)
            //: cell.collectionLayoutSubviewBlock = {[weak self] collectH in
            cell.collectionLayoutSubviewBlock = { [weak self] collectH in
                //: guard let self = self else { return }
                guard let self = self else { return }
        ...
        case .giftType:
            ...
            let cell = tableView.dequeueReusableCell(withIdentifier: InfoProcessor.className(), for: indexPath) as! InfoProcessor
...
extension QuantityStart {
    //: private func setProfileCellHeight(signAttribute: NSAttributedString) {
    private func medium(signAttribute: NSAttributedString) {
        ...
        var labelH = signAttribute.boundingRect(with: CGSize(width: mainCellMsg - 58, height: CGFloat(MAXFLOAT)), options: [.usesLineFragmentOrigin, .usesFontLeading], context: nil).size.height
...
extension QuantityStart: JXPagingViewListViewDelegate {
...
extension QuantityStart {
    ...
    func frequency(_ giftArr: [ValueTranslate],
                   //: _ sectionArr: [TUserDetailCellType],
                   _ sectionArr: [MultiStatus],
                   //: _ userModel: TalkingUserInfoModel) {
                   _ userModel: EstimatedCharacteristic)
    {
