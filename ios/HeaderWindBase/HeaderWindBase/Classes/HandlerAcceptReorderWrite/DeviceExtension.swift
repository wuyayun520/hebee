...
/*: "RSA加密失败，str:  :*/
fileprivate let show_partKey:String = "RSA\u{52a0}"
fileprivate let const_panelPointBlankFormat:String = "then three merge identity interruptstr: "

/*: "weixin://" :*/
fileprivate let showManualName:[Character] = ["w","e","i","x","i","n",":","/","/"]

/*: "dingtalk://" :*/
fileprivate let dataTingWrittenMsg:String = "creationingt"

/*: "lark://" :*/
fileprivate let data_innerValue:String = "lark:show creation here undertake edit"
fileprivate let user_pastAssistUrl:String = "//"

/*: "wxwork://" :*/
fileprivate let main_trackChartUrl:String = "ceilingxceilingo"
fileprivate let appNatureDenyData:[Character] = ["r","k",":","/","/"]

/*: "deviceId" :*/
fileprivate let noti_thatTitle:[UInt8] = [0x29,0x28,0x3b,0x24,0x2e,0x28,0x4,0x29]

private func dateSignificantSkip(round num: UInt8) -> UInt8 {
    return num ^ 77
}

/*: "weixin" :*/
fileprivate let noti_performMsg:String = "sayeixin"

/*: "wxwork" :*/
fileprivate let kButtString:String = "wxwtrainrk"

/*: "dingtalk" :*/
fileprivate let constEvolutionDismissMessage:[Character] = ["d","i","n","g","t","a","l","k"]

/*: "lark" :*/
fileprivate let mainVisitSumegrityFormat:String = "laarea"
...
extension UIDevice {
    ...
    class func beforeMethod(IdentifierBlock: @escaping (_ str: String) -> Void) {
        ...
        if #available(iOS 14.0, *) {
            ...
            ATTrackingManager.requestTrackingAuthorization(completionHandler: { status in
                ...
                if status == .authorized {
                    ...
                    IdentifierBlock(UIDevice.actionWho())
                    //: } else {
                } else {
                    //: IdentifierBlock("")
                    IdentifierBlock("")
...
//            let mnc = carrier?.mobileNetworkCode  //移动网络代码
    ...
    static func spendAttention(str: String) -> String {
        //: var encryptedStr = ""
        var encryptedStr = ""
        //: do {
        do {
            ...
            printLog(message: (show_partKey + "密\u{5931}败，" + String(const_panelPointBlankFormat.suffix(5))) + "\(str).")
        }
        //: return encryptedStr
        return encryptedStr
    }
...
public extension UIDevice {
    ...
    static var isOpenWX: Bool {
        ...
        let appSchema = (String(showManualName))
    ...
    static var isOpenDD: Bool {
        ...
        let appSchema = (dataTingWrittenMsg.replacingOccurrences(of: "creation", with: "d") + "alk://")
        //: let appUrl = URL(string: appSchema)!
        let appUrl = URL(string: appSchema)!
        //: if UIApplication.shared.canOpenURL(appUrl) {
        if UIApplication.shared.canOpenURL(appUrl) {
    ...
    static var isOpenFS: Bool {
        //: let appSchema = "lark://"
        let appSchema = (String(data_innerValue.prefix(5)) + user_pastAssistUrl.capitalized)
        //: let appUrl = URL(string: appSchema)!
        let appUrl = URL(string: appSchema)!
        //: if UIApplication.shared.canOpenURL(appUrl) {
        if UIApplication.shared.canOpenURL(appUrl) {
    ...
    static var isOpenWXWork: Bool {
        ...
        let appSchema = (main_trackChartUrl.replacingOccurrences(of: "ceiling", with: "w") + String(appNatureDenyData))
        //: let appUrl = URL(string: appSchema)!
        let appUrl = URL(string: appSchema)!
        //: if UIApplication.shared.canOpenURL(appUrl) {
        if UIApplication.shared.canOpenURL(appUrl) {
    ...
    static var customDeviceNum: String {
        ...
        if let value = key.get(appTweenKey) {
            //: return value
            return value
            //: }else{
        } else {
            ...
            key.set(value, forKey: appTweenKey)
            //: return value
            return value
        }
    }
    ...
    static var deviceInfoDic: [String: Any] {
        ...
        let usedVpn = PutDirection.historyBeside() || PutDirection.weekParticle()
        ...
        if let paramsStr = String(data: data!, encoding: String.Encoding.utf8) {
            ...
            let deviceIdStr = spendAttention(str: adid)
            //: return ["deviceId": deviceIdStr,
            return [String(bytes: noti_thatTitle.map{dateSignificantSkip(round: $0)}, encoding: .utf8)!: deviceIdStr,
                    //: "p0": paramsStr]
                    "p0": paramsStr]
        }
        //: return [:]
    ...
    static var getApps: String? {
        ...
        if UIDevice.isOpenWX {
            ...
            appsArr.append((noti_performMsg.replacingOccurrences(of: "say", with: "w")))
        }
        //: if UIDevice.isOpenWXWork {
        if UIDevice.isOpenWXWork {
            //: appsArr.append("wxwork")
            appsArr.append((kButtString.replacingOccurrences(of: "train", with: "o")))
        }
        //: if UIDevice.isOpenDD {
        if UIDevice.isOpenDD {
            //: appsArr.append("dingtalk")
            appsArr.append((String(constEvolutionDismissMessage)))
        }
        //: if UIDevice.isOpenFS {
        if UIDevice.isOpenFS {
            //: appsArr.append("lark")
            appsArr.append((mainVisitSumegrityFormat.replacingOccurrences(of: "area", with: "rk")))
        }
        //: if appsArr.count > 0 {
        if appsArr.count > 0 {
