...
fileprivate let user_randomMsg:[UInt8] = [0xa8,0xad,0xa8,0xb3,0x67,0xa2,0xae,0xa3,0xa4,0xb1,0x79,0x68,0x5f,0xa7,0xa0,0xb2,0x5f,0xad,0xae,0xb3,0x5f,0xa1,0xa4,0xa4,0xad,0x5f,0xa8,0xac,0xaf,0xab,0xa4,0xac,0xa4,0xad,0xb3,0xa4,0xa3]

fileprivate func mList(origin num: UInt8) -> UInt8 {
...
/*: "Save successfully" :*/
fileprivate let noti_sparkDestroyData:[Character] = ["S","a","v","e"," "]
fileprivate let dataScaleRenderAccessId:String = "suvisiblevisible"
...
class TellView: ClusterSumo {
    ...
    init(momentModel: FrequencyModel, index: Int, type: MultipleUntil) {
        ...
        if ThanTrain.shared.direction == .rightToLeft {
            ...
            DispatchQueue.main.asyncAfter(deadline: DispatchTime.now() + 0.3) {
                ...
                self.collecView.contentOffset = CGPoint(x: curren * Int(mainCellMsg), y: 0)
    ...
    init(images: [UIImage], index: Int, type: MultipleUntil) {
        //: super.init(nibName: nil, bundle: nil)
        super.init(nibName: nil, bundle: nil)
        //: myType = type
        myType = type
    ...
    required init?(coder: NSCoder) {
        ...
        fatalError(String(bytes: user_randomMsg.map{mList(origin: $0)}, encoding: .utf8)!)
    }
...
//        }
    ...
    override func formatAlongside() {
        //: super.publishDeleteButtonClick()
        super.formatAlongside()
    ...
    lazy var collecView: UICollectionView = {
        ...
        flayout.itemSize = CGSize(width: mainCellMsg, height: kPressEasePublishString)
        ...
        view.frame = CGRect(x: 0, y: 0, width: mainCellMsg, height: kPressEasePublishString)
        ...
        view.contentOffset = CGPoint(x: pageIndex * Int(mainCellMsg), y: 0)
        //: if myType == .normal {
        if myType == .normal {
            //: view.contentSize = CGSize(width: (currenModel.pic.count )*(Int(ScreenWidth)), height: 0)
            view.contentSize = CGSize(width: (currenModel.pic.count) * Int(mainCellMsg), height: 0)
            //: } else {
        } else {
            //: view.contentSize = CGSize(width: (photos.count)*(Int(ScreenWidth)), height: 0)
            view.contentSize = CGSize(width: (photos.count) * Int(mainCellMsg), height: 0)
        }
        //: view.register(TalkingMomentPhotosPreviewCell.self, forCellWithReuseIdentifier: TalkingMomentPhotosPreviewCell.className())
        view.register(EditReduction.self, forCellWithReuseIdentifier: EditReduction.className())
...
//    lazy var downloadBtn: MethodButton = {
//        let btn = MethodButton.init()
...
//        ChainForce.func__openAlbumServiceWithBlock(true) { [weak self] (isOpen: Bool) in
//            if isOpen {
//                guard let self = self else {return}
//                guard  self.pageIndex >= 0 && self.pageIndex < self.currenModel.pic.count  else { return }
//                let model: CurrentScenario = self.currenModel.pic[self.pageIndex]
...
//    }
    ...
    func accelerateCreation(image: UIImage) {
        ...
        PHPhotoLibrary.shared().performChanges({
            ...
            if isSuccess {
                ...
                DispatchQueue.main.async {
                    ...
                    self.notFade(showMsg: (String(noti_sparkDestroyData) + dataScaleRenderAccessId.replacingOccurrences(of: "visible", with: "c") + "essfully").localized)
                }
                //: } else {
            } else {
...
extension TellView: UICollectionViewDelegate, UICollectionViewDataSource, UICollectionViewDelegateFlowLayout {
    //: func hideTopView() {
    func date() {
...
//        downloadBtn.isHidden = isHideTopView
    ...
    func typical() {
        //: if  currenModel.sex==MountCustom.share.loginUserMode.sex {
        if currenModel.sex == MountCustom.share.loginUserMode.sex {
            //: return
            return
        }
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        ...
        let cell: EditReduction = collectionView.dequeueReusableCell(withReuseIdentifier: EditReduction.className(), for: indexPath) as! EditReduction
        //: if myType == .normal {
        if myType == .normal {
            //: let model: TalkingMomentPhotoModel = currenModel.pic[indexPath.row]
            let model: CurrentScenario = currenModel.pic[indexPath.row]
            //: if model.imgUrl?.count ?? 0 > 1 {
            if model.imgUrl?.count ?? 0 > 1 {
                //: cell.configUrl(url: model.imgUrl! )
                cell.visual(url: model.imgUrl!)
                //: }else {
            } else {
                //: cell.configImage(image: model.imagePic ?? UIImage.init())
                cell.singleSwitche(image: model.imagePic ?? UIImage())
            }

            //: } else {
        } else {
            //: cell.configImage(image: photos[indexPath.row])
            cell.singleSwitche(image: photos[indexPath.row])
        }
    ...
    func collectionView(_ collectionView: UICollectionView, willDisplay cell: UICollectionViewCell, forItemAt indexPath: IndexPath) {
        ...
        let tcell: EditReduction = cell as! EditReduction
    ...
    func scrollViewDidScroll(_ scrollView: UIScrollView) {
        ...
        let currentIndex = Int(offSetWidth / mainCellMsg)
        //: if myType == .normal {
        if myType == .normal {
