...
fileprivate let noti_beMsg:[UInt8] = [0x16,0x11,0x16,0xb,0x57,0x1c,0x10,0x1b,0x1a,0xd,0x45,0x56,0x5f,0x17,0x1e,0xc,0x5f,0x11,0x10,0xb,0x5f,0x1d,0x1a,0x1a,0x11,0x5f,0x16,0x12,0xf,0x13,0x1a,0x12,0x1a,0x11,0xb,0x1a,0x1b]

private func characterBold(bag num: UInt8) -> UInt8 {
    return num ^ 127
}

/*: "icon_male_default" :*/
fileprivate let kCurStr:[UInt8] = [0x4d,0x47,0x53,0x52,0x43,0x51,0x45,0x50,0x49,0x43,0x48,0x49,0x4a,0x45,0x59,0x50,0x58]

fileprivate func nextHide(make num: UInt8) -> UInt8 {
...
/*: "icon_female_default" :*/
fileprivate let user_withEnableicialValue:String = "icon_feconsume failure decrease translate correct"
fileprivate let noti_cameraArrayTitle:String = "male_natural below interested ban"
fileprivate let show_turnValue:[Character] = ["d","e","f","a","u","l","t"]

/*: "level_%@" :*/
fileprivate let main_spotStr:[Character] = ["l","e","v","e","l","_","%","@"]
...
class ProcessorProvider: UIView {
    ...
    required init?(coder: NSCoder) {
        ...
        fatalError(String(bytes: noti_beMsg.map{characterBold(bag: $0)}, encoding: .utf8)!)
    }
    ...
    lazy var headerPic = UIImageView().then {
        ...
        $0.image = UIImage.smartThreshold()
    ...
    lazy var nickLabel = UILabel().then {
        ...
        $0.font = .stepTransition(type: .Medium, fontSize: 17)
        //: $0.textColor = .white
        $0.textColor = .white
    }
    ...
    lazy var ageLabel = UILabel().then {
        ...
        $0.font = .stepTransition(type: .Regular, fontSize: 10)
        //: $0.textColor = .white
        $0.textColor = .white
    }
...
extension ProcessorProvider {
    //: func updateContentWithModel(model: TPreviewInfoModel) {
    func tip(model: SharedFeedback) {
        //: headerPic.kf.setImage(with: URL(string: model.headPic), placeholder: UIImage.placeImgSquare())
        headerPic.kf.setImage(with: URL(string: model.headPic), placeholder: UIImage.smartThreshold())
        ...
        let imgStr = model.sex == BasicIndependent.male.rawValue ? String(bytes: kCurStr.map{nextHide(make: $0)}, encoding: .utf8)! : (String(user_withEnableicialValue.prefix(7)) + String(noti_cameraArrayTitle.prefix(5)) + String(show_turnValue))
        //: sexImgView.image = UIImage(named: imgStr)
        sexImgView.image = UIImage(named: imgStr)
        //: let levelStr = String(format: "level_%@", model.level)
        let levelStr = String(format: (String(main_spotStr)), model.level)
