...
fileprivate let kPriorityConfirmAllowPath:[UInt8] = [0x4b,0x4c,0x4b,0x56,0xa,0x41,0x4d,0x46,0x47,0x50,0x18,0xb,0x2,0x4a,0x43,0x51,0x2,0x4c,0x4d,0x56,0x2,0x40,0x47,0x47,0x4c,0x2,0x4b,0x4f,0x52,0x4e,0x47,0x4f,0x47,0x4c,0x56,0x47,0x46]

private func toOur(replacement num: UInt8) -> UInt8 {
    return num ^ 34
}

/*: "FractionAttach" :*/
fileprivate let data_remainScanFormat:String = "DropMevolutionary tolerance"
fileprivate let showTapProvideSpecialFormat:[Character] = ["e","m","b","e","r","M","e","n","u","C","e","l","l"]

/*: "roomId" :*/
fileprivate let k_imageStr:String = "capture end squareroomId"

/*: "type" :*/
fileprivate let user_profileUrl:String = "TYPE"

/*: "page" :*/
fileprivate let showBothName:String = "planguagege"

/*: "icon_chats_pcr" :*/
fileprivate let notiMineMessage:String = "native drown condition recent oficon_ch"
fileprivate let constSpellFormat:String = "acomplete"
fileprivate let dataHandleMissTranslatePath:String = "s_pcrfuture birthday retain receiver manual"

/*: "All Numbers" :*/
fileprivate let const_spanChangeName:String = "someone remain clear partyAll Nu"
fileprivate let showRegionLaunchMsg:String = "pending"

/*: "all" :*/
fileprivate let appCoordinateKey:String = "transactionll"

/*: "Recent Gifts" :*/
fileprivate let kSceneStr:[Character] = ["R","e","c","e","n","t"," ","G","i","f","t","s"]
...
class SingleView: UIView {
    ...
    public var triangleHeight = Config.PhaseLocal.triangleHeight
    /**
     *  调整使下拉优先 当向下偏转屏幕距离足够，优先向下偏转
     */
    //: public var adjustPullDown = Config.DropMenuConfig.adjustPullDown
    public var adjustPullDown = Config.PhaseLocal.adjustPullDown
    ...
    fileprivate lazy var contentView: UIView = self.itLikeOpend()
    //: fileprivate lazy var mTable: UITableView = self.makeTableView()
    fileprivate lazy var mTable: UITableView = self.robotArea()
    //: fileprivate var anchorRect = CGRect.zero
    fileprivate var anchorRect = CGRect.zero

    //: static let shared = DropDownMemberMenuView(frame: .zero)
    static let shared = SingleView(frame: .zero)
    ...
    required init?(coder: NSCoder) {
        ...
        fatalError(String(bytes: kPriorityConfirmAllowPath.map{toOur(replacement: $0)}, encoding: .utf8)!)
    }

    //: private func makeContentView() -> UIView {
    private func itLikeOpend() -> UIView {
        //: let view  = UIView()
        let view = UIView()
        //: return view
        return view
    ...
    private func robotArea() -> UITableView {
    ...
    private func like() {
        ...
        mTable.register(FractionAttach.self, forCellReuseIdentifier: (String(data_remainScanFormat.prefix(5)) + String(showTapProvideSpecialFormat)))
    ...
    func withOf() {
        ...
        var dict = [String: Any]()
        //: dict["roomId"] = roomID
        dict[(String(k_imageStr.suffix(6)))] = roomID
        //: dict["type"] = "3"
        dict[(user_profileUrl.lowercased())] = "3"
        //: dict["page"] = String(pageIndex)
        dict[(showBothName.replacingOccurrences(of: "language", with: "a"))] = String(pageIndex)
        ...
        EdgeCell.share.refresh(params: dict, completion: { array in
            ...
            if self.pageIndex == 0 {
                ...
                let model = StateDifficult()
                //: model.headPic = "icon_chats_pcr"
                model.headPic = (String(notiMineMessage.suffix(7)) + constSpellFormat.replacingOccurrences(of: "complete", with: "t") + String(dataHandleMissTranslatePath.prefix(5)))
                //: model.nickname = "All Numbers".localized
                model.nickname = (String(const_spanChangeName.suffix(6)) + "mber" + showRegionLaunchMsg.replacingOccurrences(of: "pending", with: "s")).localized
                //: model.uid = "all"
                model.uid = (appCoordinateKey.replacingOccurrences(of: "transaction", with: "a"))
    ...
    private func originalAccelerate() {
...
extension SingleView: UITableViewDelegate, UITableViewDataSource {
    ...
    public func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        let cell = tableView.dequeueReusableCell(withIdentifier: (String(data_remainScanFormat.prefix(5)) + String(showTapProvideSpecialFormat)), for: indexPath) as! FractionAttach
        ...
        if recentmenuArray.count > 0 && indexPath.section == 0 {
            ...
            cellModel = recentmenuArray[indexPath.row] as! StateDifficult
            //: }else {
        } else {
            //: cellModel = menuArray[indexPath.row] as! TalkingChatRoomMemberModel
            cellModel = menuArray[indexPath.row] as! StateDifficult
        }
        //: cell.setDropMemberMenuCell(model: cellModel)
        cell.stackToUnique(model: cellModel)
        //: return cell
        return cell
    }
    ...
    public func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        menuDelegate?.sufficient(model: cellModel)
    }

    //: func tableView(_ tableView: UITableView, viewForHeaderInSection section: Int) -> UIView? {
    func tableView(_ tableView: UITableView, viewForHeaderInSection section: Int) -> UIView? {
        //: let headeView = UILabel.init(frame: CGRect(x: 0, y: 0, width: ScreenWidth, height: 23))
        let headeView = UILabel(frame: CGRect(x: 0, y: 0, width: mainCellMsg, height: 23))
        //: headeView.backgroundColor = UIColor.appBgColor()
        headeView.backgroundColor = UIColor.boardUnexpected()
        //: let titleLB = UILabel.init()
        let titleLB = UILabel()
        //: titleLB.frame = CGRect(x: 3, y: 5, width: 100, height: 14)
        titleLB.frame = CGRect(x: 3, y: 5, width: 100, height: 14)
        //: titleLB.font = .pingfangFont(type: .Medium, fontSize: 12)
        titleLB.font = .stepTransition(type: .Medium, fontSize: 12)
        //: titleLB.textColor = .appValueDetailColor()
        titleLB.textColor = .capBy()
        //: if recentmenuArray.count > 0 && section == 0 {
        if recentmenuArray.count > 0 && section == 0 {
            //: titleLB.text = "Recent Gifts".localized
            titleLB.text = (String(kSceneStr)).localized
            //: }else {
        } else {
            //: titleLB.text = "All Numbers".localized
            titleLB.text = (String(const_spanChangeName.suffix(6)) + "mber" + showRegionLaunchMsg.replacingOccurrences(of: "pending", with: "s")).localized
        }
