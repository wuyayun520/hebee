...
/*: "Search" :*/
fileprivate let k_rankMessage:String = "very tradeSearch"

/*: "#F5F5F5" :*/
fileprivate let userRemoteTitle:String = "#F5F5F5define boy infrastructure weight norm"

/*: "Search by UID" :*/
fileprivate let constAnonymousId:String = "Searcrecord photo thumb lower"
fileprivate let kSnapData:String = "with"

/*: "icon_search_sea" :*/
fileprivate let user_petStr:[Character] = ["i","c","o","n","_"]
fileprivate let const_hiAgileMainMessage:[Character] = ["s","e","a","r","c","h","_","s","e","a"]

/*: "Can’t find~" :*/
fileprivate let show_visualName:String = "Can’delay period"

/*: "icon_kong_kong_default" :*/
fileprivate let dataSumSinkStr:String = "icon_kchoose literal key title pi"
fileprivate let data_gravityName:String = "KONG"
fileprivate let const_reasonBesideData:[Character] = ["a","u","l","t"]

/*: "Please enter the query UID" :*/
fileprivate let data_meTitle:[Character] = ["P","l","e","a"]
fileprivate let userSecurityPath:[Character] = ["s","e"," ","e","n","t","e","r"," ","t","h","e"," ","q","u","e","r","y"," ","U","I","D"]
...
class CombineEnd: PetViewController {
    ...
    private lazy var searchBtn: UIButton = {
        ...
        btn.setTitle((String(k_rankMessage.suffix(6))).localized, for: .normal)
        ...
        btn.setBackgroundImage(UIImage.untilFactory(colors: UIColor.opInMay(), size: CGSize(width: 72, height: 32)), for: .normal)
        //: btn.layer.cornerRadius = 16
        btn.layer.cornerRadius = 16
        //: btn.clipsToBounds = true
        btn.clipsToBounds = true
        //: btn.addTarget(self, action: #selector(searchButtonClick), for: .touchUpInside)
        btn.addTarget(self, action: #selector(hostFromCrushCharacter), for: .touchUpInside)
        //: btn.alpha = 0.5
        btn.alpha = 0.5
    ...
    private lazy var searchTF: UITextField = {
        ...
        tf.placeholder = (String(constAnonymousId.prefix(5)) + "h by UI" + kSnapData.replacingOccurrences(of: "with", with: "D")).localized
    ...
    private lazy var unlessEditingView: UIView = {
        ...
        let image = UIImage.evolutionary(name: (String(user_petStr) + String(const_hiAgileMainMessage)))
    ...
    private lazy var tableView: UITableView = {
        ...
        tabV.register(SmallMaster.self, forCellReuseIdentifier: SmallMaster.className())
    ...
    private lazy var emptyV: EmptyView = {
        ...
        style.TipsTitle = (String(show_visualName.prefix(4)) + "t find~").localized
        //: style.TipsIcon = "icon_kong_kong_default"
        style.TipsIcon = (String(dataSumSinkStr.prefix(6)) + "ong_" + data_gravityName.lowercased() + "_def" + String(const_reasonBesideData))
        ...
        let v = EmptyView(frame: CGRect(x: 0, y: 14 + showBuildData, width: mainCellMsg, height: kPressEasePublishString - (14 + showBuildData + mainItemFormat)), style: style)
    ...
    private lazy var reqManager: MediumBullet = //: return MediumBullet()
        .init()
    //: }()
}
...
extension CombineEnd {
    ...
    private func lock() {
        ...
        guard searchTF.text?.count ?? 0 > 0 else {
            ...
            self.signWhen(showMsg: (String(data_meTitle) + String(userSecurityPath)).localized)
            //: return
            return
        }
        ...
        reqManager.style(uid: searchTF.text ?? "") { [weak self] succeed, result, errorModel in
            //: ProgressHUD.dismiss()
...
extension CombineEnd {
    ...
    @objc private func hostFromCrushCharacter() {
        //: req_searchUid()
        lock()
    }
...
extension CombineEnd: UITableViewDataSource, UITableViewDelegate {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        let cell = tableView.dequeueReusableCell(withIdentifier: SmallMaster.className(), for: indexPath) as! SmallMaster
        //: let model = reqManager.searchListDataArr[indexPath.row]
        let model = reqManager.searchListDataArr[indexPath.row]
        //: cell.refreshCell(model: model)
        cell.smallDown(model: model)
...
extension CombineEnd: UITextFieldDelegate {
...
extension CombineEnd: JXPagingViewListViewDelegate {
...
extension CombineEnd {
    ...
    private func equipmentOrSymbol() {
        // 监听搜索框文本长度
