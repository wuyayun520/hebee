...
class BuildLast: UINavigationController {

    //: var tabBarType: TabBarItemType?
    var tabBarType: EquivalentBecome?
    //: var pushing = false                             
    ...
    override func pushViewController(_ viewController: UIViewController, animated: Bool) {
        ...
        if viewControllers.count >= 1 {
            ...
            if viewController.navigationItem.leftBarButtonItem == nil {
                ...
                viewController.navigationItem.leftBarButtonItem = viewController.replacement()
            }
        }
    ...
    func lineHighlight() {
        ...
        if #available(iOS 15.0, *) {
            ...
            barApp.backgroundImage = UIImage.magnitudeensity(color: UIColor.white)
            //: barApp.shadowImage = UIImage.init()
            barApp.shadowImage = UIImage.init()
            //: barApp.shadowColor = UIColor.appBgColor()
            barApp.shadowColor = UIColor.boardUnexpected()
            ...
            barApp.titleTextAttributes = [NSAttributedString.Key.foregroundColor: UIColor.labelMomentum(), NSAttributedString.Key.font: UIFont.stepTransition(type: .Medium, fontSize: 17)]
        ...
        //: } else {
            ...
            navigationBar.setBackgroundImage(UIImage.magnitudeensity(color: UIColor.white), for: .any, barMetrics: .default)
            ...
            navigationBar.titleTextAttributes = [NSAttributedString.Key.foregroundColor: UIColor.labelMomentum(), NSAttributedString.Key.font: UIFont.stepTransition(type: .Medium, fontSize: 17)]
        }
        //: UINavigationBar.appearance().tintColor = UIColor.appTitleColor()
        UINavigationBar.appearance().tintColor = UIColor.labelMomentum()
...
extension BuildLast: UINavigationControllerDelegate {
    ...
    func navigationController(_ navigationController: UINavigationController, willShow viewController: UIViewController, animated: Bool) {
        ...
        if (viewController.hidesBottomBarWhenPushed) {
            ...
            CanvasGreen.shared.advSuperView?.isHidden = true
        //: } else {
        } else {
...
extension BuildLast: UIGestureRecognizerDelegate {
