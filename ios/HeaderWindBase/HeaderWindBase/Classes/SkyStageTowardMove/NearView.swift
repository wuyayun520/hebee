...
fileprivate let showPureSpineName:[UInt8] = [0x64,0x65,0x74,0x6e,0x65,0x6d,0x65,0x6c,0x70,0x6d,0x69,0x20,0x6e,0x65,0x65,0x62,0x20,0x74,0x6f,0x6e,0x20,0x73,0x61,0x68,0x20,0x29,0x3a,0x72,0x65,0x64,0x6f,0x63,0x28,0x74,0x69,0x6e,0x69]

/*: "Reply" :*/
fileprivate let const_burnString:String = "combine front speaker month buildingReply"

/*: "#F95151" :*/
fileprivate let notiSineCapMessage:String = "hard adjust page#F95151"
...
let constPriorityFormat = (data_visitorPath + 80.0)
...
class NearView: UIView {
    ...
    var startDismissBlock: ((_ model: VerticalName) -> Void)? // 开始移除视图Block
    //: var finishDismissBlock: ((_ view: TalkingMessageNotificationView) -> Void)?
    var finishDismissBlock: ((_ view: NearView) -> Void)? // 完成移除视图Block
    ...
    required init?(coder: NSCoder) {
        ...
        fatalError(String(bytes: showPureSpineName.reversed(), encoding: .utf8)!)
    }
    ...
    private lazy var nickNameLabel: UILabel = {
        ...
        label.font = .stepTransition(type: .Medium, fontSize: 15)
    ...
    private lazy var replyBtn: UIButton = {
        ...
        btn.titleLabel?.font = .stepTransition(type: .Medium, fontSize: 15)
        //: btn.setTitle("Reply".localized, for: .normal)
        btn.setTitle((String(const_burnString.suffix(5))).localized, for: .normal)
        //: btn.setTitleColor(UIColor(hex: "#F95151"), for: .normal)
        btn.setTitleColor(UIColor(hex: (String(notiSineCapMessage.suffix(7)))), for: .normal)
        //: btn.isEnabled = false
        btn.isEnabled = false
        //: return btn
        return btn
...
extension NearView {
    ...
    func offer(_ model: VerticalName?) {
    ...
    func large(needAlpha: Bool = true) {
        ...
        DispatchQueue.main.async {
            ...
            UIView.animate(withDuration: 0.5) {
                ...
                if needAlpha {
                    ...
                    self.frame.origin.y = -constPriorityFormat
                }
                //: } completion: { _ in
            } completion: { _ in
    ...
    @objc private func equal() {
        ...
        if currModel.chatType == .service {
            ...
            //: RetainProcess.share.func__pushToPriveteChatVC(chatID: currModel.targetId)
            RetainProcess.share.roundBeside(chatID: currModel.targetId)
        }
    }

    /// 滑动手势
    //: @objc private func swipeGestureEvent(gesture: UISwipeGestureRecognizer) {
    @objc private func flexibleFor(gesture: UISwipeGestureRecognizer) {
...
extension NearView {
    ...
    func createFor() {
        ...
        if let userInfo = currModel.gj_userInfo {
            ...
            headImgView.containRetain(urlStr: userInfo.headPic, placeImg: UIImage.rapidTo(sex: userInfo.sex))
            //: if !userInfo.headPicFrame.isEmptyString {
            if !userInfo.headPicFrame.isEmptyString {
                //: iconBorder.isHidden = false
                iconBorder.isHidden = false
                //: iconBorder.setHeadFrameUrlImage(urlStr: userInfo.headPicFrame)
                iconBorder.pendingRemote(urlStr: userInfo.headPicFrame)
                //: } else {
            } else {
            ...
            headImgView.image = UIImage.untilAlways()
            //: iconBorder.isHidden = true
            iconBorder.isHidden = true
        }

        //: msgShowLabel.attributedText = TalkingChatListTableCell.handleShowC2CDetailTextOfMessage(model: currModel)
        msgShowLabel.attributedText = ImplementationFilter.operation(model: currModel)
    }
    ...
    private func previous() {
        ...
        self.frame = CGRect(x: 0, y: -constPriorityFormat, width: mainCellMsg, height: constPriorityFormat)
    ...
    private func anonymousRank() {
        //: let tap = UITapGestureRecognizer(target: self, action: #selector(tapGestureEvent))
        let tap = UITapGestureRecognizer(target: self, action: #selector(equal))
        //: self.addGestureRecognizer(tap)
        self.addGestureRecognizer(tap)

        //: let swipe = UISwipeGestureRecognizer(target: self, action: #selector(swipeGestureEvent))
        let swipe = UISwipeGestureRecognizer(target: self, action: #selector(flexibleFor))
        //: swipe.direction = .up
        swipe.direction = .up
