...
fileprivate let notiInstancePartyStr:[UInt8] = [0x64,0x65,0x74,0x6e,0x65,0x6d,0x65,0x6c,0x70,0x6d,0x69,0x20,0x6e,0x65,0x65,0x62,0x20,0x74,0x6f,0x6e,0x20,0x73,0x61,0x68,0x20,0x29,0x3a,0x72,0x65,0x64,0x6f,0x63,0x28,0x74,0x69,0x6e,0x69]

/*: "btnJQbEAB_etihw_kcab_shadow" :*/
fileprivate let noti_stateId:String = "over than appearancebtnJQ"
fileprivate let noti_lightResponseFormat:String = "written certain shorttihw_kc"
fileprivate let show_themeTargetMsg:String = "thumb hour filter goab_shadow"

/*: "Edit" :*/
fileprivate let const_secureTitle:[Character] = ["E","d","i","t"]

/*: "Finish" :*/
fileprivate let constDrawData:[Character] = ["F","i","n","i","s","h"]

/*: "btnYDo2AS_oediv_play" :*/
fileprivate let appCapacityStr:String = "ago zonebtnYDo"
fileprivate let notiSearchTitle:String = "oeinterrupt"

/*: "Cancel" :*/
fileprivate let showGiftString:String = "Cancelchart ok despite data"

/*: "bgQkRTbF_oediv_toast" :*/
fileprivate let dataCycleSceneName:String = "bgQkRTbaccelerate print medal draw"
fileprivate let k_cityBelowTodayString:[Character] = ["F","_","o","e","d","i","v"]
fileprivate let k_aboveFormat:String = "_toasttolerance container beyond"

/*: "The video length should be %@-%@ seconds. Please intercept it" :*/
fileprivate let kSlideStr:[UInt8] = [0x52,0x6e,0x63,0x26,0x70,0x6f,0x62,0x63,0x69,0x26,0x6a,0x63,0x68,0x61,0x72,0x6e,0x26,0x75,0x6e,0x69,0x73,0x6a,0x62,0x26,0x64,0x63,0x26,0x23,0x46,0x2b,0x23,0x46,0x26,0x75,0x63,0x65,0x69,0x68,0x62,0x75,0x28,0x26,0x56,0x6a,0x63,0x67,0x75,0x63,0x26,0x6f,0x68,0x72,0x63,0x74,0x65,0x63,0x76,0x72,0x26,0x6f,0x72]

private func wantCos(very num: UInt8) -> UInt8 {
    return num ^ 6
}
...
typealias VideoEditBlock = (_ videoInfo: AnalyzeBridgeDepth?) -> Void

//: let kMinVideoDuration: CGFloat = 5.0
let kEstimatedJoinString: CGFloat = 5.0

//: class TalkingVideoEditorController: TalkingBaseViewController {
class ObjectViewController: PetViewController {
    ...
    private var slider: DuringShade?
    //: private var videoInfo = TalkingVideoInfoModel()
    private var videoInfo = AnalyzeBridgeDepth() // 编辑结果模型
    //: private var currStyle = TalkingVideoEditorStyle.normal
    private var currStyle = MediumSpeed.normal // 当前样式
    ...
    override func viewWillAppear(_ animated: Bool) {
        ...
        //: if TalkingSocketManager.shared.usage == true {
        if PanCurrentStatus.shared.usage == true { // 正在音视频通话，静音播放
    ...
    init(style: MediumSpeed = .normal, maxDuration: CGFloat, videoURL: URL, completionHandler: @escaping VideoEditBlock) {
        ...
        imgWidth = (mainCellMsg - 2 * padding) / (framesInScreen * 1.0)
        //: imgHeight = 50
        imgHeight = 50
        //: duration = 0
        duration = 0
    ...
    required init?(coder: NSCoder) {
        ...
        fatalError(String(bytes: notiInstancePartyStr.reversed(), encoding: .utf8)!)
    }
    ...
    private lazy var backBtn: UIButton = {
        ...
        v.setImage(UIImage.evolutionary(name: (String(noti_stateId.suffix(5)) + "bEAB_e" + String(noti_lightResponseFormat.suffix(7)) + String(show_themeTargetMsg.suffix(9)))), for: .normal)
        //: v.contentHorizontalAlignment = LanguageManager.shared.direction == .rightToLeft ? .right:.left
        v.contentHorizontalAlignment = ThanTrain.shared.direction == .rightToLeft ? .right : .left
        //: v.addTarget(self, action: #selector(backAction), for: .touchUpInside)
        v.addTarget(self, action: #selector(pushBy), for: .touchUpInside)
    ...
    private lazy var editBtn: UIButton = {
        ...
        v.setTitle((String(const_secureTitle)).localized, for: .normal)
        //: v.setTitleColor(UIColor.appThemeColor(), for: .normal)
        v.setTitleColor(UIColor.alongOccur(), for: .normal)
        //: v.titleLabel?.font = UIFont.pingfangRugularFont(fontSize: 14)
        v.titleLabel?.font = UIFont.observeFor(fontSize: 14)
        //: v.contentHorizontalAlignment = LanguageManager.shared.direction == .rightToLeft ? .right:.left
        v.contentHorizontalAlignment = ThanTrain.shared.direction == .rightToLeft ? .right : .left
        //: v.isHidden = true
        v.isHidden = true
//        v.addTarget(self, action: #selector(editAction(sender:)), for: .touchUpInside)
    ...
    private lazy var bottomDoneBtn: UIButton = {
        ...
        v.setTitle((String(constDrawData)).localized, for: .normal)
        //: v.setTitleColor(UIColor.white, for: .normal)
        v.setTitleColor(UIColor.white, for: .normal)
        //: v.titleLabel?.font = UIFont.pingfangMediumFont(fontSize: 14)
        v.titleLabel?.font = UIFont.monsterColumn(fontSize: 14)
        //: v.layer.cornerRadius = 14.0
        v.layer.cornerRadius = 14.0
        //: v.layer.masksToBounds = true
        v.layer.masksToBounds = true
        //: v.backgroundColor = UIColor.appThemeColor()
        v.backgroundColor = UIColor.alongOccur()
        //: v.addTarget(self, action: #selector(bottomDoneAction), for: .touchUpInside)
        v.addTarget(self, action: #selector(sinceReward), for: .touchUpInside)
        //: self.botBar.addSubview(v)
        self.botBar.addSubview(v)
        //: return v
        return v
    ...
    private lazy var playBtn: UIButton = {
        ...
        v.setImage(UIImage.evolutionary(name: (String(appCapacityStr.suffix(6)) + "2AS_" + notiSearchTitle.replacingOccurrences(of: "interrupt", with: "di") + "v_play")), for: .normal)
        //: v.addTarget(self, action: #selector(playAction(sender:)), for: .touchUpInside)
        v.addTarget(self, action: #selector(sinceDataWindow(sender:)), for: .touchUpInside)
        //: self.view.addSubview(v)
        self.view.addSubview(v)
        //: return v
        return v
    ...
    private lazy var cancelBtn: UIButton = {
        ...
        v.setTitle((String(showGiftString.prefix(6))).localized, for: .normal)
    ...
    private lazy var confirmBtn: UIButton = {
        ...
        v.addTarget(self, action: #selector(dangerous), for: .touchUpInside)
        //: self.botBar.addSubview(v)
        self.botBar.addSubview(v)
        //: v.snp.makeConstraints { make in
        v.snp.makeConstraints { make in
    ...
    private lazy var toastImageView: UIImageView = {
        ...
        let v = UIImageView(image: UIImage.evolutionary(name: (String(dataCycleSceneName.prefix(7)) + String(k_cityBelowTodayString) + String(k_aboveFormat.prefix(6)))))
        //: self.view.addSubview(v)
        self.view.addSubview(v)
        ...
        lab.text = String(bytes: kSlideStr.map{wantCos(very: $0)}, encoding: .utf8)!.fieldOf(minDuration, Int(maxduration))
        //: lab.textColor = .white
        lab.textColor = .white
        //: lab.numberOfLines = 0
        lab.numberOfLines = 0
    ...
    lazy var useVideoView: ClipView = {
        //: let v = TalkingSendIntimateInfoView(frame: CGRect.zero, type: .intimateVideo)
        let v = ClipView(frame: CGRect.zero, type: .intimateVideo)
        //: v.isHidden = true
        v.isHidden = true
        //: self.view.addSubview(v)
        self.view.addSubview(v)
...
extension ObjectViewController {
    ...
    private func topicDown(animated: Bool = true) {
    ...
    @objc func provisionBlack() {
        ...
        self.previewLayer?.frame = CGRect(x: 26, y: 26, width: mainCellMsg - 2 * 26, height: kPressEasePublishString - 46 - 127 - noti_stageFormat)
        //: imgWidth = ScreenWidth / framesInScreen
        imgWidth = mainCellMsg / framesInScreen

        //: if slider == nil {
        if slider == nil {
            //: let frame = CGRect(x: 0, y: ScreenHeight, width: ScreenWidth, height: CGFloat(imgHeight))
            let frame = CGRect(x: 0, y: kPressEasePublishString, width: mainCellMsg, height: CGFloat(imgHeight))
            // 创建预览区域
            ...
            slider = DuringShade(frame: frame, url: self.videoURL!, imgWidth: imgWidth, maxDuration: maxduration, minVideoDuration: kEstimatedJoinString)
            ...
            slider?.snp.makeConstraints { make in
                ...
                make.bottom.equalToSuperview().offset(-60 - noti_stageFormat)
...
//                self.playing = false
            ...
            slider?.slidingBlock = { [weak self] (direction: EditIndex) in
                //: guard let self = self else { return }
                guard let self = self else { return }
...
//                        self.playBtn.isHidden = true
    ...
    @objc func sinceReward() {
        ...
        if self.videoEditHandler != nil {
            ...
            ThinEnhance.shared.future(videoInfo: videoInfo, containerView: self.view, saveDocuments: true) { [weak self] outputPath, coverPath in
                //: guard let self = self else { return }
                guard let self = self else { return }
                //: self.popCurrentViewController()
                self.topicDown()
    ...
    @objc func sinceDataWindow(sender: UIButton) {
...
extension ObjectViewController {
    ...
    private func everySink() {
        ...
        NotificationCenter.default.addObserver(self, selector: #selector(suspendEnable), name: NSNotification.Name.AVPlayerItemDidPlayToEndTime, object: item)
        ...
        self.playerTimer = self.player?.addPeriodicTimeObserver(forInterval: step, queue: DispatchQueue.main, using: { [weak self] time in
            ...
            if self.slider != nil, self.slider!.isHidden == false {
                ...
                self.slider?.overlay.satisfyResolve(xposition)
            }
            //: }) as AnyObject?
        }) as AnyObject?
    ...
    private func visualCorner() {
        ...
        self.botBar.snp.makeConstraints { make in
            ...
            make.height.equalTo(48 + noti_stageFormat)
        }
