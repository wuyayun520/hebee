...
fileprivate let app_integrityString:[UInt8] = [0x11,0x16,0x11,0xc,0x50,0x1b,0x17,0x1c,0x1d,0xa,0x42,0x51,0x58,0x10,0x19,0xb,0x58,0x16,0x17,0xc,0x58,0x1a,0x1d,0x1d,0x16,0x58,0x11,0x15,0x8,0x14,0x1d,0x15,0x1d,0x16,0xc,0x1d,0x1c]
...
@objc protocol AssociatedEquallyThread: NSObjectProtocol {
    ///
    //: @objc optional func startScroll()
    @objc optional func someone()

    //: @objc optional func endScroll()
    @objc optional func towardCapture()
}

//: class TalkingVideoDanmuMsgListTableView: UIView {
class DirectView: UIView {
    ...
    override init(frame: CGRect) {
        ...
        let tapGesture = UITapGestureRecognizer(target: self, action: #selector(ready))
    ...
    required init?(coder: NSCoder) {
        ...
        fatalError(String(bytes: app_integrityString.map{$0^120}, encoding: .utf8)!)
    }
...
extension DirectView {
    ...
    func locateMe() {
        ...
        refreshTimer = Timer.scheduledTimer(timeInterval: reloadTimeSpan, target: self, selector: #selector(selectImmediate), userInfo: nil, repeats: true)
    }

    //: func stopTimer() {
    func responseMomentum() {
...
extension DirectView {
    ...
    func clip(msgModel: ResponseInstead?) {
    ...
    func content() {
        ...
        if AllHeight > show_changeSubsequentMsg {
            //: if self.msgTableView.height < VideoMsgTableViewHeight {
            if self.msgTableView.height < show_changeSubsequentMsg {
                ...
                self.msgTableView.height = show_changeSubsequentMsg
            }
            //: } else {
        } else {
            //: var frame = self.msgTableView.frame
            ...
            frame.origin.y = show_changeSubsequentMsg - AllHeight
        ...
        videoDisable(animated: false)
    }

    /** 执行插入动画并滚动 */
    //: func scrollToBottom(animated: Bool) {
    func videoDisable(animated: Bool) {
...
extension DirectView: UITableViewDelegate, UITableViewDataSource {
    ...
    func tar(_ scrollView: UIScrollView) {
        //: self.delegate?.endScroll?()
        self.delegate?.towardCapture?()
    ...
    func tableView(_ tableView: UITableView, heightForRowAt indexPath: IndexPath) -> CGFloat {
        ...
        return msgModel.msgHeight + Double(user_distinctionPath)
    }
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        let cell = FactoryAround.runForLargeClub(tableView: tableView, msg: msgModel, indexPath: indexPath)
...
extension DirectView {
    ...
    func magnitudeeraction() {
        ...
        self.msgTableView.frame = CGRect(x: 0, y: 0, width: mainSuccessGenerateNetName, height: show_changeSubsequentMsg)
    }
}
