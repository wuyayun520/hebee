...
fileprivate let notiGoingPackUrl:[UInt8] = [0x8f,0x88,0x8f,0x92,0xce,0x85,0x89,0x82,0x83,0x94,0xdc,0xcf,0xc6,0x8e,0x87,0x95,0xc6,0x88,0x89,0x92,0xc6,0x84,0x83,0x83,0x88,0xc6,0x8f,0x8b,0x96,0x8a,0x83,0x8b,0x83,0x88,0x92,0x83,0x82]

private func layCrush(mode num: UInt8) -> UInt8 {
    return num ^ 230
}
...
@objc protocol OverFrame: NSObjectProtocol {
    ///
    //: @objc optional func startScroll()
    @objc optional func sectionScenario()

    //: @objc optional func endScroll()
    @objc optional func blindForSmart()
}
...
class AddUp: UIView {
    ...
    required init?(coder: NSCoder) {
        ...
        fatalError(String(bytes: notiGoingPackUrl.map{layCrush(mode: $0)}, encoding: .utf8)!)
    }
...
extension AddUp {
    ...
    func thenSh() {
        ...
        refreshTimer = Timer.scheduledTimer(timeInterval: reloadTimeSpan, target: self, selector: #selector(enterForStack), userInfo: nil, repeats: true)
    }
...
extension AddUp {
    ...
    func frameCenter(msgModel: ShapeCalculate?) {
    ...
    func challenge() {
        ...
        if AllHeight > main_onePath {
            //: if self.msgTableView.height < MsgTableViewHeight {
            if self.msgTableView.height < main_onePath {
                ...
                self.msgTableView.height = main_onePath
            }
            //: } else {
        } else {
            //: var frame = self.msgTableView.frame
            ...
            frame.origin.y = main_onePath - AllHeight
        ...
        characterPlain(animated: false)
    }

    /** 执行插入动画并滚动 */
    //: func scrollToBottom(animated: Bool) {
    func characterPlain(animated: Bool) {
...
extension AddUp: UITableViewDelegate, UITableViewDataSource {
    ...
    func visitorInFlag(_ scrollView: UIScrollView) {
        //: self.delegate?.endScroll?()
        self.delegate?.blindForSmart?()
    ...
    func tableView(_ tableView: UITableView, heightForRowAt indexPath: IndexPath) -> CGFloat {
        ...
        return msgModel.msgHeight + Double(mainSegmentPath)
    }
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        let cell = PlateTarget.banRemark(tableView: tableView, msg: msgModel, indexPath: indexPath)
