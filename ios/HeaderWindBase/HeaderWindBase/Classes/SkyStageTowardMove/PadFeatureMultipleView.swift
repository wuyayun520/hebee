...
fileprivate let constEmbraceId:[UInt8] = [0x6b,0x6c,0x6b,0x76,0x2a,0x61,0x6d,0x66,0x67,0x70,0x38,0x2b,0x22,0x6a,0x63,0x71,0x22,0x6c,0x6d,0x76,0x22,0x60,0x67,0x67,0x6c,0x22,0x6b,0x6f,0x72,0x6e,0x67,0x6f,0x67,0x6c,0x76,0x67,0x66]
...
class PadFeatureMultipleView: UIView {
    ...
    required init?(coder: NSCoder) {
        ...
        fatalError(String(bytes: constEmbraceId.map{$0^2}, encoding: .utf8)!)
    }
    ...
    private lazy var progressView: OwnerAccelerate = {
        ...
        let v = OwnerAccelerate(frame: self.frame, progressBgColor: self.progressBgColor, progressPlayedColor: self.progressPlayedColor, progressCachedColor: self.progressCachedColor)
    ...
    private lazy var panGesture: UIPanGestureRecognizer = {
        ...
        let v = UIPanGestureRecognizer(target: self, action: #selector(commonOf(gesture:)))
        //: return v
        return v
        //: }()
    }()
    ...
    private lazy var tapGesture: UIPanGestureRecognizer = {
        ...
        let v = UIPanGestureRecognizer(target: self, action: #selector(towardContain(gesture:)))
        //: return v
        return v
        //: }()
    }()
...
extension PadFeatureMultipleView {
    //: @objc func panGesture(gesture: UIPanGestureRecognizer) {
    @objc func commonOf(gesture: UIPanGestureRecognizer) {
        // 移动的距离
    ...
    @objc func towardContain(gesture: UIPanGestureRecognizer) {
