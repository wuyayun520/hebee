...
class HebeeChatScreen extends StatefulWidget {
  final String userId;
  final String userName;
  final String userAvatar;

  const HebeeChatScreen({
    super.key,
  ...
  @override
  State<HebeeChatScreen> createState() => _HebeeChatScreenState();
}

class _HebeeChatScreenState extends State<HebeeChatScreen> {
  List<_HebeeChatMessage> _messages = [];
  ...
  Future<String> _getMediaDirectory() async {
    ...
    final mediaDir = Directory('${dir.path}/hebee_chat_media');
  ...
  Future<void> _loadMessages() async {
    final dir = await getApplicationDocumentsDirectory();
    final file = File('${dir.path}/hebee_chat_history_${widget.userId}.json');
    if (await file.exists()) {
      try {
        ...
        setState(() {
          _messages = jsonList.map((e) => _HebeeChatMessage.fromJson(e)).toList();
        });
  ...
  void _sendMessage() {
    ...
    setState(() {
        _messages.add(_HebeeChatMessage(
        text: text,
        isMe: true,
        time: _getTimeStamp(),
          type: _HebeeChatMessageType.text,
      ));
    });
  ...
  Future<void> _sendImage(File imageFile) async {
    try {
      ...
      final fileName = 'hebee_image_${DateTime.now().millisecondsSinceEpoch}.jpg';
      ...
      setState(() {
        _messages.add(_HebeeChatMessage(
          ...
          isMe: true,
          time: _getTimeStamp(),
          type: _HebeeChatMessageType.image,
        ));
      });
      _scrollToBottom();
      _saveMessages();
  ...
  Future<void> _sendVoice(String audioPath, Duration duration) async {
    try {
      final mediaDir = await _getMediaDirectory();
      final fileName = 'hebee_voice_${DateTime.now().millisecondsSinceEpoch}.m4a';
      ...
      setState(() {
        _messages.add(_HebeeChatMessage(
          ...
          isMe: true,
          time: _getTimeStamp(),
          type: _HebeeChatMessageType.audio,
        ));
      });
      _scrollToBottom();
      _saveMessages();
    ...
    try {
      if (_isRecording) {
        ...
        if (await _voiceRecorder.hasPermission()) {
          ...
          final filePath = '${dir.path}/hebee_voice_${DateTime.now().millisecondsSinceEpoch}.m4a';
...
enum _HebeeChatMessageType { text, image, audio }

class _HebeeChatMessage {
  final String? text;
  ...
  final bool isMe;
  final String time;
  final _HebeeChatMessageType type;
  ...
  static _HebeeChatMessage fromJson(Map<String, dynamic> json) => _HebeeChatMessage(
    text: json['text'],
    ...
    isMe: json['isMe'] ?? true,
    time: json['time'] ?? '',
    type: _HebeeChatMessageType.values.firstWhere(
      (e) => e.name == json['type'],
      orElse: () => _HebeeChatMessageType.text,
    ),
  );
}

class _HebeeChatBubble extends StatefulWidget {
  final _HebeeChatMessage message;
  final void Function(File file)? onImageTap;
  const _HebeeChatBubble({required this.message, this.onImageTap});

  @override
  State<_HebeeChatBubble> createState() => _HebeeChatBubbleState();
}

class _HebeeChatBubbleState extends State<_HebeeChatBubble> {
  ...
  void initState() {
    super.initState();
    if (widget.message.type == _HebeeChatMessageType.audio) {
  ...
  Future<void> _togglePlayPause() async {
    try {
      ...
      final dir = await getApplicationDocumentsDirectory();
      final absPath = '${dir.path}/hebee_chat_media/${msg.audioPath}';
  ...
  Widget build(BuildContext context) {
    ...
    if (msg.type == _HebeeChatMessageType.audio && msg.audioPath != null) {
        ...
        child: Container(
          ...
          decoration: BoxDecoration(
            gradient: msg.isMe 
                ...
                : null,
            color: msg.isMe ? null : Colors.white.withOpacity(0.2),
    ...
    if (msg.type == _HebeeChatMessageType.image && msg.imagePath != null) {
      return Align(
        alignment: msg.isMe ? Alignment.centerRight : Alignment.centerLeft,
        child: GestureDetector(
          onTap: () async {
            final dir = await getApplicationDocumentsDirectory();
            final absPath = '${dir.path}/hebee_chat_media/${msg.imagePath}';
                ...
                ClipRRect(
                  ...
                  child: FutureBuilder<Directory>(
                    ...
                    builder: (context, snapshot) {
                      ...
                      final absPath = '${snapshot.data!.path}/hebee_chat_media/${msg.imagePath}';
                      return Image.file(
                        File(absPath),
                        width: 160,
                        height: 160,
    ...
    return Align(
      ...
      child: Container(
        ...
        decoration: BoxDecoration(
          ...
          border: msg.isMe ? null : Border.all(color: Colors.white.withOpacity(0.3), width: 1),
          boxShadow: [
            BoxShadow(
              color: msg.isMe 
                  ? AppTheme.primaryEnd.withOpacity(0.3)
...
class _HebeeChatInputBar extends StatelessWidget {
                ...
                children: [
                  ...
                  Container(
                    decoration: BoxDecoration(
                      ...
                      shape: BoxShape.circle,
                      border: Border.all(color: AppTheme.primaryEnd.withOpacity(0.4), width: 1.5),
                    ),
                    child: IconButton(
                      icon: Icon(
                        Icons.image_rounded,
