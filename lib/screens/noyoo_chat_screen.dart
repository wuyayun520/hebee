...
class NoyooChatScreen extends StatefulWidget {
  final String userId;
  final String userName;
  final String userAvatar;

  const NoyooChatScreen({
    super.key,
  ...
  @override
  State<NoyooChatScreen> createState() => _NoyooChatScreenState();
}

class _NoyooChatScreenState extends State<NoyooChatScreen> {
  List<_NoyooChatMessage> _messages = [];
  ...
  Future<String> _getMediaDirectory() async {
    ...
    final mediaDir = Directory('${dir.path}/noyoo_chat_media');
  ...
  Future<void> _loadMessages() async {
    final dir = await getApplicationDocumentsDirectory();
    final file = File('${dir.path}/noyoo_chat_history_${widget.userId}.json');
    if (await file.exists()) {
      try {
        ...
        setState(() {
          _messages = jsonList.map((e) => _NoyooChatMessage.fromJson(e)).toList();
        });
  ...
  void _sendMessage() {
    ...
    setState(() {
        _messages.add(_NoyooChatMessage(
        text: text,
        isMe: true,
        time: _getTimeStamp(),
          type: _NoyooChatMessageType.text,
      ));
    });
  ...
  Future<void> _sendImage(File imageFile) async {
    try {
      ...
      final fileName = 'noyoo_image_${DateTime.now().millisecondsSinceEpoch}.jpg';
      ...
      setState(() {
        _messages.add(_NoyooChatMessage(
          ...
          isMe: true,
          time: _getTimeStamp(),
          type: _NoyooChatMessageType.image,
        ));
      });
      _scrollToBottom();
      _saveMessages();
  ...
  Future<void> _sendVoice(String audioPath, Duration duration) async {
    try {
      final mediaDir = await _getMediaDirectory();
      final fileName = 'noyoo_voice_${DateTime.now().millisecondsSinceEpoch}.m4a';
      ...
      setState(() {
        _messages.add(_NoyooChatMessage(
          ...
          isMe: true,
          time: _getTimeStamp(),
          type: _NoyooChatMessageType.audio,
        ));
      });
      _scrollToBottom();
      _saveMessages();
    ...
    try {
      if (_isRecording) {
        ...
        if (await _voiceRecorder.hasPermission()) {
          ...
          final filePath = '${dir.path}/noyoo_voice_${DateTime.now().millisecondsSinceEpoch}.m4a';
...
enum _NoyooChatMessageType { text, image, audio }

class _NoyooChatMessage {
  final String? text;
  ...
  final bool isMe;
  final String time;
  final _NoyooChatMessageType type;
  ...
  static _NoyooChatMessage fromJson(Map<String, dynamic> json) => _NoyooChatMessage(
    text: json['text'],
    ...
    isMe: json['isMe'] ?? true,
    time: json['time'] ?? '',
    type: _NoyooChatMessageType.values.firstWhere(
      (e) => e.name == json['type'],
      orElse: () => _NoyooChatMessageType.text,
    ),
  );
}

class _NoyooChatBubble extends StatefulWidget {
  final _NoyooChatMessage message;
  final void Function(File file)? onImageTap;
  const _NoyooChatBubble({required this.message, this.onImageTap});

  @override
  State<_NoyooChatBubble> createState() => _NoyooChatBubbleState();
}

class _NoyooChatBubbleState extends State<_NoyooChatBubble> {
  ...
  void initState() {
    super.initState();
    if (widget.message.type == _NoyooChatMessageType.audio) {
  ...
  Future<void> _togglePlayPause() async {
    try {
      ...
      final dir = await getApplicationDocumentsDirectory();
      final absPath = '${dir.path}/noyoo_chat_media/${msg.audioPath}';
  ...
  Widget build(BuildContext context) {
    ...
    if (msg.type == _NoyooChatMessageType.audio && msg.audioPath != null) {
    ...
    if (msg.type == _NoyooChatMessageType.image && msg.imagePath != null) {
      return Align(
        alignment: msg.isMe ? Alignment.centerRight : Alignment.centerLeft,
        child: GestureDetector(
          onTap: () async {
            final dir = await getApplicationDocumentsDirectory();
            final absPath = '${dir.path}/noyoo_chat_media/${msg.imagePath}';
                ...
                ClipRRect(
                  ...
                  child: FutureBuilder<Directory>(
                    ...
                    builder: (context, snapshot) {
                      ...
                      final absPath = '${snapshot.data!.path}/noyoo_chat_media/${msg.imagePath}';
                      return Image.file(
                        File(absPath),
                        width: 160,
                        height: 160,
...
class _NoyooChatInputBar extends StatelessWidget {
